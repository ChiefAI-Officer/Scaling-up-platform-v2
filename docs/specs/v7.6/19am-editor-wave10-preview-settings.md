...
| **D12** *(corrected during build)* | Duplicate Sections card | **Drop it from Settings** (candidate-8 dedup); section CRUD lives solely in the Build tab; stop threading `onSections*` into the Settings tab. ~~AND add the impact confirm to Build's ⋯ Delete~~ **CORRECTION (T9):** the Build ⋯ Delete **already shows the impact confirm** — `useEditorCommands.deleteSection` (`hooks/useEditorCommands.ts:219-289`) runs `window.confirm(buildSectionDeletePrompt(...))` before `model.deleteSection` and has since ED9; `FormSectionCard` uses that wrapped command. The grill's "Build deletes with no confirm" was a misread of `commandsModel.deleteSection = model.deleteSection` (the raw input to the wrapper). So dropping the Metadata Sections card **loses no confirmed path** (when ED10 is active, Metadata is unreachable and SettingsTab has no section-delete → the only reachable delete is Build's, which confirms). **T9 is a verified no-op.** |
...
- **Section CRUD not lost (D12) — no change needed.** Build's delete affordance uses `commands.deleteSection` from `useEditorCommands`, which **already** runs the `buildSectionDeletePrompt` `window.confirm` before `model.deleteSection` (`useEditorCommands.ts:219-289`, since ED9). So dropping the Metadata Sections card removes a *duplicate surface*, not the confirm — the Build path (the only reachable section-delete when ED10 is active) confirms unchanged. The earlier "add a gated confirm to Build" plan (and the related Codex C5 delete-behavior concern) is **void**: nothing about the delete confirm changes in either flag state.
