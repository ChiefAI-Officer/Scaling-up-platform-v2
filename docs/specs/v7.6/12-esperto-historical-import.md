...
| D7 | **Additive `AssessmentCampaign.externalId String? @unique`** (= Esperto `campaignid`), mirroring the `Organization.externalId` partial-unique pattern. DB-enforced re-import dedupe. | First-class provenance + idempotency guard; one safe additive column. |
...
- `externalId` = Esperto `campaignid` (idempotency). `alias` = slug e.g. `imported-qsp-v2-bdvhudorxz`. `name` = operator-editable default. `status=CLOSED`, `accessMode=INVITED`, `endMode=OPEN_END`, `openAt`=earliest row date, `closeAt`=latest row date, `language`= the **pinned version's** language (Esperto `enUS` ignored).
...
- Pre-existing `From Jeff/` committed-PDF PII leak (separate cleanup task).
