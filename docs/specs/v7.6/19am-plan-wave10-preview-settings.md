...
| **C5** flag-OFF default is Build in forms mode (`TabbedShell.tsx:351`), so "inactive default = metadata" *regresses* ED9; T9 delete-confirm changes ED9 flag-OFF behavior (innerHTML can't catch it) | ✅ confirmed | **T3/T9/T10** — one `ed10Active` gate; flag-OFF default = existing computed default; delete-confirm gated on `ed10Active` (supersedes O4) |
...
| Section delete is cascade-safe but confirm is caller-side | `useTemplateEditorDraft.ts:399+` (safe delete); `FormsBuilder.tsx:164` (raw `commands.deleteSection`); `TabbedShell.tsx:468-491` (the confirm wrapper) | Add the same confirm to Build (D12) |
...
### T9 — Build section-delete confirm (D12, gated — C5)
- [ ] Wire `commands.deleteSection` (FormsBuilder via the controller) through `collectSectionDeleteImpact` + `buildSectionDeletePrompt` + confirm before `model.deleteSection` — matching the Metadata/Sections card behavior — **only when `ed10Active`**. Flag-OFF keeps ED9's current confirm-less delete (byte-identical; the innerHTML golden can't catch a confirm-dialog behavior change, so the gate is the guard, C5). Cancel = no-op.
- [ ] Tests: **`ed10Active` on** ⇒ confirm shows question count / freed dependents, cancel aborts, confirm deletes cascade-safely; **`ed10Active` off** ⇒ delete fires with no confirm (unchanged).
