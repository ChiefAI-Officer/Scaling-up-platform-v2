...
| #7 removal | the remove capability (UI + endpoint) | login block + API/page liveness checks | no more removals — removed admins **stay locked out**. A kill switch must never un-fire an offboarding. |
