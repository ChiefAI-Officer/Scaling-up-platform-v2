...
Renderer visual/print evidence does not need that database. The DB-free harness
uses `react-dom/server` to render the exact Boardroom and Dashboard components
from the same allow-listed fixture builder, then loads the production scoped
CSS into Chromium. `next/font` compiler output is represented only by empty
class variables, so this evidence exercises the declared CSS fallback fonts;
the bundled fonts remain covered by the Task 14 build and Task 15 authenticated
captures. No route, auth check, database query, or production markup is mocked.
...
| Browser suite discovery | `npx playwright test e2e/report-styles.spec.ts --list` | Found 8 tests (Chromium + Mobile Chrome): two isolated workflow/race tests, one authenticated fixture-route test, and one DB-free exact-renderer test per browser. |
| DB-free exact-renderer matrix | `PLAYWRIGHT_SKIP_WEBSERVER=1 npx playwright test e2e/report-styles.spec.ts --project=chromium --grep 'DB-free exact renderer'` | Passed: 1 Playwright test, 12 style×variant cases, 12 Axe scans, 24 responsive overflow checks/screenshots, and 12 full Letter PDFs. `pdfinfo` confirmed 612×792 pt pages; per-page `pdftotext` confirmed all 111 physical pages nonblank with recurring confidentiality and matching `Page N of M`. |
| Whole E2E spec without app server | `PLAYWRIGHT_SKIP_WEBSERVER=1 npx playwright test e2e/report-styles.spec.ts` | 2 exact-renderer project cases passed (Chromium and Mobile Chrome); 6 authenticated/database cases skipped by their explicit environment gates. Mobile Chrome uses the suite's 60-second renderer timeout because it repeats all 12 PDF generations. |
| Fixture-only browser attempt | `NEXTAUTH_SECRET=<ephemeral> NEXTAUTH_URL=http://localhost:3000 E2E_REPORT_STYLES_PREVIEW_BASE_URL=http://localhost:3000 npx playwright test e2e/report-styles.spec.ts --project=chromium --grep 'fixture-only renderer'` | Failed as expected before rendering: signed JWT authenticated, then dashboard liveness query required `DATABASE_URL` at `src/app/(dashboard)/layout.tsx:32`. No browser/PDF success claimed. |
| Completed-campaign PDF — Boardroom | `e2e/report-styles.spec.ts` | **NOT RUN.** Requires a completed synthetic campaign/report URL. Exact-renderer PDF evidence is observed below, but it does not substitute for authorization/loading through the product route. |
...
Provision a disposable migrated PostgreSQL database and set the app's
`DATABASE_URL` to it, with the report-style feature enabled. The fixture setup
must create a unique organization/respondent plus three **Classic, unlocked**
...
The DB-free exact-renderer suite above is the observed renderer/CSS visual gate.
The separately retained authenticated fixture-route suite can additionally be
run against a Task 15 preview server with
`E2E_REPORT_STYLES_PREVIEW_BASE_URL`; it verifies the protected route itself
and its representative one-page captures.

Then run the production app and acceptance suite with the same isolated
database, inspect the Playwright output PNG/PDF files, and replace every `Not
run` cell above with the observed page count and any defect/resolution. The
workflow capture must verify full report routes—not the single-page preview
