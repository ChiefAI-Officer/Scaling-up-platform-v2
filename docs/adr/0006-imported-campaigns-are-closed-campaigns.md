...
- Idempotent re-import depends on the unique keys across all four entities: `AssessmentCampaign.externalId` (stored namespaced as `esperto:<campaignid>`); `(campaignId, respondentId)` on **participant and invitation**; and **`invitationId`** for the **submission** — *not* its `(campaignId, respondentId)`, which exists only as a raw-SQL partial index that Prisma's `upsert` cannot target. A re-run must **upsert** across the chain, never blind-insert.
