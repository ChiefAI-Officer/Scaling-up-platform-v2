...
- ⚠️ **AUDIENCE CLAUSE SUPERSEDED (2026-07-29, ADR-0027).** The "coach/admin-gated" property below described the *route*, not the artifact. A campaign may now opt in to showing an invited **respondent their own** report in place at submit (`AssessmentCampaign.showResultsOnScreen`, spec 19an). Everything else here stands: the report is still the canonical view, still per-individual with no cohort data, and there is still **no durable per-respondent results URL**.
