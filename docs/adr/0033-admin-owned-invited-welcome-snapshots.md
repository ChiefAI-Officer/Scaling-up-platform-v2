...
template-row Welcome default at request time and renders every schema-valid value,
including values equal to a code seed or migration backfill. Schema-v1 values
normalize to schema v2 before rendering because v1 lacks `sharingDescription`.
The Campaign's related Template is the sole content source; template, Campaign,
and URL aliases do not select copy. Only missing or malformed JSON retains the
standing public fallback and Campaign description. Question-bank facts (count,
...
- Existing public campaigns intentionally reflect their current schema-valid
  template Welcome copy without repinning or recreating the campaign, including
  seed- or backfill-equivalent values.
