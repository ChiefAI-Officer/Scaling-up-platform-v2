...
- governed reports retain the exact phase/frozen disclosure, while legacy reports
  now say `Peers use the governed historical baseline for reports scored before
  phase-aware peer snapshots were frozen. This is not an industry-, geography-, or
  cohort-matched comparison.`;
- one typed disclosure model owns both disclosure selection and the exact
  `Legacy baseline · <sourceId>` / `Phase P<n> · <sourceId>` formatting consumed by
  the generic and landscape reports;
