...
Task 7 is not approved and was not executed in this run. A future authorization to begin activation permits only guarded draft creation. The operator must capture the exact draft/source receipt and content hash, verify no historic or campaign mutation, obtain an independent review of that exact draft, and then stop. A second approval must name the exact reviewed code SHA and separately authorize PR/integration, merge, and Production deployment while the draft remains unpublished; the deployed SHA must be provider-Ready and pass canonical `/api/health`, or the process stops without publishing. Only then may a third, separate, explicit human publish/activation approval name the exact draft ID, content hash, actor, and verified deployed SHA. Create, deploy, and publish must never share one approval or receipt. Existing campaign pins and frozen submissions must never be rewritten.
