...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-09-01 LAST_UPDATED_SLUG:all-public-welcome-authority-implemented --> September 1 — **Every valid saved Public Welcome is now authoritative locally; verification and protected-branch release are in progress before the approved SunHub v7 successor cutover.** |
...
- **Admin-owned Welcome screens:** **INVITED LIFECYCLE LIVE; ALL-PUBLIC AUTHORITY CORRECTION IMPLEMENTED LOCALLY, RELEASE IN PROGRESS.** PR #327 squash-merged as `8433f9cf`; enabled deployment `dpl_C5mJHCDHwDgUTLidkjfbhC4SpoTx` is Ready on both canonical aliases. ADMIN/STAFF author eight safe plain-text Welcome fields in the first Build card, including the sharing explanation. Every future INVITED campaign freezes that default at creation; existing invited snapshots remain immutable. New Jeff acceptance evidence superseded PR #412's baseline discriminator: every schema-valid current Template Welcome value is authoritative for every existing/future PUBLIC link, while only absent/malformed data uses the anonymous fallback. The correction is not yet merged or deployed. Full detail: `plans/CHANGELOG.md` entries `all-public-welcome-authority-implemented`, `public-welcome-unedited-baseline-corrected`, `public-welcome-live-template-wiring-implemented`, `invited-welcome-sharing-explanation-implemented`, and `admin-owned-assessment-presentation-launched`.
