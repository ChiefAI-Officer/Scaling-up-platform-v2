...
| **Integrated main history** | **Jeff #65 stable reminder links are LAUNCHED** from PR #282 (squash `050573fa`) with the Production global flag enabled, the kill switch off, and the claim released. Its additive token-history migration backfilled all 103 invitations without a missing parent or duplicate hash/sequence group. **Jeff #48 shipped in PR #251** (squash `d676aa77`) after the protected gates. The assessment-email duplicate-delivery hotfix from PR #250 (`d4df6db1`) is launched and its controlled Inngest cutover is complete. Wave EV remains shipped from PR #241 (`cc370aa9`). **Wave OSR (#71) is LAUNCHED, not dark**; its Production flag is enabled and its fast kill remains the campaign checkbox. |
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-03 LAST_UPDATED_SLUG:gh-220-branded-invitation-html-dark --> August 3, 2026 — **GH #220 branded invitation HTML is implemented, locally verified, default-off, and not production-activated.** The new behavior flag composes campaign HTML inside the shared branded shell only after a separately authorized activation and fresh read-only audit. GH #257 remains claimed design-first with no implementation started; GH #228 report branding, GH #256 image policy, and GH #257 outbox behavior are unchanged. Full detail in CHANGELOG entry `gh-220-branded-invitation-html-dark`. |
...
| **Latest progress** | Jeff #65 is the latest launched product outcome. GH #217 is owner-accepted and closed after final receipt correction. GH #257 residual assessment-email outbox reconciliation is the next active design item; implementation remains gated on an approved recovery contract. |
