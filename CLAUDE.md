...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-11 LAST_UPDATED_SLUG:picker-visible-invitation-banner-canaries-final-review-fix --> August 11, 2026 — **The universal INVITED-email banner is independently reviewed, locally verified, and default-off.** Empty canary configuration now skips all banner visibility queries. Final gates passed 687 suites / 8,578 tests, all 47 migrations, and the 94/94-page Turbopack build. It is not merged, deployed, or activated. |
...
- **Universal invitation banner:** **IMPLEMENTED + LOCALLY VERIFIED + DEFAULT-OFF; independently reviewed and merge-ready.** INVITED initial/manual, fan-out, reminder, and resend emails share the platform-owned Scaling Up/Coach shell; custom HTML controls only the sanitized body. PUBLIC and results/report flows are excluded. Canary snapshots expose only authorized live Organization IDs and live picker-visible Template IDs; global/KILL/empty states expose none and skip visibility queries. `WAVE_INVITATION_BANNER_KILL` remains the containment switch. [Runbook](docs/specs/v7.6/17d-ops-runbook.md#universal-invitation-banner--dark-rollout-and-rollback) · [design](docs/superpowers/specs/2026-08-10-universal-invitation-email-banner-design.md) · [plan](docs/superpowers/plans/2026-08-10-universal-invitation-email-banner.md). Not deployed or activated.
