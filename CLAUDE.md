...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-11 LAST_UPDATED_SLUG:picker-visible-invitation-banner-canaries-release-ready --> August 11, 2026 — **Universal invitation banner is release-ready for independent whole-branch review and remains default-off; it is not merged, deployed, or activated.** The picker-visible canary fix passes configured Template IDs through the shared live campaign-picker scope and configured Organization IDs through existing organization access before serializing a browser snapshot. Deleted, disabled, invalid, inaccessible, and stale-grant Template IDs never enter that snapshot; it never contains the raw configured allowlist. The latest local gate passed 4 focused suites / 32 tests, all 47 migrations, a 94/94-page Turbopack build, and 687 full suites / 8,576 tests. Production flags were neither read nor changed, and no deployment, database write, or customer email occurred. |
