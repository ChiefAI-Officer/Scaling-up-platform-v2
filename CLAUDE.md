...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-07 LAST_UPDATED_SLUG:report-comparison-deployed-dark --> August 7, 2026 — **The Jeff-anchored individual report-comparison MVP is DEPLOYED + DEFAULT-OFF in Production.** PR #314 merged as `73ff4fab`; exact deployment `dpl_UsFiaQSBMjrph7sr94F3BXGWbgSE` is Ready on both canonical aliases, both health endpoints are healthy, and every report-comparison rollout/capability variable remains absent. Coach/Admin/STAFF and CEO self-only comparison code is present but dark; the existing group report remains unchanged. Full detail in CHANGELOG entry `report-comparison-deployed-dark`. |
