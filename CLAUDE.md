...
| **Latest progress** | Jeff #65 is implemented, locally verified, and open for review as PR #282; it remains default-off and is not a shipped or reportable product outcome. GH #217 is the latest launched reliability outcome from PR #280 and remains dormant because Production leaves `ASSESSMENT_INVITE_BRANDED` unset. |
...
- **GH #217 legacy invitation fallback hardening:** **MERGED + LIVE, dormant unless the existing kill switch is activated** from PR #280 (squash `a683e55d`) on Ready production deployment `dpl_uM3anRWbD3Nmzuqi3dStdEGyVfLW`. The dormant `ASSESSMENT_INVITE_BRANDED=0` renderer threads Coach identity, sends a plain-text twin, and supplies an escaped bottom fallback URL. Production still leaves the flag unset, so the branded renderer remains active; launch verification was read-only and sent no invitation. Full detail: `plans/CHANGELOG.md` entry `gh-217-legacy-invite-fallback-launched`.
