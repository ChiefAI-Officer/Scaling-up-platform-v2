...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-09-02 LAST_UPDATED_SLUG:sunhub-successor-transaction-timeout-hotfix --> September 2 — **The all-Public Welcome correction is live; the SunHub successor cutover remains unapplied while a bounded Prisma transaction-timeout hotfix clears protected release gates.** |
...
- **Admin-owned Welcome screens:** **INVITED + ALL-PUBLIC AUTHORITY LIVE; SUNHUB CUTOVER PAUSED BEFORE MUTATION.** PR #327 is `8433f9cf`; PR #414 is `83a18dc9`, deployed with healthy aliases. ADMIN/STAFF author eight Welcome fields. Future INVITED campaigns freeze immutable snapshots; every valid current Template value drives every PUBLIC link, with fallback only for missing/malformed data. The first guarded SunHub quiesce exceeded Prisma's default transaction timeout and rolled back; the source remains ACTIVE with 15 submissions while the bounded-timeout hotfix clears release gates. Full detail: `plans/CHANGELOG.md` entries `sunhub-successor-transaction-timeout-hotfix`, `all-public-welcome-authority-implemented`, `public-welcome-unedited-baseline-corrected`, `public-welcome-live-template-wiring-implemented`, `invited-welcome-sharing-explanation-implemented`, and `admin-owned-assessment-presentation-launched`.
