...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-17 LAST_UPDATED_SLUG:su-full-individual-peer-comparison-dark-verified --> August 17, 2026 — **The Scaling Up Full individual peer comparison is implemented and locally verified, but remains dark and is not deployed or activated.** The Classic overview and matching detail cards use paired You/Peers bars; `scaling-up-full` remains absent from `PEER_RENDER_ENABLED_ALIASES`, so ordinary runtime reports do not query benchmark rows or render the new UI. |
