...
| **Latest progress** | GH #256's narrow Circle-sync validation slice is the latest production-verified release; the separate host-policy decision remains open. The next consolidated report retains its existing seven-outcome window through GH #228. GH #220 branded invitation HTML is implemented, locally verified, default-off, and not production-activated; it remains excluded until its separate closeout. GH #257 residual assessment-email outbox reconciliation remains excluded while its separate implementation and closeout are incomplete. |
...
- **GH #220 branded campaign invitation HTML:** **IMPLEMENTED + LOCALLY VERIFIED + DEFAULT-OFF; not production-activated.** `WAVE_D_CUSTOM_HTML_EMAIL_ENABLED` remains the capability gate, and new `ASSESSMENT_INVITE_BRANDED_CUSTOM_HTML_ENABLED` controls branded-body composition without rewriting stored bytes. Activation remains a separately authorized operation: verify the branded-renderer kill switch `ASSESSMENT_INVITE_BRANDED` is not `"0"`, read the current Production flag state using the safe value-verification guidance below, run the dedicated read-only override audit, manually review every live override, write the new flag through the Vercel REST API as `type:"encrypted"` for `target:["production"]`, and redeploy. Audit mode classifications apply only while the branded renderer is active. GH #228 report branding, GH #256 image policy, and GH #257 outbox reconciliation remain out of scope. Full detail: `plans/CHANGELOG.md` entry `gh-220-branded-invitation-html-dark`.
- **GH #257 residual assessment-email outbox reconciliation:** **PRIMED + CLAIMED; DESIGN ONLY.** The remaining pre-database failure can commit a submission without creating its expected outbox row. The next gate must define reconstructible historical inputs and fingerprints, `(submissionId, recipientRole)` idempotency, campaign/template drift, and terminal-state eligibility before implementation. Branch `codex/257-outbox-reconciliation` is clean and no code has started. Send leases, provider delivery, email copy/chrome, production backfill, and unrelated outbox types are excluded until the design is approved.
