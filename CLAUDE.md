...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-07 LAST_UPDATED_SLUG:universal-individual-report-imported-provenance-correction --> August 7, 2026 — **Universal individual-report imported provenance is locally corrected and remains pending protected review plus hosted acceptance.** Executive Boardroom and Modern Dashboard preserve the safe canonical `Imported from Esperto (historical)` fact only when imported provenance is true, while continuing to hide submission IDs, version IDs, and content hashes. Public/non-imported results omit the marker. Production remains globally off with only the existing disposable exact-campaign canary live. Full detail in CHANGELOG entry `universal-individual-report-imported-provenance-correction`. |
