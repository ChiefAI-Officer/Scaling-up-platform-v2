...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-06 LAST_UPDATED_SLUG:template-creation-simplification-launched --> August 6, 2026 — **Template creation simplification and plain-language scoring are launched and globally enabled in Production.** PR #309 squash-merged as `1433d95e`; exact deployment `dpl_BBUMTPFiPV46RRarnko7iH9JmNbh` is Ready on all Production aliases, the enable flag is present, the kill is absent, and the health endpoint reports a healthy database and safe auth posture. Full detail in CHANGELOG entry `template-creation-simplification-launched`. |
