...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-06 LAST_UPDATED_SLUG:template-creation-simplification-implemented-locally --> August 6, 2026 — **Template creation simplification and plain-language scoring are implemented, reviewed, and verified locally; ready for a protected draft PR.** The release remains default-off, and authorization extends only to pushing the branch and opening that PR—not merging, deploying, enabling flags, or touching Production data. Full detail in CHANGELOG entry `template-creation-simplification-implemented-locally`. |
...
- **Scaling Up Full report-style selection:** **DEPLOYED DARK + EXACT-CAMPAIGN CANARY LIVE; global gate remains off.** PR #306 squash-merged as `44a6733c`; the canary-discovered draft-reset correction shipped through PR #307 as `f7617969`. Production has only `WAVE_REPORT_STYLES_CANARY` set to disposable campaign `cmsgz5uw900043ath2xee900i`, while `WAVE_REPORT_STYLES_ENABLED` and `WAVE_REPORT_STYLES_KILL` are absent. Synthetic Coach verification selected Executive Boardroom, saved with HTTP `200`, reloaded with the selection retained, and confirmed `CAMPAIGN_OVERRIDE` in Production. The campaign remains DRAFT, unlocked, and empty: zero participants, invitations, or submissions and no email. Broader activation and the synthetic completion/lock/report acceptance path remain separate gates. Runbook: [docs/runbooks/report-style-rollout.md](docs/runbooks/report-style-rollout.md); full evidence: `plans/CHANGELOG.md` entry `report-style-canary-launched`.
