...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-03 LAST_UPDATED_SLUG:gh-228-report-email-branding-pr-ready --> August 3, 2026 — **GH #228 Results report email branding is implemented and locally verified, default-off.** No production flag was enabled, no deployment or live email check was performed, and GH #220, #233, #256, and #257 remain untouched. Full detail in CHANGELOG entry `gh-228-report-email-branding-pr-ready`. |
...
- **GH #228 Results report email branding:** **IMPLEMENTED + LOCALLY VERIFIED + DEFAULT-OFF.** All three Results report roles now have Scaling Up-first email chrome and trusted frozen Coach-byline provenance when the dedicated global or exact-campaign canary gate is enabled; the dedicated kill restores legacy for new renders. `ASSESSMENT_SENDS_PAUSED` stops applicable new enqueue activity, while containing queued rows requires pausing both Inngest functions `quick-assessment-lead-email` and `quick-assessment-lead-email-cron` before quarantine or rollback. The approval hash, outbox schema, send leases, retries, and provider handoff are unchanged. No production flag was enabled, deployment or live email check was performed, and GH #220, #233, #256, and #257 were untouched. Full evidence: `plans/CHANGELOG.md` entry `gh-228-report-email-branding-pr-ready`.
