...
- ~~Per-recipient pre-send idempotency · immediate-path `executionId` synthesis · SEND_FILE_LINK/EMAIL_ATTENDEES FAILED-child writes~~ — **RESOLVED by audit PR-3 (#83, 2026-06-24):** `sendFanoutRecipients` + `ensureExecutionParent` (`deliveryBatchKey` dedup) cover all 3 attendee step types on both Inngest send paths, with terminal-vs-transient SMTP classification (`isTerminalSmtpError`).
- Deterministic parent.id via `inngestRunId` for forceResend audit trail
- Error redaction codes for `WorkflowStepExecution.errorMessage` — **SMTP terminal paths DONE (PR #93, 2026-06-26):** stable codes (`smtp_auth_failed`/`smtp_send_failed`) via `redactSmtpError`, raw kept in `console.error`. **Still TODO:** the general function-level step-failure recorder (`execute-workflow.ts`) stores raw `err.message` for *any* error (incl. transient-SMTP host/IP) → needs a general, non-SMTP-specific redactor.
...
- ENH-MAY6-6 — affiliate provider switch (owner decision needed)
