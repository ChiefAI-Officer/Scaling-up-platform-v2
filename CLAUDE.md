...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-07 LAST_UPDATED_SLUG:universal-individual-report-appearances-launched --> August 7, 2026 — **Universal individual-report appearances are merged, deployed, and globally enabled in Production.** PR #311 squash-merged as `ab7dacca`; enabled deployment `dpl_D7WnRzpiuY1PGMukp9LAjVqsANyV` is Ready on `platformtest.scalingup.com`, with healthy database/auth posture. Scored, qualitative, custom, and public individual reports can use Classic, Executive Boardroom, or Modern Dashboard on screen and in print/PDF. Authenticated Admin/Coach visual acceptance remains the final manual closeout check because no signed-in browser session was available. Full detail in CHANGELOG entry `universal-individual-report-appearances-launched`. |
