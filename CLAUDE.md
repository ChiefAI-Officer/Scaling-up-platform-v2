...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-11 LAST_UPDATED_SLUG:coach-profile-fields-aligned-release-ready --> August 11, 2026 — **Coach profile-field alignment is locally verification-complete.** Professional Title now maps to `Coach.title` and Company Name to `Coach.company` across profile, BIO, Circle, and new landing/default paths; legacy reads retain a company fallback without a migration or data rewrite. The post-fix full Jest and authoritative Turbopack gates passed, while authenticated local visual acceptance remains pending because the displayed localhost demo credentials are invalid and no authorized local test account is available. Full evidence is in CHANGELOG entry `coach-profile-fields-aligned-release-ready`. |
...
- **Coach profile-field alignment:** **LOCAL VERIFICATION COMPLETE; not deployed.** `Coach.title` and `Coach.company` are separated across Admin/self-profile, BIO, Circle, coach creation/details, landing defaults, and duo-workshop metadata. Legacy reads retain the company fallback; no migration, data rewrite, or saved-snapshot rewrite occurred. The full suite passed **688 suites / 8,531 tests / 16 snapshots** and Turbopack generated **94/94** pages. Authenticated visual acceptance remains pending because no valid authorized local account is available. Full detail: `plans/CHANGELOG.md` entry `coach-profile-fields-aligned-release-ready`.
