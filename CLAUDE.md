...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-09-01 LAST_UPDATED_SLUG:public-welcome-live-template-wiring-implemented --> September 1 — **Public Welcome edits now render on existing Public campaign links; implementation is locally verified and not yet merged.** |
...
- **Admin-owned Welcome screens:** **INVITED LIFECYCLE LAUNCHED + GLOBALLY ENABLED + PRODUCTION-VERIFIED; PUBLIC live-template follow-up implemented locally.** PR #327 squash-merged as `8433f9cf`; enabled deployment `dpl_C5mJHCDHwDgUTLidkjfbhC4SpoTx` is Ready on both canonical aliases. ADMIN/STAFF author eight safe plain-text Welcome fields in the first Build card, including the sharing explanation. Every future INVITED campaign freezes that default at creation; existing invited snapshots remain immutable. The pending PUBLIC follow-up reads the current template default live so existing public links reflect saved edits immediately, without changing Coach campaign controls or invited snapshot semantics. Full detail: `plans/CHANGELOG.md` entries `public-welcome-live-template-wiring-implemented`, `invited-welcome-sharing-explanation-implemented`, and `admin-owned-assessment-presentation-launched`.
