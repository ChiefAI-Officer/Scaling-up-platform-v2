...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-01 LAST_UPDATED_SLUG:gh-217-legacy-invite-fallback-launched --> August 1, 2026 (Asia/Manila) — **GH #217 legacy invitation fallback hardening is MERGED + LIVE, dormant unless the existing kill switch is activated.** PR #280 squash-merged as `a683e55d` on Ready deployment `dpl_uM3anRWbD3Nmzuqi3dStdEGyVfLW`; launch-record PR #281 then squash-merged as `6ba3221e` on exact Ready deployment `dpl_Cdk5VxnKcjs6RofueeAJw9tGVzA9`, with both health aliases returning HTTP 200 / healthy / safe afterward. Production still leaves `ASSESSMENT_INVITE_BRANDED` unset, so the branded renderer remains active; no flag, invitation, or production data was changed during verification. Full detail in CHANGELOG entry `gh-217-legacy-invite-fallback-launched`. |
...
| **Latest progress** | GH #217 **legacy invitation fallback hardening is Complete and LAUNCHED** from PR #280. The hardened renderer remains dormant because Production still leaves `ASSESSMENT_INVITE_BRANDED` unset. This is the fifth reportable product or reliability outcome after the sent July 27–31 report, following GH #222, GH #242, GH #243, and GH #224. |
