...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-27 LAST_UPDATED_SLUG:summary-reporting-final-aggregate-fix --> August 27, 2026 — **Summary Reporting final aggregate fix: locally verified and independently reviewed, NOT LAUNCHED.** All eight final-review findings addressed; no new scoped defects. Uncertain-commit artifact retention, frozen tier policy, safe wizard errors, bounded frozen coach images and exact JSON/UTC persistence corrected. Private Blob/Redis, deployment, canary and actual-output visual acceptance remain pending. See CHANGELOG entry `summary-reporting-final-aggregate-fix`. |
...
| **Latest progress** | GH #242 **retired pinned-edition warning is Complete and LAUNCHED** from PR #273. Campaign detail now preserves the served-edition provenance and uses a destructive warning when that exact edition is retired; retirement takes precedence over the stale-edition warning. This is the second reportable product outcome after the sent July 27–31 report, following GH #222. |
...
- **Summary Reporting foundation + Scaling CEO Full:** implemented behind `SUMMARY_REPORTING_ENABLED` / exact-campaign `SUMMARY_REPORTING_CANARY` / winning `SUMMARY_REPORTING_KILL`; local proof only. Keep global off until the actual canary UI/PDF is accepted. Dedicated private Blob and distributed Redis are prerequisites. [Approved design](docs/superpowers/specs/2026-08-27-summary-group-reporting-design.md), [local evidence and rollout](docs/research/evidence/summary-reporting-local-proof/README.md), [visual differences/gate](docs/research/evidence/summary-reporting-scaling-ceo-full-renderer-v1-review.md). Other families remain separate future slices; this is not a shipped outcome.
- **Jeff July-10 #65 stable reminder links:** claimed on issue #261 and product contract locked in `docs/specs/v7.6/20-jeff-65-stable-reminder-links-contract.md`. This is **design only**: current production still overwrites the invitation's sole token hash after a successful reminder, invalidating older links. Technical design, implementation, merge, and launch remain pending; manual Resend is outside the item unless separately approved.
