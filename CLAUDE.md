...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-05-18 LAST_UPDATED_SLUG:assessment-v7-6-task-g-access-group-ui --> May 18, 2026 — Assessment Tool v7.6 Task G admin AccessGroup management UI shipped (Issue #10). Closes the real ops gap from Tasks A–F: admins can now create / edit / archive AccessGroups and add+remove coaches + templates via the app instead of manual scripts. Wires Wave 5 wireframes 21 (list) + 22 (detail with evaluateAccessChange preview modal) into the real product on top of Task A's transactional guard. 9 new admin-only API routes + a DRY-RUN preview endpoint (`POST /api/admin/access-groups/[id]/preview-change`) that wraps evaluateAccessChange in a rolled-back transaction so the UI can render the per-coach BEFORE/AFTER diff before the admin commits. Two-stage UI flow: preview → if 409 BLOCKED_ZERO_ACCESS, second-stage modal requires non-empty `forceReason` audit-log entry. 1603 tests (+35 from Task G across 6 suites). Prior Tasks A–F (full v1 INVITED loop: service-layer, org/team/respondent CRUD, coach portal wizard, magic-link survey, admin aggregate, coach detail page) all live on prod and unchanged by this task. |
