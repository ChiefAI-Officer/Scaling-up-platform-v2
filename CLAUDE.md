...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-07-31 LAST_UPDATED_SLUG:gh-222-welcome-screen-accuracy-launched --> July 31, 2026 — **GH #222 Welcome-screen question-bank accuracy is MERGED + LIVE.** PR #269 merged as squash `b350ff22`; the exact commit deployed successfully and production health is green. A scale is described only when every respondent-visible question shares one valid slider range; mixed banks use neutral format-aware wording and omit only the scale chip. Time estimates, question/section chips, APIs, stored data, and assessment content are unchanged. Full detail in CHANGELOG entry `gh-222-welcome-screen-accuracy-launched`. |
