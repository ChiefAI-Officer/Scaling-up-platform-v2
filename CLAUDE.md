...
- **🔴 The PRODUCTION Vercel project is under the `scaling-up` team, NOT `chief-aio-fficer` (corrected 2026-07-29)**: **two** projects are named `scaling-up-platform-v2`, and the older guidance in this file pointed at the wrong one. Ground truth, verified via the REST API on 2026-07-29:
  ...
  **The local `.vercel` link is STALE** (`vercel env pull` → "Your Project was either deleted, transferred… or you don't have access"), and `vercel link` writes `.vercel/` + edits `.gitignore` — avoid it. Read env via the REST API with the CLI's own token (`~/Library/Application Support/com.vercel.cli/auth.json` → `.token`): `GET https://api.vercel.com/v9/projects/<id>/env?teamId=<team>&decrypt=true`. **`decrypt=true` did NOT decrypt on this plan** — non-empty values come back as ~970-char ciphertext, so you can reliably tell **set vs empty** but NOT `"1"` vs `"0"`. That distinction is usually enough, because `isOn()` only accepts `"1"|"true"|"TRUE"|"yes"`. Never paste secret values anywhere; filter to the `WAVE_*` keys you actually need.
- **Prod feature-flags: verify VALUES via the Vercel CLI, and WRITE them via the REST API — NOT piped `vercel env add` (2026-07-22, ROOT-CAUSED + FIXED)**: An authenticated `vercel env pull --environment=production` audit found ~15 "launched" wave-flags (`WAVE_ED9/ED10/O/X/P/Q/S/T/U/U3/V/W`, plus `ED1/2/4/6/8`) stored EMPTY (OFF) in Production despite the docs. **Proven cause:** `vercel env add <KEY> production` fed from **piped stdin** (`printf '1' | …`) silently stores an EMPTY value (reproduced live: add via pipe → pull shows `=""`; add the same key via REST → pull shows `="1"`). It is **NOT** `scripts/push-env-to-vercel.mjs` — that script only touches keys present in local `src/.env` (the wave toggles aren't there — only `WAVE_O_ESPERTO_IMPORT_HASH_SALT` is) and its line-172 guard skips empty values before the delete; **no SKIP-list change is needed** (the earlier suspicion was wrong). **Correct write path = the Vercel REST API** (`POST /v10/projects/{id}/env`, `type:"encrypted"`, `target:["production"]`) — the same path `push-env-to-vercel.mjs` uses, which is why `WAVE_D_*`/`F`/`J`/`M`/`N`/`WORKSHOP_CUSTOM_HTML` stayed "1". After any env change, **redeploy** (env is injected at build time). On 2026-07-22 the 12 clean flags were re-set via REST + redeployed (build `mcflc46pq`) and live-verified as admin: **9 confirmed live in-app** (ED9/ED10/O/X/Q/T/U/W/V); **S** flag-on but the LVA "Peer averages" panel needs the published version to expose SLIDER_LIKERT keys (follow-up); **P/U3** flag-on but only observable in a sent email. `isOn("")` is false. ⚠️ **CORRECTION (2026-07-29) — the previous claim here, that `ED1` Test Mode and `ED8` version lifecycle "are now flagless", was WRONG and is retracted.** Both are still flag-gated in code and both flags are **EMPTY in Production**, so **both features are DARK in prod today** despite being recorded as launched:
  - `isOn()` accepts only `"1"|"true"|"TRUE"|"yes"` (`wave-ed1-flags.ts:8-10`); there is **no default-on path**, so an empty value is unambiguously OFF.
  - **ED1** → `TabbedShell.tsx:574` `testModeAvailable = !isPublished && testModeEnabled`, gating the Test Mode button (`:756`) and drawer (`:1117`). Both hidden.
  - **ED8** → `VersionsTab.tsx:124` early-returns, and the APIs **404 opaquely** (`versions/[versionId]/route.ts:468`, `.../archive/route.ts:82`).
  - Prod values confirmed 2026-07-29 on the correct project: `WAVE_ED1_TEST_MODE_ENABLED`, `WAVE_ED2/ED4/ED6/ED8` all **empty**; `WAVE_S_PEER_BENCHMARKS_ENABLED` set with `KILL` empty; `WAVE_Q_ADMIN_CONTROLS_ENABLED` set.
  **Root cause of the bad claim:** the 2026-07-22 re-flip deliberately excluded `ED1/2/4/6/8` as "superseded". That is right for the *presentation* waves (ED2/ED4/ED6, superseded by ED10 — ADR-0024) but **wrong for ED1 and ED8, which are distinct capabilities ED10 does not replace**; the "they render anyway" sighting that produced the flagless conclusion was a misread. **ED10 does NOT depend on ED6** — `isPreviewSettingsEnabled()` reads only its own ENABLED/KILL, and the edit page passes `singleColumnEnabled` / `previewSettingsEnabled` independently, so the ED6-empty/ED10-on pairing is *not* an inconsistency.
  **Standing lesson:** a live sighting is only truth if you can name the gate it passed through. Prefer reading the gate in code + the flag value over "I saw it render". Detail in memory `project_prod_flag_state_discrepancy` + CHANGELOG `prod-flag-reflip-live-verify` and `flag-state-correction-ed1-ed8`.
