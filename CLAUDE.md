...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-10 LAST_UPDATED_SLUG:admin-owned-assessment-presentation-release-ready --> August 10, 2026 — **Admin-owned invited Welcome screens and Coach report-style simplification are release-ready and default-off.** Every future invited campaign freezes its template Welcome default; the Admin Build card owns authoring, Coach report-appearance controls disappear only when enabled, and PUBLIC campaigns remain separate. Full local evidence is in CHANGELOG entry `admin-owned-assessment-presentation-release-ready`; nothing is yet pushed, merged, deployed, enabled, or written to Production. |
...
- **Admin-owned invited Welcome screens:** **BUILT + LOCALLY VERIFIED + DEFAULT-OFF; protected review and Production rollout pending.** ADMIN/STAFF author seven safe plain-text Welcome fields in the first Build card; every new INVITED campaign receives an immutable creation-time snapshot even while the presentation flag is off or killed, so active and historical campaigns never change retroactively. Coach campaign creation/detail lose Report appearance controls only while enabled and inherit the assessment default; PUBLIC quiz behavior remains separate. The additive migration, exact-value/trigger verifier, rollback runbook, response-contract stripping, and independent clean review are complete. Full detail: `plans/CHANGELOG.md` entry `admin-owned-assessment-presentation-release-ready`.
