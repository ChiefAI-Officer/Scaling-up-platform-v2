...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-11 LAST_UPDATED_SLUG:universal-invitation-banner-dark-merge --> August 11, 2026 — **Universal invitation banner implementation is complete, locally verified, and default-off; it is not deployed or activated.** Final review enforces INVITED-only authoring, gives the universal renderer precedence over the old non-universal `ASSESSMENT_INVITE_BRANDED=0` switch, and prevents raw cross-tenant canary IDs from entering the browser snapshot. The receipt records 20 focused suites / 429 tests and 686 full suites / 8,568 tests; Production flags were neither read nor changed, and no deployment, database write, or customer email occurred. Full evidence is in CHANGELOG entry `universal-invitation-banner-dark-merge`. |
...
- **Universal invitation banner:** **IMPLEMENTED + LOCALLY VERIFIED + DEFAULT-OFF; not deployed or activated.** The separate `WAVE_INVITATION_BANNER_ENABLED`, exact-ID `WAVE_INVITATION_BANNER_CANARY`, and overriding `WAVE_INVITATION_BANNER_KILL` controls cover only INVITED initial/manual sends, automatic fan-out, reminders, and resends. While active, the platform owns the universal shell and treats permitted custom HTML as a sanitized body fragment without synchronizing `ASSESSMENT_INVITE_BRANDED_CUSTOM_HTML_ENABLED`; PUBLIC campaign authoring and results/report email paths remain excluded. `ASSESSMENT_INVITE_BRANDED=0` remains a non-universal renderer switch and does not disable the universal path; `WAVE_INVITATION_BANNER_KILL` is the universal containment switch. Create-page snapshots contain only canary Organization/Template IDs the signed-in actor can access, never the raw configured allowlist. Fresh validation recorded 20 focused suites / 429 tests, migration safety for all 47 migrations, changed-file ESLint, a 94/94-page Turbopack build, and 686 full suites / 8,568 tests. Standalone `tsc --noEmit` remains nonzero with 422 pre-existing diagnostics across 70 broad test/config files; the branch-local nullable Organization-ID regression was corrected and its 42-test detail-route suite passes. The safe read-only audit, all seven synthetic capture states, exact canary, Ready/health, and organic telemetry sequence are in [the ops runbook](docs/specs/v7.6/17d-ops-runbook.md#universal-invitation-banner--dark-rollout-and-rollback). Design: [universal invitation banner design](docs/superpowers/specs/2026-08-10-universal-invitation-email-banner-design.md); plan: [universal invitation banner plan](docs/superpowers/plans/2026-08-10-universal-invitation-email-banner.md). No Production flag was read or mutated; no Production deployment, database write, or customer email occurred. |
