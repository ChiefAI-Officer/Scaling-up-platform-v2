...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-30 LAST_UPDATED_SLUG:summary-reports-coach-dropdown-implemented --> August 30 — **Summary Reports coach-only dropdown is merge-authorized; the Production canary was cleared and the pre-merge redeploy is Ready.** |
...
- **Public Assessments Coach removal:** **IMPLEMENTED + LOCALLY VERIFIED; NOT MERGED OR DEPLOYED.** An eligible referring Coach can confirm removal of a submission from their own Public Assessments collection. A purpose-specific tombstone hides it from every Coach collection/export/report read while preserving the submission and ADMIN/STAFF oversight; the ownership write and bounded audit commit atomically. Full detail: `plans/CHANGELOG.md` entry `referred-results-coach-removal-implemented`.
