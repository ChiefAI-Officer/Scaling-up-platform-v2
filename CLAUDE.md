...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-09-01 LAST_UPDATED_SLUG:public-assessment-contact-fields-implemented --> September 1 — **Per-template Public assessment contact fields PR #409 open and merge-authorized; hosted checks pending, not deployed.** |
...
- **Admin-owned invited Welcome screens:** **LAUNCHED + GLOBALLY ENABLED + PRODUCTION-VERIFIED; sharing-explanation extension implemented locally.** PR #327 squash-merged as `8433f9cf`; enabled deployment `dpl_C5mJHCDHwDgUTLidkjfbhC4SpoTx` is Ready on both canonical aliases. ADMIN/STAFF author eight safe plain-text Welcome fields in the first Build card, including the sharing explanation; every future INVITED campaign freezes that default at creation. Schema-v1 snapshots remain readable without stored-data rewrites, while explicit saves and new campaigns write v2. Coach campaign creation/detail and PUBLIC Welcome behavior remain unchanged. Full detail: `plans/CHANGELOG.md` entries `invited-welcome-sharing-explanation-implemented` and `admin-owned-assessment-presentation-launched`.
