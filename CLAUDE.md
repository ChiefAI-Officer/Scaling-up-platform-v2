...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-09-01 LAST_UPDATED_SLUG:public-welcome-unedited-baseline-corrected --> September 1 — **Edited Public Welcome copy remains live while untouched templates retain the anonymous public presentation; PR #412 is live and production-verified.** |
...
- **Summary Reporting:** B2 locally adds CEO-only Condensed as one-click, two-page HTML; not deployed. Comparison remains B3. [Receipt](docs/research/evidence/summary-reporting-production-release-2026-08-27.md).
...
- **Admin coach-password actions:** **LAUNCHED + GLOBALLY ENABLED + PRODUCTION-VERIFIED.** PR #358 merged as `b26dc685`; Ready deployment `dpl_3rKZaLtqTcrWtCP7ckNJzcyZezao` owns both healthy aliases. The capability flag is present and kill absent. Full evidence: `plans/CHANGELOG.md` entry `admin-coach-password-actions-launched`.
- **Universal invitation banner:** **LAUNCHED + GLOBALLY ENABLED + PRODUCTION-VERIFIED.** INVITED initial/manual, fan-out, reminder, and resend emails now share the platform-owned Scaling Up/Coach shell; custom HTML controls only the sanitized body. PUBLIC and results/report flows remain excluded. PR #331 squash-merged as `dc0026fd`; enabled deployment `dpl_H7JnKCB9eXoMoYF4cUDthxnG1sCy` owns both healthy canonical aliases. `WAVE_INVITATION_BANNER_ENABLED=1` is encrypted in Production; CANARY and KILL are absent, with KILL available for immediate containment. [Runbook](docs/specs/v7.6/17d-ops-runbook.md#universal-invitation-banner--dark-rollout-and-rollback).
- **Coach profile-field alignment:** **LAUNCHED + PRODUCTION-HEALTH-VERIFIED.** PR #333 squash-merged as `8b03c948`; Ready deployment `dpl_AdSEqp9H4HxToFuCbRTQgKU1EHXX` owns both healthy canonical aliases. `Coach.title` and `Coach.company` are separated across Admin/self-profile, BIO, Circle, coach creation/details, landing defaults, and duo-workshop metadata, with a legacy read fallback and no migration or saved-snapshot rewrite. The merged tree passed **688 suites / 8,538 tests / 16 snapshots** and Turbopack generated **94/94** pages. Authenticated visual acceptance remains pending because no valid authorized account is available. Full detail: `plans/CHANGELOG.md` entry `coach-profile-fields-aligned-launched`.
- **Report preview disclosure:** **LAUNCHED + PRODUCTION-HEALTH-VERIFIED.** PR #332 squash-merged as `9cbfa304`; Vercel completed and both canonical health aliases are healthy. Report Appearance pickers now use compact responsive tiles with accessible on-demand previews while preserving selection/page state. Simplified public creation and summary-only Campaign Detail remain unchanged. No schema, API, report-output, flag, environment, or customer-data change occurred. Pre-merge read-only visual QA covered the affected Admin/Coach screens at 1280px and 393px; no authenticated Production account was used. Full detail: `plans/CHANGELOG.md` entry `report-preview-disclosure-launched`.
...
- **Admin-owned Welcome screens:** **INVITED LIFECYCLE LAUNCHED + GLOBALLY ENABLED + PRODUCTION-VERIFIED; PUBLIC edited-template correction LIVE + PRODUCTION-VERIFIED.** PR #327 squash-merged as `8433f9cf`; enabled deployment `dpl_C5mJHCDHwDgUTLidkjfbhC4SpoTx` is Ready on both canonical aliases. PR #412 squash-merged as `c51af95e`; Production deployment `6201111666` is successful and healthy on the exact deployment plus both canonical aliases. ADMIN/STAFF author eight safe plain-text Welcome fields in the first Build card, including the sharing explanation. Every future INVITED campaign freezes that default at creation; existing invited snapshots remain immutable. PUBLIC links use current template copy only after it differs from the normalized code baseline; untouched templates retain the anonymous public presentation and Campaign description. Full detail: `plans/CHANGELOG.md` entries `public-welcome-unedited-baseline-corrected`, `public-welcome-live-template-wiring-implemented`, `invited-welcome-sharing-explanation-implemented`, and `admin-owned-assessment-presentation-launched`.
...
- **Jeff July-10 assessment-feedback closeout:** the tracked [53-row canonical ledger](docs/agents/jul10-feedback-closeout.md) is authoritative at **50 DONE / 0 PARTIAL / 3 NEEDS DECISION**. #47 is DONE because its exact coach-logo, coach-name, revised-copy, and larger-CTA ask is implemented and live; a received-email smoke is optional QA. Only #33, #41, and #45 remain decision-gated. Exact Section 2 wording is the acceptance boundary; generated PDFs and `tmp/` dictionaries remain derivatives.
...
- **Jeff July-10 closeout rows:** use the [canonical ledger](docs/agents/jul10-feedback-closeout.md) for each exact gate. Only #33 needs one annotated report pair and #41/#45 need content intent. #32, #44, #47, #57/#58, #75, and #84 are closed. A controlled received-email smoke, cohort-matched Scaling Up Full norms, persistent LVA peer data, and optional #44 copy clarification are separately scoped follow-ons.
...
- No unsanitized HTML in JSX; custom HTML requires server sanitization and one typed render seam.
