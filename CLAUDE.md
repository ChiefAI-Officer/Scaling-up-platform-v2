...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-06 LAST_UPDATED_SLUG:report-style-rollout-prepared --> August 6, 2026 — **Scaling Up Full individual report styles are prepared for a controlled dark rollout, not enabled in Production.** `CLASSIC` remains the migration, flag-off, kill, and fallback behavior until both new styles receive authoritative isolated production-route visual acceptance. Full detail in CHANGELOG entry `report-style-rollout-prepared`. |
...
- **Scaling Up Full report-style selection:** **PREPARED FOR DARK ROLLOUT; Production remains Classic.** The additive enum migration defaults template and campaign records to `CLASSIC`; `WAVE_REPORT_STYLES_ENABLED`, `WAVE_REPORT_STYLES_CANARY`, and `WAVE_REPORT_STYLES_KILL` gate only `scaling-up-full` individual scored on-screen/browser-print reports, with kill precedence and no stored-choice erasure. Admin sets the future-campaign template default in ED10 Settings; the owning coach may override a campaign only before its first completion, which atomically locks the choice. The supplemental component-renderer evidence passed 2 styles × 6 variants, 12 Axe scans, 24 responsive screenshots, and 12 Letter PDFs / 111 physical pages, but the authoritative isolated admin → campaign → coach → submission → lock → report workflow is **NOT RUN**: this worktree has no disposable fixture database and strong sentinel. Do not enable beyond Classic until that workflow passes and both new styles receive visual acceptance. Runbook: [docs/runbooks/report-style-rollout.md](docs/runbooks/report-style-rollout.md); full evidence: `plans/CHANGELOG.md` entry `report-style-rollout-prepared`.
