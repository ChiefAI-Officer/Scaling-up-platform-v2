...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-11 LAST_UPDATED_SLUG:coach-profile-fields-aligned-release-ready --> August 11, 2026 — **Coach profile-field alignment is locally verification-complete.** Professional Title now maps to `Coach.title` and Company Name to `Coach.company` across profile, BIO, Circle, and new landing/default paths; legacy reads retain a company fallback without a migration or data rewrite. All local code gates passed, but authenticated local visual acceptance is pending because the in-app browser became unavailable after a configured local NextAuth retry. Full evidence is in CHANGELOG entry `coach-profile-fields-aligned-release-ready`. |
