...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-01 LAST_UPDATED_SLUG:gh-217-legacy-invite-fallback-launched --> August 1, 2026 — **GH #217 legacy invitation fallback hardening is MERGED + LIVE, dormant unless the existing kill switch is activated.** PR #280 squash-merged as `a683e55d`; exact production deployment `dpl_uM3anRWbD3Nmzuqi3dStdEGyVfLW` is Ready and both health aliases are safe. Production still leaves `ASSESSMENT_INVITE_BRANDED` unset, so the branded renderer remains active; no flag, invitation, or production data was changed during verification. Full detail in CHANGELOG entry `gh-217-legacy-invite-fallback-launched`. |
