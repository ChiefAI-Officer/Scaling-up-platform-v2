...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-11 LAST_UPDATED_SLUG:coach-profile-fields-aligned-launched --> August 11, 2026 — **Coach profile-field alignment is LAUNCHED + PRODUCTION-HEALTH-VERIFIED.** PR #333 squash-merged as `8b03c948`; exact Vercel deployment `dpl_AdSEqp9H4HxToFuCbRTQgKU1EHXX` is Ready on both canonical aliases, whose health endpoints report healthy database and safe auth posture. Authenticated visual acceptance remains pending because no valid authorized test account is available. Full evidence is in CHANGELOG entry `coach-profile-fields-aligned-launched`. |
