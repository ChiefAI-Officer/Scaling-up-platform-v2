...
| **Integrated main history** | **Coach mobile drawer containment fix is RELEASE-READY** with protected-main merge and Production verification pending. **SU Full Esperto-faithful landscape is LAUNCHED + ENABLED + PRODUCTION-VERIFIED** from PR #367 (`c1c76082`) on deployment `dpl_GKWACwsP9AocxTJr87E2BeNRPhPv`. **GH #233 benchmark auditability is LAUNCHED** from PR #300 (`df3f2f51`). **GH #257 email reconciliation, GH #220 branded campaign HTML, and GH #228 report-email branding are LAUNCHED + DEFAULT-OFF** from PRs #296, #292, and #288. **Jeff #65 stable reminder links are LAUNCHED** from PR #282 (`050573fa`), with 103 invitation histories backfilled. **Jeff #48 shipped in PR #251** (`d676aa77`). The PR #250 email dedup hotfix and its Inngest cutover are complete. Wave EV remains shipped from PR #241 (`cc370aa9`). **Wave OSR (#71) is LAUNCHED** with its campaign kill retained. |
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-20 LAST_UPDATED_SLUG:coach-mobile-drawer-containment-fixed --> August 20 — **Coach mobile drawer containment fix implemented, visually verified, and authorized for Production.** |
...
- **Coach mobile drawer containment:** **IMPLEMENTED + LOCALLY VERIFIED + USER-AUTHORIZED FOR PRODUCTION; PROTECTED-MAIN MERGE/DEPLOYMENT PENDING.** The responsive Coach drawer is portalled to `document.body`, escaping the sticky header's backdrop-filter containing block while preserving flag-off markup and outside-click dismissal. The real 390×844 browser receipt measures an 844px overlay/panel instead of the broken 64px containing block. Evidence: `plans/CHANGELOG.md#coach-mobile-drawer-containment-fixed`.
