...
- **Prod feature-flag VALUES must be verified via the Vercel CLI — do NOT trust CHANGELOG "launched" claims (2026-07-22)**: An authenticated `vercel env pull --environment=production` audit found the durable Production values for `WAVE_ED9/ED10/O/X/P/Q/S/T/U/U3/V/W` are EMPTY (OFF), contradicting their documented launches; only `WAVE_D_*`, `WAVE_F`, `WAVE_J`, `WAVE_M`, `WAVE_N`, `WORKSHOP_CUSTOM_HTML_EDITOR`, and (newly) `WAVE_QSP_ROCK_GROUP_REPORT_ENABLED` = "1". Git auto-deploy is ON, so the live build compiled with those OFF → those features are likely **dark in prod pending live-app confirmation**. Before reporting any wave as "live," confirm on the live app; before re-flipping en masse, root-cause the durability gap (suspect `scripts/push-env-to-vercel.mjs` clobbering wave-flags with an empty local `.env` — add them to its SKIP list). Detail in memory `project_prod_flag_state_discrepancy`.
