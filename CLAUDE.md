...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-11 LAST_UPDATED_SLUG:universal-invitation-banner-dark-merge --> August 11, 2026 — **Universal invitation banner implementation is complete, locally verified, and default-off; it is not deployed or activated.** The three absent/default-off `WAVE_INVITATION_BANNER_*` controls preserve current output until separately authorized rollout. The receipt records 14 focused suites / 301 tests and 685 full suites / 8,555 tests; Production flags were neither read nor changed, and no deployment, database write, or customer email occurred. Full evidence is in CHANGELOG entry `universal-invitation-banner-dark-merge`. |
