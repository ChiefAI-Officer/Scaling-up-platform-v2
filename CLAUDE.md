...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-06 LAST_UPDATED_SLUG:report-style-canary-picker-fix-prepared --> August 6, 2026 — **Scaling Up Full individual report styles are deployed dark and enabled only for one disposable exact-campaign canary.** Live verification found an unsaved picker choice could be reset by an equivalent server-projection identity change; the test-first correction is prepared for protected review while the global gate remains off. Full detail in CHANGELOG entry `report-style-canary-picker-fix-prepared`. |
...
- **Scaling Up Full report-style selection:** **DEPLOYED DARK + EXACT-CAMPAIGN CANARY ONLY; global gate remains off.** PR #306 squash-merged as `44a6733c`; Production has only `WAVE_REPORT_STYLES_CANARY` set to disposable campaign `cmsgz5uw900043ath2xee900i`, while `WAVE_REPORT_STYLES_ENABLED` and `WAVE_REPORT_STYLES_KILL` are absent. Live Coach verification exposed a client synchronization defect that reverted an unsaved choice to Classic before save; a focused test-first correction is prepared for protected review and broader activation remains blocked. The additive enum migration still defaults template and campaign records to `CLASSIC`, and no real campaign, response, invitation, or email was used. Runbook: [docs/runbooks/report-style-rollout.md](docs/runbooks/report-style-rollout.md); full evidence: `plans/CHANGELOG.md` entry `report-style-canary-picker-fix-prepared`.
