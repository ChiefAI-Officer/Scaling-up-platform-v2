...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-07-19 LAST_UPDATED_SLUG:ed10-preview-settings-merged-dark --> July 19, 2026 — **Wave ED10 (Metadata→Preview tab + one Settings tab) BUILT + MERGED DARK; launch BLOCKED by a Neon Free-tier compute-quota outage.** PR #195 squash-merged to `main` (`cfe21566`), behind flag `WAVE_ED10_PREVIEW_SETTINGS_ENABLED` (+ `_KILL`), effective ONLY when ED9 forms mode is also on (`ed10Active = previewSettingsEnabled && formsBuildEnabled && singleColumn`); **flag-OFF byte-identical to ED9**. Ticket #6 of the 10-ticket editor-simplification pipeline. The editor's **Metadata** tab becomes a read-only **Preview** tab (the questionnaire rendered exactly as respondents see it — the real `SectionPager` in a new additive default-off `previewMode`; Active↔draft version toggle; SU-Full non-CEO audience filter applied so no CEO-only leak) and all settings fold into one plain-language **Settings** tab (Access read-only · humanized aggregation · language · both emails · Access-groups link · read-only alias). **Split save model** (co-validate revision): invitation email + language stay Save-Draft (invitation email is in the version `contentHash`); aggregation + results email use explicit per-card **Save** editable-while-published, with the SEC-H2 approval interlock (approval disabled while the results-email card is dirty; approve binds the hash to saved content atomically). Header pills + copy de-jargoned; dead PUBLIC radio + duplicate Sections card removed; `CONTEXT.md` gained Preview-tab/Settings-tab glossary terms. Built **extract-don't-fork** (new `assemble-survey-pages` composes `org-survey-client`'s pure fns; `preview-version-adapter`; `wave-ed10-flags`; `PreviewTab`/`SettingsTab`; one `ed10Active` gate in `TabbedShell`). 12 TDD tasks (subagent-driven); ED10 test surface all green (template-editor **62 suites / 716**); frozen `editor-byte-equivalence` 15 + `three-pane-parity` 20 + `ed10-golden-snapshots` **zero-diff** (byte-identity proven); eslint clean; `CI=true npx next build --turbopack` exit 0; Codex **co-validated** (no-go-as-written → 6 findings folded; **D9 re-decided** — invitation email is `contentHash`-governed); final whole-diff review **0 Critical / 0 Important** + **review-loop 5/5**. **T9 was a verified no-op** (Build ⋯Delete already confirms via `useEditorCommands`). **LAUNCH BLOCKED (infra, not code):** every Vercel deploy fails at `prisma migrate deploy` with `P1001` because the Neon project ("Scaling Up Platform", account Joshua/**Free**) hit its monthly compute cap (**110/100 CU-hrs, 100%**) → compute suspended → **prod DB unreachable** (affects the live app + all deploys, not just ED10). Fix = **Josh upgrades Neon** (or the ~Aug-1 monthly reset); then redeploy prod + flag flip (separate auth). Diagnosed via `/diagnosing-bugs` (proxy up / compute quota-suspended; NOT code/cold-start/ED10). Detail in [plans/CHANGELOG.md](plans/CHANGELOG.md) (`ed10-preview-settings-merged-dark`) + memory `project_neon_free_quota_outage`. _(Prior: July 17 — **Wave ED9 (Build-tab "Google Forms" overhaul) LAUNCHED**: `WAVE_ED9_FORMS_BUILD_ENABLED=1` on prod (PR #193 `bc5a6955`, redeploy `k4t84f3ce`, aliased `platformtest.scalingup.com`); the ED6 single-column Build tab becomes a true Forms builder (presentation-only — hero card, inline type picker, section cards, footer bar); 13 TDD tasks, frozen guards zero-diff, review-loop 5/5. Detail: CHANGELOG `ed9-forms-build-launched`.)_ _(Prior: July 17 — **Wave ED8 (version lifecycle: Roll back · Archive · Draft-delete) LAUNCHED**; ED7/ED6/ED1–ED5 editor waves + Waves A–Z all live. Detail: CHANGELOG.)_ **Full history (Waves A–ED9, all earlier sprints) in [plans/CHANGELOG.md](plans/CHANGELOG.md).** |
