...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-07-01 LAST_UPDATED_SLUG:wave-o-esperto-sufull-import-phase-1 --> July 1, 2026 — **Wave O — historical Esperto SU-Full import, Phase 1, MERGED DARK to production (PR #113 `94a5a87`; additive, behind a default-OFF flag `WAVE_O_ESPERTO_SUFULL_IMPORT` AND a `locked:false` crosswalk, so deploying changed nothing reachable).** Coach-operated (ADR-0017), recompute-not-store historical import of Scaling Up Full results from Esperto — the marquee gap for retiring Esperto (renders as normal per-respondent reports + feeds the Wave N "Year 1 vs Year 2" longitudinal). Phase 1 = safe plumbing + the **anti-corruption layer**: coach-supplied round label as stable round identity (export has no round field); single-cid-per-batch; explicit target-org selection + org-level `cid` provenance pin (never inferred from file contents); entitlement parity with campaign-create re-checked in-transaction; exact/superset/divergent reuse classification (a changed/missing respondent 409s, never silent overwrite); append binds to the pinned version; per-round `pg_advisory_xact_lock`; redacted salted-hash manifest. Additive migration (`AssessmentCampaign.importManifest`, `Organization.espertoSuFullCid`) verified applied to prod. Built `/claudex:plan` (3 adversarial rounds, 26 findings, all accepted)→subagent-driven TDD; `CI=true npx next build --turbopack` green; full Wave O suite + prod deploy green. **NOT launched — flag off + crosswalk locked.** **Phase 2 (verified `Q3_1…Q12_10 → Q01…Q61` crosswalk + lock-checklist, then canary flag-flip) is gated + pending; Rockefeller/LVA parked.** Runbook `docs/specs/v7.6/18o-ops-runbook.md`. _(Prior: June 30 — Waves M + N LAUNCHED, PR #108 `7d4a87d`; custom slides #19 + per-respondent longitudinal #23.)_ **Wave M** coach-authored custom slides (#19) — sanitized-HTML interstitials woven into the participant survey as NON-counted pages (`AssessmentCampaign.customSlides` JSONB; a STRICT `sanitizeSlideHtml` — the admin `sanitizeCustomHtml` was NOT reused; pure `mergeCustomSlides`; `CustomSlidesPanel` in the campaign wizard + CampaignDetail; flag `WAVE_M_CUSTOM_SLIDES_ENABLED`). **Wave N** per-respondent longitudinal comparison (#23) — coach view of one person's scored results across campaigns, incl. Esperto-imported "Year 1 vs Year 2" (scored-only + same-version deltas, ADR-0016; match by `normalizedEmail`-within-org; flag `WAVE_N_RESPONDENT_LONGITUDINAL_ENABLED`; **post-launch P0 hotfixed same-day — PR #111: a `submittedAt: { not: null }` filter against the NON-nullable `AssessmentSubmission.submittedAt` 500'd campaign-detail pages; caught by the live smoke, fixed + re-verified**). Closes the last two buildable Jeff June-9 items (Spec 17). Built brainstorm→grill-with-docs→grill-me→/frontend-design (signed off)→/claudex:plan (3 rounds, 22 findings)→subagent-driven TDD→whole-branch adversarial review (2 confirmed defects fixed); `CI=true next build --turbopack` green; 436 (M) + 152 (N) targeted tests. **LAUNCHED 2026-06-30** — both flags set `=1` on Vercel **Production** (non-sensitive, verified) + redeployed (deploy `7gwoftxjt`, aliased `platformtest.scalingup.com`). Kill = zero the flag + redeploy; runbooks in `docs/specs/v7.6/18m`/`18n`. **Full history (Waves A–L, Wave J launch, Waves B/D/F launch, all earlier sprints) in [plans/CHANGELOG.md](plans/CHANGELOG.md).** |
