...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-08 LAST_UPDATED_SLUG:public-campaign-optional-organization-ready --> August 8, 2026 — **Organization-free PUBLIC campaigns are implemented, rebased onto current `main`, and ready for protected production review.** The Admin create flow no longer loads or requests an organization; new PUBLIC rows store `organizationId = NULL`, while INVITED creation and every invited-only reader continue to require a real organization and fail closed on invalid rows. Existing campaign links are preserved by the nullability-only migration. Full detail in CHANGELOG entry `public-campaign-optional-organization-ready`. |
