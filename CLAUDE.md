...
- **Jeff July-10 #65 stable reminder links:** **Jeff #65 is implemented on the branch and locally verified only; it is not merged, deployed, canaried, globally enabled, or closed.** The additive child-token model retains the parent `tokenHash` compatibility mirror: enabled originals stage over a never-delivered rollback root, enabled bulk reminders append sibling tokens, and exact `REJECTED` tombstones fail closed without parent fallback. `WAVE_J65_STABLE_LINKS_ENABLED`, `_CANARY`, and `_KILL` are all default-off; kill keeps the exact legacy parent-only lookup, with definite-rejection quarantine restoring a viable parent mirror first. Manual **Resend** remains parent-only and is deliberately outside the stable-link guarantee. Migration `20260731110000_add_stable_invitation_tokens` backfills only SHA-256 hashes; no UI or invitation-copy change is included. Full evidence and rollout boundary: `plans/CHANGELOG.md` entry `jeff-65-stable-reminder-links-implemented-local-only`.
