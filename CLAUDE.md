...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-14 LAST_UPDATED_SLUG:su-full-peer-benchmark-store-implemented --> August 14, 2026 — **Scaling Up Full peer values now have one canonical snapshot plus an audited database refresh path.** The existing report keeps its current layout and values; authorized operators can explicitly populate or refresh the editable benchmark rows without ordinary seeding overwriting them. |
