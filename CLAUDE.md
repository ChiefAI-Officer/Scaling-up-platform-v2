...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-01 LAST_UPDATED_SLUG:jeff-65-stable-reminder-links-launched --> August 1, 2026 — **Jeff #65 is merged, globally enabled, production-verified, and claim-released.** Original invitation links and successfully delivered bulk-reminder links now remain valid against one invitation lifecycle. Exact merge deployment `dpl_HAF1FS2BpUXQxjVLXz4Tu1h3YoJB` is Ready on SHA `050573fa`; both production aliases report healthy database/auth posture. The Production flag is on, kill is off, and no canary override remains. Manual **Resend** stays outside the locked contract. Full detail in CHANGELOG entry `jeff-65-stable-reminder-links-launched`. |
...
| **Latest progress** | Jeff #65 is the latest launched product outcome: PR #282 is merged, its exact production SHA is globally enabled and healthy, the token-history audit is clean, and the shared claim is released. GH #217 remains launched but dormant because Production leaves `ASSESSMENT_INVITE_BRANDED` unset. |
