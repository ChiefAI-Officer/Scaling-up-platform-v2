...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-07-20 LAST_UPDATED_SLUG:ed10-launched --> July 20, 2026 — **Wave ED10 (Metadata→Preview tab + one Settings tab) LAUNCHED to production — the editor-simplification pipeline's 10th and final pass; ED1–ED10 are ALL now live.** The Neon Free-tier compute-quota outage that had blocked it is **cleared: Josh upgraded the Neon project to the Launch plan** → prod DB reachable (`prisma migrate status` green, no P1001), live app recovered. Launch = **dark redeploy** of `main` (`e4b4iezx7`, flag OFF, byte-identical to ED9, build green now DB is up) → **set `WAVE_ED10_PREVIEW_SETTINGS_ENABLED=1`** on Vercel Production + redeploy (`4z6ge1u7a`, aliased `platformtest.scalingup.com`). ED10 is active only with ED9 forms mode on (`ed10Active = previewSettingsEnabled && formsBuildEnabled && singleColumn`); **kill = `WAVE_ED10_PREVIEW_SETTINGS_KILL=1` / unset the flag → byte-identical ED9.** Headless admin **launch-walk** (Rockefeller Habits Checklist v3 editor) confirmed: tab bar **Preview (default) · Build · Scoring & Tiers · Settings · Versions** (no Metadata); Preview renders the questionnaire read-only exactly as respondents see it (Active↔draft toggle; humanized `Invited · Everyone` pills); Settings folds the whole metadata wall into one plain-language column (per-card **Save** for aggregation/results-email; Save-Draft for invitation email/language). Built **extract-don't-fork**; 12 TDD tasks; byte-identity proven (`editor-byte-equivalence` 15 + `three-pane-parity` 20 + `ed10-golden-snapshots` zero-diff); Codex co-validated; review-loop 5/5 (build detail in CHANGELOG `ed10-preview-settings-merged-dark`). A comprehensive **ED1–ED10 progress report** (self-contained HTML + PDF + a 2-min talk track, real prod screenshots + ED10 locked-design renders) was produced for Jeff (repo-root `Scaling-Up-Assessment-Editor-Report-2026-07-20.*`). Remaining pipeline items = docs (#9) + a live e2e follow-on (#10). _(Prior: July 17 — **Wave ED9 (Build-tab "Google Forms" overhaul) LAUNCHED** (`WAVE_ED9_FORMS_BUILD_ENABLED=1`, PR #193 `bc5a6955`); **ED8** version lifecycle, **ED7** de-jargon, **ED6** single-column, **ED1–ED5** editor waves + Waves A–Z all live. Detail: CHANGELOG.)_ **Full history (Waves A–ED10, all earlier sprints) in [plans/CHANGELOG.md](plans/CHANGELOG.md).** |
