...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-10 LAST_UPDATED_SLUG:create-assessment-welcome-parity-release-ready --> August 10, 2026 — **Create assessment Welcome parity is built and locally verified.** ADMIN/STAFF simplified creation and draft Build now share the same fixed Welcome card and validation contract; valid authored defaults persist atomically with the template and empty v1 draft, while off/kill preserves the name-only form and generic default. Authenticated non-Production visual acceptance remains pending because no safe environment was available. Full evidence is in CHANGELOG entry `create-assessment-welcome-parity-release-ready`. |
...
- **Create assessment Welcome parity:** **BUILT + LOCALLY VERIFIED; not pushed, merged, deployed, or Production-accepted.** Simplified ADMIN/STAFF creation now uses the same fixed, collapsed-by-default seven-field Welcome card as draft Build, directly after Assessment name and before Advanced / Internal ID. Valid authoring fields persist with the template and v1 draft in one transaction; local errors prevent the POST and focus the first invalid field on every failed submit. Enabled create accepts exactly the seven authoring keys and rejects arbitrary nested properties before any transaction. Off/kill retains the name-only strict contract and generic default. Fresh gates passed 9 focused suites / 143 tests, 3 kill suites / 103 tests, the complete 683 suites / 8,505 tests / 16 snapshots, ESLint, Prisma generation, all 47 migrations, and two 94/94-page Turbopack builds. No safe authenticated current-branch Preview or seeded local environment was available, so visual acceptance remains pending and no Production record or flag was changed. Full detail: `plans/CHANGELOG.md` entry `create-assessment-welcome-parity-release-ready`.
