...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-09-21 LAST_UPDATED_SLUG:member-portal-release-two-implemented --> September 21 — **Member portal Releases 1–2 are implemented and verified behind default-off gates: passwordless member access now derives CEO/founder and department-head report scope live, and eligible full cohorts receive the canonical group report. The branch is not merged, deployed, or enabled.** |
...
- **Admin-owned Welcome screens:** **INVITED + ALL-PUBLIC AUTHORITY LIVE; SUNHUB v7 SUCCESSOR LIVE.** PR #414 (`83a18dc9`) made every valid Template Welcome authoritative for PUBLIC links while preserving invited snapshots; PR #415 (`73b0db16`) bounded the guarded successor transactions. Retired v1 retains 15 historical submissions, while `sunhub-quick-quiz` resolves to the ACTIVE v7 successor. Safe Preface and Closing/CTA fragments are present for new individual browser/print reports; group/aggregate/email custom HTML remains excluded. The durable verifier ignores only database/workflow timestamps and still enforces identities, receipt, allow-listed fields, and zero inherited relations. Full detail: `plans/CHANGELOG.md` entries `sunhub-successor-durable-verifier`, `sunhub-v7-successor-cutover-live`, and `all-public-welcome-authority-implemented`.
