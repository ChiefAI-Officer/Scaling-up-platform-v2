...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-07-31 LAST_UPDATED_SLUG:jeff-65-stable-reminder-links-implemented-local-only --> July 31, 2026 — **Jeff #65 is implemented on the branch and locally verified only; it is not merged, deployed, canaried, globally enabled, or closed.** The default-off stable-link implementation preserves parent-token compatibility while the enabled path retains sibling invitation tokens. Definite rejection durably tombstones the exact child and restores a monotonic parent fallback. Exhausted synchronous repair first writes an ID-only strict-audit outbox intent; an immediate Inngest event is only the fast path, while a five-minute single-concurrency drain provides durable recovery. Partial-batch 503 errors visibly warn against whole-request retry. There is no UI component or invitation-copy change. Full detail in CHANGELOG entry `jeff-65-stable-reminder-links-implemented-local-only`. |
