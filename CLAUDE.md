...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-17 LAST_UPDATED_SLUG:admin-coach-password-actions-implemented --> August 17, 2026 — **Admin coach-password actions are implemented and release-ready.** ADMIN can set a coach password with immediate session revocation and a non-secret notification; ADMIN/STAFF can send a separate 15-minute reset link. |
...
- **Admin coach-password actions:** **IMPLEMENTED + LOCALLY VERIFIED; PRODUCTION PENDING.** ADMIN can immediately rotate a coach credential with session revocation and non-secret notification; ADMIN/STAFF can send a separate 15-minute reset link. The additive migration is applied and inert. Full evidence: `plans/CHANGELOG.md` entry `admin-coach-password-actions-implemented`.
