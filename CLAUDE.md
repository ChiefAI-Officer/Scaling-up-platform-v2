...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-10 LAST_UPDATED_SLUG:admin-owned-assessment-presentation-launched --> August 10, 2026 — **Admin-owned invited Welcome screens and Coach report-style simplification are launched and globally enabled.** New INVITED campaigns freeze the current ADMIN-owned template default at creation; existing campaigns retain their exact backfilled legacy copy, PUBLIC campaigns remain separate, and the kill switch is absent. Full Production evidence is in CHANGELOG entry `admin-owned-assessment-presentation-launched`. |
