...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-11 LAST_UPDATED_SLUG:picker-visible-invitation-banner-canaries-final-review-fix --> August 11, 2026 — **Universal invitation banner’s final-review correction is locally verified and remains default-off; it is not merged, deployed, or activated.** With all banner variables absent, the authoring gate now returns its disabled empty snapshot before picker visibility, so the new-campaign page performs no Template, Organization, access-group, or grant query and gains no dormant-feature SSR failure path. The latest local gates passed 2 focused suites / 18 tests, all 47 migrations, a 94/94-page Turbopack build, and 687 full suites / 8,578 tests. Production flags were neither read nor changed, and no deployment, database write, or customer email occurred. |
