...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-11 LAST_UPDATED_SLUG:report-preview-disclosure-launched --> August 11, 2026 — **Report preview disclosure is LAUNCHED + PRODUCTION-HEALTH-VERIFIED.** PR #332 squash-merged as `9cbfa304`; its Vercel Production deployment completed successfully and both canonical health endpoints are healthy. Existing Report Appearance pickers now use compact tiles and hide preview assets by default behind Show/Hide preview. No Production environment or customer data was changed. Full evidence is in CHANGELOG entry `report-preview-disclosure-launched`. |
...
- **Report preview disclosure:** **LAUNCHED + PRODUCTION-HEALTH-VERIFIED.** PR #332 squash-merged as `9cbfa304`; the associated Vercel Production deployment completed and both canonical health aliases report healthy database and safe auth posture. Every existing Report Appearance picker now uses compact responsive style tiles and starts with Cover/Summary/Detail preview assets hidden behind an accessible Show/Hide disclosure. Same-visit page state persists across close/reopen and style changes; locked or saving radios remain distinct from usable preview controls. Simplified public-campaign creation and summary-only Campaign Detail remain unchanged because they intentionally expose no picker. The rollout introduced no schema, migration, API, report-output, feature-flag, or Production-environment change. Authenticated live picker interaction was intentionally not performed with a real Production account; pre-merge read-only visual QA covered Admin Settings, Coach Report Setup, and editable Campaign Detail at 1280px and 393px. Full detail: `plans/CHANGELOG.md` entry `report-preview-disclosure-launched`.
