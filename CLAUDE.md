...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-05-18 LAST_UPDATED_SLUG:assessment-v7-6-foundation-spec-library --> May 18, 2026 — Assessment Tool v7.6 foundation slice shipped (Issue #10). 13 new tables (Organization + `ownerCoachId`, OrgTeam, OrgRespondent, AssessmentTemplate + immutable Version, AssessmentCampaign + `createdByCoachId`, AssessmentCampaignParticipant, AssessmentInvitation, AssessmentSubmission, AccessGroup + Coach/Template joins, OrganizationOwnershipEvent). INTERSECTION-semantics RBAC via AccessGroup (replaces dropped TemplateAccessGrant + OrganizationMembership). Rockefeller seeded (alias `RockHabits`, v1 enUS, golden fixture `countAchieved=37`). PLAN.md restructured into 7-file spec library at `docs/specs/v7.6/` (~795 lines) + history archive at `plans/history/v6-v7.5-archive.md` (2,541 lines). 4 Wave 5 admin wireframes (Access Groups list/detail, aggregate report, platform nav entry) + 4 paired markdown specs. `src/scripts/db-fingerprint.ts` + mandatory `dotenv-cli` for deploy safety. Prod DB rebuilt May 18 after Free-tier Neon found wiped — full v7.6 schema, 26 migrations baselined, seed cascade complete, admin login verified. 1310 tests (+48 lib/assessments). |
