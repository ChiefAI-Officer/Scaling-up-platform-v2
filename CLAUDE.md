...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-07-30 LAST_UPDATED_SLUG:wave-ev-campaign-edition-visibility --> July 30, 2026 — **Wave EV SHIPPED — the campaign screen now says which template EDITION it is serving** (PR #241, squash `cc370aa9`). Read-only, **flagless**, no migration; rollback = revert; admin + coach at once. A campaign pins a version at creation and **can never move off it** (PATCH only *reads* `versionId`) — correct, but **invisible**, so a tester opening a campaign created before a fix published read old wording and filed it as broken, **correctly, from what was on screen**. 🔑 Most likely mechanism behind **#40/#43** being re-reports; the CHANGELOG records the re-reports, **not** the mechanism — that inference is ours. ⚠️ **Framing correction: first written up as something only Jeff could fix by testing differently. Wrong — he had no way to know.** Tile now reads `Edition 3 · published Jul 2, 2026` + a `Not the latest edition` chip. "Edition" not "Version" (v2-vs-v1 is a different *product*); "Not the latest" not "available" (a campaign cannot move editions); **exactly one date**, the served edition's. 🔑 **Two traps, same failure — the tile asserting "you are on the newest edition" when it cannot know:** an empty-sibling fallback on a read failure, and a **narrowed `SELECT`** (drop `versionNumber` → `Number.isFinite(undefined)` false → every sibling rejected → **suite green**). Only `archivedAt` is shielded. Projection pinned by `toEqual` on **values** (Prisma reads `select:{f:false}` as exclusion). Query spreads **`activePublishedWhere`** (the ONE definition) so a sibling counts only if campaign-create would offer it. 32 tests / 3 suites. ⚠️ **FIVE review rounds; after round 1 NO further runtime defects — every later round found the WRITE-UP disagreeing with the code, and each fix left a fresh one** (R4 caught R3 fixing a false claim in the PR body while the identical sentence sat in `campaign-detail.ts`). **Standing lessons: (a) when you correct a claim, GREP for every copy; (b) never state counts you have not audited, including of your own errors.** Split out: **GH #242** (archived-edition pin renders reassuring text), **GH #243** (campaign *list* has no edition signal). Detail in CHANGELOG `wave-ev-campaign-edition-visibility`. _(Prior, July 29 — **Jeff July-10 item #71 SHIPPED — Wave OSR: invited respondents see their own report on screen at submit** (PR #236, squash `26f18701`). Sixth item in the one-item-at-a-time pass. Ships **DARK** behind `WAVE_OSR_RESPONDENT_RESULTS_ENABLED` (+ `_KILL`); migration additive; **ADR-0027** supersedes ADR-0007's audience clause and extends ADR-0008. A campaign may opt in (`AssessmentCampaign.showResultsOnScreen`) to rendering the respondent's **own** `BrandedReport` in place instead of the thank-you page — the SAME artifact for a new audience; the submit route already computed the score and discarded it. **Worth more than the row suggests:** the results-email lane is dormant, so on-screen is currently the ONLY way an invited respondent ever sees their result. Disclosure is decided **server-side under the Phase-2 lock** via `emailRenderFingerprint`, with **no client-visible flag** — payload presence IS the signal. **TWO review rounds, and round 1's security fix was itself wrong.** Round 1 (10 findings, 3 blocking) fixed a **PII leak** — the stored report was read BEFORE `/me`, so a tokenless reload served it with no credential; 🔑 the token-exchange purge did NOT cover it because **the exchange strips the fragment**, making the tokenless reload the COMMON path. Round 1 gated rehydrate on a `/me` **410**. Round 2 proved that **time-bounds the leak but does not close it**: 🔑 **`sessionStorage` is per-TAB while cookies are per-origin**, so a 410 proves only that *some* live invitation exists in this browser — never *whose* report is in a tab's slot. Co-invitee B exchanging in tab 2 replaces the shared cookie and purges only tab 2; B's later reload of tab 1 gets a VALID 410 and would render **A's** full report. Closed by adding **ownership on top of authorization** (the 410 echoes `respondentKey`, the envelope records its owner, they must match; no skip path; envelope `v` 1→2). **Accepted, NOT fixed:** an abandoned *own* browser exposes the report until the cookie lapses (≤29 min) — bounded, not prevented. Round 2 also caught that the purge branch **destroyed a legitimate respondent's only copy** on a transient `/me` 500 (Neon cold-start) — permanently, under show-once; now only 401/404 purge. **F4** made `RespondentReport.templateAlias` **required**: `public-quiz-client` omitted it, so EVERY public report silently resolved to `DEFAULT_REPORT_CONFIG` — requiring the field IS the fix, because it is what lets the compiler find the omission. Round 2 flagged F4 as a **real public dispatch change** justified by a prod claim in a source comment; that claim was therefore **verified** (1 PUBLIC campaign, `scaling-up-quick`, unmapped → inert) and **moved into ADR-0008 with its date**. **92 tests, jest-verified per suite.** ⚠️ **Open before launch:** `showResultsOnScreen` is **create-only (no PATCH)**, so flipping the flag alone surfaces nothing; and this wave puts an unvalidated coach-controlled `<img>` in front of unauthenticated respondents for the first time (**GH #229**). ⚠️ **Correction to a recurring claim: the "floor of 9 failing suites / 32 tests" is NOT stable** — identical code measured 9, 11 and 14; state a range and a mechanism, never one sample. Detail in CHANGELOG `jeff-jul10-71-onscreen-respondent-results`. _(Prior, same day — 🔴 **SoT RE-CORRECTION (docs-only): yesterday's flag correction was ITSELF wrong — an empty read is not an empty value.** Two adversarial reviews (Vercel primary docs + this repo) both returned "NOT safe to publish" on the draft, so this is the third version. **The error:** I inferred "flag reads empty ⇒ feature dark" from a REST read without checking the read could see the value — **8 of 29 prod `WAVE_*` vars are `type:"sensitive"`, whose values are NEVER returned.** **The proof was already in our own CHANGELOG:** ED10/ED9 both require `activeAuthoringMode === "single"`, produced only by `isOn(WAVE_ED6_SINGLE_COLUMN_ENABLED)` — and the 07-22 session read ED6 EMPTY, excluded it from the re-set, then live-verified ED9+ED10 **in that same session**. ED6 was ON while being reported empty. **`sensitive` never meant dark** (values still inject at build+runtime; local proof: `WAVE_O_..._HASH_SALT` reads empty yet its resolver THROWS when falsy, and the Wave O/X import handlers ran clean in prod). **RETRACTED:** (a) ED1/ED8 "dark" — never measured, and for **ED8 the evidence says it was ON** (the 07-22 "Roll back/Archive" sighting matches labels existing only in the ED8 branch, so my "misread" explanation is refuted); (b) **"ED10 does not depend on ED6"** — it does; **I wrongly rejected an audit agent that was right**; (c) the 07-22 **"piped stdin stores empty"** root cause — CLI 51.8.0 made `vercel env add` default to `sensitive` and skip the prompt on piped input, explaining it with **no value loss**; (d) "`WAVE_S_PEER_BENCHMARKS_KILL` is empty" — it is itself unreadable, so peers may be killed (**GH #233**). **SURVIVES:** the prod-project correction (team `scaling-up`, 77 env vars vs 0) — metadata-based, untouched by the confound; sharpened with `.vercel/project.json` holding the right projectId with the WRONG orgId, so `push-env-to-vercel.mjs` would target the wrong team. ED1/ED8 now rewritten as `encrypted "1"` (readable, **not live until the next build**); their prior values are unrecoverable. **Lesson, inverted from yesterday: "I saw it render" BEATS "the flag reads empty"** — the render is downstream of the real value. Detail in CHANGELOG `flag-state-recorrection`. _(Prior, same day — **SoT CORRECTION: the prod Vercel project was mis-documented, and `ED1`/`ED8` are DARK in prod, not "flagless".** Found while re-baselining the 23 unaccounted rows of Jeff's July-10 tracker — a sweep triggered by discovering **#43 was a re-report of work shipped 8 days before he wrote it** (Wave P, 2026-07-02), with **#40** turning out to be a second such row (same commit). **(1)** Production lives in the **`scaling-up` team** (`prj_xcAWuAmGZAU3DCHgAauRv2WPKneo`) — it owns `scaling-up-platform-v2.vercel.app` + `platformtest.scalingup.com`, has **77 env vars**, and its 2026-07-28 12:26/12:34 UTC deploys match PRs #230/#231. The `chief-aio-fficer` project this file used to name has **0 env vars** — so our own documented command returned an empty list and would support a false "no flags set" reading. **(2)** `ED1` Test Mode + `ED8` version lifecycle are **flag-gated and their prod flags are EMPTY**, so both are **dark despite being recorded as launched** (`isOn()` has no default-on path; ED1 hides the Test Mode button/drawer, ED8 404s its APIs). Root cause: the 07-22 re-flip excluded `ED1/2/4/6/8` as "superseded" — right for the presentation waves ED2/ED4/ED6 (ADR-0024) but **wrong for ED1/ED8, which ED10 does not replace**. Flipping them on is a separate ops decision, NOT done here. Rejected with evidence: the claim that ED10 requires ED6 (they are independent flags). **Lesson: a live sighting is only evidence if you can name the gate it passed through.** Detail in CHANGELOG `flag-state-correction-ed1-ed8`. **Prior entries (Waves A–OSR, July 2026 and earlier): see [plans/CHANGELOG.md](plans/CHANGELOG.md).** |
