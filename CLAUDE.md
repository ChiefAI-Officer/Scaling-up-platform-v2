...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-05-14 LAST_UPDATED_SLUG:round-16-wave-1-affiliate-cookie --> May 14, 2026 — Round 16 Wave 1: affiliate tracking iDev→PAP migration kickoff. Tracker registry + Strangler-fig staging. `(public)/layout.tsx` mounts cookie script on every public-route-group page; `next.config.ts` CSP `script-src` + `connect-src` allowlist `scalingup.idevaffiliate.com` and `*.postaffiliatepro.com`. Three-form `ScriptDescriptor` (image / externalScript / inlineScriptGroup) ready for Wave 2-3. Legacy `<img>` pixel `customCode` attribution UNTOUCHED. Env vars `AFFILIATE_TRACKER_IDEV_MODE` + `AFFILIATE_TRACKER_IDEV_COOKIE_URL` (mode default `off`). 1262 tests (+18); Waves 2-4 + S2S roadmapped in `~/.claude/plans/do-we-need-to-cryptic-swan.md`. |
