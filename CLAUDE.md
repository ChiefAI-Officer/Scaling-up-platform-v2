...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-07-31 LAST_UPDATED_SLUG:gh-243-campaign-list-edition-visibility-launched --> July 31, 2026 — **GH #243 campaign-list edition visibility is LAUNCHED and production-verified.** Shared admin and coach campaign rows display factual `Edition N` identity with lifecycle-aware markers, while closed historical rows keep identity but suppress actions. PR #275 merged and its exact Ready production deployment passed both health checks and read-only admin/coach smoke; no migration, write path, repinning, filter, sort, or feature flag was added. Full launch evidence in CHANGELOG entry `gh-243-campaign-list-edition-visibility-launched`. |
