...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-09-02 LAST_UPDATED_SLUG:sunhub-v7-successor-cutover-live --> September 2 — **All-Public Welcome authority is live, and `sunhub-quick-quiz` now resolves to its zero-history v7 successor with safe Preface and Closing/CTA content.** |
...
- **Admin-owned Welcome screens:** **INVITED + ALL-PUBLIC AUTHORITY LIVE; SUNHUB v7 SUCCESSOR LIVE.** PR #414 (`83a18dc9`) made every valid current Template Welcome authoritative for PUBLIC links while preserving invited snapshots. PR #415 (`73b0db16`) bounded the guarded successor transactions after the first quiesce attempt rolled back on Prisma's default timeout. The completed cutover retains all 15 historical submissions on retired v1 and serves new `sunhub-quick-quiz` responses from a zero-history ACTIVE v7 successor. Safe v7 Preface and Closing/CTA fragments are present for new individual browser/print reports; group/aggregate/email custom HTML remains excluded. Full detail: `plans/CHANGELOG.md` entries `sunhub-v7-successor-cutover-live`, `sunhub-successor-transaction-timeout-hotfix`, and `all-public-welcome-authority-implemented`.
