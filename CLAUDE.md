...
| **Integrated main history** | **Scaling Up Full report HTML authoring and source restoration are LAUNCHED + ENABLED** from PR #374 (`133a57f9`) on Ready deployment `dpl_t8xbV6VXPA5ngekuYhg9NchdDMjG`; forward-only Edition 6 carries the published Verne preface and CTA. The Esperto-faithful landscape remains enabled from PR #367 (`c1c76082`), and the Coach mobile drawer fix remains live from PR #370 (`e40b826a`). Older launch history is retained in `plans/CHANGELOG.md`. |
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-25 LAST_UPDATED_SLUG:qsp-v2-canonical-report-ready --> August 25 — **Canonical QSP v2 preface and full representative report locally verified; release pending.** |
