...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-07-21 LAST_UPDATED_SLUG:jeff-jul10-feedback-batch --> July 21, 2026 — **Jeff Verdun's 2026-07-10 assessment-feedback batch: 8 fixes shipped + merged to prod (12 of items #30–#87 resolved).** Triaged all 53 open items against the live code, then shipped every buildable no-decision item as 8 independent PRs (#198–#205) — each TDD + two-axis review + Turbopack build gate + Notion task — merged to `main` via a CI-gated cascade (Build + Migration Safety Gate; no `--admin` bypass; no schema migrations). Shipped: **#79/#59 P0 hotfix** (SU-Full non-CEO submit `MISSING_REQUIRED_KEY` + participant-removal 204-with-body false error), **#50** coach-completion email names the respondent, **#85** Observability admin-nav link, **#87** actionable import lane-mismatch message (shared `resultsLaneMismatchMessage`), **#60** editable member email (dedupe-safe, P2002→409), **#63/67/73/78/81** coach name as visible text on all report headers + FiveD-only "Talk to coach" CTA removal (`report-config.showCoachCta`), **#86** admin Organizations-by-coach toggle, **#83** public-quiz submissions surfaced per campaign. **Remaining Jeff items are gated on external input:** invite-email copy (#47/61/69/76/80) on prod-flag verification (Vercel connector unavailable in the build session); Five Dysfunctions findings text (#75, Jeff/Suzanne content); ~11 decision forks (#48/#64/#72/#32/#84/…) + FOG items (#33/#42/#45) awaiting product calls. Full detail + ledgered follow-ups in CHANGELOG `jeff-jul10-feedback-batch`. _(Prior: July 20 — **Wave ED10 LAUNCHED**; the editor-simplification pipeline ED1–ED10 is all live; detail in CHANGELOG `ed10-launched`.)_ **Full history (Waves A–ED10, all earlier sprints) in [plans/CHANGELOG.md](plans/CHANGELOG.md).** |
