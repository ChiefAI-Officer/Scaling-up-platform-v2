...
| **Latest progress** | GH #220 branded invitation HTML is the latest production-verified release and remains default-off. Issue #220 auto-closed; its claim release and Notion closeout are still pending, so the next consolidated report retains its existing seven-outcome window through GH #228 under the established completed-closeout rule. GH #256's narrow Circle-sync validation slice remains launched while its separate host-policy decision stays open. GH #257 residual assessment-email outbox reconciliation remains excluded while its separate implementation and closeout are incomplete. |
...
- **GH #220 branded campaign invitation HTML:** **LAUNCHED + DEFAULT-OFF + PRODUCTION-VERIFIED.** Independently approved PR #292 squash-merged as `fe3deeba`; exact Ready deployment `dpl_AJ2gQhRtmuYYP6dU8bhT2a4mQfx4` is tied to that SHA and owns all four production aliases. `WAVE_D_CUSTOM_HTML_EMAIL_ENABLED` is present as the pre-existing capability gate, while new `ASSESSMENT_INVITE_BRANDED_CUSTOM_HTML_ENABLED` and renderer kill `ASSESSMENT_INVITE_BRANDED` are absent, so branded-body composition remains inactive and existing `full_replace` semantics remain active. No flag value was read or changed, no live email was sent, and no audit, activation, or production-data write ran. Issue #220 auto-closed; claim release and Notion closeout remain pending. GH #228 report branding, GH #256 image policy, and GH #257 outbox reconciliation remain out of scope. Full detail: `plans/CHANGELOG.md` entry `gh-220-branded-invitation-html-launched-default-off`.
