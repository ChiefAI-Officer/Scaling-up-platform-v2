...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-04 LAST_UPDATED_SLUG:gh-220-branded-invitation-html-launched-default-off --> August 4, 2026 — **GH #220 branded campaign invitation HTML is merged, deployed, production-verified, default-off, and fully closed out.** The new behavior remains inactive because its Production flag is absent; issue #220 is closed, the shared claim is released, and the matching Notion task is Done. Full detail in CHANGELOG entry `gh-220-branded-invitation-html-launched-default-off`. |
...
| **Latest progress** | GH #220 branded invitation HTML is the latest fully closed production release and remains default-off. Issue #220 is closed, its claim is released, and its Notion task is Done, so it becomes the eighth eligible outcome in the next consolidated report. GH #256's narrow Circle-sync validation slice remains launched while its separate host-policy decision stays open. GH #257 residual assessment-email outbox reconciliation remains excluded while its separate implementation and closeout are incomplete. |
