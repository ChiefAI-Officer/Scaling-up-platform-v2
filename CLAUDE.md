...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-05-18 LAST_UPDATED_SLUG:assessment-v7-6-task-h-trends --> May 18, 2026 — Assessment Tool v7.6 Task H coach trends/longitudinal page shipped (Issue #10). Wave 1 wireframe 10 made real: coach picks (template, organization) at `/portal/assessments/trends` and sees year-over-year score progression. Service `getLongitudinalTrend` resolves the latest published version, partitions campaigns into included/excluded, builds per-campaign means + per-question sparkline series + per-section trend table. v1 single-version constraint enforced (older-version campaigns counted + excluded with banner). Composite score uses `tierMetricValue` per v7.1 spec (not blind numeric mean). Three UI states: zero campaigns (empty CTA), single campaign (banner "Trends require 2+ campaigns"), multi-campaign (SVG composite-score line chart + section trend table color-shaded by improvement + collapsible per-question sparkline grid). New `GET /api/assessment-templates/[id]/longitudinal?organizationId=…` route (canAccessOrganization gate). "View Trends" deep-link added to `/portal/assessments/[id]`. 1622 tests (+19 from Task H across 3 suites). Closes the v1 "year-over-year" feature gap that's the entire reason for Issue #10. Prior Tasks A–G unchanged. |
