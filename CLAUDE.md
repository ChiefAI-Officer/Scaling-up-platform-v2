...
| **Integrated main history** | **Jeff #48 shipped in PR #251** (squash `d676aa77`) after the protected Build and Migration Safety gates. The presentation-only QSP story grouping preserves the original answer/import contract. The assessment-email duplicate-delivery hotfix from PR #250 (`d4df6db1`) is launched and its controlled Inngest cutover is complete. Wave EV remains shipped from PR #241 (`cc370aa9`). **Wave OSR (#71) is LAUNCHED, not dark** — corrected here: `WAVE_OSR_RESPONDENT_RESULTS_ENABLED` is set as `type:"encrypted"` in the `scaling-up` prod project, PR #249 (`36131fe4`) closed the create-only reachability gap, PR #252 (`002e58fd`) carries its review corrections, and a controlled prod walk (`closeout-71-20260730`) rendered a respondent's own report in place before being soft-deleted. There is **no `_KILL` var**, so the fast kill is unticking the campaign's box (no deploy). |
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-01 LAST_UPDATED_SLUG:jeff-65-outbox-fairness-correction-local-only --> August 1, 2026 — **Jeff #65 remains implemented and locally verified only; it is not merged, deployed, canaried, globally enabled, or closed.** The strict-audit repair drain now guarantees forward progress for every selected row: resolved duplicates are consumed, missing targets and malformed metadata terminalize deterministically, and transient failures rotate an ID-only retry to the tail. Fifty failing head rows cannot indefinitely starve a valid row 51. The additive migration now indexes `AuditLog(action, timestamp)` for the bounded oldest-first query. There is no UI, flag-default, kill-path, or invitation-copy change. Full detail in CHANGELOG entry `jeff-65-outbox-fairness-correction-local-only`. |
...
| **Latest progress** | Jeff #65 is implemented on its branch and locally verified only; it remains default-off and is not a shipped or reportable product outcome. GH #224 remains the latest launched outcome from PR #278. |
...
- **Jeff July-10 #65 stable reminder links:** **Jeff #65 is implemented on the branch and locally verified only; it is not merged, deployed, canaried, globally enabled, or closed.** The additive child-token model retains the parent `tokenHash` compatibility mirror: enabled originals stage over a never-delivered rollback root, enabled bulk reminders append sibling tokens, and exact `REJECTED` tombstones fail closed without parent fallback. The parent separately persists a monotonic last-known-deliverable fallback, so quarantine can restore it without predecessor traversal. Exhausted synchronous repair persists one allowlisted `AssessmentInvitationToken` pending audit keyed by token-row ID with only `invitationId` metadata; the ID-only event is a fast path and the bounded scheduled drain is the durable recovery path. The drain consumes resolved, deleted-target, and malformed rows and rotates transient failures to the tail with deterministic ID-only audit states, so bounded selection cannot starve behind poison work. `WAVE_J65_STABLE_LINKS_ENABLED`, `_CANARY`, and `_KILL` are all default-off, and kill keeps the exact legacy parent-only lookup. Partial-batch 503 errors include progress identifiers, `retrySafe: false`, and a visible do-not-retry-whole-request warning. Manual **Resend** remains parent-only and is deliberately outside the stable-link guarantee. Migration `20260731110000_add_stable_invitation_tokens` backfills only SHA-256 hashes and adds the action-aware AuditLog drain index; no UI component or invitation-copy change is included. Full evidence and rollout boundary: `plans/CHANGELOG.md` entry `jeff-65-outbox-fairness-correction-local-only`.
- **GH #217 legacy invitation fallback hardening:** **IMPLEMENTED + LOCALLY VERIFIED; not yet merged or live** on branch `codex/217-legacy-invite-fallback-hardening`. The dormant `ASSESSMENT_INVITE_BRANDED=0` renderer now threads the supplied Coach name, sends a plain-text twin, and supplies an escaped bottom fallback URL. The branded and custom-HTML renderers, invitation copy, telemetry, attachment behavior, and SMTP error propagation remain unchanged. Production still leaves the flag unset, and this work did not mutate it. GH #217 becomes an eligible reliability outcome only after protected merge, exact production-deployment verification, issue closeout, claim release, and launch SoT.
