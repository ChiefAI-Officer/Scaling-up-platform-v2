...
| **Last Updated** | <!-- LAST_UPDATED_ISO:2026-08-10 LAST_UPDATED_SLUG:create-assessment-welcome-parity-launched --> August 10, 2026 — **Create assessment Welcome parity is launched and Production-verified.** PR #329 merged as `235b18f4`; exact Ready deployment `dpl_FFUVXcZ9AnUwQQpAoYkwHGZHk3kx` owns both canonical aliases, whose health checks passed. ADMIN/STAFF creation and draft Build now share the same fixed Welcome card; this affects newly created assessments moving forward and does not rewrite existing templates or campaigns. Full evidence is in CHANGELOG entry `create-assessment-welcome-parity-launched`. |
