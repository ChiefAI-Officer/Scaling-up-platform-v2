...
**Report Welcome section** (report preface):
The optional Template-Version-owned HTML/CSS that replaces the default preface near the start of an individual Results report. ADMIN/STAFF authors it beside the report's **Closing message** in the Reports tab; after publication it can follow the template's Active version for presentation while the Campaign's questions, answers, and scoring remain pinned. It is sanitized static report content that participates in browser/print pagination, not a participant entry screen.
_Avoid_: "welcome screen" for this region. The **Welcome screen** below appears before a respondent starts answering and is campaign/template-row presentation metadata; the Report Welcome section appears only after results exist and belongs to a Template Version.

**Welcome screen**:
The first screen a **Respondent** sees for a **Campaign** — before any question and before the **Section pager** begins: the campaign title, the **welcome lede** (the introductory paragraph — or two — under the title), a what-to-expect summary, and a "Start the assessment" affordance. It belongs to the campaign, not to a section, so it is **never counted** in "Section N of M". ADMIN/STAFF authors a structured plain-text default on the **Assessment Template** row, including both the sharing heading and its explanation, during simplified assessment creation or later in the draft Build tab; both surfaces use the same Welcome card and validation contract. Every new invited Campaign copies that default into an immutable **Welcome snapshot** when it is created; later template edits never change an existing DRAFT, ACTIVE, CLOSED, or imported invited campaign (ADR-0033). A public Campaign reads and renders every schema-valid current default from its related Template at request time, including a value equal to a code seed or migration backfill; it neither creates nor reads an invited snapshot. Only a missing or malformed public default uses the anonymous public presentation and Campaign description. Valid schema-v1 values normalize in memory to schema v2 with the former sharing sentence; new saves and new invited campaigns use schema v2 without rewriting stored history. Template, Campaign, and URL aliases never select Welcome content. The default is presentation metadata, not part of a Template Version or its content hash. Invitation-email content is a separate surface and lifecycle.
...
**Peers (benchmark)**:
The external reference values shown alongside an assessment score — Esperto describes them as companies that previously took the assessment. A respondent's standing is expressed as **deviation from Peers** (▲/▼), *not* a percentile or tier band (we do not have the peer distribution or Esperto's private cohort formula; see ADR-0015). **SU-Full** reports already render static domain, section, question, and overall Peer values. Its 61 question values now share one canonical, verified 2026-08-14 source snapshot; an explicit audited refresh can mirror that snapshot into template-level `AssessmentBenchmark` rows, and the existing editor can maintain those rows manually. The current SU-Full report still consumes the canonical static snapshot, not admin DB rows; the approved paired-micro-bar UI and its DB-backed render path remain separate follow-ons. **LVA** peers remain admin-set per rating question; no Esperto LVA dataset exists, so reports omit empty keys (ADR-0019).
