...
describe("getAggregateReport", () => {
  ...
  // Filters follow-on — Decision #8 deferred MVP now landed.
  describe("filters", () => {
    it("organizationId filter is forwarded into the campaign WHERE clause", async () => {
      ...
      await getAggregateReport(db, "tpl-1", "ver-1", {
        organizationId: "org-X",
      });
      const call = findMany.mock.calls[0][0];
      expect(call.where.campaign.organizationId).toBe("org-X");
      expect(call.where.campaign.templateId).toBe("tpl-1");
    });

    it("startDate + endDate are forwarded as submittedAt range", async () => {
      const findMany = jest.fn().mockResolvedValue([]);
      const db: AggregateReportDb = {
        assessmentTemplateVersion: {
          findUnique: jest.fn().mockResolvedValue({
      ...
      const call = findMany.mock.calls[0][0];
      expect(call.where.submittedAt).toEqual({ gte: start, lte: end });
    });

    it("no filters → no submittedAt key + no organizationId key", async () => {
      const findMany = jest.fn().mockResolvedValue([]);
      const db: AggregateReportDb = {
        assessmentTemplateVersion: {
          findUnique: jest.fn().mockResolvedValue({
      ...
      expect(call.where.campaign.organizationId).toBeUndefined();
    });
  });
});
