...
describe("July 10 feedback closeout ledger", () => {
  ...
  it("records the evidence-backed 44 done / 4 partial / 5 decision state", () => {
    ...
    expect(tally).toEqual({ DONE: 44, PARTIAL: 4, "NEEDS DECISION": 5 });
  });
  ...
  it("drives the artifact generator from the tracked ledger", () => {
    ...
    expect(result.stdout).toContain(
      "53 rows: 44 DONE / 4 PARTIAL / 5 NEEDS DECISION",
    );
  });
});
