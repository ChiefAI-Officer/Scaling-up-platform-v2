...
describe("July 10 feedback closeout ledger", () => {
  ...
  it("records the evidence-backed 45 done / 3 partial / 5 decision state", () => {
    ...
    expect(tally).toEqual({ DONE: 45, PARTIAL: 3, "NEEDS DECISION": 5 });
  });
  ...
  it("drives the artifact generator from the tracked ledger", () => {
    ...
    expect(result.stdout).toContain(
      "53 rows: 45 DONE / 3 PARTIAL / 5 NEEDS DECISION",
    );
    expect(result.stdout).toContain("Status date: 10 August 2026");
  });
});
