...
describe("July 10 feedback closeout ledger", () => {
  ...
  it("records the evidence-backed 48 done / 2 partial / 3 decision state", () => {
    ...
    expect(tally).toEqual({ DONE: 48, PARTIAL: 2, "NEEDS DECISION": 3 });
  });
  ...
  it("drives the artifact generator from the tracked ledger", () => {
    ...
    expect(result.stdout).toContain(
      "53 rows: 48 DONE / 2 PARTIAL / 3 NEEDS DECISION",
    );
