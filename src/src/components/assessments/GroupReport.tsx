...
/**
 * Cohort-report footer. Thin wrapper over the shared `ReportFooter` — kept as a
 * named export because QualitativeGroupReport + ScoredGroupReport both import
 * it, and it owns the group-specific date formatting + testid.
 */
export function GroupReportFooter({
  ...
  return (
    <ReportFooter
      dateLabel={formatGroupDate(generatedAt)}
      coachLogoUrl={coachLogoUrl}
      coachName={coachName}
      testId="group-report-footer"
    />
  );
}
