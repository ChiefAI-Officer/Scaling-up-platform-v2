...
export function NewWorkshopForm({ isCoachPortal = false, prefilledCoach }: NewWorkshopFormProps) {
  ...
  useEffect(() => {
    ...
    setFormData((prev) => {
      ...
      // A title is considered auto-generated if it's blank OR matches any variant of the base pattern.
      const prevIsAutoGenerated =
        !prev.title ||
        prev.title === baseTitle ||
        // Edge case: if a user manually types a title starting with the category's base string,
        // it will be treated as auto-generated and may be overwritten on category switch.
        // This is acceptable UX but documents the known behavior.
        prev.title.startsWith(`${baseTitle} with `);
