/**
 * Assessments lane layout (wireframe 24).
 *
 * Renders the assessment-specific sidebar to the left of the page body for
 * every route under /admin/assessments/*. Mirrors the portal sidebar pattern
 * but uses the admin chrome's role gate (ADMIN + STAFF only; COACH bounces
 * to /unauthorized).
 *
 * Note: the coach-lane portal already has its own sidebar (`(portal)/layout.tsx`);
 * this layout is exclusively the admin-side surface.
 */

import { redirect } from "next/navigation";
import { getServerSession } from "next-auth/next";
import { authOptions } from "@/lib/auth/auth";
import { AssessmentsSidebar } from "@/components/nav/assessments-sidebar";
import { isMobileResponsiveEnabled } from "@/lib/mobile-responsive-flags";
import "@/styles/wireframes-scoped.css";

export default async function AdminAssessmentsLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  const mobileResponsiveEnabled = isMobileResponsiveEnabled();
  const session = await getServerSession(authOptions);
  if (!session) {
    redirect("/login");
  }
  const role = (session.user as { role?: string } | undefined)?.role;
  if (!role || role === "COACH") {
    redirect("/unauthorized");
  }

  return (
    <div
      className={
        mobileResponsiveEnabled
          ? "flex flex-col sm:flex-row sm:items-stretch sm:min-h-[calc(100vh-4rem)] -mx-4 sm:-mx-6 lg:-mx-8 -my-6"
          : "flex flex-col md:flex-row md:items-stretch md:min-h-[calc(100vh-4rem)] -mx-4 sm:-mx-6 lg:-mx-8 -my-6"
      }
    >
      <AssessmentsSidebar
        session={session}
        responsiveEnabled={mobileResponsiveEnabled}
      />
      <div
        className={
          mobileResponsiveEnabled
            ? "wf-scope min-w-0 max-w-full flex-1 px-4 py-6 sm:px-6 lg:px-8"
            : "wf-scope flex-1 min-w-0 px-4 sm:px-6 lg:px-8 py-6"
        }
      >
        {children}
      </div>
    </div>
  );
}
