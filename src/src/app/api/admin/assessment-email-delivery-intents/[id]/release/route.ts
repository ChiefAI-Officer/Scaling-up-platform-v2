...
const releaseSchema = z
  .object({
    ...
    reasonCode: z.literal("DRIFT_REVIEWED_SEND_FROZEN"),
...
export async function POST(
  ...
  try {
    ...
    const parsedBody = releaseSchema.safeParse(body);
    if (!parsedParams.success || !parsedBody.success) {
      return privateJson({ error: "INVALID_REQUEST" }, 400);
    }

    const resolution = await releaseHeldIntent(
      productionAssessmentEmailIntentOperatorDeps({
        reviewTokenSecret:
      ...
      }),
      {
        ...
        reviewToken: parsedBody.data.reviewToken,
      },
    );
    return privateJson({
      data: {
