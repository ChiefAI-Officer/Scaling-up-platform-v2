...
export function publicDraftKey(campaignAlias: string): string {
  if (typeof window === "undefined") return `assessment-draft:pub:${campaignAlias}:ssr`;
...
export function useAnswerDraft(storageKey: string | null, answers: AnswersMap, setAnswers: (m: AnswersMap) => void) {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      if (raw) { const parsed = JSON.parse(raw); if (parsed && typeof parsed === "object") setAnswers(parsed); }
    } catch { /* ignore corrupt draft */ }
  }, [storageKey, setAnswers]);
