 ...
 *
 * Usage: npx tsx scripts/wave-v-walk-campaign.ts
 */
...
const TEMPLATE_ID = "cmr8wocif00028lbz5625bobh"; // WALK V SCORED TEST (throwaway)
const VERSION_ID = "cmr8wocyf00048lbzahgxi1uu"; // v1 (published 2026-07-06)
...
async function main() {
  const campaign = await db.assessmentCampaign.create({
    data: {
      ...
      language: "en",
      alias: "walk-v-campaign",
      name: "WALK V CAMPAIGN (throwaway)",
      description: "Wave V launch-walk throwaway — quarantined after the walk",
      status: "ACTIVE",
      accessMode: "INVITED",
      openAt: new Date(),
      endMode: "OPEN_END",
  ...
  console.log(`survey URL: http://localhost:3000/org-survey/walk-v-campaign#t=${rawToken}`);
