...
**Status: IMPLEMENTED + REVIEWED LOCALLY; READY FOR PROTECTED PR; DEFAULT-OFF.** The ADMIN/STAFF new-assessment route now has a release-gated name-first handoff into the existing editor: a collapsed **Advanced** section exposes the generated or manually overridden immutable Internal ID, the server atomically creates one empty unpublished v1 draft with canonical defaults and bounded alias-collision retries, and success opens the exact Build URL at `.../edit?tab=questions`. The existing ED9 FormsBuilder remains the sole section/question authoring surface and retains Slider, Short text, Number, and Multiple choice under the existing Wave T rules. The same release gate presents assessment-wide scoring and tier configuration in approved plain language while preserving raw stored keys, calculation behavior, validation, report behavior, and flag-off legacy rendering. There is no schema or Prisma migration.

**Fresh final verification.** After the final whole-branch review fixes at `8196934a`, changed-file ESLint exited `0`; migration safety checked **45 migrations** with no unapproved destructive operation; the exact targeted release matrix passed **11 suites / 218 tests / 0 snapshots**; the complete Jest suite passed **646 suites / 7,946 tests / 16 snapshots**; and `CI=true npx next build --turbopack` compiled successfully, completed TypeScript, and generated **93/93** static pages. Independent whole-branch review found no Critical or Important issue. Its two Minor interaction findings—stale generated-ID errors after name changes and whitespace-only tier-label accessible names—were fixed test-first, and a scoped re-review found no new breakage.
...
**Rollout boundary and explicit non-actions.** Implementation ends dark: the new enable and kill variables were supplied only to local processes and were not written to Vercel or any shared environment. Authorization now extends only to pushing this reviewed branch and opening a draft pull request. No Production database, customer data, flag, draft, template version, publish action, email, deployment, redeploy, merge, or Production smoke was read, created, changed, or performed. Production remains on the legacy surfaces until a separately authorized protected merge, flag activation, deployment, and Production acceptance smoke.
