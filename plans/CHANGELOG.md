...
**Tests:** 97 new across 10 suites (flags 18, resolver 16, scoring wave-u 15, reader-audit 2, PATCH wave-u 7, serialization wave-u 15, panel 11, reports 8, section model + walk prefix from the review; +2 in #140). Full sweep 5,292 pass / 448 suites (7 failing suites pre-existing on main: `db.$transaction` mock drift in the submit tests, stale `GROUP_RENDER_VERSION=lva-fidelity-v1` pin from before Wave S, dashboard-stats/exchange/portal-filter/date-lint — none import a Wave U surface). `CI=true npx next build --turbopack` green. Adversarial whole-branch review completed INLINE (all three TDD subagents died on session usage limits — third wave running; the entire build was completed inline).
