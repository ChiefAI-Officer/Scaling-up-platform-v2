...
### 2026-07-21 — CHI-17 Vercel/Neon transfer and approval packet prepared <!-- ENTRY_ISO:2026-07-21 ENTRY_SLUG:chi17-transfer-approval-packet -->

**Status: PREPARED, LIVE EXECUTION BLOCKED ON NAMED OWNERS/DESTINATIONS AND SEPARATE APPROVALS.** Added a source-derived, secrets-free handoff runbook, approval packet, and environment-name manifest under `docs/operations/`. The packet separates the Vercel project-ownership window from the Neon ownership/data window; records the current Vercel project/team/deployment/domain facts; makes Josh or Jeff the eligible source-owner executor rather than elevating the preparation operator; and defines preflight, stop, validation and rollback gates.

The inventory surfaced two change-window hazards: Vercel reports Node `24.x` while `.nvmrc` pins Node `20`, and every production build runs `prisma migrate deploy`, so Window A explicitly prohibits a verification redeploy. Neon is not assumed transferable: the executor must identify native-org versus Vercel Marketplace provisioning and integration status, then separately approve native transfer, Marketplace resource transfer, or a backup-tested migration. No provider state, secret, billing setting, deployment, environment variable or database was changed.
