...
### 2026-07-21 — Jeff July-10 assessment-feedback batch: 8 fixes shipped + merged to prod (12 items resolved) <!-- ENTRY_ISO:2026-07-21 ENTRY_SLUG:jeff-jul10-feedback-batch -->

**Status: LAUNCHED (2026-07-21).** Triaged Jeff Verdun's 2026-07-10 assessment-feedback tracker (items #30–#87, 53 open) against the live code, then shipped every buildable no-decision item as **8 independent PRs** — each TDD (red→green), two-axis code-review (Standards + Spec), Turbopack build gate green, and a Notion task. All 8 merged to `main` via a **CI-gated cascade** (Build + Migration Safety Gate; no `--admin` bypass — branch stayed up-to-date per protection). No schema migrations. **12 of Jeff's items resolved:**

- **PR #198 — P0 hotfix.** #79 SU-Full non-CEO submit no longer 500s with `MISSING_REQUIRED_KEY` — the submit/scoring path now applies the same CEO-only `S_BACKGROUND` audience drop as the render path (`SU_FULL_ALIAS`/`SU_FULL_BACKGROUND_SECTION` exported from `assemble-survey-pages.ts`; `computeScoreResult` receives audience-filtered questions/sections). #59 participant removal no longer shows a false "could not remove" — the DELETE route returned a 204 **with** a body (throws post-commit → 500); now `new NextResponse(null, {status:204})`.
...
- **PR #205 — #83.** Public-quiz submissions (self-enrolled taker + referring coach + date) are surfaced per campaign via a new admin `GET /api/admin/public-campaigns/[id]/submissions` + a "View submissions" expander on the Public Campaigns page (reads the persisted `referringCoachEmail` + `publicTaker`).

**Remaining Jeff items — gated on external input, NOT built:** T04 invite-email copy (#47/61/69/76/80) blocked on prod-flag verification (`WAVE_P`/`WAVE_O`/`WAVE_X` — Vercel connector not authorized in the build session); Five Dysfunctions findings text (#75 — Jeff/Suzanne content, capability already exists via Wave U); ~11 decision forks (#48 core-values consolidation, #64 Print/Download, #72 group reports for all 4 types, #32/57/58 benchmarking, #84 8-Q SunHub quiz, …) awaiting product calls; FOG items (#33/#42/#45) needing Jeff/Suzanne clarification.

**Follow-ups ledgered (not blocking):** extract a shared `resolveSuFullAudience` helper (SU-Full audience rule now spans 4 sites); extract a shared `publicTaker` name/email helper (the CSV aggregate route has a local copy); extract the P2002→409 dedupe-conflict response (recurs across create/bulk/campaigns/edit); a coach-facing (non-admin) view of public-quiz submissions.
