...
### 2026-06-26 — SMTP error redaction + repo/CI housekeeping (PRs #91/#92/#93/#94) <!-- ENTRY_ISO:2026-06-26 ENTRY_SLUG:smtp-error-redaction-and-repo-ci-housekeeping -->

A batch of four small PRs merged to prod (`main` → `461232e`), plus a source-of-truth reconciliation. The one substantive code change is **#93**; the rest are repo/CI/docs housekeeping.

**#93 — SMTP error redaction (`fix/redact-workflow-smtp-error-codes`, prod; no migration, no flag).** `WorkflowStepExecution.errorMessage` is rendered to operators (admin workflow-executions panel `workflow-executions.tsx:210` + the "trigger now" toast `trigger-now-button.tsx`), but the two terminal-SMTP failure paths stored the **raw** SMTP server text (`EAUTH`/`535` auth detail, hostnames, 5xx specifics), leaking infra detail into the UI. Now mapped to stable, non-sensitive codes — `smtp_auth_failed` | `smtp_send_failed` — via a shared `redactSmtpError()` in `lib/workflows/fanout-delivery.ts`; the raw error is kept only in `console.error` (ephemeral server logs). Applied at the two SMTP-terminal stores: the `sendFanoutRecipients` terminal branch (covers all 3 attendee step types across both Inngest send paths) and the `trigger-workflow-step.ts` EMAIL_COACH/STAFF/CUSTOM generic send (whose duplicated inline terminal-classifier regex was DRY'd onto the shared `isTerminalSmtpError`). TDD (redactSmtpError mapping + FAILED-child code assertions); 188 inngest/fanout tests green; Turbopack build clean; ESLint clean. Adversarial review 4/5, merge-ready, zero P0/P1. **Deliberately out of scope (new tracked follow-on):** the general function-level step-failure recorder (`execute-workflow.ts` step-failure catch) still stores raw `err.message` for *any* error — and a *transient* SMTP error reaches it, so it can persist an internal host/IP. Redacting that correctly needs a **general** (non-SMTP-specific) redactor, since `redactSmtpError` would mislabel a non-SMTP terminal error (e.g. a DB `P2002`) as `smtp_send_failed`.

**#91 — repo-reference cleanup (`chore/repo-move-rescaffold`).** Re-pointed stale `jcbdelo26/...` → `ChiefAI-Officer/...` references and `@MonksKoala` → `@GabrielChiefAIOfficer` across `.github/CODEOWNERS`, `CLAUDE.md`, and `docs/runbooks/branch-protection-setup.md`, after the June-24/25 org migration.

**#92 — backfill assessment specs 14 + 15 (`docs/backfill-specs-14-15`).** Landed `docs/specs/v7.6/14-totals-by-category-and-five-dysfunctions.md` + `15-scaling-up-quick-assessment.md` (authored in the never-merged #44, but their features shipped long ago). **Closed the stale #44** as fully superseded (its ADR-0008 was already on `main`).

**#94 — CI actions bumped to Node-24 majors (`chore/ci-bump-actions-node24`).** `actions/checkout@v4→v5` + `actions/setup-node@v4→v5` in `.github/workflows/ci.yml`, clearing GitHub's Node-20 deprecation warning on the Build + Migration Safety Gate jobs. The app build stays on Node 20 (`node-version-file: src/.nvmrc`, Vercel parity) — only the action-wrapper runtime moved.

**Branch-protection misconfiguration (found + resolved during this batch).** `main`'s required status check was set to the context **`CI`** — but that's the *workflow* display name; the real check-runs are **`Build`** + **`Migration Safety Gate`**. So the required `CI` check never reported and every PR hung on "Expected — Waiting for status to be reported." `GabrielChiefAIOfficer` is `maintain` (not admin), so it couldn't edit protection (404). jcbdelo26 (sole admin/org-owner) corrected the required contexts to `Build` + `Migration Safety Gate`; there is **no required review** (`reviewDecision` empty), so Gabriel can now self-merge own PRs via **web-UI "Update branch" → squash** (the `gh pr update-branch` CLI was throwing flaky GraphQL errors; `strict:true` remains, so branches must be synced before merge).

**SoT reconciliation (stale-catalog correction).** Verified against `main` that two items the local notes still listed as "queued/buildable" were already shipped: **Wave K.1** (move Esperto historical import admin→coach) shipped June 5 (PR #39 follow-up — coach route `/api/assessments/import` + `/portal/members/import`, roster *and* results), and the **entire audit-remediation stream** (#81/#82/#83) merged June 24 (PR-3's `deliveryBatchKey` dedup already resolved the per-recipient idempotency / immediate-path `executionId` / FAILED-child-writes follow-ons — pruned from CLAUDE.md's Open follow-ons here). Spec 18 §D/§E + memory corrected.
