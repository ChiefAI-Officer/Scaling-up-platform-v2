...
**Anatomy-matched visual evidence.** The picker now selects scored, qualitative, or sparse-custom synthetic preview content from the template's existing report family/capabilities while preserving selection and Save when an image fails and exposing `Preview unavailable` plus retry. Fixtures are deeply frozen, contain no loader/database/customer-content path, and traverse the same presentation adapters and appearance renderers as real individual reports. The capture pipeline generated and visually reviewed **27 committed WebPs** (3 anatomies × 3 appearances × Cover/Summary/Detail), retained the nine scored compatibility paths, and passed one-page PDF assertions using A4 for Classic and US Letter for both alternates. Maximum-length, partial, degraded, missing-block, and long-branding cases assert no empty cards, blank columns, color-only status, horizontal overflow, or inaccessible selection/read-only state. The real DB-free Playwright supplemental lane passed all **36 anatomy × alternate-style × stress-variant combinations**, including Axe, desktop/mobile screenshots, and nonblank page-numbered Letter PDFs.

**Verification.** Focused preview/policy/picker/print coverage passed **7 suites / 81 tests**; the caller regression matrix passed **5 suites / 60 tests**; and the three full-run corrections passed **3 suites / 8 tests / 4 snapshots**. The first fresh full-repository Jest run correctly exposed five branch regressions in legacy Campaign Detail fixtures and one intended golden copy change; after fail-safe Classic fallback and snapshot refresh, the finished repository passed **643 suites / 7,987 tests / 16 snapshots**. Changed-file ESLint, changelog freshness, diff hygiene, migration safety across **45 migrations**, and the Turbopack production build passed; the build completed TypeScript and generated **93/93** static pages. Repository typecheck remains at the inherited **402-diagnostic** baseline with no diagnostic in a Task 8 changed file. The real PostgreSQL lock suite remains locally unavailable because its isolated test URL and explicit safety opt-in are unset and Docker is unavailable; it must run as a protected hosted gate.
