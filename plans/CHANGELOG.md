...
### 2026-06-30 — Waves B/D/F LAUNCHED to production + Wave J publish-prepped (ops session) <!-- ENTRY_ISO:2026-06-30 ENTRY_SLUG:waves-bdf-production-launch -->
...
**Wave J publish-prepped (still DARK + Jeff-gated):** verified prod SU-Full state in the admin UI — **v1 was already PUBLISHED** (since May 25; so the group-report publish guard is satisfied, i.e. single-dark behind `WAVE_J_*` only, not "double-dark"), and the v2 DRAFT lacked the J-1 `S_BACKGROUND`/FTE section (the J-1 re-seed had never run in prod). Ran the guarded **DRAFT-only** SU-Full re-seed against prod (origin/main's seed via temporary overlay — the local checkout was on a stale pre-J-1 branch; `forceSupersedeDraft` in `main()` appends past the divergent draft) → created **v3 DRAFT** (`9a23eca…`, 11 sections incl. `S_BACKGROUND`); v1 untouched. Then deleted the orphaned old v2 draft via a guarded one-off (verified unpublished + 0 referencing campaigns; wrote an `ASSESSMENT_VERSION_DELETED` audit row). SU-Full now = **v1 PUBLISHED + v3 DRAFT (publish-ready)**. **To LAUNCH Wave J (Jeff-gated):** Jeff signs off on v3 content (wording, provisional ScaleUp 0–100 cutoffs, single-cohort Peers numbers, #32 universal-vs-report-specific) → Publish v3 → canary `WAVE_J_SUFULL_GROUP_CANARY` → `WAVE_J_SUFULL_GROUP_ENABLED=1` + redeploy.
