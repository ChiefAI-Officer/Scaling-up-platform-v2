...
**Acceptance and security-runtime correction.** Every fixture submission is invitation-bound, including a same-normalized-email other-organization completed submission that is intentionally absent from the focused CEO picker. The Playwright suite queries frozen per-question rows to derive coverage, derives a signed CEO exchange capability from the real submitted invitation, proves its fragment-free exact-path session and own-history-only candidates, and verifies immediate disclosure/CEO-designation revocation plus non-CEO exchange denial. It also derives candidate IDs, imported badges, and deployed renderer test IDs instead of trusting parent-process report facts. Contract coverage proves the deterministic topology and hard refusal. A real `iron-session` runtime regression test reproduced Jest/jsdom resolving `uncrypto`'s browser ESM export through CommonJS `iron-session`; Jest now maps that one dependency to its supplied Node CommonJS export. This is not a mock or a bypass of cookie capability behavior.
...
**Executable acceptance contract and evidence.** `src/e2e/report-comparison.spec.ts` is an opt-in, sentinel-guarded disposable-database Playwright suite. The shared launcher now rejects competing report-style/comparison fixture lanes, binds the app server to the comparison database, validates its sentinel before build/start, and disables ambient-server reuse; its contract suite passed **12/12**. The comparison fixture contract requires native and imported CEO history, an unrelated same-email respondent, a non-CEO, all three report styles, and the real invitation/API paths. It declares the expected most-recent baseline, excluded cross-organization candidates, and parity facts across styles. The browser flow proves that the default candidate is initially inactive, then drives coach/admin campaign-detail **View report** links; tests both native and imported selections, canonical URL/remove behavior, print/PDF text, and desktop/narrow captures in every style. It also drives an invited CEO flow through fragment exchange, exact-path self access, own-history selection, denials for another report/group/Trends/campaign/altered focus, immediate disclosure revocation, CEO-designation revocation, and absence of a non-CEO link. The prescribed comparison Jest matrix ran **14 suites / 162 tests passing**, with `report-comparison.test.ts` unable to initialize because Jest loads `iron-session`'s untransformed `uncrypto/dist/crypto.web.mjs`; the legacy matrix similarly had **7 suites / 125 tests passing**, with `group-report-route.test.tsx` blocked at that same inherited ESM boundary. Changed-file ESLint passed; migration safety passed across **45 migrations**; and `CI=true npx next build --turbopack` completed successfully after compilation and TypeScript.
