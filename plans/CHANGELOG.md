...
<a id="su-full-esperto-landscape-dark-verified"></a>
### 2026-08-18 — Scaling Up Full Esperto-faithful landscape report dark implementation verified <!-- ENTRY_ISO:2026-08-18 ENTRY_SLUG:su-full-esperto-landscape-dark-verified -->

**Status: APPROVED + IMPLEMENTED + BROWSER/PDF VERIFIED + DEFAULT-OFF; NOT YET PUSHED, DEPLOYED, OR ACTIVATED.** The resolved Classic Scaling Up Full individual report now has a dedicated, deterministic 26-page A4 landscape composition behind `NEXT_PUBLIC_WAVE_SU_FULL_LANDSCAPE_REPORT_ENABLED`, which defaults off and is independently contained by `NEXT_PUBLIC_WAVE_SU_FULL_LANDSCAPE_REPORT_KILL`. When enabled with a complete governed peer presentation, chapter opener pages 7, 11, 14, 19, and 21 render the approved respondent bars with one thin, solid peer contour; the thirteen detail pages retain question-level **You / Peers** paired bars before each frozen score-selected feedback; page 26 repeats all five readable vertical comparisons in Appendix A. The opening sequence includes the frozen ScaleUp Score, the FTE-derived growth phase when available, the page-3 five-color chapter key, the supported peer dashboard, five chapter aggregates plus ten visibly subordinate subsection rows on page 5, Scaling Up and Coach identity on the cover/footer, and fixed physical page numbers 1–26.

**Truthfulness, fallback, accessibility, and data boundary.** The composition now fails closed unless its caller supplies the resolved `CLASSIC` style, a finite frozen ScaleUp Score, all 61 canonical scored records with nonblank frozen feedback, and the complete peer payload. A stored unavailable style that resolves to Classic can render; a genuinely resolved Executive or Modern style cannot. Composition failure emits one bounded structured warning containing only the reason and resolved style, then preserves the current shipped paired report; missing or invalid peer data retains the generic Classic fallback. The report states that Peers are a current benchmark reference and are not yet matched to company size, growth phase, geography, or industry; no unsupported cohort claim, dotted team/previous series, placeholder feedback, or vendor attribution is rendered. An explicit contact email wins, with the frozen referring-coach email as fallback. The standalone report layout supplies exactly one route-level `main`; the 26 physical page bodies remain ordinary containers, avoiding nested landmarks. Real Chromium proves kicker text and the separate non-scaling peer-line colors meet their thresholds in all five chapter colors while lighter peer bar fills remain decorative. The existing 61 governed Production benchmark values, respondent answers, scoring, ScaleUp score, feedback selection, template content, group report, Executive/Modern styles, report email, public mini-quiz/CTA, schema, and migrations are unchanged.

**Canonical browser and print receipt.** The DB-free route-wrapper capture uses the canonical Edition 4 seeded labels and the exact feedback band selected by each fixture score, including the real 482-character Q35 feedback and fixture score **55/100**. With print media enabled before measurement, Real Chromium verifies every contour on opener pages 7, 11, 14, 19, and 21 plus all five Appendix charts against its rendered 0–10 track and actual row center within **1px**. Those six print surfaces contain exactly **10** charts, **10** solid undashed contours, and **122** canonical rows—Q01-Q61 once on its opener and once in Appendix A. The same 122 labels reject any scroll-height, visual-bound, hidden-overflow, line-clamp, or ellipsis clipping. At **375px** and **760px**, document width stays within the viewport; all ten contours are hidden while the same 122 rows retain visible, aligned, truth-scaled `Peers` bars and numeric values. Chromium generated exactly **26** physical pages at **841.92 × 594.96 pt A4 landscape** with no browser chrome; Poppler text extraction found two ScaleUp Score placements, **61** frozen-feedback headings, all canonical labels, and every selected feedback after extraction-safe word normalization. Page 5's fifteenth row stays above its footer. All 26 regenerated PNGs were inspected, including every opener and Appendix A; no chart label is clipped or missing.

**Verification.** The corrected release matrix passed **9/9 suites and 108/108 tests**, including four real Chromium integration tests covering geometry, full-label bounds, responsive peer visibility, landmark count, contrast, physical page count, and extracted report content. Changed-file ESLint emitted no diagnostics; migration safety approved all **48** migrations; and both staged/uncommitted diff checks passed. The Production-equivalent `CI=true npx next build --turbopack` compiled in **17.8s**, passed TypeScript, and generated **94/94** pages, retaining only the established middleware-deprecation and optional missing Inngest-key/`DATABASE_URL` diagnostics. No Production environment, benchmark row, assessment, campaign, response, invitation, email, Esperto data, schema, or migration was read or changed. This entry does not claim merge, deployment, activation, or launch.
...
**Status: IMPLEMENTED + LOCALLY VERIFIED + USER-AUTHORIZED FOR PRODUCTION; MERGE, FLAG ACTIVATION, SUCCESSOR-DRAFT SEED, AND LIVE TESTER SMOKE PENDING.** Assessment creation now requires one explicit persisted delivery choice when the wave is enabled: **Public marketing quiz** or **Invited assessment**. The choice is correctable until first publication, then protected by both API policy and a PostgreSQL trigger. Migration backfill classifies only the two established public aliases (`scaling-up-quick`, `sunhub-quick-quiz`) as public and every other template as invited; runtime code never infers type from aliases. Regular and Public campaign creation enforce mutually compatible stored types, while every campaign continues to pin one immutable version at creation.

**Versioned admin experience and treatment.** Public drafts expose a tester-friendly Settings card with three copied starting snapshots — Full Marketing, Scaling Up Quick, and Start blank — plus plain Text/Image/Button/Divider editing, managed PNG/JPEG/WebP upload, accessible ordering/removal controls, and no raw HTML. The server discards caller-supplied compiled markup, validates HTTPS/mail/phone/dynamic-coach destinations, deterministically compiles an allow-listed audit snapshot, includes reportConfig in the content hash, and blocks public publication until an action-ready CTA is valid. Full Marketing carries official Scaling Up book artwork and the approved assessment, follow-up, and book destinations; Quick retains its separate resource and coach treatment.

**Runtime boundary and rollout.** The Marketing CTA loads only when the global wave is effective, the persisted template type is public, the Campaign is PUBLIC, and the campaign-pinned published version contains a compiler-matching safe CTA. It renders after the existing interactive public result with authored score bands, resolves the Quick coach action to the verified referring coach or coach directory, and is explicitly hidden from print. Invited results, admin/coach report routes, Results report email, PDF/print, existing campaigns, and malformed CTA storage remain unchanged. ADMIN/STAFF can open a no-write draft preview. An idempotent guarded script can create forward-only successor drafts for Scaling Up Quick and SunHub; it never rewrites or auto-publishes the current version, so only campaigns created after an operator publishes the successor can receive the new treatment.

**Verification.** Test-first coverage records the flag kill precedence, schema/backfill/type lock, API and editor classification, fixed presets, reportConfig preservation, unsafe-link rejection, server compilation, publication gate, visual block editor, upload authorization, campaign compatibility, safe runtime parsing, exact SunHub score bands, real book artwork, dynamic coach fallback, and public-result rendering. The focused implementation matrix passed, changed-file ESLint emitted no diagnostics, and the Production-equivalent `CI=true npx next build --turbopack` compiled, passed its TypeScript phase, and generated **95/95** pages. Expected local missing Inngest-key and `DATABASE_URL` static-data messages remained non-fatal. The independent kill switch restores legacy creation/pickers/results after redeploy without deleting persisted version content.

<a id="su-full-peer-ui-production-accepted"></a>
### 2026-08-17 — Scaling Up Full peer UI Production accepted <!-- ENTRY_ISO:2026-08-17 ENTRY_SLUG:su-full-peer-ui-production-accepted -->

**Status: LAUNCHED + PRODUCTION END-TO-END VERIFIED.** Activation PR [#361](https://github.com/ChiefAI-Officer/Scaling-up-platform-v2/pull/361) squash-merged as `af2b0a47dae28353f93998b364d7cacbee231402`; closeout PR [#362](https://github.com/ChiefAI-Officer/Scaling-up-platform-v2/pull/362) squash-merged as `cd766c6414b8227c16868d71262ad3cc9357d386`. Ready activation deployment `dpl_DT1MeunntHmts1WbFkTQpMAzTWtK` captured `WAVE_S_PEER_BENCHMARKS_ENABLED`; the kill switch remains logically off, and later Ready main deployments retained the enabled capability. The prerequisite read confirmed active English Edition **4** and exactly **61** governed/stored QUESTION keys `Q01`–`Q61`, all finite and inside `[0, 10]`.

**Controlled Production fixture.** With explicit operator authorization, the dedicated Production Coach test account created Classic Edition 4 campaign `cmsx9g4al000p8dp4dlqkfkw4` (**TEST Peer UI Production Smoke 2026-08-17**) inside the pre-existing DELETE-ME canary organization. Respondent `cmsx9fdg100078dp4dwv2towj` uses reserved non-deliverable address `codex-peer-ui-20260817@fixture.invalid` and is visibly labelled **TEST DATA — safe to delete**. Because the address intentionally could not receive SMTP, one random 90-day raw invitation token was issued directly in the existing hash-only format and recorded by audit action `TEST_TOKEN_ISSUED`; the raw token was never persisted or logged. The temporary Production environment directory was moved to Trash after use. No real recipient or customer email was used or delivered.

**Canonical respondent and report receipt.** The invited flow collected two unscored company-size inputs, displayed the unchanged **phase 3 – Management phase** interstitial, and then accepted all **61** scored questions across sections 2–11. The completed submission produced overall score **60/100** and immediately rendered the on-screen Classic report. That respondent report and the authenticated Coach **View report** route each showed **122 Peers labels** (61 section-overview pairs plus 61 question-detail pairs) and **123 You labels** (the same 122 pairs plus the Four Decisions `You` domain), with no `NaN`, no `undefined`, and no page-level horizontal overflow (`scrollWidth === clientWidth` at the 1265px verification viewport). Visual inspection confirmed Jeff's approved gold **You** / purple **Peers** paired micro-bars, explicit labels, numeric values, and frozen feedback on both surfaces.

**Print and scope boundary.** **Download PDF** invoked the browser print path and set the expected default filename title `Scaling Up Full Assessment — Codex Peer UI Smoke 20260817`; native Save-as-PDF output is not claimed because the system print dialog is outside browser automation. Turning on `WAVE_S_PEER_BENCHMARKS_KILL` remains the immediate generic-report rollback. The group report, growth-phase survey/interstitial, report email, public mini-quiz/CTA, Executive/Modern layouts, benchmark values, scoring, and feedback selection were not changed by this release or smoke. The authorized fixture remains available for operator inspection. Two evidence images are retained outside the repository in the task's Codex visualization directory.
...
**Status: IMPLEMENTED + LOCALLY VERIFIED + DARK; NOT DEPLOYED OR ACTIVATED.** The two approved Classic report surfaces are implemented from one immutable 61-question presentation: each section first shows an independent paired-bar overview with explicit **You** and **Peers** labels, then matching question detail cards repeat those bars before the frozen feedback. There is no connected contour or second calculation path. Authorized respondent reports, authorized public-submission reports, and the single disclosed invited on-screen result can consume the shared resolver, while session persistence revives the additive presentation without recomputing it.

**Dark-launch boundary.** `PEER_RENDER_ENABLED_ALIASES` still contains only `leadership-vision-alignment`; `scaling-up-full` is intentionally absent. A dedicated default-path guard proves that a Scaling Up Full report performs no template or benchmark query while LVA remains enabled. Scaling Up Full benchmark database rows are therefore reachable only through the explicit test-only `enabledAliases` override until a separately approved activation changes the production alias list. No Production environment, deployment, benchmark row, template version, campaign, response, invitation, email, schema, or migration was read or changed for this work.

**Scope exclusions preserved.** Existing LVA behavior is unchanged. Scaling Up Full group reports, growth-phase behavior, report-email HTML, public mini-quiz results, aggregate/longitudinal exports, benchmark values, assessment content, and feedback text remain unchanged. The implementation is local Phase A only and does not constitute release approval.

**Verification.** The required focused matrix passed **14/14 suites, 313/313 tests, and 1/1 snapshot**; the fresh repository-wide run passed **698/698 suites, 8,715/8,715 tests, and 16/16 snapshots**. Migration safety approved all **47 migrations**. Changed implementation/test ESLint emitted no diagnostics. Repository-wide `tsc --noEmit` retains **427 known baseline diagnostics** outside this task; exact attribution reports **0 diagnostics** in every Task 1–6 implementation/test path after correcting one task-owned fixture cast. The Production-equivalent `CI=true npx next build --turbopack` compiled successfully, completed its TypeScript gate, and generated **94/94** pages; expected local middleware-deprecation, missing Inngest-key, and missing `DATABASE_URL` messages remained non-fatal.

**Local visual and print receipt.** A DB-free real-component fixture rendered with the production SU report CSS and made no external or application-data mutation. Desktop and 393px mobile captures each contained all **61 overview rows**, **61 detail cards**, explicit You/Peers labels, one row-derived **August 18, 2026** disclosure, and no horizontal overflow; mobile rows stacked and no contour/vector markup was present. The A4 print produced **13 pages** with deliberate overview page boundaries and `break-inside: avoid` detail cards; visual inspection of the all-page contact sheet found no clipped detail card. Uncommitted evidence is retained outside the repository at `/tmp/su-full-task6-visual-20260817/desktop-full.png`, `/tmp/su-full-task6-visual-20260817/mobile-full.png`, `/tmp/su-full-task6-visual-20260817/print-a4.pdf`, and `/tmp/su-full-task6-visual-20260817/print-contact-sheet.png`.
...
**Status: LIVE ON PRODUCTION + POPULATED + VERIFIED.** PR #352 launched the governed 61-value snapshot and explicit audited refresh path. The first authorized Production population attempt reached the direct database connection but exceeded Prisma's default five-second interactive-transaction window while performing sequential inserts from a cross-continent operator connection. The transaction rolled back atomically; follow-up reads confirmed **0 benchmark rows and 0 matching audit rows**, so that attempt left Production unchanged.
...
**Production receipt.** Fix PR [#353](https://github.com/ChiefAI-Officer/Scaling-up-platform-v2/pull/353) passed Build, Migration Safety Gate, Assessment Email Lease (PostgreSQL), Vercel, and Vercel Preview Comments, then squash-merged as `070744441bdef37736d231a6998ecf0c89d73b18`. Exact Production deployment `dpl_9R5NjSJAVsKuYmYLEWbcTp17xzc6` is Ready and owns both canonical aliases; both `/api/health` endpoints reported a healthy database and safe auth posture. The authorized refresh, attributed to `gabriel@chiefaiofficer.com`, populated **61/61** rows with version `2026-08-14.esperto-controlled-v1` and effective date `2026-08-14`. A separate read-only process found **61 exact matches**, no missing/unexpected keys, and no mismatches; audit `cmssw6mqq001q63xlslc1nmbx` recorded an after-count of 61. Temporary Production environment and project-link files were removed and their absence verified. No Slack message or customer email was sent.
...
**Status: LIVE ON PRODUCTION.** PR [#350](https://github.com/ChiefAI-Officer/Scaling-up-platform-v2/pull/350) passed Build, Migration Safety Gate, Assessment Email Lease (PostgreSQL), Vercel, and Vercel Preview Comments, then squash-merged to protected `main` as `8495485827947c22964efcf267325d0cd4a1e858`. Production deployment `dpl_6RGwvAHmbdKVE56pK6i6ivgnAQQF` is Ready and owns both canonical aliases. Coach **My Campaigns** and Admin **Campaigns** reuse `CampaignsListWithFilter`, so both surfaces now present the same customer-first hierarchy. Every customer starts collapsed; activating its full-width, keyboard-accessible header reveals only that customer's campaigns; opening another customer closes the first; activating the open header closes it; and choosing **All / Draft / Active / Closed** resets the list to the collapsed overview. Filter counts, filtered customer visibility, campaign detail routes, staged-progress metrics, and empty states retain their existing behavior.

**Compatibility and accessibility.** The change is client-only and adds no API, query, schema, migration, flag, environment, or customer-data mutation. The trigger exposes `aria-expanded` and `aria-controls`; the controlled region remains mounted and hidden while collapsed so that relationship is always valid. It retains the company heading hierarchy, shows a directional chevron, and supplies an inset keyboard focus ring. The newer campaign edition identity plus **Not latest** / **Retired** warnings remain unchanged inside expanded campaign rows.

**Visual and live acceptance.** The customer-overview mockup was reviewed in conversation against Jeff's supplied desktop reference and explicitly approved as “perfect” before implementation. On the deployed Preview, the seeded Coach view visibly started with its customer collapsed, expanded to reveal the pinned Edition 3 campaign and existing metrics, and returned to the collapsed overview when **Draft** was selected. A 393px inspection kept the accordion visible and usable; the page's separately tracked pre-existing Quick Assessment-link/shell horizontal overflow remains outside this feature. The documented Preview admin seed password was stale, so no real account was substituted; Admin remains covered by the same shared component and passing route matrix.

**Production verification and safety.** Both canonical `/api/health` endpoints returned HTTP `200` with a healthy database and safe auth posture from the exact Ready deployment. Unauthenticated requests to `/portal/assessments` and `/admin/assessments/campaigns` returned the expected `307` sign-in redirects. Verification was read-only: no Production login, campaign, assessment version, response, invitation, email, schema, migration, feature flag, environment value, or other customer data was created or changed.

**Test-first verification.** On the latest `main` baseline, the related matrix first passed **3 suites / 31 tests**. The new accordion expectations then failed **22 tests** because customer headings were not controls and all campaign rows rendered immediately. After implementation, the focused coach/admin matrix passed **3 suites / 34 tests**. Independent review then caught the collapsed-state `aria-controls` target being unmounted; a regression test failed first, the controlled region was kept mounted and hidden, and the final focused matrix passed **3 suites / 33 tests**. The repository-wide run passed **689/692 suites, 8,621/8,624 tests, and 16/16 snapshots**; its three failures were unrelated async UI tests that timed out only after prolonged single-process contention, and all three passed immediately in isolation (**55/55 tests**). Changed-file ESLint emitted no diagnostics; migration safety approved all **47 migrations**; `git diff --check` passed; and the final Production-equivalent Turbopack build compiled in **65s**, passed TypeScript, and generated **94/94** static pages. The build retained only the expected local middleware-deprecation and missing Inngest-key/`DATABASE_URL` diagnostics.
...
**Status: BUILT + LOCALLY VERIFIED; not yet pushed, merged, deployed, or written to Production.** When simplified assessment creation and the coordinated ADMIN-owned presentation capability are active, ADMIN and STAFF now see the same fixed `WelcomeScreenCard` used in Build directly after **Assessment name** and before **Advanced** / **Internal ID**. It remains collapsed by default, clones the generic Welcome authoring values without mutating their readonly paragraph array, and expands to the same seven fields and respondent preview. The creation preview truthfully uses `Example campaign`, zero questions, and zero sections. **Cancel** and **Create and start building** remain the only page actions; no card-level save was added.

**Validation, request, and atomicity.** Shared authoring-schema errors stop the POST, expand the card, preserve the entered values, remain associated with their controls, and focus the first invalid field on every failed submit, including an unchanged repeated submit while the card is already expanded. An enabled valid POST sends exactly the seven authoring fields only; a route-local exact-key guard rejects arbitrary nested properties such as layout, styles, disclosure, or calculated facts before any transaction while preserving shared-schema compatibility. The API supplies server-owned `schemaVersion: 1` and `finePrint: null`, then writes the template Welcome default with the template and empty v1 draft inside the existing transaction. Internal ID generation, retry/collision handling, audit timing, response shape, duplicate-submit latch, Cancel behavior, and redirect to the new draft's Build tab remain unchanged. Omission remains compatible and persists the generic default. With the feature off or killed, Create returns to the name-only form, the old strict request contract rejects a forged Welcome property before any transaction, and persistence remains generic.

**Lifecycle and isolation.** This capability changes only a newly created template default. It does not alter any existing template, invited campaign Welcome snapshot, DRAFT/ACTIVE/CLOSED/historical campaign, or response. A future INVITED campaign still freezes the then-current template default through the already-shipped snapshot path; PUBLIC campaigns remain null on that path and keep their separate campaign-authored Welcome behavior. No Production template, campaign, response, email, environment flag, or database row was created, edited, or deleted while implementing or verifying this release.

**Fresh release evidence.** After the final review fixes, the prescribed enabled matrix passed **9 suites / 143 tests / 0 snapshots**, the coordinated enabled-plus-kill matrix passed **3 suites / 103 tests / 0 snapshots**, and the complete repository passed **683 suites / 8,505 tests / 16 snapshots**. The focused route/form pair passed **2 suites / 94 tests**, and the affected card suite passed **1 suite / 6 tests**. Git-derived changed-file ESLint emitted no diagnostics; Prisma client generation succeeded; migration safety approved all **47 migrations**; and both Production-equivalent Turbopack builds—enabled and enabled with kill overriding—compiled successfully, completed TypeScript, and generated **94/94** static pages. The complete Jest stream retained established negative-path logs and React `act(...)` warnings, and static generation retained established missing local `DATABASE_URL` and Inngest-key warnings; none failed a suite, test, TypeScript phase, build, or gate. The post-receipt changelog-freshness suite passed **1 suite / 4 tests / 0 snapshots**, and final-fix diff hygiene was silent.

**Visual-acceptance limitation.** No authenticated current-branch Preview or seeded local environment was safely available. The in-app browser had no authenticated tab, this branch has no deployed Preview, and the worktree has no local environment; the only locally available database configuration points to the remote Production Neon database and was deliberately not used. Browser policy also blocked localhost wireframe rendering. Therefore no desktop collapsed, desktop expanded, 1024 px, redirected Build, or screenshot acceptance is claimed. The Markdown and HTML visual contracts now contain the Create collapsed and expanded states, shared fields-before-preview stack, and zero-question/zero-section preview, but authenticated non-Production visual acceptance remains required before release.
...
**Status: BUILT + LOCALLY VERIFIED + DEFAULT-OFF; not yet pushed, merged, deployed, enabled, or written to Production.** ADMIN and STAFF can author the seven approved plain-text invited Welcome fields in a fixed, collapsible, nearly full-width card at the top of each assessment's Build canvas. The card uses the existing page-level Save Draft action, preserves sequential blank-line paragraph typing, provides a safe live respondent preview, and exposes no editor for calculated facts, disclosure copy, fine print, icons, layout, or styles. Coach campaign creation and detail hide Report appearance/style authoring only while the coordinated flag is active; new Coach campaigns then inherit the ADMIN-owned assessment report default while existing report renderers, stored campaign styles, report links, and first-response locks remain intact.

**Lifecycle and safety boundary.** Additive migration `20260810160000_add_invited_welcome_snapshots` adds nullable template defaults and campaign snapshots, backfills exact frozen legacy per-alias copy for every non-deleted template and every INVITED campaign regardless of DRAFT/ACTIVE/CLOSED state, leaves PUBLIC campaigns null, and installs an immutable non-null snapshot trigger. Every Coach and historical-import creation path writes the snapshot inside its transaction even when presentation is off or killed; template edits therefore affect future campaigns only and cannot retroactively alter an already-created campaign. The verifier checks schema validity, exact alias values, null PUBLIC snapshots, and an enabled trigger bound to the expected function. Participant responses receive only the validated campaign snapshot while enabled; management responses strip the persistence fields from legacy client contracts. Forged Coach report-style create/PATCH writes fail with stable ownership errors and no mutation or audit. ADR-0033, the superseded ADR-0026 marker, domain glossary, wireframes, and the rollout/rollback runbook record the ownership boundary.

**Fresh release evidence.** The active and enabled-plus-kill matrices each passed **35 suites / 542 tests / 0 snapshots**. After the final response-contract repair, focused validation passed **4 suites / 84 tests**, and the complete repository passed **683 suites / 8,475 tests / 16 snapshots**. Prisma client generation passed; migration safety approved all **47 migrations**; changed-file ESLint and diff hygiene passed. Independent review completed with no remaining Critical, Important, or Minor findings and a **Ready to merge: Yes** verdict. Both final Production-equivalent Turbopack builds—feature enabled and enabled with kill overriding—compiled successfully, completed TypeScript, and generated **94/94** static pages. Static generation emitted the repository's expected missing local `DATABASE_URL` and Inngest-key messages without failing either build. Standalone repository-wide `tsc --noEmit` still reports pre-existing test/config diagnostics; filtered output contained no branch-owned diagnostic, and both authoritative Next builds completed their TypeScript phase. Source wireframes were reviewed; local-file browser rendering was blocked by the browser's file-navigation policy, so authenticated Preview/Production responsive visual acceptance remains part of rollout. No Production assessment, template, campaign, response, submission, report, email, flag, environment value, database row, or customer data was read or changed during local implementation.
...
**Status: CODE COMPLETE + DEFAULT OFF; not pushed, merged, deployed, Production-enabled, or Production-accepted.** The approved visual design replaces the jargon-heavy combined screen, when enabled, with a list-first management page and a dedicated creation page. The active path says **Public campaigns**, **Create campaign**, **Draft / Live / Closed**, natural-language availability, and **responses**; it does not expose raw aliases, enum values, API/schema commentary, or an organization selector. The unchanged legacy `PublicCampaignsManager` remains the exact flag-off/kill-switch fallback.

**Organization-free workflow and eligible choices.** Creation still sends the established PUBLIC body without `organizationId`, and the endpoint persists `organizationId = NULL` and `createdByCoachId = NULL`; the prior organization-free authorization and data boundaries remain intact. The new server-only `listPublicCampaignCreateOptions()` service supplies only non-deleted, non-disabled assessments with an unarchived published `en-US` version, ordered by name, plus report-design capability metadata. The flagged list response maps its internal submission count to a plain `responseCount`; the default-off response shape remains unchanged. No Prisma schema or migration changed in this UI wave.

**Preserved management behavior.** Draft rows publish through the existing confirmation operation; Live rows copy the complete public link, with announced success and a labelled manual-copy fallback; Live and Closed rows retain lazy, cached inline response review and report links. Report-design disclosure continues to use the existing capability, appearance, lock-on-first-response, conflict, and save behavior. Creation preserves immediate or scheduled starts, optional scheduled end, report-design inheritance/override, accessible field validation and first-invalid focus, cancel back to the list, and successful redirect to a focused status message plus highlighted draft. The responsive list follows wireframe 25 at desktop and narrow-laptop breakpoints. `CONTEXT.md`, the wave-6 wireframe note, the current HTML wireframe/index, the approved design, and the implementation plan record the list-first/dedicated-create language and organization-free invariant.

**Flags and rollout boundary.** `WAVE_PUBLIC_CAMPAIGNS_SIMPLE_UI_ENABLED` activates the new presentation; `WAVE_PUBLIC_CAMPAIGNS_SIMPLE_UI_KILL` wins over it and restores the legacy page while `/admin/assessments/public-campaigns/new` redirects to the list. Both are read at call time and the feature defaults OFF. This receipt does not authorize a push, PR, merge, deployment, environment change, seed, or Production-data operation.

**Fresh local verification on `b10af1f2`.** The prescribed focused matrix passed **13 suites / 136 tests / 0 snapshots** (exit 0). Git-derived ESLint checked all **24** changed TypeScript/TSX files with no diagnostics (exit 0). The complete repository passed **673 suites / 8,374 tests / 16 snapshots** (exit 0); its established console stream included deliberate negative-path errors, React `act(...)` warnings, report fallback telemetry, and missing local integration-key notices, but no failed suite or test. Migration safety checked all **46 migrations** with no unapproved destructive operation (exit 0). `CI=true npx next build --turbopack` compiled in **13.1 s**, completed TypeScript, and generated **94/94** static pages (exit 0); the established non-fatal local warnings were the deprecated middleware convention, absent Inngest keys, and absent `DATABASE_URL` during static data fetches. After this receipt was written, the changelog-freshness check passed **1 suite / 4 tests** (exit 0). `git diff --check` passed, the legacy-manager diff was empty, and no forbidden jargon matched any new component or added production line. The task's literal route-wide jargon command still found the two unchanged legacy fallback lines already present on `origin/main`; they are intentionally retained for byte-neutral rollback.

**Visual-acceptance limitation and retained review ledger.** A fresh isolated local PostgreSQL fixture was provisioned with eligible assessments, Draft/Live/Closed rows, and local-only responses, and the development server reached Ready with the feature and supporting local flags enabled. The only connected in-app browser rejected `localhost`, `127.0.0.1`, and the private LAN URL with `ERR_BLOCKED_BY_CLIENT`, so no fresh browser screenshot, responsive geometry, focus-trap walk, clipboard walk, or flag-off visual acceptance is claimed in this receipt. The focused component and route suites cover those interaction contracts, but controller-side browser acceptance remains required before rollout. Two deferred minors remain recorded in the SDD ledger: the pre-success raw-link absence assertion and list-level response-cache/exclusivity coverage. The human-adjudicated temporary `SubmissionResult` duplication also remains accepted rollback debt until feature-flag retirement after Production acceptance; none of these was silently erased.
...
**Status: SOURCE-BACKED BUILD IN PROTECTED REVIEW; Production activation remains pending.** PR [#320](https://github.com/ChiefAI-Officer/Scaling-up-platform-v2/pull/320) carries the build from implementation commit `53d48fab`. The tracked SunHub workbook supplied the previously missing authoritative package: eight ordered 0-10 questions, four score bands with feedback, source result visuals, and three CTA destinations. The implementation adds a separate `sunhub-quick-quiz` PUBLIC template and leaves the existing 32-question `scaling-up-quick` instrument unchanged.

**Bounded implementation.** The additive seed path creates only a DRAFT through the existing guarded template-version machinery and is not invoked automatically. Eight fresh `sunhub_*` stable keys preserve isolation. Shared scoring produces a 0-100 result from the eight-question mean with exact 24/25, 49/50, and 74/75 display transitions. The public lead screen and report-email output use the source's four feedback messages and three CTA destinations while omitting generic detail, score-table, and report-metadata sections not present in the source. Public SunHub results remain on the source-faithful Classic presentation; every other template and non-public report keeps its existing behavior.

**Evidence and safety boundary.** After independent review found that the inner SunHub report could be Classic while its outer surface still advertised a frozen alternate style, a red regression test pinned the full-surface contract and the shared source-result policy now forces both layers to Classic. The post-review focused matrix passed **9 suites / 189 tests / 2 snapshots**, covering exact source content, section order, schema validity, tier seams, CTA destinations, immediate public results, public email output, later authorized public-result review, invited-report isolation, renderer compatibility, outer-style agreement, and tracked-ledger integrity. The complete repository then passed **663 suites / 8,286 tests / 16 snapshots**; one pre-existing focus-timing test failed on the first run, passed alone, and the complete rerun passed. Changed-file ESLint, Python compilation, the artifact generator check, migration safety across **46 migrations**, and diff hygiene passed. After regenerating the worktree's copied Prisma client, `CI=true npx next build --turbopack` compiled successfully, completed TypeScript, and generated **94/94** static pages. The 14-page status overlay and unchanged one-page progress delta were regenerated and visually reviewed; the source-backed result itself was reviewed in DB-free desktop and mobile captures without clipping or overlap. Row #84 moves from NEEDS DECISION to PARTIAL, producing **44 DONE / 4 PARTIAL / 5 NEEDS DECISION**. The August 1-7 progress delta stays at exactly twelve outcomes because this work began on August 8. No seed was run; no template version, publish state, campaign, response, submission, report, email, flag, schema, migration, benchmark, customer data, environment value, or other Production state was read or changed.
...
**Status: IMPLEMENTED + REBASED + LOCALLY VERIFIED + DEFAULT-OFF; ready for protected review, not yet merged, deployed, or activated.** The Jeff-anchored MVP adds report-native comparison only to an authorized individual `scaling-up-full` report. Coach, ADMIN, and STAFF viewers can select one eligible frozen earlier submission for the same person, organization, and template. An invited participant designated CEO can exchange a short-lived fragment capability for a sealed, exact-path session and compare only their own eligible history. The selector is inactive until a baseline is chosen, supports native and Esperto-imported history, exposes bounded coverage and compatibility states, canonicalizes Remove comparison, and prints the same frozen current/previous/change facts. The existing single-campaign group report, group access rules, aggregate reports, Trends, and cross-time group behavior are unchanged.

**Current-main integration and final repairs.** The feature branch is rebased onto `cd611191`, after the globally launched universal individual-report appearance work. Comparison now composes with the server-authorized `ReportStyleScope` and the shared `IndividualReportPresentation` contract rather than restoring the retired alternate-renderer model: Classic, Executive Boardroom, and Modern Dashboard consume the same authorized comparison facts, safe imported provenance, and presentation labels. The public submission route preserves the closed report-style candidate catalog and shared email-render cache while binding any CEO link to the locked participant authorization before persistence or disclosure. Campaign Detail retains the current appearance picker/capabilities and restores the legacy Coach-only **Over time** entry only while report-native comparison is inactive. ADMIN never receives that rollback link. Final fixed-point repairs also make OFF/KILL short-circuit before token/rate-limit/database work, version the fixture content hash, null incompatible non-finite question values, report the actual bounded candidate count, revalidate the live CEO grant before candidate-history reads, preserve distinct Coach/Admin visual artifacts, select the correct A4/Letter PDF contract per appearance, and complete role, revocation, cross-organization, responsive, PDF, style-lock, and zero-delivery acceptance assertions.

**Fresh release evidence.** From `src/`, the prescribed comparison matrix passed **15 suites / 215 tests**; the legacy longitudinal/group matrix passed **8 suites / 156 tests / 1 snapshot**; the audit, on-screen, portal/admin, provisioner, cleanup, launcher, and cookie-runtime group passed **9 suites / 99 tests**; and the focused final-repair group passed **5 suites / 47 tests**. The complete repository then passed **659 suites / 8,216 tests / 16 snapshots**. ESLint passed across every changed TypeScript, TSX, JavaScript, MJS, and CJS file. Migration safety checked **45 migrations** with no unapproved destructive operation. `CI=true npx next build --turbopack` compiled successfully, completed TypeScript, and generated **94/94** static pages. `PLAYWRIGHT_SKIP_WEBSERVER=1 npx playwright test e2e/report-comparison.spec.ts --list` discovered the four expected desktop/mobile Coach/Admin and CEO/non-CEO cases. Diff hygiene is clean, and no schema or Prisma migration is added.

**Unrun acceptance and rollout boundary.** The destructive browser workflow remains sentinel-guarded and intentionally unrun because this checkout has no proven disposable database URL, matching sentinel row, exact fixture scope, fixture credentials, or 32+ character report-access secret. Therefore no real browser, screenshot, PDF, print-preview, authenticated Production, email, or customer-data claim is made. Merging this branch must keep `WAVE_RC_REPORT_COMPARISON_ENABLED` unset/false, `WAVE_RC_REPORT_COMPARISON_CANARY` empty, and the capability dark; `WAVE_RC_REPORT_COMPARISON_KILL=1` remains the immediate containment control. `ASSESSMENT_REPORT_ACCESS_SECRET` is required before any later authorized CEO canary or global activation. A Production code deployment is not an activation decision.
...
**Status and scope.** **IMPLEMENTED + LOCALLY VERIFIED + READY FOR PROTECTED REVIEW; not pushed, merged, deployed, or globally activated.** Classic, Executive Boardroom, and Modern Dashboard are now one closed appearance policy for every individual on-screen/browser-print report family rather than only `scaling-up-full`. Admin template defaults, Coach campaign creation and pre-completion overrides, public-campaign ownership, read-only campaign detail, and the public and invited report routes use the same registry and server-authorized policy. Group, aggregate, longitudinal/trend, CSV, and report-email HTML surfaces remain unchanged.

**Data and concurrency behavior.** Existing template and campaign values remain stored enums; no value is rewritten, re-scored, or inferred from preview content. New campaigns copy the template default, an explicit campaign choice records `CAMPAIGN_OVERRIDE`, and the first successful completion atomically freezes the authoritative campaign appearance before submission plus durable delivery work commits. Public and invited immediate responses use the final row-ordered style, failed transactions roll the lock back, idempotent replay re-reads the committed lock, and both historical import writers preserve the stored style while applying the earliest imported completion as the lock minimum. No repair migration or production-data mutation was added.

**Flags, canaries, and rollout boundary.** `WAVE_REPORT_STYLES_KILL` overrides both global `WAVE_REPORT_STYLES_ENABLED` and the exact template/campaign allowlist in `WAVE_REPORT_STYLES_CANARY`; unset, invalid, ineligible, or killed paths fail closed to Classic without erasing stored values. Production is unchanged: the global and kill variables remain absent and the existing disposable exact-campaign canary remains the only live exposure. Protected review, hosted database-backed concurrency coverage, merge/deploy, separately authorized synthetic completion → frozen report acceptance, and any global activation are still required. No customer campaign, response, report, email, flag, or Production record was read or changed in this closeout.

**Anatomy-matched visual evidence and reviewer correction.** The picker selects scored, qualitative, or sparse-custom content from canonical report configuration plus stored active-version question types; it does not classify from aliases, labels, keys, or synthetic defaults. Those capabilities are threaded through Settings, campaign creation, campaign detail, and public-campaign controls, so an arbitrary narrative-only template reaches sparse-custom previews. Fixtures are deeply frozen, contain only invented content, and have behaviorally distinct normal, partial, degraded, maximum-length, and missing-block sparse variants without invented scores. The renderer harness now loads the actual compiled Next font variable classes and font bytes from the production build rather than empty test stubs. Capture executes real image/PDF checks for dimensions, foreground/color variation, visible content/provenance, descendant bounds, non-hidden overflow, extracted PDF markers, and nonblank rasterized pages. The final pipeline regenerated and visually reviewed **27 committed WebPs** (3 anatomies × 3 appearances × Cover/Summary/Detail), retained the nine scored compatibility paths, and asserted one physical content-bearing page using A4 for Classic and US Letter for both alternates.

**Browser, parity, and gate evidence.** Both the authenticated preview-route matrix and the DB-free renderer matrix now contain Classic, Executive Boardroom, and Modern Dashboard across all three anatomies and all six stress variants. The locally executable DB-free lane passed all **54 combinations**, each with real production-font intent, Axe, empty-composition/color-only checks, desktop/mobile screenshots, horizontal-overflow checks, and a complete PDF whose every physical page is nonblank; it exposed and corrected Classic contrast plus long-branding overflow while the available/unavailable Classic DOM remained byte-identical. The authenticated matrix additionally requires exactly one selected preview page per PDF. It was attempted against a local production server with a signed ADMIN JWT and reached the route's current-user authorization, but Prisma correctly stopped because no `DATABASE_URL` is available; it is not counted as a pass.

**Final verification.** Reviewer-fix focused coverage passed **7 suites / 55 tests**. The finished repository passed **643 suites / 7,993 tests / 16 snapshots** in **129.819 seconds**. Changed-file ESLint, diff hygiene, migration safety across **45 migrations**, final 27-entry capture integrity, and `CI=true npx next build --turbopack` passed; the build compiled in **22.3 seconds**, completed TypeScript, and generated **93/93** static pages. Repository-wide `tsc --noEmit` remains non-green at **401 inherited diagnostics**, with **zero diagnostics in a changed file**. The real PostgreSQL lock suite compiled and reached its explicit destructive guard, then stopped because `TEST_DATABASE_URL` and `ASSESSMENT_EMAIL_LEASE_TEST_ALLOW=isolated-schema` are unset; Docker is unavailable. Authenticated preview-route execution and the three PostgreSQL concurrency cases remain required protected hosted gates.

<a id="template-creation-simplification-launched"></a>
### 2026-08-06 — Template creation simplification launched <!-- ENTRY_ISO:2026-08-06 ENTRY_SLUG:template-creation-simplification-launched -->

**Status: LAUNCHED + GLOBALLY ENABLED IN PRODUCTION.** The ADMIN/STAFF new-assessment route now has a name-first handoff into the existing editor: a collapsed **Advanced** section exposes the generated or manually overridden immutable Internal ID, the server atomically creates one empty unpublished v1 draft with canonical defaults and bounded alias-collision retries, and success opens the exact Build URL at `.../edit?tab=questions`. The existing ED9 FormsBuilder remains the sole section/question authoring surface and retains Slider, Short text, Number, and Multiple choice under the existing Wave T rules. The same release gate presents assessment-wide scoring and tier configuration in approved plain language while preserving raw stored keys, calculation behavior, validation, report behavior, and kill-restored legacy rendering. There is no schema or Prisma migration.

**Protected delivery and fresh verification.** PR [#309](https://github.com/ChiefAI-Officer/Scaling-up-platform-v2/pull/309) passed Build, Migration Safety Gate, Assessment Email Lease (PostgreSQL), Vercel, and Vercel Preview Comments, then squash-merged as `1433d95e4eec0adc95c1a48ff8cc190b31504c2f`. Immediately before merge, the complete Jest suite passed **646 suites / 7,946 tests / 16 snapshots** on the exact branch head. The earlier final gate also passed changed-file ESLint, migration safety across **45 migrations**, the exact targeted release matrix at **11 suites / 218 tests**, and `CI=true npx next build --turbopack` with TypeScript complete and **93/93** static pages. Independent whole-branch review found no Critical or Important issue; its two Minor interaction findings were fixed test-first and scoped re-review was clean.

**Local-only visual smoke.** The in-app browser exercised `http://127.0.0.1:4317` against a disposable loopback-only PostgreSQL 16 database at `127.0.0.1:55432`, bootstrapped from the current Prisma schema with one local ADMIN user. With `WAVE_TEMPLATE_CREATION_SIMPLIFIED_ENABLED=1`, ED6, ED9, Wave T, and ED10 enabled, and the new kill absent, the new route showed only Assessment name plus collapsed Advanced; entering `Task 10 Local Smoke` generated `task-10-local-smoke`; creation opened the new v1 Build tab at the exact `?tab=questions` URL; the empty builder showed its existing message and **+ Add section**; adding a section and question exposed all four approved question types; and Scoring & Tiers showed **How results are calculated**, **Overall result tiers**, **Before you can publish**, and **Example result** with zero hits for the ten forbidden raw/schema terms. Restarting locally with `WAVE_TEMPLATE_CREATION_SIMPLIFIED_KILL=1` restored the legacy metadata/content creation form and the legacy raw Scoring & Tiers copy. The disposable database and app processes were stopped after the smoke; the local draft was never published.

**Production activation and acceptance boundary.** `WAVE_TEMPLATE_CREATION_SIMPLIFIED_ENABLED=1` is present only in Production; `WAVE_TEMPLATE_CREATION_SIMPLIFIED_KILL` remains absent. Flag-enabled redeployment `dpl_BBUMTPFiPV46RRarnko7iH9JmNbh` reached Ready and owns `platformtest.scalingup.com` plus all three Vercel production aliases. The public health endpoint returned HTTP `200` with a healthy database and safe auth posture, and the protected admin route continued enforcing authentication/authorization. The automated browser's existing non-admin session was correctly denied, so no authenticated Production form submission was attempted: no Production template, version, publish action, customer data, or email was created or changed. The production feature can be visually reviewed by an ADMIN/STAFF user at `/admin/assessments/templates/new`; rollback remains a one-variable kill plus redeploy.

---

<a id="report-comparison-final-review-closed"></a>
### 2026-08-06 — Report-comparison final review gaps closed <!-- ENTRY_ISO:2026-08-06 ENTRY_SLUG:report-comparison-final-review-closed -->

**Status: IMPLEMENTED, DEFAULT-OFF, AND NOT LAUNCHED.** Runtime and test corrections are implemented in `e1fab9ce04a8f5b73fe1f6e16280ea81d7ba218a`, strict-limiter/fixture correction `4c045769bd104fa8e8b32d6ea0a9212a42e67207`, and acceptance completion `b012ba5c5c6d9afddf92368cc5017f85df29fdf8`; the tracked closeout documentation completes the remaining evidence and guidance findings. The public CEO exchange now applies `RateLimits.standard` through the strict fail-closed wrapper before parsing or token examination; a missing or failing distributed backend returns the generic privacy-safe unavailable response. CEO access tokens cannot exceed 30 days; canonical capability audits use `CEO_SELF`; selected comparisons revalidate the invitation, campaign, organization, respondent, disclosure, participant CEO designation, and exact focus binding inside the same Serializable transaction that reloads the focus and baseline. Raw bearer URLs are exchanged function-locally after invited submission, and only the clean exact report path can enter React state or rendered markup.

**Comparison correctness and rollback.** Incompatible question types, changed slider bounds, or non-finite bounds now produce dashes for both Previous and Change. Imported provenance is visible and printable in Classic, Executive Boardroom, and Modern Dashboard. Empty `compareTo` values fail generically, controls reset when server props change, and reaching either the 50-identity or 200-row inspection ceiling marks the candidate list bounded even after one-per-campaign collapse. Global OFF/KILL short-circuits comparison services before reads or transactions; middleware opens the self-access shell, exchange, and individual report only while some rollout scope is possible. While report-native comparison is disabled, the coach campaign detail restores the existing eligible Wave N “Over time” links and checks; when enabled, those N+1 checks and links are suppressed. Admin never receives the coach-only rollback entry.

**Executable acceptance contract; no browser claim.** The disposable fixture uses distinct same-person identities per renderer, one CEO per campaign, lifecycle-faithful style locks, deterministic reuse of its namespaced published version, and a separate active Classic focus with pending CEO/non-CEO invitations plus its own native/imported history. The guarded Playwright workflow repeats native/imported Change/Remove facts across all three renderers through both coach and admin actions, then answers and submits the real pending invitations, proves the CEO sees and compares only their own live native/imported history, and proves the non-CEO receives no comparison link. It also covers imported print provenance, 1440px and 390px captures, Letter PDFs, exact-path CEO access, direct cross-organization denial, and independently restored disclosure, CEO designation, invitation states, and style lock. The live focus disables results email, and the launcher explicitly disables Wave D results-email and delivery-intent gates while enabling only the fixture process’s comparison, report-style, and on-screen-result flags after exact URL plus live sentinel-row proof. In this checkout all five required database/sentinel/secret inputs were absent, so no server, database, browser, screenshot, PDF, visual review, Production request, deployment, or canary operation ran. Playwright discovery alone listed four cases.

**Verification.** The fresh prescribed comparison matrix passed **15 suites / 193 tests**; the legacy matrix passed **8 suites / 149 tests / 1 snapshot**; the added audit, on-screen, portal/admin, provisioner, cleanup, launcher, and cookie-runtime group passed **9 suites / 93 tests**. The final review correction slice passed **4 suites / 45 tests**, and the pre-correction full repository run passed **653 suites / 8,007 tests / 16 snapshots** before every affected matrix was rerun. Changed-surface ESLint passed. Migration safety checked **45 migrations** with no unapproved destructive operation. `CI=true npx next build --turbopack` compiled successfully, completed TypeScript, and generated **94/94** static pages. Earlier `report-comparison-verification-prepared` language described intended browser assertions as though they were executable evidence; this entry is the controlling correction: the contract is complete and statically executable, but authoritative browser/print evidence remains pending a proven disposable environment and human visual inspection.
...
**Acceptance and security-runtime correction.** Every fixture submission at this checkpoint was invitation-bound, including a same-normalized-email other-organization completed submission intentionally absent from the focused CEO picker. The Playwright code queried frozen rows and minted a CEO report capability in the test process against a provisioned already-submitted invitation; it did **not** yet drive an invitation through answering and submission, nor did this unrun source constitute browser evidence. The later `report-comparison-final-review-closed` entry records the actual-submission contract. A real `iron-session` runtime regression test reproduced Jest/jsdom resolving `uncrypto`'s browser ESM export through CommonJS `iron-session`; Jest now maps that one dependency to its supplied Node CommonJS export. This is not a mock or a bypass of cookie capability behavior.
...
**Status: IMPLEMENTED, DEFAULT-OFF, AND NOT LAUNCHED.** The report-native comparison implementation is contained in commits `6d41a937011d624b2c7101eb87b20a34d03e3625`, `77c2abc45dae9e2a0ef154f030e4e9754b5d03d7`, `c614ad012358f27861e2d5c021e142948484eb12`, `7c9a69bb0f0698178c83dbbb058f11bde705d9e5`, `dc3cef80684c65f2b0ded515386b62f9d6f7c048`, `385d7ebfdc7fd0e778e3dda7193228817b05cb68`, `65401a43ee8236bdc2eb3fc0a341abecdde5c4d5`, `e7ea9a51b927e7bcd102e96d5b9cc57b6f6e71c2`, `9e00e37ab8de3ec422904da44d35f90aae7c9d40`, `71202269d66e2fe9f45cb1e6c8042c6879653ec9`, `5dc9129ec27ca2252473015eb036d1034e286035`, `fe64e93e22080f1077c582baeaa9d4f138982ba1`, `297c3df58070b47fc741bdfd6efc06bfad370615`, `0bdb1b8900297af238f311981d35fcc882161c13`, `6707766ef0341078ca076368cb39fdf2d3c43877`, and `d3e750dad6e2ebe1ad6d9e85fce0fa3e9676888a`. It compares one frozen current Scaling Up Full submission with one eligible frozen earlier submission inside the canonical individual report; Classic, Executive Boardroom, and Modern Dashboard receive the same current/previous/change facts. No migration was added. The existing group report remains a single-campaign CEO-versus-team artifact and has no report-comparison controls or cross-time behavior.

**Dark configuration and rollout boundary.** `.env.example` now records `ASSESSMENT_REPORT_ACCESS_SECRET`, `WAVE_RC_REPORT_COMPARISON_ENABLED=0`, `WAVE_RC_REPORT_COMPARISON_CANARY=`, and `WAVE_RC_REPORT_COMPARISON_KILL=0`. Production must set a unique secret of at least 32 characters before enabling CEO self access. No Production variable, deployment, organization/template canary, or canary value was set or inferred. A later authorized canary must use only the exact disposable organization or template identifier in `WAVE_RC_REPORT_COMPARISON_CANARY`; global enablement remains a separate decision. To contain the feature, set `WAVE_RC_REPORT_COMPARISON_KILL=1` and redeploy. The kill switch overrides both the global flag and a canary; clearing the global/canary variables is the slower default-off rollback.

**Prepared acceptance contract and evidence at this checkpoint.** `src/e2e/report-comparison.spec.ts` was an opt-in, sentinel-guarded disposable-database suite, but its intended coach/admin, actual invited submission, responsive-capture, and PDF assertions were incomplete and no browser ran. The source did cover DB-derived candidates/renderers plus a manually minted submitted-invitation capability. The prescribed comparison Jest matrix ran **14 suites / 162 tests passing**, with `report-comparison.test.ts` unable to initialize because Jest loaded `iron-session`'s untransformed `uncrypto/dist/crypto.web.mjs`; the legacy matrix similarly had **7 suites / 125 tests passing**, with `group-report-route.test.tsx` blocked at that inherited ESM boundary. Changed-file ESLint passed; migration safety passed across **45 migrations**; and `CI=true npx next build --turbopack` completed successfully after compilation and TypeScript. The later final-review entry supersedes the incomplete browser-contract description and records the fully green Jest runtime.

**Browser/print status.** No managed disposable fixture database, sentinel, exact canary scope, credentials, or report-access secret is available in this checkout. The literal Playwright command was attempted without that environment and did not produce trustworthy browser evidence; the guarded no-webserver check enumerated **6 skipped** tests. No screenshot, PDF, browser-print review, visual acceptance, Production check, or canary acceptance is claimed. Run the suite only after the isolated fixture provisioner supplies its required environment and inspect its generated 1440px/390px screenshots and Letter PDFs before launch.

---

<a id="report-style-canary-launched"></a>
### 2026-08-06 — Report-style exact-campaign canary launched <!-- ENTRY_ISO:2026-08-06 ENTRY_SLUG:report-style-canary-launched -->
...
**Test-first correction and protected delivery.** A regression test recreates the live object-identity churn and proves an unsaved Executive Boardroom choice remains selected and Save remains enabled when the server projection is equivalent. The test failed against the shipped code, then passed after splitting overview adoption from report-style synchronization and keying the latter only to campaign ID, authoritative style, and lock state. A companion case confirms a genuinely changed authoritative style/lock is still adopted. The full Campaign Detail component matrix passed **12 suites / 71 tests** before that companion assertion was added; the fresh repository run passed **641 suites / 7,869 tests / 16 snapshots**. Changed-file ESLint, changelog freshness, migration safety across **45 migrations**, diff hygiene, and the Turbopack production build passed, with TypeScript complete and **93/93** static pages generated. Independent follow-up review found no Critical, Important, or Minor issue. Correction PR [#307](https://github.com/ChiefAI-Officer/Scaling-up-platform-v2/pull/307) passed all five hosted checks and squash-merged as `f7617969ff6e87073a7e70e4b93d4e416b288c02`.

**Live canary acceptance.** Exact Production deployment `dpl_Ft3KLtZ1UqqC4mdSvqyRrNaxXX6x` is Ready on all four aliases, and both public health endpoints return a healthy database and safe auth posture. In a fresh isolated browser, the synthetic Demo Coach saw all three choices on the exact canary. Executive Boardroom remained selected beyond the former reset window, Save became enabled, the PATCH returned HTTP `200`, the success toast rendered, and reload retained Executive Boardroom. A Production readback confirmed `reportStyle=EXECUTIVE_BOARDROOM`, `reportStyleSource=CAMPAIGN_OVERRIDE`, `reportStyleLockedAt=null`, DRAFT status, and zero participants, invitations, or submissions. Modern Dashboard also remained selected in an unsaved interaction and returning to the persisted Executive choice disabled Save, confirming both alternate cards use the repaired draft path. A React hydration warning also reproduced on the shared Admin Campaign Detail surface where the report-style picker is absent, so it is not specific to this canary gate or correction; no visible or functional failure accompanied it.

**Remaining gate.** The controlled campaign-level canary is live and accepted, but the global gate remains off. Do not enable `WAVE_REPORT_STYLES_ENABLED` or change the Scaling Up Full template default until the separately authorized synthetic completion → atomic lock → respondent report → desktop/mobile/Axe/Letter-print acceptance path passes for both new styles. No real Coach campaign, participant, invitation, response, submission, report email, or customer data was used.
...
**Status: EVIDENCE AND DECISION PACKAGE PREPARED; no runtime or Production change.** The original July 10 feedback report's Section 2 is now represented by a tracked, tested 53-row ledger at [docs/agents/jul10-feedback-closeout.md](../docs/agents/jul10-feedback-closeout.md). It reconciles the exact asks against origin/main at `15ee442b124f43155ffd3dfdc1dc08fbfa37cd5e` and records **42 DONE / 4 PARTIAL / 7 NEEDS DECISION**. Rows #31, #34, #36, #38, and #82 are intentionally absent, and page-boundary row #57 is explicitly guarded. Every DONE row links evidence; every unresolved row names an actionable resume gate and decision owner. The tracked ledger replaces scratch `tmp/` status dictionaries as the authority.

**Independent reconciliation.** Fresh evidence did not justify the four likely verification-only closures. #44 still lacks a product decision on its source-faithful priority triplet. #57/#58 have tested Wave S authoring and both report renderers, but the latest retained Production evidence shows the capability dark and no approved populated peer set. #75 has Five Dysfunctions scoring/tier guidance plus the generic Wave U engine, but no instrument-specific authored findings or live report receipt. These rows remain visible instead of being closed through adjacent capability evidence.

**Approval-ready resumes.** [Decision packets](../docs/agents/jul10-feedback-decision-packets.md) cover #32, #41, #42, #44, #45, #47, #57/#58, #75, and #84 with current truth, recommendation, compatibility/stable-key impact, acceptance, and a copyable approval sentence. The [report-fidelity matrix](../docs/agents/jul10-report-fidelity-matrix.md) keeps #33 bounded to one sanitized annotated source/current pair at a time and identifies LVA group rehire presentation as the first confirmed visual comparison candidate. The tracked [August 1-5 progress delta](../docs/agents/jul10-progress-delta-2026-08-01-to-2026-08-05.md) contains exactly nine post-cutoff outcomes and preserves default-off/dormant rollout truth.

**Reproducible evidence boundary and verification.** `src/scripts/build-jul10-closeout-artifacts.py` validates the ledger contract, regenerates the 14-page status overlay from the tracked rows rather than a hard-coded dictionary, and builds the review-only post-cutoff progress PDF. The source scan found the expected **83 status occurrences**: 30 Section-1 repeats plus all 53 Section-2 rows. Exact PDF dependencies are declared in `src/scripts/requirements-jul10-closeout.txt`; the generator emits an actionable install error when they are absent. ReportLab invariant mode fixes generated metadata/IDs, and two clean-directory builds from identical inputs produced byte-identical SHA-256 pairs for both PDFs. The focused ledger/freshness matrix passed **2 suites / 10 tests**. The fresh full repository run passed **625 suites / 7,699 tests / 14 snapshots**; changed-test ESLint, Python compilation, migration safety across **44 migrations**, and `git diff --check` passed. `CI=true npx next build --turbopack` compiled successfully, completed TypeScript, and generated **92/92** static pages; the expected local missing-Inngest and `DATABASE_URL` messages did not fail the build. All 14 status-overlay pages, the one-page delta, and the copy-decision visual received a full visual pass, including page-boundary row #57; one header-contrast issue was corrected before the final generation. Generated PDFs remain derivatives and must not be sent externally without separate authorization. No benchmark row, template version, feature flag, schema, migration, customer data, email, deployment, or other Production state was read or changed.
...
**One item, not five — and not by preference.** All three report renderers (`BrandedReport`, `QualitativeReport`, `GroupReport`) emit structurally identical brandbar markup, `report-config.ts` has **zero** header/layout knobs, and `CoachLogo` takes only `url`/`name`/`variant`. There is no selector, prop, or config field that can mean "LVA only" — splitting by tracker row would have produced **four empty diffs**. This is a different argument from the previous item's grouping (which was merely *convenient* to group); do not reuse that framing.

**Only ONE delta was actually unshipped.** Two of the five rows were already closed before this PR: the **coach NAME** half (#63b/#67a/#73a/#81a) shipped in **PR #203** via the one shared `CoachLogo` (`Coached by {name}` inside a `display:inline-flex` wrapper = literally "next to the photo"), and **#81c**'s CTA removal was complete on **both** emitters (`report-config.ts` `showCoachCta:false` for `five-dysfunctions` only → `BrandedReport` on screen, `report-email.ts` in email). What remained on all five rows was the **layout**.

**What changed.**

- **Cover** (CSS only, 0 renderer edits, reaches all 3 renderers + group + print): `.su-public-brand .su-report-cover .su-brandbar` gains `flex-direction:column; align-items:flex-start; gap:12px` — the report-scoped seam, **not** the global `.su-brandbar` (`su-public-brand.css:56`), which is shared with the thank-you hero + landing-page chrome and has no test guarding it (ADR-0005). Cover coach logo **56px → 40px** so it reads as a byline under the wordmark rather than a peer beside it; `margin-left:20px` dropped (spacing for the superseded horizontal lockup — `gap` owns it now).
- **`position:relative; z-index:1` added to the same rule.** `.su-report-cover-inner::after` (the decorative blob) is positioned at `z-index:0` and therefore paints **above** non-positioned in-flow siblings in that stacking context. `.su-report-title` already carried this exact guard; the now-taller brandbar needed it too. Found by reading the stacking rules, not by a reviewer.
- **Footer** — new shared **`ReportFooter`** component, SU mark **ahead of** the coach byline. Extracted rather than edited three times because all three footers were **byte-identical** — the same condition that justified extracting `CoachLogo` in PR #203. `GroupReportFooter` stays exported as a thin wrapper (2 existing importers untouched); the three renderers' differing date computations are handled by a pre-formatted `dateLabel: string` prop rather than centralising formatting.

**The footer was NOT Jeff's ask, and the justification is not aesthetic.** All five rows say "header"/"top"; #81's only "bottom" reference is the CTA. Codex's review recommended dropping the footer from scope for exactly that reason. It was **kept, and the evidence is decisive**: A4 print at 14mm margins is **~688 CSS px — below the 720px breakpoint** — and that `@media (max-width: 720px)` block is *not* `screen`-qualified, so **printed and saved PDFs take the `flex-direction: column` path**. Because `CoachLogo` was emitted first, **every PDF a coach saved or sent a client rendered the coach ABOVE the Scaling Up mark** — the literal inverse of the requested layout, on the artifact clients actually keep. Confirmed by print emulation before/after (`coachRendersAboveSU: true → false`). This is a correctness argument, not scope creep.

**A real DOM reorder, not `order: -1`.** `order` is one line and fixes both widths visually, but it leaves the DOM lying — screen-reader and tab order stay wrong — and it makes the ordering untestable, since `next/jest` stubs CSS and DOM order is the only thing jsdom can see.

**#78 needed no round-trip to Jeff.** His tracker logs it "suspected — needs Jeff to confirm". `scaling-up-full` is `reportType:"scored"` (`report-config.ts:53`) → the **identical** `BrandedReport` as Rockefeller/Five-Dysfunctions; no SU-Full-specific header code exists anywhere. Settled in code and fixed by the same change. Consistent with the standing "don't over-defer to Jeff" rule. Report it as *verified in code*, **not** as a Jeff-confirmed ask.

**7 aliases inherit, not the 5 he named** — `qsp-v1` (`report-config.ts:41`) and `scaling-up-quick` (unmapped → `DEFAULT_REPORT_CONFIG`) ride along, as will every future template. Accepted deliberately: uniform correct layout, and the **audience is coach/admin only** — both report surfaces live under the `(report)` route group behind the Report access gate, keyed by campaign/respondent. The public quiz gets a thank-you page, not a report.

**The emailed report is provably NOT the surface behind these complaints.** `report-email.ts` + `results-email.ts` contain **zero `<img>` tags** and zero `coachName`/`coachLogo`/`su-logo`/`brandbar` references. Every one of the five rows describes a coach image that **is visibly present** ("the coach's photo currently appears at the very beginning/top", "has the coach logo but no coach name"). An email with no images cannot generate a complaint about where the coach photo sits. This **falsified** the pre-scoped gate's largest flagged risk ("if his screenshots came from an email, all five items come back as not fixed") rather than deferring it. Filed as **GH #228** — a real but unrequested gap, not this item's surface.

**Geometry measured on real component output, not estimated.** Markup was dumped from the actual renderers (jsdom render → `container.innerHTML`), wrapped in the real stylesheets in a scratchpad HTML file served over localhost — **no throwaway route** (the previous item's review ranked a public preview route its #1 risk), no DB, no auth. Cover **+20px** (brandbar 56→76px); tallest cover in A4 print **533px of 1016px printable → no overflow risk**; **footer height unchanged at 76px** (the reorder adds no row). Verified at 1280 / 375 / A4-print, before and after.

**Tests — `report-chrome-order.test.tsx`, 9 tests / 1 suite.** DOM-order guards (`compareDocumentPosition`) on all three footers **and** all three covers, plus name-beside-image and the no-logo / no-coach-at-all degrades. RED was genuine: 4 of 5 failed pre-implementation for the right reason, and the 5th (no coach at all) passed legitimately — which also proves the shared helper discriminates, so the later cover assertions aren't vacuous. **Cover guards became load-bearing with this change**: now that the brandbar is `flex-direction: column`, DOM order **is** the visual top-to-bottom order, so a JSX swap would reproduce the exact defect being fixed. **Accepted limit, stated in the test file:** jsdom cannot observe `flex-direction`, so deleting the cover CSS rule would fail nothing — that rule is covered by screenshot/PDF review at merge.

**`/co-validate` reversed 3 gate decisions; 1 of its 5 points was overridden.** Accepted: **drop the CSS source-drift-guard** (it pins implementation text without proving behaviour, and a legitimate selector refactor would break it for nothing); **no new ADR** — amend spec-13's **G7** instead, since this fails the domain-modeling skill's own hard-to-reverse bar (it is two CSS numbers) and G7 lives in `docs/specs/`, not `docs/adr/`, so there was no ADR to supersede; **add a row-level acceptance matrix** (renderer-level screenshots don't verify every sub-ask); **build the mockup from real component output** rather than hand-typed markup. **Overridden:** dropping the footer (see the print evidence above).

**Two-axis code review found the gap both the grill and Codex missed.** Standards and Spec agents **independently converged**: the cover — Jeff's *literal* ask — had **no automated guard at all**, a hole opened by dropping the CSS drift-guard. Fixed by adding cover DOM-order assertions (file renamed `report-footer-order` → `report-chrome-order`) rather than the suggested `ReportCoverBrandbar` extraction — same gap closed at a fraction of the risk, and a fourth extraction was never gated. Also fixed: a **prose overclaim** (the spec note implied the 40px cap expressed subordination everywhere; in the *footer* the coach cap is 40px against a 22px SU mark — both **pre-existing and untouched here**, now recorded as *seen-and-judged*, since the footer wordmark is 120px **wide** so it isn't a straight height ratio, and shrinking it is an unrequested visual change), and a **dead `vertical-align: middle`** left inert on a flex item in the very rule being rewritten.

**One review finding REJECTED, with evidence.** The Spec agent claimed the blast radius is smaller because "`qsp-v1` is a disabled template" — that repeats a **stale CHANGELOG claim**. A read-only prod check on 2026-07-27 found `disabledAt` **NULL**. Same doc/prod drift that tripped a reviewer on the previous item; the 7-alias framing stands.

**Caveat on reporting #67 closed.** The coach byline renders **only** when the campaign has a `creatorCoach` (`respondent-report.ts`: `coachName = creatorCoach ? … : null`, by design since Wave K so admin-created PUBLIC campaigns degrade to SU-logo-only rather than a broken image). An **admin-created** QSP campaign with no creator coach still shows no coach name, and #67's words were *"coach's name is missing from the report header"*. Code-wise the name half is shipped and template-agnostic; whether it closes **what Jeff actually saw** wants a live check on his campaign. This is the **#76 pattern** — his QSP invite complaint turned out to be a campaign-level override, not a template regression (GH #220).

**Docs.** spec-13 **G7 marked partly superseded**: the coach is a **subordinate provenance byline** in the masthead/footer — acknowledged, never co-branded — where G7 had specified coach-as-text-in-the-CTA *only*, explicitly to avoid needing a coach-mark asset. Wave K already broke that by adding a coach *logo*; #81 then **deleted** the CTA text G7 designated as the acknowledgement on `five-dysfunctions`. The note records the end state so the layout doesn't read as drift, and tells a future engineer **not** to restore peer sizing or re-add the Five-Dysfunctions CTA. `CONTEXT.md` gains a **Coach byline** glossary entry: "coach logo" and "coach photo" are *both* unsafe as the general term, because one `Coach.profileImage` serves as headshot **and** firm logo.

**Gates.** Turbopack build ✓ (104s, then 2.2min after review fixes) · ESLint clean on all changed files · `tsc` clean on changed files · 32 suites / **313 tests** green across the assessment component surface · all 4 required CI checks green first run, `mergeStateStatus: CLEAN` · prod deploy `success` on `febbdcc1`. Full-suite failures are the **documented pre-existing floor** (9 suites / 32 tests; a second run of the same tree gave 9 vs 12, confirming load flake) — all proven unrelated by import-grep (zero references to any touched file), with the two plausible ones traced to **Wave S PR #132 (2026-07-04)** for `GROUP_RENDER_VERSION` and **2026-05-19** for the `toLocaleDateString` lint hit in `public-quiz-client.tsx`.

**Files:** `ReportFooter.tsx` (new) · `BrandedReport.tsx` · `QualitativeReport.tsx` · `GroupReport.tsx` · `su-report.css` · `report-chrome-order.test.tsx` (new) · `docs/specs/v7.6/13-assessment-brand-and-results-report.md` · `CONTEXT.md`.

---

### 2026-07-28 — Jeff July-10 #62/#66/#70/#77: per-template Welcome-screen copy (+ a Five Dysfunctions paragraph we drafted) <!-- ENTRY_ISO:2026-07-28 ENTRY_SLUG:jeff-jul10-welcome-lede-copy -->

**Status: DONE (2026-07-28, PR #225, squash `0663641a`).** Fourth item in the one-item-at-a-time July-10 pass, after the invitation-email family (#61 LVA, #69 Rockefeller, #76/#80 SU-Full + Five Dysfunctions). Full loop: state-check → grill (9 decisions) → Codex co-validate → thin-spec gate → **visuals approved before code** → TDD → PR → review-loop → merge → SoT. **Flagless. No migration, no feature flag, no prod write, no seed edit.** Rollback = revert.

**What changed.** The participant **Welcome screen** ("Screen 1" of `/org-survey/[campaignAlias]`, before the Section pager) showed **one hardcoded JSX paragraph shared by every INVITED template**. It is now per-template copy in a code-owned per-alias map — `src/src/lib/assessments/welcome-copy.ts`, resolved by `resolveWelcomeLede()`, failing open to the previous paragraph for any unkeyed, unknown, or missing alias. Five aliases carry bespoke copy; `qsp-v1` keeps the previous copy byte-for-byte.

**Code, not data — ADR-0026.** Deliberately *not* a template-row column. The discriminator against **ADR-0025** is **authorship**: invitation copy is data because coaches *author* it (and that has cost a migration, four atomic-CAS patch scripts, a coverage receipt, drift guards, and the campaign-override bypass in GH #220); nobody has asked to author this paragraph. `resolveWelcomeLede()` is the seam if that ever changes — bounded, though not free: it runs synchronously in a `"use client"` render body, so a promotion to data means the value arrives on the `/me` payload. Same shape as the existing per-alias maps at `report-config.ts` (ADR-0010) and `ORG_LINE_SUPPRESSED_ALIASES`. ADR-0026 also states two consequences in its own words rather than borrowing the report doctrine: the screen is re-derived on **every mount** (including back-exit from the pager), so copy reaches in-flight campaigns and can change within one sitting — flagged to be revisited if Screen 1 ever becomes answer-bearing — and the copy is **invited-surface only**, because `accessMode` is per-campaign.

**Deviations from Jeff's dictated bytes** — all deliberate, itemised in the PR, classed grammar / style / anchor / layout:

| Item | His bytes | Shipped | Class |
|---|---|---|---|
| #62 | `members view` | `members' views` | grammar (#66 uses the plural) |
| #62 | `the priorities and its future` | `its priorities and its future` | **style** — his was awkward, not ungrammatical |
| #66 | `members views` | `members' views` | grammar |
| #70 | `each item was rated` | `each item is rated` | anchor — past tense before question 1 |
| #70 | `The table on page 4 therefore shows values ranging from 0 up to 12, …` | clause **OMITTED** | see below |
| #77 | `throughout this report` | `throughout your report` | anchor |
| #70, #77 | one paragraph | two paragraphs | layout — 475/495 chars is a wall at 15px |

**#70's omitted clause is a decision routed to Jeff (GH #223), not a completed ask.** Two independent reasons, both verified: Rockefeller renders **no score table** — his own June item **#24** had it removed (`report-config.ts:40`, `showScoreTable:false`, CHANGELOG Wave E / PR #67) — and **we print no page numbers at all** (`su-report.css`, the `@page { @bottom-center { content: counter(page) } }` block is commented out; `counter(page)` appears only in commented lines repo-wide). So no wording of that clause is shippable *even if the table returned*. The two true facts it carried (0–3 scale; four items per habit — confirmed against the seed: 10 sections × 4 questions at `{min:0,max:3}`, so 0–12 is right) are kept. Restoring it is a one-line change.

**Five Dysfunctions copy is OURS, drafted-not-dictated.** Jeff asked for four and not this one, the same day he sent Five Dysfunctions wording for the invitation email (#80), so the omission reads as an oversight; leaving one instrument on generic copy while four siblings describe themselves was the worse option. Every clause traces to `prisma/seed-five-dysfunctions.ts`. **Three earlier drafts were rejected** — one claimed "there are no right answers" (the seed states `higher always = healthier`, no reverse scoring), one claimed to capture "what the whole team is experiencing" (unverifiable — nothing guarantees full response), and two named Patrick Lencioni on the participant screen (licensed, but no other paragraph in the family names a third party). **Report it to Jeff as drafted, not as his.**

**Also fixed: a regression we would have introduced.** The replaced sentence was the **only** place the invited card told a respondent they could stop and come back — and the promise is true (`useAnswerDraft`, `localStorage`, same browser). Jeff asked to *add* intro copy, not remove that. Bespoke templates now carry it in the fine print, gated by `shouldShowResumeNote()` so a template still on the default lede does not state it twice. **Caught by the RED step**: three assertions passed *before* implementation because they matched the sentence in the lede, exposing the duplication bug before any code existed.

**Accepted trade-offs, both visually verified** (throwaway harness against the shipped module at 1280×900 and 375×812; the ED10 Preview tab renders `SectionPager` only and cannot show Screen 1). #77's copy pushes the CTA **below the fold at 375px** (#62 keeps it visible, #66 is borderline) — accepted rather than trimming his copy or reordering the approved participant mockup, since the respondent arrives by clicking a button in a coach's invitation email. And the resume promise **loses prominence**: lede → fine print, 0.9375rem left-aligned → 0.71875rem centred (76.7%).

**Tests: 39 / 2 suites.** Resolver coverage (fail-open, per-alias resolution by distinctive substring, paragraph counts, a byte-exact + 135-char pin on the default) plus DOM wiring. Two guards worth naming: one pins that resolution **keys off `templateAlias`**, not `campaign.alias` or the route param — a fixture sets the campaign slug to a real map key with `templateAlias: null` and asserts the default still renders, because an implementation keyed off either would pass a looser fixture and be a **prod no-op**; the other asserts **every map key is really declared by a seed**, matched against the seeds' own `const ALIAS` / `const TEMPLATE_ALIAS` as text, closing the highest-probability silent failure (a typo or seed rename reverting a template to the default with a green suite *and* a green build).

**Review-loop: 3 adversarial rounds + a targeted verification → 5/5, 17 findings, exactly ONE code defect.** That defect was real: `resolveWelcomeLede("constructor")` returned a **function** — `map[alias] ?? DEFAULT` walks the prototype chain, `??` only fires on null/undefined, `Object.freeze` blocks writes not prototype reads — so the render site's `.map()` would have white-screened the Welcome screen. Reachable: the admin create-template validator `/^[a-z0-9][a-z0-9-]*$/` admits `constructor`, and seeds bypass that regex entirely. Fixed with `Object.hasOwn`; the five inherited-key tests were proven to fail against the pre-fix resolver. **The other 16 findings were prose** — three separate citation-drift instances (one caused by a round-1 commit shifting the lines it cited), three stale test counts, overstated claims, and a fix landing in one of two homes. Two durable lessons: **line numbers belong in code comments, never in `docs/adr/`**, and **prose gets verified like code**.

**Split out, not folded in:** **GH #222** (the Welcome-screen stat chip mislabels non-slider banks — LVA renders "67 short statements, rated 1–3" but is 40 TEXT + 16 slider + 10 NUMBER + 1 MC; QSP v2 "22 … rated 1–10" but 15 TEXT + 6 slider + 1 NUMBER; `deriveScaleLabel` takes the first slider, `deriveTimeEstimate` is count-only) · **GH #223** (the #70 clause decision) · **GH #224** (the card promises "Honest & confidential" unconditionally while `five-dysfunctions` is `aggregationMode: "FULL_VISIBILITY"` — predates and survives this change).

**Two things worth carrying forward.** (1) **`report-config.ts` has the same prototype unsoundness** this item fixed in its own resolver — the reachability argument now written into `welcome-copy.ts` applies verbatim, though it degrades to a bad config object rather than a crash; unfiled. (2) **Doc/prod drift:** the Wave Q CHANGELOG records `qsp-v1` as disabled at launch, but a read-only prod check shows `disabledAt` **null**. Immaterial here (it keeps the default lede either way), but one reviewer tripped on it.

**Process caveat — this item is NOT Codex-signed-off.** Codex reviewed revision 1 and its five findings reshaped the plan (they are why #70 was split out and why visuals moved before code). The **second** call, on the revised plan, died on an **1800s MCP idle timeout with no `threadId`**, so the co-validate skill's `codex-reply` resume path was unavailable and the scripted `model-chat` fallback is unrunnable here (no `anthropic` SDK, no API key, stale hardcoded paths). A **three-lens adversarial panel** substituted (correctness/wiring · ruthless simplification · client editorial) and found more than the first Codex pass did — the resume regression, the commented-out page counter, and a public preview harness left on disk — but it was not Codex.

---

### 2026-07-27 — Jeff July-10 #76/#80: SU-Full + Five Dysfunctions invitation emails — invite family complete 4/4 <!-- ENTRY_ISO:2026-07-27 ENTRY_SLUG:jeff-jul10-76-80-invite-copy -->

**Status: DONE (2026-07-27, PR #219).** Third item in the one-item-at-a-time July-10 pass (after #61 LVA, #69 Rockefeller), same loop: grill + domain-modeling → thin-spec gate → TDD → PR → review-loop → merge → live prod patch → SoT. Takes the **remaining 2 of 4** invitation emails, so the coach-forward invite family is now complete. Reused the #69 pattern (seed edit as factory default + atomic-CAS prod-row patch, ADR-0025). No migrations.

**The state-check reshaped the item before any code was written.** Jeff filed #76 as "SU-Full **and QSP**", flagging QSP as a possible regression of the 2026-07-03 Wave-P fix. A read-only prod check disproved the regression: `qsp-v2`'s live `invitationBodyMarkdown` had been coach-forward all along.

- **#76 SU-Full — SHIPPED.** Body now leads `{{coachName}} has invited you to complete the {{templateName}}` (was `{{organizationName}} invited you…`). Unlike #69 the assessment name is **not** hardcoded — `{{templateName}}` renders "Scaling Up Full Assessment". The duplicate above-button raw `{{invitationUrl}}` line is gone; "Click the link below to begin:" → "Click the button below to begin." That URL removal is an **extension of the #61/#69 pattern**, not a literal #76 ask (Jeff listed two). It is a genuine rendered change — a bare URL line is not a markdown link, so `dropRedundantCta` does not strip it. Subject unchanged.
- **#80 Five Dysfunctions — SHIPPED.** Body names the coach instead of the generic "Your coach", and uses Jeff's wording "the Five Dysfunctions assessment" (`{{templateName}}` renders the clunky "The Five Dysfunctions of a Team — Team Assessment"). Subject unchanged.
- **#80 ask 3 (suspected duplicate link) — NO-OP on the branded renderer.** Jeff hedged it ("likely… worth confirming"); the answer is no. `dropRedundantCta` strips any standalone markdown-link line whose URL equals the invitation URL, and `{{assessmentUrl}}` resolves to exactly that, so the old `[Take the Assessment]({{assessmentUrl}})` line rendered **zero body anchors**. It **is** a real fix on the dormant legacy renderer (`sendLegacyInvitationEmail`, `ASSESSMENT_INVITE_BRANDED=0`), which has no `dropRedundantCta` — GH #217. Removed from source either way; two renderer tests pin both halves of the asymmetry.
- **#76/#80 ask 1 (coach logo below the SU logo) — ALREADY TRUE**, as with #69 ask 2a. Shared, template-independent chrome (`invitation-email.ts`) emits the SU cid logo then the coach logo; the existing logo-order regression test covers it.
- **#76 QSP — NO prod write; root-caused instead.** Jeff's *observation* was accurate, his *diagnosis* was not. A campaign-level `invitationBodyHtml` override on `2026 QSP Q2` **replaces the branded shell entirely** — no SU logo, no coach logo, no coach-forward body — and is unreachable by a template-row patch. **Evidenced, not inferred:** prod `EMAIL_DELIVERY` telemetry records **4 `renderer:"custom_html"` invitation sends on 2026-07-10**, the day of Jeff's report, all from that campaign (soft-deleted 2026-07-24). The real defect on QSP was **seed drift** — the seed still carried the pre-Wave-P copy, so re-seeding a fresh environment would have reintroduced exactly the wording Jeff complained about. Seed pinned byte-for-byte to the live row; no prod write.

**Delivery (ADR-0025).** Two new atomic compare-and-swap scripts — `scripts/patch-scaling-up-full-invitation-copy.ts` and `scripts/patch-five-dysfunctions-invitation-copy.ts` — run LIVE + verified: `scaling-up-full` `b83288bc4ce4` → `31dd729e162b`, `five-dysfunctions` `fc88f616501f` → `56e4f11ebdcb`, both dry-run idempotent afterwards. **0 campaign overrides** on either template, so the patches reach all 9 SU-Full and all 6 Five Dysfunctions campaigns.

**New shared receipt + two live findings it surfaced.** ADR-0025's required version-staleness preflight and campaign-override inventory were unimplemented (including in the shipped #69 script). Extracted `scripts/patch-invitation-copy-coverage.ts`, now imported by **all four** invite-copy scripts, printing both on dry-run and after a live patch. It immediately surfaced a previously unreported operational trap: **`RockHabits` v3 and `leadership-vision-alignment` v4 are both unpublished DRAFTs**, so their `contentHash` went stale when #69/#61 patched the rows — and because neither seed passes `forceSupersedeDraft`, **re-seeding either template now FAILS CLOSED**. Not user-facing; the fix is to publish v3/v4 or re-run those seeds with the flag. SU-Full and Five Dysfunctions are unaffected (latest versions published → a re-seed just appends a new DRAFT).

**Split out.** The `invitationBodyHtml` override-bypass gap → **GH issue #220** (it defeats the whole #61/#69/#76/#80 coach-forward programme wherever a campaign sets one; ADR-0025 §2 treats overrides as deliberately coach-writable, so closing it is a product call, not a patch). Zero live overrides across all four templates today.

**TDD / gate.** New drift-guards for SU-Full, Five Dysfunctions and qsp-v2 (coach lead, no org lead, no bare URL, button copy, **positive subject-unchanged assertions**) + seed↔script parity guards + the two renderer asymmetry tests; the #69 residual (subject proven only by omission) is closed, and the LVA script — previously the only one outside the tsc program, unexported and unguarded — gained `require.main` guarding, exported constants and its own parity test. **279 tests / 16 suites** green in `src/__tests__/seed`; `CI=true npx next build --turbopack` clean; **review-loop 4 rounds → 5/5** zero-actionable. The recurring finding across rounds was inaccurate prose, not broken code: three separate rounds retired a confident claim that did not survive checking (the "three links" bug, "never reached the inbox", "unreachable on current data", "tracked as its own issue").

---

### 2026-07-27 — Jeff July-10 #69: Rockefeller invitation email — coach-forward copy + drop duplicate URL <!-- ENTRY_ISO:2026-07-27 ENTRY_SLUG:jeff-jul10-69-rockefeller-invite -->

**Status: DONE (2026-07-27, PR #216).** Second item in the one-item-at-a-time July-10 pass (after #61 LVA), same loop: grill + domain-modeling + Codex co-validate → thin-spec gate → TDD → PR → review-loop → merge → SoT. #69 is the Rockefeller respondent invitation email. A pre-build state-check (parallel readers + Codex) found it is **not** a clean four-ask build — the four asks split unevenly:

- **Ask 1 (remove the duplicate raw URL above the Start button) — SHIPPED.** The bare `{{invitationUrl}}` lived in the Rockefeller seed body; removed it (the shell's Start button + bottom fallback URL already cover it) and changed "Click the link below to begin:" → "Click the button below to begin." Seed-copy change only — the renderer already emits exactly one button + one below-button fallback.
- **Ask 4 (coach-forward body) — SHIPPED.** Body now leads "{{coachName}} has invited you to complete the Rockefeller Habits" (was "{{organizationName}} invited you to complete the {{templateName}}"). The assessment name is **hardcoded** ("Rockefeller Habits") because `{{templateName}}` resolves to "Rockefeller Habits Checklist". The **subject line is deliberately unchanged** (Jeff #69 is body-only).
- **Ask 3 (keep the company name in the purple header) — NO-OP guardrail.** Deliberate contrast with #61 (which suppressed LVA's). `RockHabits` stays out of `ORG_LINE_SUPPRESSED_ALIASES`; nothing changed.
- **Ask 2a (coach logo below the Scaling Up logo) — ALREADY TRUE.** The renderer already emits the SU cid logo first, then the coach logo. No reorder; added a **logo-ORDER regression test** to lock it.
- **Ask 2b (fix the broken red-X coach logo) — DIAGNOSED CLEAN, no change.** No code-level layout cause (`safeImageSrc` passes any parseable https URL; the coach logo is `creatorCoach.profileImage`). A live diagnosis of all 9 Rockefeller campaigns found **both** distinct coach `profileImage`s return `200 image/jpeg` — valid, live images. Jeff's red-X was a **stale historical send** (the image at send-time pointed at a since-replaced blob); because invites read `profileImage` live at send, new Rockefeller invites render correctly. No code/data change; a real Gmail/Outlook confirm-send is the only residual.

**Delivery (ADR-0025).** Invitation copy is a TEMPLATE-row field read live at send (`campaign.invitationBodyMarkdown ?? campaign.template.invitationBodyMarkdown`); re-seeding an already-seeded template does **not** rewrite the row (`ensureTemplateVersionContent` hashes stored values). So the seed edit ships the factory default and a new **atomic compare-and-swap** script — `scripts/patch-rockefeller-invitation-copy.ts` — corrects the live prod row: `updateMany({ where: { alias:"RockHabits", deletedAt:null, invitationBodyMarkdown: EXPECTED_OLD }, data:{…} })`, `count===1`; `count===0` re-reads and classifies (soft-deleted → hard fail; already-new → idempotent; anything else → drift hard-fail, never a silent no-op). Run LIVE + verified: prod row `b294ebd969b4` → `b417f147939a`, dry-run idempotent, live body confirmed coach-forward with no bare URL. A prod override inventory found **0** of 9 Rockefeller campaigns carry an invitation override, so the template patch reaches every campaign. **F2** (the atomic-patch pattern) is captured in **ADR-0025** and supersedes the read-then-update-by-id shape of `patch-lva-invitation-copy.ts`; reusable for #76/#80.

**Scope discipline.** Both reviewers (Codex + Claude) recommended **splitting F1** — the #61 co-validate follow-on to harden the dormant `ASSESSMENT_INVITE_BRANDED=0` legacy path — out of #69, since it is not a Jeff #69 ask and touches shared code with zero live impact (flag unset). Filed as **GH issue #217**.

**TDD / gate.** New `__tests__/seed/rockefeller-invitation-copy.test.ts` drift-guard (coach lead, no `{{organizationName}}`, hardcoded name, no bare URL, button copy) + a **seed↔script parity** guard (`NEW_BODY === buildRockefellerContent().invitationBodyMarkdown`, catching future drift that would silently no-op the CAS) + the logo-order regression test; invitation-email suite 60/60 (+ seed suites). `CI=true npx next build --turbopack` passes; review-loop ran **2 rounds → 5/5** zero-actionable (round-1 fixes: dry-run soft-delete gating, classification ordering, import-safe script, parity test, ADR status). No migrations.
...
**Co-validate (Codex, retroactive, 2026-07-24):** clean bill on the shipped code — alias threading complete across all four send paths, suppression exact-alias/fail-open, branded path retains CTA + bottom fallback URL + text twin. Two **latent** follow-ons (neither a live incident — the prod branded path is correct) **folded into #69**: **(F1)** the legacy kill-switch path (`ASSESSMENT_INVITE_BRANDED=0`, unset/dormant in prod) is left button-only by the mid-URL removal → harden `sendLegacyInvitationEmail` (pass `coachName`, add the bottom fallback URL + a plain-text twin); **(F2)** make the invitation-copy patch atomic (`updateMany` on id + expected old body) as the reusable pattern for #69/#76/#80.
...
- **Ground truth (decrypted `vercel env pull --environment=production`, read in the gitignored scratchpad then deleted):** ON (`"1"`) = `WAVE_D_*` (4), `WAVE_F`, `WAVE_J`, `WAVE_M`, `WAVE_N`, `WORKSHOP_CUSTOM_HTML_EDITOR` (all created 22d ago). EMPTY (`""`) = every `WAVE_ED1/2/4/6/8/9/10`, `WAVE_O`, `WAVE_X`, `WAVE_P`, `WAVE_Q`, `WAVE_S` (+ KILL), `WAVE_T`, `WAVE_U`, `WAVE_U3`, `WAVE_V`, `WAVE_W` — and `WAVE_QSP_ROCK_GROUP_REPORT_ENABLED` (the #72 flag "set" the prior session had also landed empty).
- **Proven root cause (NOT the earlier guess):** `vercel env add <KEY> production` fed from **piped stdin** (`printf '1' | vercel env add …`) silently stores an **empty** value. Reproduced live — re-added `QSP_ROCK` via pipe → `env pull` showed `=""`; set the same key via the **REST API** (`POST /v10/projects/{id}/env`, `type:"encrypted"`, `target:["production"]`) → `env pull` showed `="1"`. This explains the cohort split exactly: the "1" flags were set via the REST path (`scripts/push-env-to-vercel.mjs` uses REST, and its header says it does so *because the CLI misbehaves*); the empty flags were flipped via the piped CLI. **The earlier suspicion — that `push-env-to-vercel.mjs` clobbers wave-flags with an empty local `.env`, fixable via its SKIP list — is WRONG:** the feature toggles aren't in local `src/.env` (only `WAVE_O_ESPERTO_IMPORT_HASH_SALT` is), and the script's line-172 guard skips empty values before its delete, so it can't touch a key absent from the local file. No SKIP-list change needed.
- **Fix applied (with explicit user consent):** re-set **12 clean flags to `"1"` via the REST path** — `WAVE_ED9/ED10/O/X/P/Q/S/T/U/U3/V/W` + `QSP_ROCK`/#72 — verified all `="1"` via a fresh `env pull`. **Excluded `WAVE_ED1/2/4/6/8`** (superseded/kill-able editor-pipeline waves — need a separate look, not a blind flip). Redeployed the live deployment → build `mcflc46pq` (Ready; prod alias confirmed pointing to it). #72 is now **fully launched** (flag correct + activated).
- **Live-verify (admin login on `mcflc46pq`; signals from a 12-agent code-grounded recipe workflow):** **9 confirmed live in-app** — **ED9** (Build-tab FormsBuilder "Untitled form" hero card), **ED10** (tab bar leads "Preview" + has "Settings" tab), **O** (Import "Scaling Up Full" historical instrument), **X** (Import "Leadership Vision Alignment" + "Rockefeller Habits Checklist" instruments), **Q** (Templates-list "Disable" control per row), **T** (question inspector interactive type-picker with "Multiple choice", no "coming soon"/no v1.5 panel/no locked chip), **U** (`q-findings-panel` on a focused question), **W** (`q-showif-panel` "Show only when…"), **V** (Observability "Alert sweep: Healthy"). **S** — flag verified `"1"` + alias `leadership-vision-alignment` + published version exist, but the LVA "Peer averages" panel did not render; the gate also requires the *published* version to expose SLIDER_LIKERT rating keys (the 16 sliders counted were the v4 **draft**, panel reads v3 **published**) → **follow-up**, not a flag failure. **P** (invite-email coach-logo/CTA chrome) and **U3** (results-email "Your recommendations" block) — flag-on but only observable in a **sent email**; verified at env/wiring level, not click-verifiable (no real sends fired at campaign recipients).
- **Side finding (flagless drift):** the "Test Mode" button (`WAVE_ED1`) and version Roll back/Archive (`WAVE_ED8`) render on the live editor even though those flags are OFF → those waves were later made **flagless**. So an empty flag does NOT always mean a dark feature — the live-app check is ground truth, not the CLI value.
- **Honest reporting takeaway:** for any consolidated report, the accurate framing is **9 assessment features confirmed live, 1 needs a quick follow-up (S), 2 verified at wiring-level (P/U3, email-only)** — do not blanket-claim the whole cohort "live."
- **Follow-ups:** (a) resolve the S peer-averages surface (confirm v3 composition / publish v4); (b) email-verify P + U3 when a controlled send is possible.

Docs-only PR (CLAUDE.md Known-Quirks bullet corrected + anchor bumped + this entry); CI-gated cascade; no migrations.

### 2026-07-21 — Jeff July-10 decision forks: group reports for QSP + Rockefeller (#72, dark) + Print/Download split (#64, live) <!-- ENTRY_ISO:2026-07-21 ENTRY_SLUG:jeff-jul10-forks-72-64 -->

**Status: MERGED (2026-07-21).** After the no-decision batch (see `jeff-jul10-feedback-batch` below), the user greenlit two decision forks from the same July-10 tracker. Each shipped as an independent PR — TDD (red→green), two-axis review (Standards + Spec), Turbopack build gate green, Notion task — and both merged to `main` via the CI-gated cascade (Build + Migration Safety Gate; no `--admin`; branch kept up-to-date). No schema migrations.

- **PR #207 — #72 (fork DT-5): group reports for "all 4 types".** The aggregate/CEO group report is now surfaced for **QSP** (`qsp-v2`, qualitative) and **Rockefeller** (`RockHabits`, scored), joining **LVA** + **SU-Full** — the 4 types Jeff asked for. **Five Dysfunctions stays OUT** (Jeff 2026-06-18: its scored group report "over-showed / confused him"). The generic engine already dispatches scored|qualitative via `reportConfigFor(alias)`, so this is allowlist + flag wiring only — no engine/model work. Ships **DARK** behind a NEW default-OFF flag set `WAVE_QSP_ROCK_GROUP_REPORT_{ENABLED,CANARY,KILL}` (campaign-id-only canary + kill precedence, mirroring Wave J) — deliberately NOT `WAVE_F` (which is ON in prod from the Wave L LVA launch, so folding them onto it would light them up on merge; a drift-guard test asserts only LVA rides WAVE_F). The Wave J SU-Full publish guard is generalized from `alias === "scaling-up-full"` to `reportType === "scored"` (new pure `groupReportRequiresPublishedVersion`) at all 3 sites (loader + admin + portal entry points), so a DRAFT scored Rockefeller can't leak while qualitative LVA/QSP stay byte-for-byte. 124 tests green. **Enable:** `WAVE_QSP_ROCK_GROUP_REPORT_ENABLED=1` (or a per-campaign `_CANARY`) + redeploy; kill `_KILL=1`.
- **PR #208 — #64 (fork DT-2): Print / Download split.** The report action bar's single combined "Print / Download PDF" button becomes a distinct **"Print"** + **"Download PDF"** pair on BOTH report surfaces (group + per-respondent). No server-side PDF — both open the browser print dialog; "Download PDF" first sets `document.title` to a descriptive filename (`<company/respondent> - <assessment> - Report`) so the Save-as-PDF default name is clean, restoring it on `afterprint` ({once:true}) with a re-entry guard. CSS: fixed positioning moved to a `.su-report-print-actions` bar (responsive + @media-print overrides retargeted; both buttons keep `no-print`). **Flagless — live on merge.** 6-test seam; page tests (mock the button) unaffected.

**Still open on the July-10 tracker (product calls / external):** #48 core-values reword (seed-vs-live caveat), #62/66/70/77 intro copy (DT-4), #74 View Trends (DT-6), #32/57/58 benchmarking (DT-7), #84 8-Q quiz (DT-8), #71 on-screen results (DT-9), #39 rate-limit (DT-10), #44 LVA triplet (DT-11); T04 invite-email copy (now verifiable via the authenticated Vercel CLI — see `jeff-jul10-feedback-batch`); #75 Five Dysfunctions findings (Jeff/Suzanne content); FOG #33/#42/#45.

### 2026-07-21 — Jeff July-10 assessment-feedback batch: 8 updates shipped + merged to prod (6 items confirmed complete; 3 follow-up areas) <!-- ENTRY_ISO:2026-07-21 ENTRY_SLUG:jeff-jul10-feedback-batch -->

**Status: LAUNCHED (2026-07-21).** Triaged Jeff Verdun's 2026-07-10 assessment-feedback tracker (items #30–#87, 53 open) against the live code, then shipped every buildable no-decision item as **8 independent PRs** — each TDD (red→green), two-axis code-review (Standards + Spec), Turbopack build gate green, and a Notion task. All 8 merged to `main` via a **CI-gated cascade** (Build + Migration Safety Gate; no `--admin` bypass — branch stayed up-to-date per protection). No schema migrations. A same-day request-to-implementation audit separated deployment from acceptance: **6 requested items match the original request, while 3 grouped areas still need verification or product work.**

- **PR #198 — P0 hotfix.** #79 SU-Full non-CEO submit no longer 500s with `MISSING_REQUIRED_KEY` — the submit/scoring path now applies the same CEO-only `S_BACKGROUND` audience drop as the render path (`SU_FULL_ALIAS`/`SU_FULL_BACKGROUND_SECTION` exported from `assemble-survey-pages.ts`; `computeScoreResult` receives audience-filtered questions/sections). The second change is also real: participant removal no longer shows a false "could not remove" because the DELETE route now returns a valid empty 204 response. However, Jeff's #59 wording asks for **individual-report removal**; do not mark that request accepted until the exact reported flow is reproduced and mapped to this fix.
- **PR #199 — #50.** The coach-completion email (`buildCoachNotifyEmail`) now names WHO completed it in subject + body (resolved via `respondentDisplayName`, email fallback per Wave P), replacing the generic "A respondent"; blank name degrades byte-identically to the old wording.
- **PR #200 — #85.** The built-but-orphaned Observability dashboard is now linked in the assessments admin sidebar (`ADMIN_ENTRIES`), next to Import.
- **PR #201 — #87.** Feeding the wrong Esperto export kind into the Results import lane returns an actionable message naming the upload + the real on-screen option ("Roster (people)" / "Historical rounds"), via a shared `resultsLaneMismatchMessage(kind)` in `classify.ts` used by BOTH import routes (admin + coach).
- **PR #202 — #60.** An org member's email is now editable end-to-end (was read-only). `updateRespondentSchema` accepts `email`; the PATCH route recomputes `normalizedEmail` always and the dedupe key `dedupeValue` only for email-sourced members (externalId-sourced keys untouched); a collision against `@@unique([organizationId,dedupeSource,dedupeValue])` returns 409 (P2002-caught, mirroring create).
- **PR #203 — #63/67/73/78/81.** Report headers now show the coach NAME as visible text ("Coached by {name}") via the shared `CoachLogo` — flows to all 5 report types + the group report, cover + footer; the name shows even with no logo image; the logo is decorative (`alt=""`) when the name is text. #81: the "Talk to your Scaling Up Certified Coach" CTA is removed for **Five Dysfunctions only** via a new optional `report-config.showCoachCta` flag gating both the on-screen `BrandedReport` and the emailed report. These improvements are live, but Jeff's requested layout placed the coach image **below** the Scaling Up logo; the current horizontal same-row brand bar does not satisfy that placement, so this group remains partially open.
- **PR #204 — #86.** Admin "Organizations by coach" view — an A–Z / By coach toggle in `MembersTeamsView` (gated by a new `allowGroupByCoach` prop; coach portal byte-identical), grouping companies under owning-coach headers; the admin page loader joins `Organization.owner` for the name.
- **PR #205 — #83.** Public-quiz submissions (self-enrolled taker + referring coach + date) are surfaced per campaign via a new admin `GET /api/admin/public-campaigns/[id]/submissions` + a "View submissions" expander on the Public Campaigns page (reads the persisted `referringCoachEmail` + `publicTaker`). This is useful progress, but it does not show the result/score and is admin-facing rather than coach-facing, so #83 remains partial.

**Remaining Jeff items — gated on external input, verification, or product decisions:** T04 invite-email copy (#47/61/69/76/80) needs live behavior/value verification (`WAVE_P`/`WAVE_O`/`WAVE_X`) without exposing secrets; Five Dysfunctions findings text (#75 — Jeff/Suzanne content, capability already exists via Wave U); ~11 decision forks (#48 core-values consolidation, #64 Print/Download, #72 group reports for all 4 types, #32/57/58 benchmarking, #84 8-Q SunHub quiz, …) awaiting product calls; FOG items (#33/#42/#45) needing Jeff/Suzanne clarification. **Vercel access correction (2026-07-21):** the connector was unavailable in the original build session, but the current local CLI is authenticated as `gabriel-3497`, can inspect `ChiefAIOfficer/scaling-up-platform-v2`, and can list its Production environment-variable names. Jeff does not need to change Vercel access now. Re-run the read-only checks documented in `CLAUDE.md` before claiming an access block; access to encrypted variable names does not itself verify their values.

**Acceptance follow-ups:** reproduce Jeff's exact #59 individual-report-removal flow; implement or explicitly re-decide the coach-image-below-logo placement for #63/#67/#73/#78/#81; decide and add #83 result details plus coach-facing access if required. **Technical follow-ups (not blocking):** extract a shared `resolveSuFullAudience` helper (SU-Full audience rule now spans 4 sites); extract a shared `publicTaker` name/email helper (the CSV aggregate route has a local copy); extract the P2002→409 dedupe-conflict response (recurs across create/bulk/campaigns/edit).
...
**Effective gate:** all ED10 UI/behavior is gated on `ed10Active = previewSettingsEnabled && formsBuildEnabled && activeAuthoringMode === "single"` — i.e. ED10 presupposes ED9 forms mode (decision D10). Flag-OFF (or any non-forms mode) renders today's Metadata tab + Access link + Build default **byte-identically**.

**What it is (flag ON):**
- **Preview tab** (new landing tab, replaces Metadata) — a facts strip (Active↔draft version toggle · question/section counts · language) above the questionnaire rendered **read-only exactly as a respondent sees it**: the real respondent `SectionPager` in a new **additive default-off `previewMode`** (controls `disabled` but content stays readable in the a11y tree — NOT `inert`; required-gate skipped; Submit disabled; no `onSubmit`; `sectionIndex` clamp gated on previewMode). The draft side renders the live in-memory edits; the Active side renders the server-loaded published version. Assembly reuses `org-survey-client`'s exact pipeline via a new shared `assembleSurveyPages` (sort → SU-Full non-CEO audience filter → `filterVisibleSurveyQuestions` → `buildSectionPages` → `filterConditionallyEmptiedPages` → `mergeCustomSlides`) so a plain-respondent preview never leaks CEO-only questions and can't drift from the live survey; a `preview-version-adapter` maps editor-draft + stored-JSON shapes to pager types.
- **Settings tab** (one plain-language column) — Access (read-only "Invited only", dead PUBLIC radio gone) · **Aggregation** (humanized "Everyone / CEO only") · Language · **Invitation email** · **Results email** + "Approved to send" · "Send results by default" · Access-groups link row · read-only **Alias** under Advanced. Old ticket #3 de-jargon folded in; header pills humanized (`enum-labels`, flag-gated); duplicate Sections card dropped.
- **Split save model (D9, revised in co-validate):** invitation email + Language stay **Save-Draft** (draft-only, read-only when published) — invitation email is part of the version `contentHash` (`template-content-hash.ts`), so editing it while published would break provenance. Aggregation + Results email + "Approved to send" get an explicit **per-card Save** (editable while a version is published, via the existing `PATCH /api/admin/assessment-templates/{id}`; no new route/schema), with the **SEC-H2 approval interlock**: the approval toggle is disabled while the results-email card is dirty; a content Save clears approval to mirror the server's auto-clear; "approve" sends content + approval in ONE PATCH so the approval hash binds to the exact saved content. `sendResultsDefault` keeps its immediate Wave-Q PATCH.

**Architecture — extract-don't-fork:** new `lib/assessments/wave-ed10-flags.ts`, `lib/assessments/assemble-survey-pages.ts` (composes org-survey-client's pure fns; org-survey-client left unrefactored), `template-editor/preview-version-adapter.ts`, `template-editor/PreviewTab.tsx`, `template-editor/SettingsTab.tsx`. Modified: `TabbedShell.tsx` (one `ed10Active` gate drives routing/default/panels/pills/seam), `hooks/useTemplateEditorDraft.ts` + `useTemplateEditorModel.ts` (split save + conditional Save-Draft trim), `section-pager.tsx` (`previewMode`), `enum-labels.ts`, the edit `page.tsx` (Active-version load, language `enUS`), `TemplateEditorTabbed.tsx`. `CONTEXT.md` gained **Preview tab** + **Settings tab** terms (and the Test Mode note now points at Preview).

**Verification:** 13 TDD tasks (T0–T11), subagent-driven. ED10 test surface all green (`template-editor` **62 suites / 716**; assessments + ED10 lib green; the 3 pre-existing failing suites `dashboard-stats`/`lva-report-display`/`migration-verification` are ED10-unrelated). Byte-identity **proven**: `ed10-golden-snapshots` + frozen `editor-byte-equivalence` (15) + `three-pane-parity` (20) **zero snapshot diffs**. eslint clean (24 files); `CI=true npx next build --turbopack` exit 0. Codex co-validate: verdict no-go-as-written → all 6 findings verified + folded (C1 contentHash → D9 split; C2 blur-race → per-card Save; C3 no /me deserializer + audience filter → adapter; C4 inert → disable-controls; C5 flag-OFF default-regression → ed10Active gate; C6 live-campaign impact + `enUS`). Final whole-diff review **0 Critical / 0 Important** (1 Minor fixed — previewMode-gated clamp); **review-loop 5/5**. **T9 was a verified no-op** — the Build ⋯Delete already confirms via `useEditorCommands.deleteSection` (since ED9); the D12 grill premise was a misread, records corrected.

**LAUNCH BLOCKER — Neon Free-tier compute quota (infra, not code):** every Vercel deploy (preview + post-merge production) fails at the build's `prisma migrate deploy` with `P1001: Can't reach database server …ep-falling-sound-aiilz991…neon.tech:5432`. Diagnosed via `/diagnosing-bugs`: raw TCP to :5432 succeeds (Neon proxy up), but Prisma P1001s on BOTH the direct and `-pooler` endpoints, from two independent networks, no cold-wake recovery. The Neon console confirms the root cause — the project ("Scaling Up Platform", account **Joshua / Free plan**) hit **"Limit reached — 110.01 / 100 CU-hrs (100%)"** monthly compute allowance → Neon **suspended the compute**. So the **prod DB is unreachable** (affects the live app + all deploys, not just ED10). **Fix:** Josh **upgrades the Neon plan** (immediate) or waits for the ~Aug-1 monthly reset (not viable for a live app). Then: re-run `prisma migrate status` to confirm green → redeploy prod to land the merged ED10-dark → flag flip + walk (separate user auth). ED10 itself is safe throughout (flag OFF ⇒ zero prod behavior change). See memory `project_neon_free_quota_outage`.

**Kill:** `WAVE_ED10_PREVIEW_SETTINGS_KILL=1` or zero the flag → byte-identical to today's ED9 editor.

### 2026-07-17 — Wave ED9: Build-tab "Google Forms" overhaul (LAUNCHED) <!-- ENTRY_ISO:2026-07-17 ENTRY_SLUG:ed9-forms-build-launched -->

**Status: LAUNCHED (2026-07-17)** — PR #193 squash-merged to `main` (`bc5a6955`, dark deploy first), then `WAVE_ED9_FORMS_BUILD_ENABLED=1` set on Vercel Production + redeploy `k4t84f3ce` (Ready, aliased `platformtest.scalingup.com`). Ticket #5 of the 10-ticket editor-simplification pipeline (governing principle: the whole editor as simple as possible). Spec/plan `docs/specs/v7.6/19al` (co-validated — real Codex, 4 findings folded).

**What it is:** the ED6 single-column Build tab becomes a true Google-Forms-style builder (**presentation only** — no schema/API/persisted-data change). Flag ON, single-column mode:
- **Form-identity hero card** atop the Build column — editable title + description (two-way synced with Metadata via the patch-object `handleTemplateFieldChange`) + per-type count meta. The page-header `<h2>` is hidden **in single mode only** (`{!(formsBuildEnabled && single) && <h2>}`), so the title lives once (decision D1); three-pane/legacy keep it.
- **`FormQuestionCard`** — collapsed = one compact line (position · type pill · prompt · glyph state badges); focused = live `QuestionCanvas` preview + inline **`QuestionTypePicker`** (4 friendly types with lucide icons; locked chip for inherited/Wave-T) + per-type settings + Findings (Wave U) / Show-if (Wave W) panels + a **footer action bar** (Duplicate/Delete icons + a Required switch that auto-disables with a hint when a show-if rule is set — Wave-W interlock).
- **`FormSectionCard`** — inline name + a **description field** (new `handleSectionsSetDescription`; the field already round-trips and renders to respondents at `section-pager.tsx`) + a ⋯ overflow menu (Add question / Move up/down / Delete), with click-outside + Escape dismissal (`useOnClickOutside`).
- **Section-local add model** (co-validate Codex #4) — question-add is section-scoped only; the bottom bar offers **"+ Add section" only**; the empty state offers only add-section (a question can't exist without a section).

**Architecture — extract-don't-fork:** a shared **`useQuestionEditorActions`** command layer owns the destructive edits (type-change clears findings **and** dependent `showIf`; option-removal; inherited-slider scale) — called by BOTH the bare `QuestionInspector` and the new picker (a pure "confirm-text builder" would have dropped that cleanup — the co-validate Codex #1 catch). A **`useSingleColumnBuilderController`** holds the DnD/grouping/focus/announcements/command wiring, so ED6 `SingleColumnFormBuilder` and ED9 `FormsBuilder` are **both thin renderers** over it (no cloned orchestration — Codex #3). `QuestionSettings` + the exported `FindingsPanel`/`ShowIfPanel` are lifted from the inspector so the card composes them.

**Flag + kill:** `WAVE_ED9_FORMS_BUILD_ENABLED` (+ `WAVE_ED9_FORMS_BUILD_KILL`), gated at the ED6 seam in `TabbedShell`. Flag-OFF (or kill) → byte-identical to today's ED6 Build tab. **Byte-identity is PROVEN** by 7 golden `innerHTML` snapshots (`ed9-golden-snapshots.test.tsx`, captured pre-refactor — the real proof the behavior-only frozen suites don't give; Codex #2) held green through every extraction, plus frozen `editor-byte-equivalence` 15/15 + `three-pane-parity` 20/20 **zero-diff**.

**Verification:** 13 TDD tasks (T0 baseline → T1 flag → T2 golden net → T3–T6 extractions → T7–T10 components + builder → T11 flag seam → T12 review-polish → T13 gate); full `template-editor` sweep **612 tests / 54 suites** green (734 incl. the polish sweep); eslint clean; `CI=true npx next build --turbopack` (flag ON) compiles; Codex co-validate (4 findings folded, no overrides); final whole-diff review + review-loop **5/5 SHIP** (0 Critical/Important). Launch walk on prod (Rockefeller Habits Checklist editor Build tab): the Forms builder renders the hero card ("Slider ×40" meta) + friendly Slider pills + "Slider settings" + the live preview; the old `single-column-builder` is absent; the page-title `<h2>` is gone. Nothing respondent/portal/three-pane touched (diff scope verified).

**Kill** = `WAVE_ED9_FORMS_BUILD_KILL=1` or zero the flag → instant revert to the ED6 builder. **Next: ED10** = the Metadata→**Preview** + **Settings** tab rebuild (approved-design candidates 7/8, deferred from ED9's Build-canvas-only scope).

### 2026-07-17 — Wave ED8: Template version lifecycle — Roll back · Archive · Draft-delete (LAUNCHED) <!-- ENTRY_ISO:2026-07-17 ENTRY_SLUG:ed8-version-lifecycle-launched -->

**Status: LAUNCHED (2026-07-17)** — PR #191 squash-merged to `main` (`e932ebaa`); dark deploy `55dfcziaf` (Ready, migration applied), then `WAVE_ED8_VERSION_LIFECYCLE_ENABLED=1` set on Vercel Production + redeploy `7uppmb69t` (Ready, aliased `platformtest.scalingup.com`). Kill = zero/unset the flag → byte-identical legacy VersionsTab (archived rows show as plain "Published"); the archived-exclusion in read paths is persisted admin intent and stays in force (a kill never un-retires a version). Spec/plan `docs/specs/v7.6/19ak` (co-validated — real Codex NEEDS-CHANGES verdict folded).

**Why:** publishing was additive-only, so a template like Leadership Vision Alignment showed 3 identical "Published" badges with no notion of which version new campaigns use, and no rollback/archive/delete. The 2026-07-16 admin walk flagged versions UX as ticket #3 of the approved 10-ticket editor-simplification pipeline (governing principle: the WHOLE editor as simple as possible; every UI/UX decision gets a visual review first — the design-review Artifact gated this wave).

**Approved model** (derived per `(templateId, language)` — **Active is DERIVED, never stored**): **Active** = highest `versionNumber` with `publishedAt != null AND archivedAt == null` (actions: Roll back… · Duplicate); **Superseded** = published, non-archived, not highest (Archive · Duplicate); **Draft** = `publishedAt == null` (Edit · Publish · Delete); **Archived** = published + `archivedAt != null`, collapsed behind an "N archived — Show" row (Unarchive · Duplicate). **Roll back = archive the Active version** → the previous published non-archived version becomes Active for NEW campaigns; existing campaigns keep their pinned version by construction (`publishedAt` never modified, so pinned submit-scoring + reports are untouched). Guard: cannot archive the LAST published non-archived version of its (template, language) — server 409 `LAST_PUBLISHED_VERSION`.

**8 TDD tasks (each spec + quality reviewed) + 1 hygiene follow-up:**
- **T1** additive `archivedAt DateTime?` on `AssessmentTemplateVersion` + **immutability-trigger replacement** (co-validate C1 BLOCKER): migration `20260716150000` uses `CREATE OR REPLACE FUNCTION` (no destructive DDL — passes the Migration Safety Gate) to allow updates that change ONLY `archivedAt` on published rows while still blocking published-row deletes + any content/scoring/`publishedAt` mutation (`to_jsonb(NEW) - 'archivedAt' IS DISTINCT FROM …` allow-list; a schema WARNING forbids ever adding `@updatedAt`). New flag module `wave-ed8-flags.ts` (`isVersionLifecycleEnabled()` = ENABLED && !KILL).
- **T2** shared `active-version.ts` — `activePublishedWhere` + `resolveActiveVersion` (one definition of Active) + centralized `DEFAULT_TEMPLATE_LANGUAGE` (co-validate C4 — version-sections defaulted `"en"` while campaign-create defaulted `"enUS"`).
- **T3** archived-exclusion across every Active read path (campaign-create, both import routes, esperto helper, version-sections, benchmarks route + edit page, trends selection) — **unconditional, never flag-gated** (Wave-Q doctrine). Wave-T lock unions deliberately keep archived (identity locks against ALL history).
- **T4** lock-union regression pins (an archived version's stableKeys/types still yield `KEY_COLLIDES_WITH_PUBLISHED` / `TYPE_LOCKED`).
- **T5** new `archive/route.ts` (POST archive / DELETE unarchive) + draft-only DELETE — **Serializable transaction + one P2034 retry** for the concurrent last-published race (co-validate C2 BLOCKER; guards re-evaluated inside the txn), campaign preflight + `P2003` → 409 `VERSION_IN_USE` (C5), `AuditAction` union + `TEMPLATE_VERSION_ARCHIVED`/`UNARCHIVED`/`DELETED`.
- **T6** `useVersionActions` handlers (confirm copy per spec §2; coded-409 → human toasts; open-version delete hard-navigates).
- **T7** VersionsTab lifecycle table (Version · Language · Status · Published · Actions; hash column + "(you are here)" removed; ≤2 verbs per status; archived collapse), MetadataTab `VersionHistoryStrip` removed when flag ON (its all-"● Active" labels were wrong), TabbedShell language-scoped `publishedSibling` + derived pill (`v3 (active)` / `(superseded)` / `(draft)` / `(archived)`), post-rollback redirect prefers highest non-archived.
- **T8** UI suites — flag-ON statuses/verbs/collapse/multi-language (EN v3 active while ES v2 active) + flag-OFF byte-identity pins.
- Hygiene follow-up: `global.fetch` restore in the useVersionActions suite (adversarial-review nit; jsdom-sandboxed, not a contamination risk).

**Verification:** CI Turbopack build ✅; frozen ED3 byte-equivalence **15/15** + ED4 parity **20/20** with **ZERO diffs** to those files across the whole wave; eslint clean on all 40 changed files; **adversarial review SHIP, 0 defects** across the 4 hardest lenses (Active-derivation drift proven impossible — UI `deriveVersionStatuses` and server `resolveActiveVersion` use the identical rule, `@@unique(templateId,versionNumber,language)` removes tie ambiguity; kill-semantics — `isVersionLifecycleEnabled` appears ONLY in the archive route, versions DELETE, edit-page threading, UI plumbing; trigger/route interplay; archived-edit safety); full-sweep triage **`ed8CausedCount = 0`** (all 14 flagged suites git-blame-proven pre-existing — stale expectations from 2026-05 → 2026-07 + pre-existing 522-suite scheduling flakiness reproduced identically across cleared-cache / parallel / `--maxWorkers=2` / `--runInBand`).

**Ships EMPTY (honest-data):** every real published version row is `language:'enUS'` and unarchived, so LVA/SU-Full/QSP render with their current version as Active and no archived rows until an admin rolls back or archives. **Out of scope** (spec §9): Preview/Settings tab rebuild, ED9 Forms pass, "Make active" sugar on Superseded rows, version-level hard-delete of published rows (never).
...
**What:** the question-authoring surface rebuilt as a single scrolling column — a THIRD flag-selected presentation over the ED3 headless model, body-swapped at the one `TabbedShell` seam via a derived `activeAuthoringMode = single | three | legacy` (single-wins). The **Build** tab replaces Edit; the **Sections** tab folds in as inline header bands (section create/rename/reorder/cascade-delete on the bands); Metadata / Scoring & Tiers / Access / Versions untouched; `?tab=sections` resolves to Build in single mode; the tab id stays the literal `"questions"`.
- Collapsed cards = fixed-height one-line summaries (drag handle · position · type · prompt · state badges · move-select · Duplicate/Delete); a plain slider shows NO badges (Google-Forms-clean). Badges: Findings / Show-if / Required / Unassigned (text+icon, never color-only).
- The focused card expands in place to the live `QuestionCanvas` preview (reused verbatim; its collision-free `canvas-q-` id prefix means no clash with the inspector's `q-` FindingsPreview) + the re-hosted **bare** `QuestionInspector`; ONLY the focused card mounts an inspector.
- Within-section drag reorder (`useSortable` INSIDE the memoized card so memo blocks parent re-renders while the hook still drives the drag; keyboard-sensor drivable in jsdom) + a reliable "Move to section…" picker (cross-section drag = fast-follow, co-validate Q1) + "+ Add question below" (`addQuestion` gained an optional insert-after index).

**~90% reuse; the genuinely new code:** a shared `useEditorCommands` hook (the confirm→command→focus orchestration EXTRACTED from `EditorOutline`, behavior-preserving — ED4 parity green), a pure `single-column-view-model` (one-pass badges/positions), the memoized `QuestionCard` with an explicit comparator (editing one card can't re-render the other 60 — render-count guard), the `SingleColumnFormBuilder` card-list container, and an additive `QuestionInspector.bare` prop (byte-identical when off).

**Correctness:** every structural mutation routes through the shared model commands (bypass-proof); save flows through the unchanged `buildVersionScoringPayload`; published/inherited read-only locks reused verbatim; flag-OFF byte-identical (frozen guard + parity green throughout). Scoring language corrected (co-validate C2, verified `scoring.ts:1356–1457`): only SLIDER_LIKERT is scored — per-question Advanced = findings + show-if + slider scale, NOT "tier bands" (those live in the template-level Scoring & Tiers tab); dropped the per-question "Scored" badge.

**Gate:** brainstorm (visual companion) + grill-with-docs (code-verified: seam/flag, sections-absorb, read-only reuse, inspector re-host, parity harness) + **/co-validate (real Codex GPT-5.5 thread `019f6477` + own 4-lens review)** — the load-bearing finding (the `QuestionInspector` is a monolithic interwoven form, so the spec's "separate basic fields + one Advanced box" was not cheaply buildable) resolved by **decision A** (the expanded card body IS the re-hosted bare inspector) plus accepted refinements (sections routing, insert-after index, single-column's OWN focused test suite — not cross-asserted vs legacy DOM, single `activeAuthoringMode`, named three-pane retirement trigger) — + inline adversarial review (0 blockers; 1 LOW ledger note).

**Tests (jest-verified):** frozen ED3 byte-equivalence 15/15 + ED4 parity 20/20 + three-pane-flag 5 green throughout; new ED6 suites — wave-ed6-flags 10 · single-column-flag 5 · useEditorCommands 15 · add-question-insert-after 5 · single-column-view-model 6 · single-column-builder 6 · single-column-reorder-move 3 · single-column-render-guard 8 · single-column-inspector-bare 2 · single-column-expanded-card 3 · single-column-readonly 2 · single-column-a11y 3 · single-column-wiring 2. Full editor sweep **38 suites / 403 tests** (clean run; the full sweep can flake on test-timeouts under parallel contention — run it uncontended). `CI=true npx next build --turbopack` green (compiled 72s + tsc + route table); eslint clean.

**Ledger / follow-ons:** cross-section DRAG (v1 ships the picker); the `unassigned` badge is currently unreachable (orphaned-section questions render in no band, matching three-pane; prevented by cascade-delete + the save-time `ORPHAN_SECTION_REF` gate) — a catch-all "Unassigned" band if ever needed; per-question collapse (v1 = per-section); virtualization for very large instruments (fixed-height collapsed cards keep it deferrable); the pre-existing `TabbedShell.handleSectionsCascadeDelete` (unsorted) vs outline (sorted) section-delete prompt-ordering divergence (three/legacy only). Named retirement: on ED6 launch a follow-on removes `ThreePaneWorkspace` + its flag + the seam branch, but not before cross-section drag lands in single-column.

**Build note:** subagent-driven TDD for PR-A + T4 (both spec + quality reviews passed); an account session limit then forced the remainder (T5–T15) INLINE with per-task self-review + the frozen guard/parity as the safety net. 16 revertible commits.

### 2026-07-14 — Wave ED5 (editor-overhaul close-out polish) BUILT — PR pending merge <!-- ENTRY_ISO:2026-07-14 ENTRY_SLUG:wave-ed5-editor-polish -->

**The final wave of the 5-wave assessment-template-editor overhaul arc (ED1 Test Mode → ED2 Safe-to-Publish → ED3 extract-hooks → ED4 three-pane → ED5 polish).** Maximal "close everything" scope: resolves every item the ED4 post-launch audit (`docs/specs/v7.6/19af-ed4-postlaunch-audit.md`) surfaced. Spec `docs/specs/v7.6/19ag-editor-overhaul-wave5-polish.md` + plan `19ag-plan-wave5-polish.md`. Gate: brainstorm + `/grill-with-docs` (9 decisions) + `/co-validate` (real Codex GPT-5.5, thread `019f6066`, 6 findings accepted + 1 kept-hardened) + inline adversarial review (0 confirmed defects). **ADR-0023** records the structure-mutation policy.

**Flag/safety model.** Three-pane polish lives inside the flag-on `ThreePaneWorkspace` → rides the existing `WAVE_ED4_THREE_PANE_ENABLED` (kill = flag off + redeploy → byte-identical legacy `QuestionsTab`; no new flag). Flagless bits (kill = revert): the **global cascade section-delete** wiring on the Sections tab, the **tier-band bar** + live tier validation on the Scoring tab. `moveQuestionToSection` + the `collapse` slice sit on the model but are consumed only by the flag-on outline. Flag-off byte-identity pinned by the frozen ED3 guard (15/15) + the ED4 parameterized parity suite (20/20).

**Built (4 revertible PR-units, one branch `feat/wave-ed5-editor-polish`):**
- **PR-A — three-pane polish + shared model (T1–T14):** shared `toQuestionForInput` + `shapeSignature` (unified the two divergent widget mappers — B-4 DRY); BOTH canvas + inspector preview keyed by `uid+shapeSignature` (resets a stale throwaway value on focus OR widget-shape change — A-3/co-validate C1); inspector preview relabeled "Test which finding fires" + canvas "preview only" hint; section-collapse lifted into `useEditorSelection` (survives the Edit-tab unmount — C); **auto-focus** the first question once on controller mount (cold-landing A-1); `computeSurvivorFocus` + a post-mutation focus/scroll rule (add/dup→new row, delete/cascade→nearest survivor + DOM focus — a11y + never-empty canvas); per-section **`labeled/total`** counter (A-2, authoring wording); show-if **row badges** (conditional / gate-with-count — B-1a); shared `EditorDrawer` shell + read-only **logic-map drawer** (B-1b/M6); **section add/rename/reorder** from the outline (B-2); **cascade `deleteSection`** command (atomic; removes section + questions; clears EXTERNAL show-if dependents only; physical delete, no tombstone) + shared `buildSectionDeletePrompt` (enumerates inherited keys + 3 Wave-T consequences + freed dependents) + outline delete (B-2b); **`moveQuestionToSection`** command + `buildMoveQuestionPrompt` (inherited-only warning) + explicit "Move to section…" control (B-3); save serializer + publish gate independently **reject orphaned `sectionStableKey`** (`ORPHAN_SECTION_REF`) — publish already rejected dangling show-if (`checkShowIfIntegrity`) + orphan refs (`checkSectionRefsResolve`); a11y landmarks + row `aria-label` (visible label+type) + widened outline column (`minmax(14rem,22%)`).
- **PR-B — global cascade wiring (T15):** the legacy Sections tab now deletes through the SAME cascade command + prompt (via shared `collectSectionDeleteImpact`), so both surfaces prompt identically (co-validate C2); fixes the latent orphan bug where dropping a non-empty section stranded its questions.
- **PR-C — tier-band bar + live validation (T16–T18):** exported `computeGlobalTierDomain` + `TierDomain`; canonical `tier-band-math` (`tiersToBoundaries`/`boundaryToTiers`/`clampBoundary` — integer `upper.min=lower.max+1`, fractional touching) shared by the bar AND the validator (co-validate C2 — fixes an inclusive-overlap bug); `TierBandBar` (drag + keyboard dividers; finite-domain only, else inputs + a note) for global + per-domain (`computePerDomainTierContexts`); wired the exported `validateTierTiling` client-side so a non-domain-spanning draft is flagged **live** (closes the Wave-V publish-vs-runtime gap).
- **PR-D — multi-container drag (T19–T20):** pure, exhaustively-tested `resolveOutlineDrop` (reorder-vs-move decision, dependency-free — jsdom can't drive dnd-kit pointer drag); `EditorOutline`'s drag layer restructured from N per-section `DndContext`s into ONE multi-container context (per-section `SortableContext` + a `useDroppable` per section so empty sections accept drops); within-section reorder byte-identical (guard + parity green); SR announcements now name the question label/key, not its uid (T13 a11y).
- **Close-out (T21–T22):** round-trip persistence tests (cascade→serialize / move→serialize — no orphan/dangling survives, key preserved); ADR-0023; SoT; Notion; memory.

**Verification.** Editor sweep **356 pass / 28 suites**; frozen ED3 byte-equivalence guard 15/15 + ED4 parity suite 20/20 green throughout; `CI=true npx next build --turbopack` green; eslint clean on all changed files. **Deferred (honest):** the slider-tick SR-cursor a11y item lives in the shared respondent `QuestionInput` (live survey), not the editor — a respondent-scoped change for its own review (`aria-hidden` there broke 9 respondent tests). Cross-section-drag *dispatch* isn't end-to-end tested (jsdom can't drive cross-container drag; `resolveOutlineDrop` + the explicit control are tested). **Build note:** implemented largely inline from the main loop after the subagent-driven TDD run hit an account session usage limit mid-T12 (same pattern as Waves T/U/V); TDD discipline (failing test → implement → pass) kept per task. **Launch = user merges the PR** (agent self-merge to protected `main` is blocked); merge auto-deploys and the three-pane polish appears on the already-live ED4 flag.
...
**The /co-validate pivot — ONE shell, not two.** The original design built a parallel `ThreePaneShell`. Both real Codex (GPT-5.5 @ xhigh, read-only) and the own-review independently rejected it: a second shell duplicates header/nav/modal/action wiring that can drift, for a *weaker* kill guarantee. Adopted instead: a **flag-selected *workspace* inside the existing `TabbedShell`** — flag ON, the **Questions tab body** renders the three-pane and the tab relabels "Edit" + becomes default; everything else (header actions, tab nav, Metadata/Sections/Scoring/Versions/Access surfaces, modals) stays **single-source**. "Flag off ⇒ identical" is then provable by construction (supersedes the brainstorm Q4-A "sibling shell" + grill G7 "header re-implementation").

**Architecture.** `TemplateEditorController` builds the one shared ED3 `model`; `TabbedShell` gains ONE conditional (`threePaneEnabled ? <ThreePaneWorkspace> : <QuestionsTab>`). New: `ThreePaneWorkspace` (composes the three panes over the shared model), `EditorOutline` (sections-as-headers tree; add/duplicate/delete/focus + within-section **drag AND keyboard** reorder; empty-state → Sections tab; `isPublished` gating), `QuestionCanvas` (renders the focused question via the real `QuestionInput`; **local throwaway answer-state, keyed by the focused uid, never touches the model** — G4/C4), `wave-ed4-flags.ts` (`isThreePaneEnabled()`, mirrors `wave-ed2-flags.ts`, resolved server-side + passed as a prop). `QuestionInspector` reused verbatim (right pane).

**Shared question commands (co-validate C2 — the prerequisite).** The add/delete semantics were split: the model handlers returned `void` and the delete-confirm + inherited-warning + show-if **dependent discovery/cleanup** lived in `QuestionsTab`'s presentation, so a new outline calling the raw handlers would **bypass** the cleanup. Fix: lifted `addQuestion`/`duplicateQuestion` (return the new uid → focus it) + `deleteQuestion` (returns affected dependents + clears their `showIf`) + the shared `buildQuestionDeletePrompt`/`computeShowIfGates` into `question-commands.ts`; `QuestionsTab` refactored to call them (behavior-preserving); the outline uses the same commands → **bypass-proof by construction**, no forked logic.

**Other co-validate findings (7 accepted, 0 overridden).** C3 → the safety guard is **one parameterized parity contract suite** run through `TemplateEditorController` over `threePaneEnabled ∈ {false,true}`, asserting the identical **fetch transcript + UI + dirty state** for every mutation (edit→Save, Publish, Duplicate, serializer-failure→zero-requests, double-save, failed-PATCH→retry, Wave-T data-loss regression, 422/409) — flag-ON ≡ flag-OFF; the frozen ED3 guard freezes flag-OFF, so flag-ON is transitively frozen. C4 → canvas invariant structured (keyed remount; tests assert Save-stays-disabled + zero-fetch on canvas interaction). C5 → `QuestionInput` gains an optional `idPrefix` (default `"q-"`, backward-compatible) so the canvas (`"canvas-q-"`) can't collide on DOM ids with the inspector's simultaneous FindingsPreview. C6 → kill is flip+redeploy. C7 → proactive render memoization dropped (self-defeating + off-objective; hygiene only, measure first).

**TDD build (subagent-driven, 8 tasks).** T1 shared commands (`e4592e9f`) · T3 flag+workspace-pick+stub (`30875e46`) · T2 parity suite (`696e0a8c`) · T4 EditorOutline (`0ba6ce63`) · T5+T6 QuestionCanvas + idPrefix (`d73f60bc`) · T7 canvas-invariant + narrow-width (`e7c92f51`). T1 + T3 got fresh-eyes reviews (PASS); T4–T7 were finished + reviewed inline after two subagents hit session limits mid-task. Final multi-lens adversarial review **PASS** — flag-OFF byte-identity + canvas invariant airtight, bypass-proof deletes confirmed, parity genuine (0 runtime skips).

**Verification (jest-verified).** Parity **19/19** (0 skips); ED3 byte-equivalence guard **15/15**; editor sweep **280/280** (27 suites); full sweep **5,757 pass / 5,786** (8 pre-existing failing suites, NONE ED4 — proven: none import an ED4 file; failure reasons unrelated: LVA `lva-fidelity-v2` provenance, auth-allowlist drift, quiz/assessment route codes, date-lint, the known `metrics.total` portal read); `CI=true npx next build --turbopack` green; eslint clean.

**Two LOW/informational (non-blocking, documented).** (1) the model's `deleteQuestion` clears dependents *unconditionally* (vs. the flag-gated presentation) — benign + more-correct + prod-unreachable (Wave W conditional authoring is live, so both flags are on together), and it's the *intended* bypass-proof behavior. (2) deleting a whole section (via Sections) while a question in it is focused leaves focus resolving to `null` → canvas/inspector empty states (no crash) — a W5-polish UX edge.

**Launch (flag flip + redeploy + internal editor walk with the ~3 admin authors) is a separate authorized step.** W5 (polish: show-if badges + read-only logic map, drag-to-set tier bands, section-create-from-outline) remains.
...
**Gate (all before any code).** Brainstorm (5 decisions) → `/grill-with-docs` (3 decisions; guard determinism code-verified — `uid` is never emitted in the PATCH payload, so byte-exact snapshots are deterministic) → **`/co-validate` with REAL Codex GPT-5.5 @ xhigh — 5 findings, ALL accepted, 4 of them OVERRODE earlier brainstorm/grill decisions**: C1 add a `TemplateEditorController` that owns ONE model instance and hands it to the active view (precise form of single-owner); C2 lift selection NOW (else W4 still does a stateful extraction); C3 do NOT over-split — one `useTemplateEditorDraft` transactional aggregate, not six per-surface hooks; C4 the guard snapshots the COMPLETE request transcript + transition scenarios; C5 extract ONE public `QuestionInspector`, sub-panels private. Own review added F1/F3/F6/F7 (jsdom-friendly reorder, exact body strings, preserve useCallback identities, mandatory prod smoke).

**The extraction (subagent-driven TDD; 9 commits `20a13415`→`78dc2bdb`).**
- **T1 — golden byte-equivalence guard, written FIRST** (`editor-byte-equivalence.test.tsx`, **15 tests**): renders the live editor, asserts the exact outgoing `fetch` transcript (method+URL+ordered body STRINGS+count), derived behaviors (`isAnyDirty`/Save-disabled/`?tab=` routing), post-save `assignedKeys`/raw-ref reconciliation (by stableKey), and transition scenarios — serializer-failure→ZERO requests, double-save prevention, failed-save→retry, the **Wave-T sections-only-follow-up-save data-loss regression**, publish 422/409, duplicate, and a real keyboard `@dnd-kit` question-reorder. `genUid` mocked to a counter. Held byte-green through every later slice.
- **T2** — split the 1,393-line `TemplateEditorTabbed` monolith into `TemplateEditorController` (owner) + `TabbedShell` (view); `TemplateEditorTabbed.tsx` becomes a compatibility re-export so the page + all tests resolve unchanged.
- **T3** — `useEditorSelection` (lift `focusedQuestionUid`/`selectedSectionStableKey` out of `QuestionsTab`; a mount-effect reproduces the pre-ED3 remount-reset; a new characterization test pins tab-switch selection behavior before + after).
- **T4** — `useTemplateEditorDraft`, the transactional aggregate (document state + dirty + raw refs + all change handlers + `handleSaveDraft` incl. serialize-before-fetch fail-atomic + the Wave-T reconciliation), extracted in three guard-green sub-steps (T4a/b/c). *(The T4 implementer subagent hit a session usage-limit mid-C; completed inline from the clean T4b checkpoint.)*
- **T5** — `useVersionActions` (publish/duplicate + `publishIssues`); the hook returns raw snapshots as plain values (refs stay internal) so the view never reads `ref.current` during render (`react-hooks/refs`).
- **T6** — `useTemplateEditorModel` composer; the controller calls it ONCE and passes one `model` to the thin view (Codex C1). *(Caught + fixed a T5 staging slip where the hook's plain-value return was left uncommitted — commit `2eca18c1`.)*
- **T7** — extract the public `QuestionInspector` (was `QuestionConfigForm`) with `FindingsPanel`/`ShowIfPanel`/`FindingsPreview` private+collocated (Codex C5); strictly one-directional imports (no cycle); + 3 structural inspector tests.

**Verification (jest-verified counts).** Byte-equivalence guard **15/15**; **241 template-editor tests / 23 suites** green (incl. the wave-t/u/u3/w suites, now rendering through a `QuestionsTabHarness` that wires the real `useEditorSelection` — so the inspector's findings/show-if/type paths stay deeply exercised); **full sweep 5,705 pass / 5,734** with **8 pre-existing failing suites (29 tests) — none ED3** (verified: none import any ED3-changed module; the lone date-lint failure is in `public-quiz-client.tsx`, untouched). `CI=true npx next build --turbopack` clean; eslint 0/0. **Multi-lens adversarial review (real Codex, opus): SHIP — 0 material findings** (traced closure-safety of the lifted `handleSaveDraft`, selection-reset fidelity across 7 cases, raw-snapshot staleness, inspector extraction, single-instance, and the one `react-hooks/set-state-in-effect` suppression on the loop-safe tab-sync effect — all faithful to the original monolith).

**Deferred (disclosed):** direct `renderHook` unit tests for `useTemplateEditorDraft`/`useVersionActions` — judged redundant (the guard drives them end-to-end incl. reorder; the wave suites exercise them through the harness; `useEditorSelection` + `QuestionInspector` have their own unit tests). **Next:** W4 three-pane (Approach A, flag-gated, kill-able) — now a view over these hooks.
...
**Wave 2 of the assessment-editor overhaul** (re-sequenced 5-wave plan; spec `docs/specs/v7.6/19ad-editor-overhaul-wave2-safe-to-publish.md` + plan `19ad-plan-wave2-safe-to-publish.md`). _(Shipped DARK — `WAVE_ED2_SAFE_TO_PUBLISH_ENABLED` was default-OFF at merge; prod was byte-identical until the flag flip above.)_

**What it is.** A **live publish-readiness readout** in the admin template editor. Today the publish gate is *reactive*: the author only learns what blocks publish after they Save, click Publish, and hit the `422 PUBLISH_VALIDATION_FAILED` modal. Wave ED2 surfaces that **same gate live**, in a persistent header badge (beside Test Mode), visible on every tab, expandable to a grouped panel:
- **Prevent** = the exact issues the publish route would return (the six `TemplateVersionForPublishSchema` checks + base schema), shown before Publish. They still block publish server-side — unchanged.
- **Warn** = 2 advisory, publish-legal structural nudges that never block: **empty section** (a section no question references) and **unassigned question** (blank `sectionStableKey` → the "Other" bucket).

**Extract-don't-fork (the anti-drift core, co-validate C1).** A new pure `getPublishValidationIssues({questions,sections,scoringConfig}) → ZodIssue[]` in `scoring.ts` is the single publish-validation entry point; the **publish route was refactored to call it** (behavior-preserving — the `422` shape is byte-identical) and the badge calls it too. So the live readout is the *literal same code* the server runs — parity by construction, not a re-implemented check (the same move Wave 1 made for `computeScoreResult`). The badge assembles the live draft via the Wave-1 `buildVersionScoringPayload` seam (so "what the badge validates" == "what Save persists"), and **soft-fails** — a serialization/assembly error becomes a Prevent-class note, never crashing the editor (mirrors the Test Mode drawer). Warnings are computed from the raw payload **independently of Prevent** (C4 — `safeParse` yields no data on any failure, so the "N blockers · M warnings" state stays reachable). Client-side, **zero db/server imports**; writes nothing.

**Passive.** The Publish button is untouched (grill Q1) — the badge is guidance, the server `422` stays the authoritative gate. On a dirty draft the badge reads "Ready after save" (never plain "Ready to publish"); on a published/read-only version it's hidden.

**Gated build.** brainstorm → `/grill-with-docs` (3 decisions: passive readout · type-based-then-dropped Warn #3 · 2-warning set) → `/co-validate` (real Codex GPT-5.5 @ xhigh, **4 findings all accepted**: shared helper for real parity; "Ready after save"; **drop the "no findings authored" warning** — its premise was code-verified false, reports omit the empty section; warnings independent of prevent) → spec `19ad` + `CONTEXT.md` "Safe-to-Publish" glossary term → subagent-driven TDD (6 tasks) → **multi-lens adversarial review** (5 lenses + refute-by-default verification): **1 real defect fixed** — the badge's readiness `useMemo` was defeated (parent passed a fresh `new Set(...)`/`{...}` every render → full publish validation re-ran on every keystroke on large instruments); fix = memoize `publishedKeys`/`dirty` in the parent + `useDeferredValue` on questions/sections (spec §3.5 debounce). **2 findings refuted with proof**: the `scoringConfig` null-parity divergence is *unreachable* (`scoringConfig` is a required non-null `Json` column; no write path passes `JsonNull`), and `isDirty = isAnyDirty` is a safe superset (never a false "Ready"; the "save first" nudge steers to the correct action).

**No ADR** (additive, flag-gated, writes nothing → fully reversible). **No schema change, no migration.** New files: `wave-ed2-flags.ts`, `template-editor/publish-readiness.ts`, `template-editor/SafeToPublishBadge.tsx`. Touched: `scoring.ts` (+helper), `publish/route.ts` (calls the helper), `TemplateEditorTabbed.tsx` (flag-gated badge + stable-identity memos), the version edit page (flag prop). **29 new tests / 6 suites green** (jest-verified), tsc + eslint clean, `CI=true next build --turbopack` green. PRs: spec+plan+code on `feat/wave-ed2-safe-to-publish`.
...
**Covers (all LIVE on prod since the Jul-8 report):** (1) **Wave U3 recommendations-in-email LAUNCHED** (#166) — findings render in the results email, both recipients + both report kinds; honest-data (empty unless authored). (2) **Templates-list View==Edit** (#167) — dropped the redundant "View" link. (3) **Wave Z admin pages** (#168 Public Campaigns wire + #169 admin Campaigns/Organizations) — the 3 "coming soon" entries now real by reuse-not-rebuild; 60 tests, adversarial review 0 defects. The report frames these as the **Jul-8 3-target roadmap CLOSED**.

**Folds in the next arc — assessment-editor overhaul, DESIGN-stage (NOT built), badged as such in the report:** a live web-grounded competitive research sweep (Qualtrics · SurveyMonkey · Culture Amp · Typeform · Jotform · Fillout · Tally · Qualtrics-360 · Leadership Circle · Spidergap · ScoreApp · Outgrow · involve.me · Riddle · NN/g · Baymard) → chosen end-state **Approach A** (outline · canvas · inspector working editor — NN/g outline/tree rule, NOT a Typeform clone) → a **/co-validated** plan (Codex env-down → repo-grounded 3-lens panel + own review; all four independently "ship-with-changes", strong convergence). Panel-accepted changes: **extract-don't-fork** (both layouts as thin shells over shared headless hooks; byte-identity by construction), **re-sequence** (capabilities first, shell rebuild last, kill-able), Test-Mode fidelity (`pruneHiddenAnswers → scoreSubmission`, no separate `resolveFindings`; report-preview is a pure-assembler extraction, not a freebie), Prevent/Warn bounded (existing publish failures stay Prevent), inspector = Content+Findings+Show-if only.

**Re-sequenced wave plan (adopted):** Wave 1 = **Test Mode** (submit-and-score sandbox drawer on today's editor, writes nothing) · Wave 2 = **live Safe-to-Publish** panel · Wave 3 = extract shared editor state/inspector hooks · Wave 4 = **three-pane (Approach A) layout, flag-gated, kill criterion** · Wave 5 = logic/scoring polish. Deferred/if-requested: benchmark generalization, logic map, anonymity mechanism (Jeff/legal). **Wave-1 Test Mode spec drafted; awaiting build greenlight — nothing built.**

**Grounding facts (live audit):** the editor supports **4 question types** — `SLIDER_LIKERT`, `TEXT`, `NUMBER`, `MULTI_CHOICE` — as a Zod discriminated union (`scoring.ts`), NOT the dead 3-value Prisma `AssessmentQuestionType` enum (omits TEXT, referenced by no column). Live content: 200 sliders / 75 text / 14 number / **1 multi-choice** across 7 templates (69% sliders; the lone MULTI_CHOICE is LVA's obstacles gate). **No radio/single-select type** — single-select = `MULTI_CHOICE` + `maxChoices:1`, rendered as a capped checkbox. Scoping mockups (layout-directions + usage-walkthrough) are private claude.ai artifacts — design aids, not shipped. **Docs/no app-code change — prod behavior identical. Do NOT rebuild or re-send the report (user sends). Codex MCP still env-down.**
...
**Working-tree sweep — a clone is now the complete project state.** Every accumulated uncommitted work product committed: progress reports Jul-1/2/3/6/7/8 (HTML+PDF), ~80 launch-walk screenshots/PDF/YAML evidence artifacts (waves C→W), `deliverables/` (data-model diagram, LVA comparison doc for Jeff, status PDF, Wave F group-report mockup), `docs/specs/v7.6/18-jeff-june22-assessment-fidelity.md`, `plans/audit-remediation-plan.md`, the Jun-22 sprint excalidraw, `src/PLAN.md` (thin hub), `src/scripts/wave-x-walk.ts`. Two walk snapshots had the retired personal test-account email redacted before commit. `.gitignore` gains a local agent/tooling session-state block (`.agents/`, `.codex/`, `.claude/skills/`, claudex state dirs, `src/test-results/`) so session detritus stays out of `git status` (tracked claudex logs stay tracked). Also folds in the one-line annotation the PR #163 session left staged (the `wave-x-launched` entry's "LVA peer numbers" line now cross-references the phantom-ask correction). **No app code changed** — docs/assets/scripts only; production behavior identical.

### 2026-07-08 — Progress report Jul-8 SENT (Waves Y · W-cleanup · U3) + roadmap + LVA-peers correction <!-- ENTRY_ISO:2026-07-08 ENTRY_SLUG:progress-report-jul8-sent -->

**`Scaling-Up-Progress-Update-2026-07-08.html/.pdf` (repo root) SENT to Jeff 2026-07-08** — Slack (terse bullets) + Loom walkthrough (`loom.com/share/ac4e9c76c7954777914008ccf021c928`). Covers the post-Wave-X window: **Wave Y** (import observability), **Wave W-cleanup** (editor ghost-UI removal), **Wave U3** (recommendations in the results email — staged/dark + the editor test-a-value preview live), and the sections-sortOrder fix. Two REAL sample findings-emails (scored + qualitative) rendered from shipped code, plus a **Roadmap section with three committed targets, each shown with target-design screenshots** (Phase-2 wireframes, chrome-stripped): (1) flip recommendations-in-email after Jeff reviews a sample; (2) build the three "coming soon" admin pages (Organizations / Campaigns / Public Quizzes — they **404 today**) per the Phase-2 blueprint; (3) fix the templates-list **View == Edit** bug (name/View/Edit all open the same URL). Export-hardened (added `@media print` the prior reports lacked; PDF via headless Chrome). **DONE — do not rebuild/re-send.**

**Correction landed here — "LVA peer numbers" was a PHANTOM waiting-on-Jeff item, now DROPPED.** Paper trail: Jeff's `From Jeff/gabriel-feedback.docx` asks for **industry benchmarking on Scaling Up Full** ("missing the industry benchmarking feature that existed in Esperto… let's discuss whether to build this universally or report-specific"); his Vision Alignment (LVA) items are only obstacles-question fidelity / remove "All Sections" / layout — **never peers**. Our own `docs/specs/v7.6/18-lva-source-fidelity-audit.md` states *"No benchmark/peers column in source — ours correctly has none."* Wave S (#12/#13) nonetheless built an (empty, inert) LVA peer panel, and "waiting on Jeff: LVA peer numbers" then propagated through the SoT + the Jul-6 report. It ships empty so nothing is wrong live — but it was **never a Jeff dependency** and is removed from the waiting-on-Jeff list. Jeff's real open benchmarking item = **SU-Full industry benchmarking** (universal-vs-report-specific decision).

**Waiting on Jeff (corrected):** SU-Full industry benchmarking decision (universal vs report-specific) · #2.3 invite copy · #15/#16/#19 wording. _(LVA peer numbers REMOVED — not a real ask.)_

### 2026-07-08 — Wave U3 BUILT: findings in the results email (dark) + editor test-a-value preview (live) <!-- ENTRY_ISO:2026-07-08 ENTRY_SLUG:wave-u3-built -->

The two ready Wave U §3 leftovers, built through the gated pipeline (brainstorm → grill-with-docs → spec `docs/specs/v7.6/19aa-wave-u3-email-findings-editor-preview.md` → user greenlit "go" [`/co-validate` offered but declined; Codex env-down] → subagent/TDD build → 2-lens adversarial review). **No ADR** (extends ADR-0021), **no schema, no migration**. The third leftover — group-report/cohort findings — is **DEFERRED** (D1: undefined aggregation semantic + no scored-group recommendations slot; its own wave, likely a Jeff product call).

**(1) Findings in the results email** (`report-email.ts`) — a findings block rendered into BOTH the scored anatomy and the qualitative twin from the **FROZEN** `report.result.findings` snapshot (via `parseResolvedFindings` — NEVER re-resolves on the read path; the `recommendations-reader-audit.wave-u` guard stays green). Ships **DARK** behind a NEW default-OFF flag `WAVE_U3_EMAIL_FINDINGS_ENABLED` (new `wave-u3-flags.ts` / `isEmailFindingsEnabled`) — its OWN flag, NOT the live Wave U flag, because reusing the already-live flag would push findings into real results emails on deploy (a sensitive send-path change; D2). Design decisions:
- **Scored: ALL kinds, INCLUDING SLIDER_LIKERT bands** (D5) — a deliberate divergence from `BrandedReport`'s non-slider filter. The email has no legacy per-row slider-rec path, so there is no double-display problem, and excluding sliders would gut recommendations for slider-heavy scored instruments like SU-Full.
- **Qualitative: reuses the shared `buildFindingsSection`, rendered BEFORE the answers block** (D5) so recommendations survive the ~90 KB `QUAL_EMAIL_BYTE_BUDGET` / answer truncation — including the extreme case (the "assessment received" empty-body fallback now leads with findings too; the adversarial-review-found edge).
- **Both recipients** (D4: `TAKER_COPY` + `REFERRING_COACH`).
- **Flag-OFF byte-identical** (D6) — injected via adjacent concatenation (`${breakdownBlock}${findingsBlock}` / `${preface}${findingsBlock}${sectionsHtml}`) so the block collapses to nothing when off; locked by a flag-OFF byte-identity test. Imports nothing from the peers module, so the Wave S email byte-identity guard stays green. Every finding text is HTML-escaped (campaign-authored content).

**(2) Editor test-a-value preview** (`QuestionsTab.tsx` `FindingsPreview`) — a preview inside the (already Wave-U-flag-gated) `FindingsPanel` that answers the question with the REAL respondent widget (`QuestionInput` — same slider scale/step, same MULTI_CHOICE checkboxes) and shows which finding text fires, live and tolerant, including the explicit no-answer⇒nothing case. **LIVE on merge** (authoring aid; reuses the live `isFindingsLogicEnabled()` — no send/prod-data effect; D2). MULTI_CHOICE fires in the question's AUTHORED option order regardless of tick order (matches the resolver).

**No-drift (D7, the core correctness property):** the emitted `recommendations` derivation was extracted from `buildQuestionsPayload` into a SHARED pure helper `buildFindingRecommendations` (in `question-serialization.ts`), now used by BOTH the save path and the preview. So "what the preview says fires" is computed from the EXACT rules a save emits → a submission resolves — the preview provably cannot disagree with a published version. Pinned by a parity test (`buildFindingRecommendations(d)` ≡ `buildQuestionsPayload` emission across SLIDER/NUMBER/MC/TEXT/no-rules).

**Verification:** `CI=true npx next build --turbopack` exit 0; ESLint clean; **37 new tests** (jest-verified) across 4 suites (`wave-u3-flags`, `question-serialization.wave-u3` no-drift parity, `report-email.wave-u3-findings`, `questions-tab.wave-u3` preview widget); full sweep **5,600 pass / 28 fail** (the 7 pre-existing failing suites only — zero new). **2-lens adversarial review** (correctness/security/regression + no-drift/spec-fidelity): **0 confirmed defects** on every attacked invariant (flag-OFF byte-identity, never-throw, escaping, helper equivalence, MC order, Wave S + reader-audit guards); **1 LOW edge** flagged by both — the qualitative empty-answer-body fallback dropped the findings block (against the D5 "survive truncation" intent) — **FIXED** (findings now lead the degraded body; still flag-OFF byte-identical) + locked by two tests.

**Kill:** email = zero the flag (frozen snapshots persist inert — flags gate rendering, never data); preview + shared helper = revert-commit. **Launch (flip `WAVE_U3_EMAIL_FINDINGS_ENABLED` after a launch-walk: send a real scored (slider-bearing) + a qualitative results email, confirm findings render in-inbox for both taker + coach) is PENDING and requires separate explicit authorization.**

### 2026-07-08 — Wave W leftovers LAUNCHED: editor ghost-UI cleanup <!-- ENTRY_ISO:2026-07-08 ENTRY_SLUG:wave-w-leftovers-launched -->

Flagless, presentation-only cleanup of dead/misleading UI in the admin assessment template editor (spec `docs/specs/v7.6/19z-wave-w-leftovers-ghost-cleanup.md`; no flag, no schema, no migration, no ADR; kill = revert-commit — Wave R posture).

**Removed:**
- The **"Peer Benchmarks" ghost card** in the Scoring & Tiers tab (`ScoringTiersTab.tsx` "Section 3 — Deferred logic placeholder") — a disabled mini-table of FAKE hardcoded benchmark values (`Q3_2: 2.4`, `Q5_1: 1.8`, `Q7_3: 2.7`) with dead buttons — plus its sibling **"Why is this section deferred?"** explanation card (Section 4). Both were superseded by the LIVE Wave S peer-averages panel (`PeerBenchmarksPanel`, `AssessmentBenchmark` rows) mounted on the edit page; the ghost showed admins fabricated numbers for a feature that exists for real elsewhere. The tab now ends cleanly after the per-domain tiers section.
- The disabled **"Preview as Respondent"** header button (`TemplateEditorTabbed.tsx`, `title="Coming in v1.5"`) — a dead no-op with stale v1.5 framing.
- Corrected **stale docstrings** in both editor files (the tabbed-editor header still described a 7-tab layout incl. the long-gone "Conditional Logic" tab; the Scoring tab header still listed the ghost cards).

**Kept — LVA `applyLvaFilter` migration DESCOPED (spec D2/D7, no ADR):** the candidate paired the ghost cleanup with migrating LVA's hardcoded show-if branch to the generic authored `showIf` engine. The mapping is byte-exact (each `S5_why_<K>` suffix equals an `S4_biggest_obstacles` option key; S5 optional TEXT), BUT deleting the branch is not clean: (1) it keys on `templateAlias` and runs for every LVA campaign regardless of pinned version — live LVA versions carry no authored `showIf`, so deletion regresses survey-side hiding for campaigns on a pre-`showIf` version (reports stay correct — `REPORT_FILTERS` is alias-keyed); (2) it flips LVA storage (hidden S5 answers are stored + report-suppressed today; authored `showIf` would make `pruneHiddenAnswers` drop them pre-persistence); (3) a true deletion requires backfilling `showIf` onto immutable published version rows + a prod launch walk. For 18 tested lines Wave W proved safe, that prod data op is not worth it now. Kept with a code comment on `applyLvaFilter` + this ledger; deferred follow-on "LVA-migrate-to-authored-showIf".

**Wireframe drift (grill Q1/Q2):** the three Phase-2 editor wireframes (`src/public/wireframes-phase2/admin/16|17|18-*.html`) share the stale tab-bar + header action row and originated these fossils; each got a **"superseded" banner** after `<body>` recording that Peer Benchmarks → Wave S, Conditional Logic → Wave U/W, Preview-as-Respondent → not built. Surfaces the drift (wireframes-are-the-spec rule) without a surgical rewrite of a historical reference the app does not render.

**Process:** gated pipeline — brainstorm → grill-with-docs (LVA descope + sweep breadth + wireframe-banner scope + no-ADR all resolved) → spec 19z → **/co-validate** (Codex unavailable — env: invalid `service_tier="priority"` in `~/.codex/config.toml` + stale token; fell back per the skill to a 3-lens Workflow staff-review panel, which caught the one real defect: deleting Section 4 also breaks the `describe("Explanation card")` test — the spec had listed only one test flip; fixed to THREE assertion flips across two files) → TDD (red: 3 flipped assertions fail against the live UI; green after deletion) → **2-lens adversarial review** (both ship; one pre-existing stale test title "all 7 tabs" → "all 6 tabs" fixed opportunistically). Tests: 3 assertions flipped from asserts-present to asserts-absent — `ScoringTiersTab.test.tsx` ×2 (peer-benchmarks ghost + explanation card) + `TemplateEditorTabbed.test.tsx` ×1 (preview button). Verification: ESLint clean; `CI=true npx next build --turbopack` exit 0; the two touched suites 36/36; full sweep 28 failures / 7 PRE-EXISTING suites (Wave Y baseline exactly — zero Wave-W-leftover failures; an 8th full-run failure was a load-induced 5000ms flake that passed on `--onlyFailures` re-run).

**Hand-off:** the template editor no longer shows fabricated benchmark numbers or a dead preview button; real peer-average authoring remains the Wave S panel. LVA conditional behavior unchanged (branch kept). No user-facing behavior or data change.

### 2026-07-07 — Wave Y LAUNCHED: import observability panel + preview/refusal signals (merged to main, live) <!-- ENTRY_ISO:2026-07-07 ENTRY_SLUG:wave-y-launched -->

**The last buildable P8 slice — LAUNCHED, merged to main (PR #159, squash `9d30845c`); flagless, so the merge IS the launch** — spec `docs/specs/v7.6/19y-wave-y-import-observability.md` (gated pipeline: brainstorm → grill-with-docs → grill-me → spec → /co-validate [Codex + independent Claude, 6 findings: 5 accepted, C6 overridden] → user greenlight → subagent/TDD build → 5-lens adversarial-review Workflow, 4 findings ALL fixed). **No ADR** (additive/revertable), **no schema change, no migration, no flag** — signal writes are UNCONDITIONAL + fail-soft (Wave Q rule; matches Wave V's own commit-signal writes), the panel is a read-only admin view. Kill = revert-commit.

**What it does:** surfaces the Wave V import-alerting signals in-app AND closes the two confirmed gaps where the PREVIEW path and route-level 4xx REFUSALS persisted nothing (they only hit the HTTP response + a console marker).
- **Signal writes** — new self-contained `esperto-import/import-activity-signals.ts`; `alert-signals.ts` (the cron + its writers) is **neither imported nor modified** (Codex C2 — "cron unaffected" is literal). A SEPARATE `entityType:"assessment_import_activity"` (actions `preview_result` + `refused`) keeps the rows OUT of the Wave V alert cron's span query (`entityType:"assessment_import"`-scoped) by construction — a regression test locks it (D5). `recordPreviewSignal` fires ONLY on a plan block or ≥1 skip (D3 — clean previews write nothing). A shared **`refuse()`** helper (`restricted-route-helpers.ts`) centralizes "every refusal emits a signal" across 7 pre-commit gates in BOTH mirrored routes (org-access, template-not-published, crosswalk-incompatible, entitlement-denied, too-many-files, file-parse, instrument-mismatch), returning the caller's EXISTING body/status VERBATIM (no response-contract change — 120 unchanged route tests prove it). The org-access refusal records `entityId:"unknown"`, never the untrusted requested org id (Codex C4). Excluded: flag-off 404 (condition-D, log-drain) + genuine system 500s.
- **Read panel** — new `esperto-import/import-health.ts` summarizer + `GET /api/admin/assessments/import-health` + `<ImportHealthPanel/>`, mounted BELOW the existing dashboard on `admin/assessments/observability` (the existing route/component left entirely untouched — the C6 override, for isolation). Shows the alert cron's ACTUAL decisions from its checkpoint rows (per-code firing counts + last-fired — NO wrong-window re-evaluation, Codex C3), cron health disambiguated by the flag (disabled / healthy / ⚠️ >30-min-stale — D8), uncapped 24h/7d totals via `count()` with parsed breakdowns capped + a **per-window** `truncated` flag (Codex C5 + the review's LOW-2 fix: the flag reflects the window actually shown, so a high-7d/low-24h org sees no false "incomplete" warning on its complete 24h view), a REFERENCE latency p95 rendered with NO threshold (condition C is a per-10-min-span verdict, not this rollup), and a recent-signals table.

**Adversarial review (5 lenses, each finding independently verified):** 4 CONFIRMED (0 CRIT/HIGH; 1 MED, 3 LOW) — all fixed in-branch: the MED (no commit-mode Wave Y test) + the LOW truncated-window code bug + two LOW coverage gaps (context/file-parse signal side-effects; admin mirror wiring). **38 new tests** (jest-verified per suite: 6 writers + 12 summarizer/isolation + 4 route + 6 panel + 8 coach-route wiring + 2 admin-route mirror); full sweep **5,563 pass / 5,591** (28 failures in 7 PRE-EXISTING suites, NONE referencing any Wave Y module — quiz/submit, org-survey exchange, dashboard-stats, lva-report-display, portal status-filter, public-quiz-client toLocale lint). `CI=true next build --turbopack` green; eslint clean. Ships EMPTY (panel blank until an import occurs). **Hand-off: merging makes it live (no flag) — the observability page then shows import health + every refused/degraded import as it happens.** Follow-ons (spec §3): composite `(entityType,timestamp)` index if volume grows; log-drain condition-D (vendor, blocked).

### 2026-07-07 — Wave X LAUNCHED: LVA + Rockefeller historical import live on production <!-- ENTRY_ISO:2026-07-07 ENTRY_SLUG:wave-x-launched -->

**LVA + Rockefeller historical Esperto import is LIVE** — `WAVE_X_ESPERTO_LVA_ROCK_IMPORT_ENABLED=1` set on Vercel Production + newest-deployment redeploy (aliased `platformtest.scalingup.com`); the P1 close-out (the last Jeff-blocked roadmap phase). Code PR #154 (dark, squash `1a0ec4bb`) → lock PR #155 (squash `56d20bf2`) → flag flip. Spec `docs/specs/v7.6/19x-wave-x-lva-rockefeller-import.md`; **ADR-0022** (per-instrument import policy registry). /co-validate ×2 (Codex; round-2 "fit to build"); adversarial review 12 findings all fixed.

**Verification + launch walk (agent-driven, 2026-07-07):** the two D4 controlled Esperto verification submissions were taken on the TEST account (RockHabits + LeadVision throwaway campaigns, auto-invite OFF, NO mail) and every binding proven exact — Rockefeller 40/40 = the designed (s+j)%4 pattern + recompute parity; LVA V1 financials positional (the survey spinbuttons exposed the live Q-codes Q1_1/Q1a_2..9), V2 `Q16a="1,10,16"` decoded to recruitment/sales/growth_financing matching the non-empty Q17 why-texts with the S3 matrix factor order confirmed live, V3 provisional tail CONFIRMED (Q31→constructive_discussions, Q32→add_leadership_position, Q34→dept_kpis; Q33 empty→dropped); zero unknown keys. Then a full launch walk drove the real import ENGINE against the prod DB with the sanitized golden data into a throwaway org: both instruments imported as CLOSED scored campaigns (Rockefeller overallTotal 60 / countAchieved 20 = exact known-answer), imported badge set, aliases instrument-segmented (`imported-rockefeller-…`/`imported-lva-…`), and the three rejection guards fired (wrong-shape reject, armed-mat reject, unresolved-roster skip); quarantined §5.5 (org + campaigns gone, verified). Prod smoke green: the import UI now offers all three historical instruments with honest-framing subtitles; SU-Full import unaffected (its own Wave O flag).

**D8 resolved:** the Esperto export `mat` is a per-participant token (LVA and Rock submissions carry different mats), NOT a per-instrument schema id, so the registry `knownMats` stays null (spec 19x D8 case-b) — schema-drift protection is the shape detector + exhaustiveness guard, not the mat gate. **Kill:** unset the Wave X flag (or `_KILL=1`) + redeploy — SU-Full untouched; bad batch = the prefix-aware quarantine script. **Roadmap: P1 is now CLOSED — every Jeff feature phase is live.** Still open with Jeff: LVA peer numbers (the "LVA Peer Data" folder had none) _(later corrected as a phantom ask — see `progress-report-jul8-sent`)_, #2.3 invite copy, #15/#16/#19 wording. Ledgered follow-ons: the Wave O multi-round cid-pin question (cid proved per-campaign). _(Post-launch update: the parked review-loop `restricted-plan.ts` plan-time answer-value-validation edit was COMPLETED in PR #158 `c7ba8cfc` on 2026-07-07 — no longer parked.)_

### 2026-07-07 — Wave X BUILT: LVA + Rockefeller historical import (merged dark, locked:false) <!-- ENTRY_ISO:2026-07-07 ENTRY_SLUG:wave-x-built -->

**The P1 close-out wave, built and merged DARK** — spec `docs/specs/v7.6/19x-wave-x-lva-rockefeller-import.md` (gated pipeline in full: brainstorm → grill-with-docs → spec → /co-validate round 1 (Codex, 6 findings: C1/C3/C4/C6 accepted, C5 partial, C2 overridden — user-locked rollout) → grill-me → /co-validate round 2 (same thread, 4 findings all accepted; verdict "fit to build") → build → adversarial review (subagent SURVIVED; 12 findings, all dispositioned) → PR). **ADR-0022** (per-instrument import policy registry). Unblocked by Jeff's 2026-07-06 sample exports; user chose verification-first sequencing (runs the two controlled Esperto assessments per `19x-esperto-verification-runsheet.md` in parallel; crosswalks lock in-PR only if the exports land clean — else follow-up lock PR, the normal path).

**What shipped (all inert until `WAVE_X_ESPERTO_LVA_ROCK_IMPORT_ENABLED` + per-crosswalk `locked:true`):** X-1 instrument registry `restricted-instruments.ts` — batchKind-keyed (`esperto-lva-restricted-v1` / `esperto-rockhabits-restricted-v1` beside the SU-Full literal; stale-client-safe), data-derived shape detectors (universe = crosswalk map∪dropped, subset per file + distinctive anchor over the BATCH union), D8 `mat` schema-identity gate (knownMats = verified-against only; armed at lock), D9 per-instrument completeness (LVA slider-core-set — the 16-factor matrix IS the instrument; scored keep required-set) INTERSECTED with crosswalk-mapped keys (the PR #116 class killed), org-cid-pin participation (SU-Full only — **new ground truth: Esperto `cid` is per-assessment-campaign, NOT per-company**; same company's samples carry different cids; latent Wave O multi-round pin question LEDGERED) · X-3 MULTI_CHOICE index decode (comma 1-based indices → the CROSSWALK's pinned `optionOrder`, set-equality-validated against the version — template-edit reorders can never remap history; malformed → per-respondent skip, never truncate/guess) · X-4 LVA crosswalk (67 map + 6 dropped = the 73-key xlsx union; provisional V3 tail Q31/Q32/Q34; Q16a↔Q17 consistency probe warn-only) · X-5 Rockefeller identity crosswalk (×40; seed stableKeys ARE Esperto's codes; known-answer test incl. 0-valued answers + countAchieved parity) · X-1b both import routes + helpers generalized (instrument-aware commit: alias `imported-<instrument>-…`, pin scoping; Wave O flag/paths untouched) · X-6 import UI instrument radio + honest framing. **SU-Full byte-identity proven: the entire Wave O suite passes unchanged.** 63 new tests (sweep 5,451 → 5,514 pass, 7 pre-existing suites only); `CI=true npx next build --turbopack` green. Kill: LVA/Rock = zero the Wave X flag; registry refactor = revert-commit (ADR-0022). Launch (later, each step user-authorized): verification exports → lock PRs → throwaway-org walk with synthesized roster (incl. Codex R2-2 rejection evidence: unknown-mat, wrong-shape, unresolved-roster) → flag flip. Same-day earlier: micro-wave PR #152 (sections sortOrder stamp) + review-loop PR #150 merged; Notion task Done.

### 2026-07-06 — Sections sortOrder stamp: editor-added sections are publishable again <!-- ENTRY_ISO:2026-07-06 ENTRY_SLUG:sections-sortorder-fix -->

**Micro-wave (walk-found in the Wave W launch walk, ledgered in spec 19w §launch):** `buildSectionsPayload` (the editor's sections serializer) never emitted `sortOrder` — an editor-added section persisted without one, and the version then failed the publish schema (`sortOrder: expected number, received undefined`) until hand-patched. Root cause was broader than the walk noted: NO editor path wrote `sortOrder` — move-up/down and drag-reorder changed only array order, so a reordered save also silently kept the OLD survey render order (`buildSectionPages` sorts by `sortOrder`). Fix: the dirty path now stamps `sortOrder: idx + 1` positionally on every row — new sections get a number, reorders persist what the editor shows; the NOT-dirty byte-for-byte passthrough (content-hash stability) is untouched. 2 new tests (suite 5 pass, jest-verified; 94 across the six editor suites), lint + `CI=true npx next build --turbopack` green. Read-only prod scan: 1 open draft, none stuck — purely preventive, no data repair needed. Also merged the review-loop's own PR #150 (Wave W walk-script hardening, 5/5 zero-findings).

### 2026-07-06 — Progress report Jul-6 SENT: the consolidated six-wave update (R→W) <!-- ENTRY_ISO:2026-07-06 ENTRY_SLUG:progress-report-jul6-sent -->

**`Scaling-Up-Progress-Update-2026-07-06.html/.pdf` SENT to Jeff 2026-07-06** — Slack (terse bullets) + Loom walkthrough (`loom.com/share/be955b0048f24b3fb532618a862cb285`) + the PDF. The consolidated report covering the SIX launches since the Jul-3 update: **Wave R** (slider tap-to-set #8, full-width answers #4, group-report print #9) · **Wave S** (LVA peer benchmarks #12/#13) · **Wave T** (question editor unlock #10) · **Wave U** (findings logic #11) · **Wave V** (P8 hardening pass) · **Wave W** (conditional/show-if authoring). Built the house way (style verbatim from the Jul-3 report; 18 embedded screenshots — every claim visual; self-contained HTML 2.7MB + 18-page PDF; honest capture labeling: prod-morning vs launch-verification vs cleared-before-launch peer values). One new capture: `wave-t-prod-editor-unlocked.png` (client-side-only scratch question on the prod QSP draft, never saved). Headline: "Six releases in three days — every feature phase of the roadmap is now live." Roadmap stamped ✓ on every phase except Rockefeller+LVA history import (blocked on Jeff's export files); asks re-sent with **real LVA peer numbers promoted to first** (the panel is live). **This report is DONE — never rebuild or re-send it; the next report covers only post-Wave-W work.** Waiting on Jeff: LVA peer numbers, Rockefeller+LVA exports, #2.3 invite copy, #15/#16/#19 wording.

### 2026-07-06 — Wave W LAUNCHED: conditional (show-if) question authoring — the fossil tab becomes real <!-- ENTRY_ISO:2026-07-06 ENTRY_SLUG:wave-w-launched -->

**Conditional/show-if question authoring is LIVE** — PR #148 (squash `207686a`) merged + same-session launch walk; `WAVE_W_CONDITIONAL_AUTHORING_ENABLED=1` set on Vercel Production + NEWEST-deployment redeploy (re-aliased `platformtest.scalingup.com`). Spec `docs/specs/v7.6/19w-wave-w-conditional-authoring-design.md` (decisions D1–D7; /co-validate with Codex: 7 findings, C1/C2/C3/C5/C6 accepted, C4/C7 partial, 0 overridden). No new ADR (extends ADR-0014's report-side stance + the ADR-0020 identity-lock taxonomy + the Wave Q flag rule).

**Ground truth that shaped the wave:** the editor's disabled "Conditional Logic v1.5" ghost tab was a FOSSIL — its copy claimed "renderer-side conditionalSections evaluation ships in v1" but that runtime never existed in this codebase (zero references outside the stub), and its conditional-REPORT-sections concept was superseded by Wave U findings (ADR-0021). The real gap was survey-form show-if — hardcoded LVA-only since Wave I (`form-visibility.ts` + `REPORT_FILTERS.conditionalFollowups`). Wave W generalizes it as authorable per-question rules; the ghost tab is REMOVED (D5; `?tab=conditional` still falls back to Metadata).

**What shipped:**
- **W-1 schema + publish gate (flagless, zero migration):** optional `showIf: {questionKey, optionKey}` on all four question types in the version `questions` JSON (the Wave U `recommendations[]` substrate — content-hash + Duplicate carry automatic). New `checkShowIfIntegrity` in the publish `superRefine` (V-1's home): gate exists · is MULTI_CHOICE · **strictly earlier in canonical render order** (shared `canonicalQuestionOrderIndex` derived FROM `buildSectionPages` — never raw sortOrder, which isn't globally unique; C1) · real optionKey · no chains · carrier never required (D4 — a hidden required question would block every submit). Issues routed under `questions[i].showIf` for the editor modal. Save tier validates shape only — drafts may dangle; publish is the wall.
- **W-2 runtime evaluator (flagless):** `form-visibility.ts` gains the generic pass as a strict pipeline — LVA alias branch FIRST (verbatim, untouched — D3), authored showIf SECOND (intersection; generic can never resurrect an LVA-hidden question — C2). Fail-open on every unpublishable state. Both survey clients got evaluation, progress counts, and client answer-pruning with ZERO client changes (they already call the module).
- **W-3 server prune (flagless, BOTH submit routes):** `pruneHiddenAnswers` via `resolveVisibleSurveyQuestionKeys` drops known-and-hidden answers ONCE before every side effect — scoring, persistence, outbox rows (C3; build-found correction: the draft said "the single submit route" — there are two, org-survey + public quiz). Unknown stableKeys still reach `UNKNOWN_STABLE_KEY`; **generic rules only** — LVA storage behavior byte-identical. Client/server visibility equivalence is a tested property.
- **W-4 pager (flagless, D7):** a section page is suppressed only when EVERY authored question in it carries `showIf` AND all are hidden — attribution **by construction** (adversarial-review hardening: the naive "has questions + 0 visible" rule would also have suppressed sections emptied by hardcoded filters; LVA was safe only because S5 happens to carry 2 always-visible required TEXT questions). Wired into both clients BEFORE the Wave M slide merge (`before-section` anchors degrade via the existing fail-safe drop).
- **W-5 editor panel (flag-gated):** collapsible "Show only when…" per question card (Findings-panel idiom): preceding-MULTI_CHOICE gate dropdown (canonical order; unsaved/conditional gates excluded) → option dropdown → Clear; **two-way Required interlock in the editor** (C6 — required questions get a note instead of controls, conditional questions get a disabled Required toggle); dangling-rule warning; **dependent confirm-drop hygiene** (deleting a referenced gate / removing a referenced option / retyping a referenced gate names the dependents, then clears their rules); editable on inherited questions (reword-class). Serializer: explicit emission with anti-resurrection; half-picked rules never save. Reports/findings/emails needed ZERO code — hidden ⇒ unanswered ⇒ omitted by the Wave E answered-only rule; `resolveFindings` skips unanswered (all test-asserted).

**Launch walk (same-session, prod DB, throwaway `walk-qual-w` template authored via the LIVE panel):** required-interlock note + no-gates hint rendered live during authoring; publish **BLOCKED** on a dependent dragged before its gate with the routed modal message (`questions[0] → showIf → questionKey: … must appear strictly earlier in the survey` — `wave-w-publish-blocked.png`) → fixed → published; survey (safe test member, token minted, NO email): first render **"Section 1 of 1"** with only the gate visible (the conditional Follow-ups page suppressed — D7 live) → ticking People revealed its why-question AND reinstated the page ("Section 1 of 2") → a typed answer under Cash was pruned when Cash was unticked → submit; frozen submission holds EXACTLY the three visible answers; the qualitative report renders answered-only (the cash question appears nowhere — `wave-w-report-answered-only.png`); **tamper spot-check on the live route** (crafted POST smuggling a hidden-question answer) → stored answers exclude it (server prune proven); Duplicate-from-published → v2 draft questions byte-exact incl. all three rules. Artifacts quarantined §5.5 order (`scripts/wave-w-walk-quarantine.ts`; smoke 0/0). Prod smokes post-flip: real LVA respondent report renders unchanged; the panel live on a real prod draft next to the Findings panel (`wave-w-prod-showif-panel.png`; nothing saved); ghost tab gone from the prod tab bar.

**Walk-found (ledgered, PRE-EXISTING, not Wave W):** sections added via the editor's Sections tab persist WITHOUT `sortOrder` → the version fails the publish schema (`sections[1] → sortOrder: expected number, received undefined`) until reordered/patched — the create-form path stamps it, the editor-add path doesn't. Fix = one-line serializer stamp; candidate for the next editor-touching wave.

**Tests: 95 new (jest-verified per-suite):** `scoring.wave-w` 27 · `form-visibility.wave-w` 30 (incl. LVA regressions, client/server equivalence property, route-wiring guard, hardcoded-filter attribution) · `wave-w-flags` 10 · `questions-tab.wave-w` 23 · `question-serialization.wave-w` 5. Full sweep 5,449 pass with only the 7 pre-existing failing suites (identical set to main's ledger, each verified individually); `CI=true npx next build --turbopack` green; eslint clean (incl. two pre-existing ScoringTiersTab warnings removed per gate protocol). Adversarial review: the subagent died on its session usage limit (the recurring 4th-wave pattern) → completed INLINE; its dying lead was real and produced the one build-found fix (the D7 attribution hardening, regression-locked with an LVA-shaped fixture).

**Kill:** panel = zero the flag (published `showIf` rules keep rendering — flags gate capability, never persisted data); engine/publish-gate/prune/pager = revert-commit (correctness hardening). **Ships EMPTY (honest-data): no live template carries `showIf` — Jeff authors rules in the panel, publishes, new campaigns honor them.**

**Hand-off:** Jeff can now make any question conditional on an earlier multiple-choice answer directly in the editor — the LVA-style "only explain what you picked" pattern, on every instrument. Next: P1 imports remain Jeff-blocked (export files); open P8 remainder + Wave U §3 follow-ons + the new sections-sortOrder fix ledgered. Notion tasks (U/V/W) pending connector reauth.

### 2026-07-06 — Wave V LAUNCHED: P8 hardening pass — tier-domain publish gate, in-app import alerting, imported badge, read-path txn budget <!-- ENTRY_ISO:2026-07-06 ENTRY_SLUG:wave-v-launched -->

**The first P8 hardening wave is LIVE** — PR #146 (squash `e8ccd0d`) merged + same-session launch walk; `WAVE_V_IMPORT_ALERTING_ENABLED=1` set on Vercel Production + redeploy (newest deployment `e7zpj1ryu` re-aliased `platformtest.scalingup.com`). Spec `docs/specs/v7.6/19v-wave-v-hardening-pass-design.md` (10 decisions; /co-validate with Codex reshaped ONE user-confirmed decision: **C1 — the stateless alert window became a PERSISTED CURSOR + checkpoint-before-send**, user re-confirmed; C2 partial — cursor semantics adopted, new tables rejected for a-few-events-per-week volume; C3 accepted — preflight scans ALL versions, published included; C4 accepted — signals persist from the marker emission points; C5 accepted — no silent ADMIN_EMAIL fallback for alerts; C6 OVERRIDDEN — the report read transactions are H14 TOCTOU protection, budgeted not removed). No new ADR (hardening within ADR-0006/0017 + the Wave Q flag rule + Wave T non-killable-validation precedent). **Direction context: user chose the P8 hardening pass over conditional/show-if authoring (2026-07-06) — conditionals slot as Wave W.**

**What shipped (4 items):**
- **V-1 — GLOBAL tier-domain publish gate (flagless, kill = revert):** closes the Wave U walk-found gap — a version whose global `scoringConfig.tiers` don't tile its metric domain PUBLISHED fine then 400'd `INVALID_SCORING_CONFIG` on every submit (the check lived only in `scoreSubmission` step 2). `computeGlobalTierDomain` extracted (SLIDER-only filter + rollup/legacy branches) and shared VERBATIM by step 2 and the new `checkGlobalTierTiling` in `TemplateVersionForPublishSchema` — parity property test: publish-pass ⇒ step-2-pass, so nothing scoreable today can be newly blocked; ambiguous domains (mixed scales, zero sliders under rollup) reject at publish too, with issues routed to the publish-failure modal under `scoringConfig → tiers[i]`. **Preflight scan run on prod (read-only, ALL 19 versions incl. published): the ONLY failure is the quarantined Wave U walk template's v1 — the exact version that found the gap; all 18 real versions CLEAN** (script kept: `scripts/wave-v-preflight-tier-scan.ts`).
- **V-2 — in-app import alerting (flag live):** runbook `18o` §7 conditions A (divergent-reimport — always alert), B (denial burst >3/span) and C (commit latency p95 >10s) implemented in-app — previously console-markers-only, observability = human-read `vercel logs`. Both import routes persist each commit marker as an AuditLog signal row (`assessment_import` / `import_commit_result`+`latencyMs` / `import_commit_conflict`+`errorCode`; console output byte-identical; NEW `unexpected-error` code for non-domain commit failures; UNCONDITIONAL writes — flags gate the cron, never the data; fail-soft; conflict rows written route-level AFTER the rolled-back commit transaction). Inngest cron `esperto-import-alert-cron` (`*/10`) evaluates the span since its last checkpoint row (persisted cursor on AuditLog — `assessment_import_alert_cron`/`run`; checkpoint written BEFORE send so late ticks/deploy pauses can't drop a span and Inngest retries can't double-email; first run bootstraps now−10min) and emails ONE consolidated PII-free alert to `ADMIN_EMAIL` (REQUIRED — missing env + a firing → loud `alert_email_unconfigured` marker, no fallback address). Zero schema migration (`AuditLog @@index([timestamp])`). Condition D (flag-drift) stays log-drain-only (documented in the runbook §7 Wave V addendum). Kill = zero the flag; rows persist inert.
- **V-3 — "Imported from Esperto (historical)" badge (flagless):** `campaign.importManifest != null` → boolean `isImported` on the respondent-report loader, group-report PROVENANCE (the page wires provenance → props) and `CampaignOverview` — the manifest payload never reaches a client model (test-asserted). `ImportedBadge` component (report pill on the dark `su-report-cover` + admin pill variant) renders on the scored + qualitative respondent covers, the group-report cover (both variants via the shared `GroupReportCover`), and the CampaignDetail header. Fail-closed: absent/false → no badge; real (non-imported) prod reports verified byte-visually unchanged.
- **V-4 — report read-path transaction budget (flagless):** explicit `{ maxWait: 10_000, timeout: 15_000 }` on the respondent-report (`respondent-report.ts:188`) and group-report (`group-report.ts:317`) interactive transactions — previously Prisma's 5s default, which Neon cold starts / high-latency clients tripped as P2028 on report views (read-path analog of the #117 commit-path fix). The transactions themselves stay — the in-file H14 contract (authorization + fetch in ONE snapshot) is deliberate TOCTOU protection. Group-report keeps RepeatableRead.

**Launch walk (same-session, prod DB via local UI + inline flags):** (1) V-1 E2E — throwaway walk template (`walk-v-scored-test`) authored with non-tiling tiers → **publish BLOCKED** with the routed message ("last tier maxMetric must equal domain max (1)…") → tier fixed to open-ended → published → INVITED submission via tap-to-set **succeeded** with an exact frozen result (the Wave U walk scenario, now failing at the correct gate); (2) V-3 on all three surfaces — badge live on the scored respondent cover, on a walk LVA campaign's group-report cover (group reports are alias-allowlisted to LVA/SU-Full, so the walk used a real-template walk campaign, full survey submitted via tap-to-set), and on the campaign-detail header (admin pill; coach portal view); (3) V-2 sweep run TWICE against the prod DB with the flag inline and **ADMIN_EMAIL overridden to the user's own inbox — never the real admin's**: synthetic `divergent-reimport` signal row → condition A fired → checkpoint written BEFORE send → email dispatched with correct recipient/subject/PII-free content (local SMTP = mock by design — SMTP creds exist only on Vercel Production, verified present; the house transport is proven daily on prod); the walk checkpoints protect the prod cron from ever re-alerting the synthetic rows. **Quarantine in §5.5 order:** both walk campaigns soft-deleted + invitations revoked, walk template soft-deleted (published version rows persist by design — the non-tiling v1 is the preflight scan's one expected finding); post-quarantine smoke 0/0. **Prod smokes:** real LVA respondent + group reports render under the V-4 budget with NO badge; flag flip individually authorized; redeploy took the NEWEST deployment (Wave U gotcha applied).

**Tests: 63 new (jest-verified per-suite):** `scoring.wave-v` 14 · `report-txn-budget.wave-v` 2 · `imported-flag.wave-v` 7 · `imported-badge.wave-v` 9 · `wave-v-flags` 9 · `esperto-import-alerting.wave-v` 22. Two Wave U publish fixtures made globally-scoreable (they had zero sliders under `overallAvg` — the exact unscoreable class V-1 now rejects); all 251 existing esperto-import tests + all scoring suites pass unchanged; full sweep 5,355 pass with only the 7 pre-existing failing suites (identical set to main's ledger); `CI=true npx next build --turbopack` green; eslint clean (incl. removing a pre-existing unused-var warning in CampaignDetail per the gate protocol). **Build: entirely INLINE** — the co-validate subagent died on a session usage limit (4th wave running: S/T/U builds + this one's review); the Codex MCP tool was called DIRECTLY from the main loop instead, so the mandatory review still ran. Adversarial review inline: 2 findings (dead CSS branch; checkpoint-failure error code conflated with send-failure) fixed + regression-tested.

**Kill:** V-2 = zero `WAVE_V_IMPORT_ALERTING_ENABLED` (or set `_KILL=1`) + redeploy — signal rows persist inert. V-1/V-3/V-4 = revert-commit (non-killable correctness/presentation hardening). Runbook: `docs/specs/v7.6/18o-ops-runbook.md` §7 Wave V addendum (in-app A/B/C, checkpoint/signal ops queries, D = log-drain-only).

**Hand-off:** the P8 ledger shrinks — SHIPPED: tier-domain publish gate, §7 alerting (in-app form), imported badge, read-path txn budget. STILL OPEN (P8): external log-drain wiring + condition D (vendor decision — user/ops), org-canary page-gate (YAGNI until a real org canary), alert dashboard panel (email-only for now; `admin/assessments/observability` exists as the natural home), preview-path block/skip signals. Jeff-blocked queue unchanged: P1 imports (export files), #2.3 invite copy, #15/#16/#19 wording, LVA peer numbers. Next natural build: **Wave W — conditional/show-if authoring** (Wave U §3's biggest follow-on; no wireframe exists — drift to surface at scoping).

### 2026-07-06 — Wave U LAUNCHED: findings logic — answer-driven report recommendations live (#11) <!-- ENTRY_ISO:2026-07-06 ENTRY_SLUG:wave-u-launched -->

**Jeff July-1 item #11 ("add logic to present findings from an assessment — drive report content or recommendations based on respondent answers, beyond survey-screen branching") is LIVE** — PR #139 (squash `d50b576`) merged dark + same-session launch walk; launch-found fix PR #140 (`8fd7975`); `WAVE_U_FINDINGS_ENABLED=1` set on Vercel Production + redeploy (aliased `platformtest.scalingup.com`). Spec `docs/specs/v7.6/19u-wave-u-findings-logic-design.md` (21 decisions; /co-validate with Codex reshaped the design: **C1 snapshot-at-scoring ACCEPTED** — replaced the draft's render-time resolution; C2 confirmed `question-meta.ts` strips rules; C3 partial — the hardcoded walk alias became the reserved `walk-qual-*` prefix; C4 preflight scan accepted; C5 field-split OVERRIDDEN with the reader-audit rider). **ADR-0021** written: type-discriminated rule shapes on `recommendations[]` + uniform snapshot-at-scoring resolution.

**Ground truth that shaped the wave:** the findings engine already existed for sliders on scored templates (305 live Esperto-verbatim bands on SU-Full, seed-authored only, frozen per-row at scoring). Qualitative templates (LVA/QSP — Jeff's main instruments) had NO findings path, and the Wave T-unlocked TEXT/NUMBER/MULTI_CHOICE types had no rule shape at all.

**What shipped:**
- **Per-type findings rules on `recommendations[]` (ADR-0021):** the question TYPE discriminates — SLIDER/NUMBER carry bands `{minScore,maxScore,text}` (sliders keep publish-time full tiling; NUMBER bands may leave gaps — unbounded domain), MULTI_CHOICE carries per-option `{optionKey,text}` (each selected option with a rule fires), TEXT carries none (publish rejects). Zero migration; the qualitative Zod arm split into three literal arms; SU-Full's live bands validate unchanged (regression-pinned).
- **Frozen snapshot (D18):** `scoreSubmission` runs the pure `resolveFindings` UNCONDITIONALLY (flags gate capability, never data correctness — Wave Q durable rule) and freezes `result.findings: ResolvedFinding[]` on every submission (PUBLIC + INVITED + Wave O import paths — all through the one engine call). Reports render the snapshot, never re-resolve: issued reports can never drift. BC snapshot re-locked with the delta PROVEN (old SHA reproduced byte-exact minus the new key). Sliders keep the legacy per-row `recommendation` (renderers select — no double display; old submissions render unchanged).
- **Editor authoring (flag-gated UI):** collapsible "Findings" panel per SLIDER/NUMBER/MULTI_CHOICE question card — band rows with an advisory slider coverage hint ("Covers part of 0–3; missing 2–3"; publish enforces), one optional text per MC option (keys required first — "save the draft first" until assigned); EDITABLE on inherited questions (D9 reword-class — the point is adding findings to existing LVA/QSP questions); ANY retype drops rules behind a confirm naming the count (D21); option removal cleans its rule up; serializer emits per-type with ANTI-RESURRECTION (a deleted rule stays deleted; unknown future fields still survive the raw spread).
- **Validation ladder (D10):** save = shape-only (validate-don't-strip preserved); publish = slider tiling unchanged + NUMBER non-overlap + MC optionKey-exists/no-dupes + TEXT-rules rejected + sentinels + 2,000-char text cap (longest live band: 482). Non-killable hardening — kill = revert (Wave T C2 precedent). **D19 prod preflight scan CLEAN** before merge (16 versions / 582 questions / 122 rule-bearing — zero would trip the new checks; script kept at `scripts/wave-u-preflight-scan.ts`).
- **Report rendering (flag-gated):** scored reports merge non-slider snapshot findings into the existing "What to work on next" block (section-grouped; frozen slider path byte-untouched; flag-off byte-identical); qualitative reports (LVA/QSP) gain a consolidated findings section after the last section (all rule kinds incl. sliders, scored-parity copy, suppression-agnostic per D21). **Results emails untouched by construction** — the Wave S byte-identical guard runs green unmodified. Group reports untouched.
- **`walk-qual-*` reserved prefix** in `reportConfigFor` (D14): any alias with that exact prefix renders qualitative, so launch walks can exercise the qualitative report E2E with throwaway TEST templates (fresh alias per wave — soft-deleted templates keep their alias claimed).

**Launch walk (local-UI pilots + inline flag vs prod DB; every prod mutation walk-scoped):** walk (a) scored TEST template — panel live, coverage hint exact, D21 retype-confirm fired live, publish BLOCKED on partial slider tiling with the named error ("Last band must end at scale.max (3); got 1") then passed on full tiling, derived keys + per-type rules persisted exact, survey filled via safe-member token (NO email), **frozen `result.findings` verified in the DB with all 3 rule kinds exact** (rule-less selected option correctly absent), scored report merged all three into "What to work on next" with no slider double-render; **Duplicate-from-published carried all rules byte-exact** (live proof of the D19 risk path). Walk (b) qualitative TEST on `walk-qual-u` — prefix routed to the qualitative renderer, findings section rendered LAST with parity copy + all 3 kinds + section grouping, print button present. **Launch-found fix #140:** the advisory coverage hint printed an inverted range ("missing 7–3") when a band overshot the scale — out-of-scale bands are now named directly (+regression test). Walk detours (not Wave U defects): the walk template's tiers didn't tile its [0,3] metric domain → submit 400 `INVALID_SCORING_CONFIG` (pre-existing publish-vs-runtime gap: the legacy tier-domain check lives only in `scoreSubmission`) — resolved via the product's own duplicate→fix-tiers→publish-v2 flow; a stale-deployment `vercel redeploy` was caught and corrected (always redeploy the NEWEST deployment). Artifacts quarantined in §5.5 order (3 campaigns closed+soft-deleted+tokens revoked FIRST, then both templates soft-deleted+disabled). Prod smokes all-pass: panel live on the LVA editor, existing LVA report unchanged (no findings section), existing SU-Full report unchanged (61 frozen recs exact).

**Tests:** 108 new across 10 suites (flags 18, resolver 16, scoring wave-u 15, reader-audit 2, PATCH wave-u 7, serialization wave-u 13, panel 11, reports 8, section model 12 + walk prefix 6 from the review; +1 in #140 — jest-verified per suite). Full sweep 5,292 pass / 448 suites (7 failing suites pre-existing on main: `db.$transaction` mock drift in the submit tests, stale `GROUP_RENDER_VERSION=lva-fidelity-v1` pin from before Wave S, dashboard-stats/exchange/portal-filter/date-lint — none import a Wave U surface). `CI=true npx next build --turbopack` green. Adversarial whole-branch review completed INLINE (all three TDD subagents died on session usage limits — third wave running; the entire build was completed inline).

**Kill:** zero/`_KILL` the flag + redeploy (panel + both render additions vanish; snapshots + authored rules persist inert; re-flip restores). Schema/validation/snapshot-write: revert commit. **Hand-off: Jeff authors findings in the editor (per question, per type), publishes, and new campaigns render them — live templates ship EMPTY (honest-data, D15).** Follow-ons (spec §3): group-report findings aggregation, findings in the results email, panel test-a-value preview, cross-question rules, input-side conditional authoring.

### 2026-07-05 — Wave T LAUNCHED: question editor type unlock — TEXT/NUMBER/MULTI_CHOICE authoring live (#10) <!-- ENTRY_ISO:2026-07-05 ENTRY_SLUG:wave-t-launched -->

**Jeff July-1 item #10 ("Need the ability to add and edit questions directly on the platform") is LIVE** — PR #135 (squash `81032e1`) merged dark + same-session launch walk; `WAVE_T_QUESTION_EDITOR_ENABLED=1` set on Vercel Production + redeploy (aliased `platformtest.scalingup.com`). Spec `docs/specs/v7.6/19t-wave-t-question-editor-design.md` (10 grilled decisions + /co-validate with Codex: blockers C1 server-side identity enforcement + C2 kill-story split ACCEPTED, C4 partially — inherited slider scale-change moved to the warn class; C3 confirm-key UX + C5 staging-env walk OVERRIDDEN with recorded rationale). **ADR-0020** written: question stableKeys are slug-derived at first save and locked once published.

**Ground truth that shaped the wave:** the admin template editor already existed end-to-end (add/duplicate/delete/reorder, full slider config, draft→publish) — the gap was that TEXT/NUMBER/MULTI_CHOICE were disabled in the UI ("v1.5 deferred") while the engine supported all 4 types everywhere (publish schema, survey render, submit validation, reports — all resolving section membership by `sectionStableKey`). Jeff's instruments are dominated by exactly those types (LVA: 26 TEXT / 11 NUMBER / 2 MC vs 2 sliders). Wave T = UI unlock + identity guardrails, zero engine changes, zero migrations.

**What shipped:**
- **Type unlock (flag-gated UI):** 4-type dropdown on new-to-draft questions; MULTI_CHOICE options editor (option labels + auto slug keys + `maxChoices`); TEXT/NUMBER helper notes (NUMBER has no engine config — bare finite-number input by design); fake TEXTAREA/COMPOUND placeholders removed (flag-on). Flag off ⇒ today's slider-only editor byte-identical.
- **stableKey policy (ADR-0020, D3/D8/D9):** new keys derive from the label at first save (`<section prefix>_<lower_snake>`, 40-char cap, unique against draft ∪ ALL published versions, `_2` suffix on collision), immutable after; inherited questions (key in any published version) have key + type + option keys locked; structure edits warn with named impact (delete → trends/locked-crosswalk/benchmark consequences; inherited-option remove → orphans its `S5_why_` followup; inherited slider scale change → measurement-semantics drift). Reword-class fields stay free.
- **Server-authoritative identity (co-validate C1, unconditional):** the version PATCH validates every row (`QuestionSchema`, key format `^[A-Za-z][A-Za-z0-9_]{0,39}$`, payload dups, MULTI_CHOICE structure) and enforces `KEY_COLLIDES_WITH_PUBLISHED` + `TYPE_LOCKED` against the published union — **validate-don't-strip** (the original payload is persisted byte-identically, so `recommendations[]`/future fields survive).
- **Serialization hardening (non-killable, C2):** pure `question-serialization.ts` replaces the legacy serializer that injected `scale{}` into qualitative rows; not-dirty saves stay byte-for-byte (content-hash contract). Kill for this + the PATCH validation = revert-commit; the flag kills only the UI unlock.
- **Adversarial-review fixes (the review subagent died on its session usage limit — review completed inline, house pattern):** (1) **pre-existing data-loss bug fixed** — raw question/section refs now sync to the persisted payload after save; previously a sections-only or language-only follow-up save passed the STALE page-load rows through and silently deleted just-saved questions; (2) MULTI_CHOICE option keys sync post-save so an option-label rename can never re-derive a persisted key.

**Launch walk (spec §5, TEST-template E2E — live instruments never published):** local-UI pilot vs prod DB with the flag inline → created throwaway template `wave-t-test-delete-me`, added one TEXT + NUMBER + MULTI_CHOICE via the editor → **derived keys exact** (`S1_what_is_your_biggest_win_this_quarter` hits the 40-char cap precisely) → persisted rows clean per-type (no stale `scale`; options `cash/sales/strategy`; `maxChoices: 2`) → published (mixed-type version passed the publish gate) → test campaign minted by script (safe member TestDelete MeSmoke, token hashed, **NO email** — no send event ever emitted; gotchas: INVITED campaigns use `/org-survey/<alias>#t=…`, `/quiz/` is PUBLIC-only; local `ASSESSMENT_SESSION_SECRET` must be ≥32 chars or iron-session rejects the token exchange) → survey rendered all 4 types with the **maxChoices cap enforced live** (3rd checkbox auto-disabled) → submit → report rendered slider score + all three answers. **One launch-found fix shipped en route (PR #136 `f8f0006`, Wave O 3a/3b/3c precedent):** MULTI_CHOICE answers on SCORED templates rendered stored option KEYS in BrandedReport's "Additional responses" (the qualitative path already resolved labels per C-H1) — `displayAnswer()` now resolves via `QuestionMeta.options`, raw-key fallback; verified live post-fix. Cleanup ordered per §5.5 (campaign CLOSED + soft-deleted + token revoked FIRST, then template soft-deleted + disabled; published version rows never hard-deleted — immutability trigger). Prod smokes: editor flag-on markup live + published versions correctly read-only; live LVA report renders byte-normal.

**Tests:** 123 new in the feature PR (serialization 48, flag 13, route 22, unions 5, E2E pipeline 6 — editor payload → publish schema → survey pages → report model, questions-tab 23, tabbed 6 incl. two adversarial-review regression proofs) + 1 in #136 (the earlier "124+3" over-counted: a fixture-only update and 3 assertions ≠ tests); targeted sweep 338/338; `CI=true npx next build --turbopack` green. **Kill = zero the flag + redeploy** (drafts already saved stay valid and publishable).

**Follow-ons (spec 19t §3, not scheduled):** conditional/show-if authoring (feeds Jeff #11), server-side inherited-lock for option keys (rename ≡ remove+add today), NUMBER bounds config (needs engine change), scoring-config wiring for new sections on scored templates.

**Next:** P6 #11 (findings logic) → P1 imports when Jeff's Rockefeller/LVA exports land. Waiting on Jeff: #2.3 invite copy, #15/#16/#19, real LVA peer numbers for the Wave S panel.

### 2026-07-04 — Wave S LAUNCHED: LVA peer benchmarks — admin-set peer averages (#12) + peer comparison in both LVA reports (#13) <!-- ENTRY_ISO:2026-07-04 ENTRY_SLUG:wave-s-launched -->

**Jeff July-1 items #12 + #13 (the P7 LVA-peers workstream) are LIVE** — PR #132 (squash `9220503`) merged dark, launch walk same session, `WAVE_S_PEER_BENCHMARKS_ENABLED=1` set on Vercel Production + redeploy (aliased `platformtest.scalingup.com`). Spec `docs/specs/v7.6/19s-wave-s-lva-peers-design.md` (15 grilled decisions + /co-validate with Codex: C2/C3/C5 accepted — schema trimmed, reconcile-not-replace saves, zero-exposure pilot; C1/C4 overridden with recorded rationale — template-level not version-level, screen-only not email). **ADR-0019** written: per-question peer benchmarks are admin-set DB rows; SU-Full's static file retained untouched.

**What shipped:**
- **`AssessmentBenchmark`** model (QUESTION kind only — no reserved enum values; template-level per ADR-0001 stableKey continuity) + additive migration `20260703230000`. **Ships EMPTY — no seeded values, ever** (the Esperto LVA source has no peers at all; fabricating a seed would violate the ADR-0015 honest-data stance).
- **#12 editor:** "Peer averages" panel on the LVA template editor (admin/STAFF, render-enabled aliases only — `PEER_RENDER_ENABLED_ALIASES`, no dead switches) + `PUT /api/admin/assessment-templates/[id]/benchmarks`: guard ladder (rate-limit → 401 → 403 → flag 404 → template 404 → alias 404), Zod, atomic full-set **reconcile** (upsert/insert/delete-missing in one txn; unchanged rows keep id/timestamps) with before/after deltas in the `BENCHMARKS_RECONCILED` audit. Generic by construction: a future template lights up by adding its alias to the list.
- **#13 group report:** each S3 rating factor with a benchmark gains `Peers N.N` + ▲/▼/● signed-1dp deviation vs the 0–10 `scaledValue`, inline in the existing row (omit-empty per factor; scale-degraded factors excluded). `GROUP_RENDER_VERSION` → `lva-fidelity-v2`; `peerBenchmarks {applied, updatedAt}` in the GROUP_REPORT_VIEW audit when ≥1 factor joined.
- **#13 individual report:** "Organizational Strengths and Weaknesses — compared to peers" section in S3's natural slot (before `S4_obstacles`): own Weak/Average/Strong rating on the shared 0/5/10 axis vs peer average + the same deviation glyphs (shared `peerDevGlyph`/`peerDevText` helpers — the two surfaces cannot drift). Built by a **separate pure builder** — `buildQualitativeModel` untouched, so the respondent results email is byte-identical **by construction**, frozen by a CI source+behavior guard test.
- Flag OFF ⇒ zero benchmark DB reads on every path (spied in tests) + byte-identical reports/pages/panel-absent. **Kill = zero the flag; rows persist.**

**Launch walk (co-validate C5 — zero fabricated-value exposure):** merged dark → deploy Ready 58s (migration applied) → **local-UI pilot against the prod DB with the flag inline** (WAVE_F + WAVE_S; house pattern): saved 4 authorized test values → group report hand-checked all three glyph cases exact (5.0−6.3=▼−1.3 · 5.0−2.5=▲+2.5 · 5.0−5.0=●0.0 · 0.0−9.9=▼−9.9; 12 unset factors untouched) → individual report verified in-slot with exact rows (10.0−6.3=+3.7 etc., only benchmarked factors listed) → 6-page print QA clean (`wave-s-lva-group-report-print.pdf`; evidence shots `wave-s-group-report-peers.png`, `wave-s-individual-peer-section.png` at repo root) → **values CLEARED** (reconcile-to-empty, byte-identity re-verified) → prod flag flip (individually authorized) + redeploy → prod smoke all-pass: panel live (16 blank inputs), group/individual reports byte-identical, QSP untouched. At no moment could an invented number render on a production report.

**Quality:** 118 new tests (lib 53 · API 17 · flags 13 · panel 7 · render 7 · model 7 · loader 6 · page gates 4 · email guard 4); 231 adjacent regression tests green; adversarial whole-branch review **0 CRIT / 0 HIGH** (1 MED — test-only type error — + 3 LOWs fixed in `249c869`); `CI=true npx next build --turbopack` green. Build note: the renders subagent died mid-task on a session usage limit — its finished group-side work was reviewed (one real catch: a Tailwind `text-destructive` class on the Tailwind-less report route, replaced with a `su-report.css` class) and the individual side + all render tests were completed inline.

**Hand-off:** the deliverable is the SET-ability — reports stay peer-free until Jeff/Suzanne enter real numbers in the panel. **Known follow-ons (spec 19s S-7):** SU-Full static-file consolidation into the table; results-email parity; per-question SU-Full tri-bar peers; non-LVA render enablement = one alias-list entry. P2028 note: the group-report read path still trips Prisma's 5s txn budget on high-latency clients (ledgered Wave O follow-on — retry passes).

### 2026-07-03 — Wave R LAUNCHED: participant slider tap-to-set + thicker track (#8), full-width report answers (#4), group-report print (#9) — FLAGLESS <!-- ENTRY_ISO:2026-07-03 ENTRY_SLUG:wave-r-launched -->

**Jeff July-1 items #8 (P4) + #4/#9 (P5) are LIVE** — PR #129 (squash `031124a`) merged as the launch (**flagless — presentation-only, no schema/writes/scoring; kill = revert commit**, Wave C precedent), prod deploy `● Ready` 3s after merge, read-only live smokes all-pass same session. Spec `docs/specs/v7.6/19r-wave-r-slider-printing-design.md` (19 grilled decisions + /co-validate with Codex: 3 accepted — R-4 moved PRE-merge, no email fanout, CSS-string tests dropped; 2 overridden with rationale — SoT stays a post-merge docs PR per house precedent, screen+print stacking was user-approved).

**Code (live):**
- **R-1 / #8 — slider UX:** track 6px→**14px** (radius 7px, all four engine rules; 30px Wave G thumb kept, recentered), the number row became **pointer-only buttons** — `tabIndex={-1}`, `aria-label "Set rating to N"`, equal-slice `flex:1` hit areas (~32px tall; fixed paddings would overflow narrow phones), committing through the same `onChange` path as drag; `aria-hidden` removed from the row; empty-state copy "Tap a number or drag the slider to rate." Range input stays the sole keyboard control (zero added tab stops). Review-loop hook contributed `onMouseDown` preventDefault so a tap can't steal DOM focus from the slider (inspected + tested, kept). Lands on all three participant surfaces via the shared `SLIDER_LIKERT` branch + `.su-assessment-brand` scope. e2e wave-c-slider harness refreshed to the new markup (12/12).
- **R-2 / #4 — full-width answers, scored reports:** `su-report-dl-row` grid `1fr 2fr` → stacked flex column (question row, answer below), screen + print, all row types; print `break-inside` rule upgraded in place; dead responsive grid line removed.
- **R-2b — the REAL live squeeze (preview-pass discovery):** QSP reports are **qualitative** reports; free-text answers rendered inside the statement table squeezed into the 96px rating column (`RatingBlock`'s own doc comment promised Q&A rendering for TEXT but never implemented it). TEXT items now render as full-width `colSpan=2` rows (`su-stmt-text-q` bold question / `su-stmt-text-a` answer below), order preserved; slider/number statements keep the two-column pill shape. Verified live: 9 rows full-width (answer 632px vs the old 113px), print PDF clean.
- **R-3 / #9 — group-report print:** the shared group-report page gained the per-respondent page's `su-report-actions no-print` bar + `PrintReportButton` (top only) on the full-render outcome — ALL report types (LVA/SU-Full/RockHabits/5D via shared dispatch). "Degraded" is a metrics flag on a full render, so it keeps the button (recorded). LVA full print QA: 6-page PDF clean (cover page-break, button absent in output, tables fit).
- **R-4 — Wave L N≠3 tail CLOSED (pre-merge, each mutation individually authorized):** existing safe test member added to "LVA test new" via coach UI → invitation token minted directly (NO email) → survey filled on the branch UI **entirely via tap-to-set** (all 16 values exact — doubled as the live slider check) → N=1→N=2 → group report hand-check **16/16 exact**: Strong+Weak→5.0, Average+Weak→2.5, Weak+Weak→0.0, every row n=2 (a divide-by-3 bug would have shown 3.4/1.7). The N=3 "LVA Spectrum" print pass independently re-confirmed true ceiling values (8.4/6.7/3.4/1.7).

**Verification:** TDD (2 parallel subagents, red→green): 18 new Jest tests, 113+ targeted green; adversarial whole-branch review 0 CRITICAL/0 HIGH (4 LOW: e2e harness + spec contradiction fixed; desktop tick alignment + print section-push folded into QA checklists); `CI=true npx next build --turbopack` green; PR checks green. Prod smokes (read-only): QSP report 9 full-width rows/0 squeezed, LVA group report button + N=2 ceil1 values on the prod build, prod CSS bundle serves `height:14px`. Gotchas recorded: Vercel **Preview** env lacks the Production wave flags (group reports 404 on previews by design — use the local-UI pilot with the flag inline); local pilots need `ASSESSMENT_SESSION_SECRET` (any local-only value) for the invitation-cookie exchange; the group-report read path still trips Prisma's 5s txn budget on high-latency clients (known Wave O follow-on, retry passes).

**Next per triage: P7 LVA peers workstream (#12/#13) → P6 bigger builds (#10/#11) → P1 imports when Jeff's exports land.** Waiting on Jeff: #2.3 invite copy, Rockefeller+LVA exports, #15/#16/#19.
...
**Code (live):**
- **#1 — Results-email template default:** `AssessmentTemplate.sendResultsDefault` seeds the campaign-wizard checkbox (only while the results-email approval hash is valid — approval always wins; flag-off wizard path byte-identical to legacy); the coach's per-campaign flip IS the override (no coach-level preference layer). Admin toggle in the template editor's Results Email card — a template-ROW field, editable regardless of version publish state; independent of the approval hash; audited `TEMPLATE_RESULTS_DEFAULT_CHANGED`.
- **#6 — Disable retired templates:** `AssessmentTemplate.disabledAt` third lifecycle state (≠ soft-delete). Hidden from ALL THREE new-campaign paths — coach + privileged wizard picker branches AND the admin public-campaign create (the third path was an adversarial-review HIGH catch) — plus unconditional 409 `TEMPLATE_DISABLED` at create. Existing campaigns/reports/trends/re-seeds untouched (trends deliberately still lists disabled templates for historical reachability; Esperto import deliberately bypasses — documented). Enable/Disable row action + Disabled badge on the templates list; stale wizard drafts get a forced re-pick state. **`qsp-v1` disabled at launch (Jeff's named example; reversible via Enable; audited).**
- **#7 — Remove departed admins (ADR-0018):** `User.deletedAt` soft removal — hard delete impossible (non-nullable `createdBy` FKs). ADMIN-only endpoint, guard ladder (no self-removal, canonical `ADMIN_EMAIL` protected, ADMIN/STAFF targets, hybrids = whole-account lock), AdminInvite row deleted in the same transaction, audited `ADMIN_USER_REMOVED`. **Enforcement UNCONDITIONAL** — login block (`authorize`), per-request API liveness (free field-check on `getUserForApiRoute`'s existing query), dashboard-layout page liveness. Re-invite **revives the same User row in place** (one identity per email; role transition audited). Settings "Admin Users" card lists real ADMIN/STAFF users with Remove + confirm.
- **Auth-surface audit (spec-mandated):** JWT-only privileged routes converted to `getApiActor()` — `files/[id]` DELETE, `workflows` GET/[id]×3/assign×2, `survey-templates` ×3 (behavior-preserving; only delta = removed users 401). `auth-surface-guard.test.ts` freezes the raw-`getServerSession` allowlist so new bypasses fail CI. Review round also fixed a `canAccessWorkshop` ADMIN-branch liveness bypass (removed hybrid could clone/request-edit workshops).

**Durable rule (co-validate, Codex + user-approved flip of the earlier grill call):** *flags gate capabilities and writes, never the enforcement of persisted admin intent* — a KILL stops further disables/removals but never un-retires a template or un-fires an offboarding. Kill = zero the flag + redeploy. Accepted+documented residual: dashboard PAGE soft-navigations in an already-open tab (App Router layouts don't re-render on client nav) — read-only until any hard load; all mutations + API reads revoke immediately.

**Launch verification:** templates list + settings card live-verified via Playwright as prod admin; qsp-v1 disable → badge/Enable + absent from picker payload (6 templates, each carrying `sendResultsDefault`); default toggle ON (PATCH 200) → OFF net-zero; **full remove-admin drill on a throwaway invited account** — invite → accept (real inbox) → Remove (deletedAt + invite-row deletion + audit verified in DB) → **live revocation proven by differential JWT probe: same-shaped session token → live admin 200, removed admin 307 /login**. Drill end state = clean tombstone, email re-invitable. (Drill detour: user-side login mixups — wrong email, then password-manager password — machinery itself worked first try.)

**Process:** brainstorm → /grill-with-docs (3 code-grounded findings incl. the revive-on-accept dead-end) → /grill-me → spec 19q + ADR-0018 → /co-validate (Codex review: 4 accepted incl. the durable-rule flip) → scaffold + 3 parallel TDD subagents (A server routes / B client UI / C admin lifecycle) + JWT-gap closure → adversarial whole-branch review (1 HIGH + 2 MEDIUM + 3 LOW, all resolved). ~150 new tests across 18 suites; full suite 4,923 pass (6 failing suites pre-existing on main — zero runtime import intersection, incl. known `dashboard-stats` + date-rot; ledgered for cleanup); `CI=true npx next build --turbopack` clean; additive-only migration (safety gate ✓).

**Open tails:** pre-existing failing test suites on main (date-rot cleanup ticket); profileImage write-side URL validation (carried from Wave P); WF11 full users directory deferred (spec-recorded); coach-session offboarding hardening out of scope (ADR-0018 notes `User.deletedAt` is reusable for it).

### 2026-07-02 — Wave P LAUNCHED: Jeff July-1 quick-fix batch live on production <!-- ENTRY_ISO:2026-07-02 ENTRY_SLUG:wave-p-launched -->

**All six grilled Wave P items are live** (PR #123 squash `2d4210d`, merged dark; launch walk same-session with each prod mutation individually authorized). Plan: PLAN.md (claudex loop 3 rounds / 19 findings, all folded); whole-branch adversarial review **MERGE-READY, 0 confirmed defects** — flag-off byte-identity proven over a 226-case old-vs-new output matrix.

**Template content (new published versions — published versions are DB-trigger immutable, so the path was guarded re-seed → DRAFT v3 → verify → publish-by-recorded-id with contentHash re-read before/after):**
- **QSP v2 → v3 published 16:20:01Z** (`cmr3pk8bv0003r9x59njo6yy2`): the three `P1_core_values_story_*` labels differentiated with "(Story N of 3)" suffixes (Jeff #3 asked for deletion — refused: the 3 slots are Esperto-faithful, carry the locked Q5a/b/c crosswalk + imported answers; deviation already communicated in the Jul-2 report). Also bakes in the previously render-time-only `P1_overall_rating` "(with 1 decimal)" suffix removal (pre-existing seed drift, `stripLegacyDecimalSuffix` covers old versions).
- **LVA → v3 published 16:20:09Z** (`cmr3pkoxp0003tmlcfeuwtjx1`): "Leadership Team" → "Leadership team" in BOTH factor arrays (Jeff #14; S5 "Why is …" headings follow by derivation; "The leadership"/"The Leadership"/"Growth Financing" deliberately untouched — #15/#16 pending Jeff); `S6_core_values` reworded to Jeff's copy "What are your core values? If not yet set, what do you think the top 3 should be?" (Jeff #17; stableKey KEPT — ADR-0001 "semantically identical" ruling recorded).
- Existing campaigns keep their pinned v1/v2 (verified live); new campaigns pin v3 (verified live). No stableKey/type/isRequired changes → locked QSP import crosswalk, trends, ADR-0016 longitudinal all unaffected.

**Jeff #18 ("possible duplicate") = NOT a duplicate, verified against the Esperto source workbook:** all three priority questions (org / this-year / this-quarter) exist verbatim as a deliberate cascade. No change; finding goes to Jeff.

**Code (live):**
- **Coach-email name fallback (Jeff #5):** new `lib/assessments/respondent-display-name.ts` (`respondentDisplayName` trims BEFORE truthiness — the OrgRespondent path produced a truthy `" "`; falls back to respondent email, never a generic) + `greetingName` guard ("there" for blank OR @-containing). Coach surfaces (subject/lead-in/cover/conclusion) show the email; respondent greetings never do — including on-screen `BrandedReport`/`QualitativeReport`, which both needed the guard.
- **Invite-email chrome (Jeff #2.1 + #2.4), flag `WAVE_P_INVITE_EMAIL_ENABLED=1` live:** coach logo in the branded header (HTTPS-only `safeImageSrc` gate — profileImage is free-text on two write paths; escaped src+alt, height-clamped; identity = `creatorCoach ?? organization.owner` mirroring `resolveCoachName`, NO image-level fall-through, rejected URLs nulled at the resolver) + larger "Start the assessment" CTA (18px 40px / 17px). Wired through all 4 send paths (invite/resend/reminders/Inngest fan-out) with PII-free telemetry `chromeVariant`/`logoIncluded`/`logoRejectedReason` (no-coach|no-image|invalid-url). Flag OFF = byte-identical email (inline-snapshot anchored). Full custom-HTML invitations excluded by design.
- **Coach name in body (Jeff #2.2):** QSP template row `invitationBodyMarkdown` was BLANK (sends used the Wave G code default — the "current copy" Jeff quoted); set via the admin PATCH API to the default copy + "invited **by {{coachName}}**", verbatim-verified re-read, BEFORE the re-seed so v3's contentHash covers the final body. Live for all future QSP sends; per-campaign overrides untouched. Jeff's revised copy (#2.3) drops in later keeping the token.

**Launch preflight/verification:** zero lingering drafts asserted; DB host fingerprint; profileImage host audit (2 set, both Vercel Blob, 0 non-https — tracking-pixel risk gate trivially green); live smoke = new campaign pins v3 + old pins v2 + real invite sent to a controlled inbox with prod logs showing `chromeVariant:'waveP', logoIncluded:true`. Blank-name completion email covered by tests (no live blank-name respondent exercised). **Kill:** email chrome = zero the flag + redeploy; bad published version = publish corrected v4 (campaigns created in a bad-version window must be enumerated by pinned versionId — none here); body copy = re-edit the row (snapshot = blank).

**Tests:** 3 parallel TDD subagents; jest 2023/2024 across touched domains (the 1 failure is pre-existing on main — `dashboard-stats` stale expectation vs a `deletedAt:null` filter, file identical to origin/main); ESLint clean; `CI=true npx next build --turbopack` exit 0. **Out of scope / open:** Jeff #2.3 invite copy (TBD), #15/#16/#19 clarifications, profileImage write-side URL validation (follow-on), smoke campaign `cmr3q0jm8000c2mvw4zlr8s1t` ("Wave P smoke — safe to delete", Test org) left for inbox verification then deletable.

### 2026-07-02 — Progress report pushed out + Jeff's July-1 19-item feedback triaged <!-- ENTRY_ISO:2026-07-02 ENTRY_SLUG:progress-report-jul2-jeff-items-triaged -->

**`Scaling-Up-Progress-Update-2026-07-02.html/.pdf` delivered (repo root, self-contained)** — covers the Wave O launch arc (before/after live import screens, the Esperto decode story, the known-answer proof table, the 5-stage launch drill + its 3 same-day catches) and a roadmap that **fully triages the 19-item feedback PDF `From Jeff/gabriel-items-2026-07-01.pdf`**: quick fixes (QSP "duplicate" = investigated as Esperto's three same-label story slots → label differentiation, not deletion; coach-email "a responder" = missing-name fallback → roster-name/email fallback fix; LVA capitalization; core-values reword — copy provided; one possible-dup pending source check) · admin/coach controls (template-level results-email default + coach override; disable retired templates; delete admins) · survey UX (thicker slider track + click-a-number-to-set; QSP invite email logo/coach-name/copy/bigger-button — mocked with annotations in the report) · reports & printing (full-width free-text layout; LVA group-report print) · bigger builds (question editor; findings logic) · LVA peers workstream (#12/#13 folded with the locked Wave L plan; SU-Full Peers is the model). **Waiting on Jeff/Suzanne:** Rockefeller+LVA export files; new QSP invite copy; clarifications on "The leadership", "Growth financing", and the media question. **Correction recorded while building the report: Wave J (SU-Full group report + Peers) has been LIVE since 2026-06-30** — v3 published + `WAVE_J_SUFULL_GROUP_ENABLED=1`, verified on a real campaign; stale "awaiting sign-off" notes superseded (this is also why the Wave O preflight found v3 published). Do not re-triage the 19 items or re-deliver this report.

### 2026-07-02 — Wave O LAUNCHED: historical Esperto SU-Full import live on production <!-- ENTRY_ISO:2026-07-02 ENTRY_SLUG:wave-o-launched -->

**Wave O Phase 3 — the canary launch — completed same-session: `WAVE_O_ESPERTO_SUFULL_IMPORT_ENABLED=1` is live on Vercel Production and the SU-Full historical import is available to every coach + admin.** Launch plan: `docs/specs/v7.6/18o-phase3-canary-launch-plan.md` (grilled 2 rounds; decisions D1–D8 recorded inline). Ramp: prep → local pilot → prod-org canary → global, each stage gated on verification evidence.

**Pre-launch verification (Stage 0).** One controlled Esperto submission with **distinct per-row values** (section row *j* → value *j*) closed the lock-checklist's last assumption: **within-block row order = ascending, CONFIRMED for all 10 families** (even a conditional-reveal slider landed on its display sub-index), and re-confirmed every family→section binding + the FTE codes. The same submission's export became the pilot's known-answer import. `WAVE_O_ESPERTO_IMPORT_HASH_SALT` generated + set on Vercel Production AND the local `.env` (an undocumented launch prerequisite — the resolver hard-fails in prod when unset, and salt parity is what makes local-written provenance hashes prod-consistent).

**Preflight finding → Phase 3a (PR #116 `c3c1fc1`).** Prod's published SU-Full version turned out to be the FTE-bearing **v3** (published 2026-06-30), not the slider-only v1 the Phase-2 crosswalk was authored against. v3's `Q_FTE_CONTRACT` is REQUIRED and the completeness gate derives its required set from `isRequired` flags → with FTE dropped, **every import would have been skipped as incomplete (Wave O bricked as shipped)**; and `answerHash` covers *mapped* answers, so a post-hoc remap would have reclassified same-file re-imports as divergent. Fixed before any real import existed: `Q1o2_2 → Q_FTE_CONTRACT`, `Q1o2_3 → Q_FREELANCE` (NUMBER; bindings from the Phase-2 controlled decode).

**Stage 1 pilot → Phase 3b (PR #117 `f0f8c6c`).** Pilot ran the real import UI from a local dev server (flag on locally) against the prod DB: fresh "Wave O Pilot" org (`cmr36wh0u0002hm2fjcwv7974`), roster import, then the known-answer round `pilot-2026-07`. Two findings en route: (1) the Stage-0 export was the report-row envelope, not the restricted-export shape — reshaped + validated through the repo's own `parseEspertoExport` (classification had refused the wrong shape cleanly — the anti-corruption layer working); (2) the commit transaction could not complete cross-continent: Prisma's **default 5s interactive-transaction timeout** expired mid-commit repeatedly (atomic rollback every time, zero partial state) → Phase 3b passes `{maxWait:10s, timeout:55s}` at the single adapter call site (also hardens the documented 300-file batch cap). Pilot then committed (`created`, 7.5s) and the formal gate passed: **all 63 stored answers exactly matched the planted scheme** (61 sliders + FTE 20/2), campaign CLOSED, manifest populated, org cid pinned, and the rendered report's every section/domain number equaled the known arithmetic (231 pts / 3.79 avg / overall 37).

**Stage 2 canary (prod runtime, `_CANARY=<pilot-org>`).** Re-commit of the same round → **`reused-noop`** (idempotency + salt parity proven on serverless); one-changed-answer commit → **409 `divergent-reimport`** (changedCount 1); non-canary org → **dark 404** ("Organization not found", flag gate first, no DB touched); SU-Full UI confirmed still hidden platform-wide; `assessment.esperto_import.*` markers confirmed visible in `vercel logs` (observability posture per D4: human-read logs + kill switch; alert wiring is a logged follow-on).

**Stage 3 global flip + smoke → Phase 3c (PR #119 `260f0b0`).** `_ENABLED=1` set, `_CANARY` removed, redeployed. Smoke: SU-Full option visible on the admin AND coach import pages; pilot report rendering on live prod; roster/QSP paths unaffected. The smoke caught stale honest-framing copy ("Supported today…" denying SU-Full on the same page that offered it) → fixed same-session, conditioned on the flag (#119 also carried a review-loop comment correction to the tx-budget rationale; redundant PR #118 closed). Copy verified live post-deploy.

**Stage 4 cleanup.** The pilot round was quarantined via `scripts/wave-o-quarantine-import.ts` — the first real-prod rehearsal of the rollback tooling: dry-run (redacted, zero writes) → `--confirm` (soft-delete + `::quarantined::` externalId rename) → **§5b post-rollback smoke all-pass** (original externalId gone, org `espertoSuFullCid` pin untouched, org campaign list clean). Pilot org + roster kept for future smokes. Esperto-side test data had already been deleted (account verified back to original state; no emails sent at any point).

**Kill/rollback:** `WAVE_O_ESPERTO_SUFULL_IMPORT_KILL=1` + redeploy stops all new imports; a bad batch is removed surgically with the (now rehearsed) quarantine script. Runbook `docs/specs/v7.6/18o-ops-runbook.md`.

**Known nuances + follow-ons (also in CLAUDE.md Open follow-ons):** admin-lane imports carry `createdByCoachId: null` and don't appear in the owning coach's campaign list (support-backfill semantics; coach self-serve imports appear normally); no "Imported from Esperto (historical)" badge on the report (the campaign name carries "imported" — cosmetic follow-on); read-path report transaction still on Prisma's 5s default (fine same-region; align with #117 eventually); import pages' flag check is global-only, so a future org-scoped canary hides the UI for the canaried org (page gate follow-on); wire the §7 log-drain alert queries.

### 2026-07-02 — Wave O: SU-Full crosswalk verified + merged DARK — Phase 2 <!-- ENTRY_ISO:2026-07-02 ENTRY_SLUG:wave-o-esperto-sufull-crosswalk-phase-2 -->

**Wave O Phase 2 merged to production DARK — the verified SU-Full crosswalk.** Phase 1 shipped the safe plumbing + anti-corruption layer behind a default-OFF flag AND a `locked:false` crosswalk (import refused). Phase 2 authors the real crosswalk `crosswalks/scaling-up-full.ts` — all 61 Esperto sliders `Q3_1…Q12_10 → Q01…Q61` — and flips `locked:true`. **It stays DARK: the flag `WAVE_O_ESPERTO_SUFULL_IMPORT` is still off and gates the coach/admin import route _before_ the crosswalk is resolved, so `locked:true` changes nothing reachable.**

**How the mapping was verified (no guessing, no Jeff).** The grill established that static source could not disambiguate the mapping: the restricted export is bare `code→value` with no question text, and Esperto's answer-code order is a **proven permutation** of survey display order (code-order family sizes `4,7,5,6,5,8,5,6,5,10` vs survey `8,5,7,4,5,5,6,5,6,10`), so position-binding is invalid. Four families were pinnable by unique item-count (Q3→Leadership Team, Q4→Strategy, Q8→Your Employees, Q12→Your Leadership), but the six **count-tied** families (four 5-item, two 6-item) had no discriminator in any file we held (seed, survey-UI screenshots, sanitized fixtures, the raw export `.xlsx`+aggregate, product workbooks all checked; Firestore read was auth-gated). Resolved by driving the **live Esperto** account (login provided by the user): created a ScaleUp2 test campaign, filled each of the 10 slider sections with a **distinct constant** (display section N → value N), submitted, and read the saved answers back by code — the family returning all-N is display section N. Cross-checks passed (Q3=all-4, Q4=all-3, Q8=all-1, Q12=all-10). The controlled decode caught two count-tied bindings a naive position guess would have gotten wrong: **`Q6`→Internal Communication and `Q10`→Scalability** (the survey shows Internal Communication before "Making a statement"/Your Leadership, the reverse of our seed's stableKey order — so the bind is by content, not position). Full verified table: `Q8→Q01–Q08`, `Q7→Q09–Q13`, `Q4→Q14–Q20`, `Q3→Q21–Q24`, `Q5→Q25–Q29`, `Q9→Q30–Q34`, `Q10→Q35–Q40`, `Q11→Q41–Q45`, `Q12→Q46–Q55`, `Q6→Q56–Q61`; within-block row order ascending (QSP-confirmed Esperto convention; affects only per-statement display + Wave-N same-question deltas, never domain scores). Test campaign deleted; the real April campaign + client member were verified untouched.

**droppedKeys (29 non-slider raw keys).** FTE (corrected by the live decode — the earlier static guess of `Q13o*` was wrong): **`Q1o2_2` = permanent/temp-contract FTE, `Q1o2_3` = freelance FTE** → dropped against the published v1 (which has no FTE stableKeys; they live in the Wave J draft — move them to `map` → `Q_FTE_CONTRACT`/`Q_FREELANCE` when an FTE-bearing version publishes, and the growth-phase tile lights up with no other Wave O change). Also dropped: `Q1o1_1`/`Q1o4` (background), all `Q2o*` (revenue/growth), all `Q13o*` (general-level firmographics), `ScoreSchatting` (self-estimate), `Q16`/`Q17`, `Q12open` (free-text, PII), and demographics (`geslacht`/`leeftijd`/`country`/`state`/`provincie`/`postcode`).

**Verification.** TDD: `crosswalk.test.ts` +6 (exact 61 mappings; Q01–Q61 unique; the 29 dropped keys; FTE-in-dropped; **exhaustiveness against the real restricted-individual fixture's 90 raw keys**; pinned-version type/scale compat). Full esperto-import suite **222/222 green**; `CI=true npx next build --turbopack` green; ESLint clean. The lock-checklist JSDoc in the crosswalk cites the controlled-decode evidence (method + cross-checks + ascending-order note); PR review is the gate. Plan: `docs/specs/v7.6/18o-phase2-crosswalk-plan.md`. **Phase 3 (canary flag-flip: pilot → allowlist → global) is separate + gated on explicit authorization; Rockefeller/LVA remain parked.**

### 2026-07-01 — Wave O: historical Esperto SU-Full import — Phase 1 (merged DARK) <!-- ENTRY_ISO:2026-07-01 ENTRY_SLUG:wave-o-esperto-sufull-import-phase-1 -->

**Wave O Phase 1 merged to production DARK via PR #113 (squash `94a5a87`) — additive, behind a default-OFF flag AND a `locked:false` crosswalk, so merging + deploying changed nothing reachable.** Lets a coach bring a company's historical **Scaling Up Full** results out of Esperto into the platform (rendering as normal per-respondent reports + feeding the Wave N longitudinal "Year 1 vs Year 2" comparison) — the marquee gap for actually retiring Esperto. Phase 1 is the safe plumbing + anti-corruption layer; Phase 2 (the verified positional crosswalk + lock-checklist) is separate and gated.

**Design (ADR-0017: coach-operated, recompute-not-store).** Import raw Esperto answers → our own `scoreSubmission` recomputes; Esperto's pre-computed `processed`/aggregate blocks are dropped. Imported campaigns are CLOSED + never email (ADR-0006). Supersedes the old glossary "admin-operated" note.

**Two independent dark gates.** `WAVE_O_ESPERTO_SUFULL_IMPORT` (default-OFF, 3-lever KILL/ENABLED/CANARY, `wave-o-flags.ts`, mirrors Wave N) AND the `scaling-up-full` crosswalk ships `locked:false` (import refused with a `crosswalk-locked` block). Either gate alone keeps it unreachable — the flag flips only after Phase 2 verifies the crosswalk.

**Anti-corruption layer** (the correctness-critical core — a wrong crosswalk silently attaches answers to the wrong questions):
- `restricted-plan.ts` (pure) — seven ordered refusal gates: invalid round label / crosswalk-locked / crosswalk-invalid-for-version / empty-batch / **invalid-file-fields** (strict cid/mid/reportid token + ISO/sane-date validation, R2-M4) / **multiple-cids** (one company per batch) / **duplicate-respondent** (same `(cid,mid)` twice = two rounds mixed → reject). Then per-respondent: unresolved `mid` → skip (never create/fail); missing a scorable → **completeness-gate** skip (no partial-scored row). Round identity = a **coach-supplied round label** (the export has NO round/wave field), `externalId = esperto:sufull:<cid>:<slug(label)>`. Manifest carries only **salted hashes** (`saltedMidHash`/`saltedReportIdHash`/`answerHash`) + counts — no raw PII.
- `restricted-commit.ts` (transactional writer) — **entitlement parity** with normal campaign-create (`canCreateCampaign`: cert + access-group), re-checked **inside** the tx (TOCTOU); **explicit target-org** selection with org-level `cid` provenance pin + mismatch refusal (never infer org from file contents, R2-M3); **low-resolution-batch** refusal (most mids unresolved = wrong-org signal) unless acked; **exact/superset/divergent reuse classification** — a changed or missing respondent on re-import → 409 `divergent-reimport` (never a silent overwrite), a strict superset → audited append, identical → true no-op; **append binds to the campaign's already-pinned version** (never latest-published, R3-H2); **P2002 race → reuse** not 500; per-round **`pg_advisory_xact_lock`** serialization (R3-M1); DB-level `openAt/closeAt` recompute over persisted submissions; redacted in-tx audit manifest.

**Routes + UI.** Both import routes (`/api/assessments/import` + admin) gain a versioned `restrictedResults` batch payload (`batchKind: "esperto-sufull-restricted-v1"`, `roundLabel`, `targetOrgId`, `files[]`, optional `aggregateFiles[]`, `expectedVersionId`, `ackLowResolution`); the existing roster/QSP `results` paths are byte-for-byte unchanged (R3-M4). `maxDuration = 60`, batch cap 300 files, PII-safe `assessment.esperto_import.*` observability markers. `EspertoImportClient.tsx` gains the SU-Full kind (round label / target-org select / multi-file / aggregate / low-res-ack retry), gated on a server-computed `suFullImportEnabled` prop (global-flag-only for Phase 1; per-org canary visibility is a documented follow-on). Honest-framing fix: both import page headlines de-overclaimed to "Import from Esperto" / "Bring in your Esperto history" + a "supported today: Members rosters + QSP-v2 results" line.

**Schema (additive, zero-backfill).** Migration `20260701000000_add_wave_o_sufull_import_provenance`: `AssessmentCampaign.importManifest JSONB?` (round manifest) + `Organization.espertoSuFullCid TEXT?` (first-import cid pin). Verified applied to prod post-deploy (`prisma migrate status` → up to date).

**Ops.** By-batch quarantine/purge script `scripts/wave-o-quarantine-import.ts` (soft-delete + externalId-rename, since the partial-unique index isn't `deletedAt`-scoped; never touches the org cid pin) + Wave O runbook `docs/specs/v7.6/18o-ops-runbook.md` (gates, flags, canary launch order, kill vs. quarantine, observability alert queries, on-call tree).

**Three gaps found + closed mid-build:** (1) the `scaling-up-full` crosswalk stub didn't exist in the registry (a real request would 500, not cleanly refuse); (2) the commit-layer campaign lookup didn't exclude soft-deleted rows (a quarantined campaign could be silently reused — fixed to the `liveCampaignWhere`/`findFirst` convention); (3) **no Prisma migration existed** — only `prisma generate` had run, which would have crashed on deploy (hand-authored the additive migration).

**Process + verification.** `/claudex:plan` 3 adversarial rounds (26 findings — R1 design / R2 security / R3 SRE — all accepted) BEFORE any code; then subagent-driven TDD. `CI=true npx next build --turbopack` green; full Wave O suite green; repo suite stable at the same pre-existing/DB-dependent failures across 5+ runs (one genuinely flaky new UI test root-caused to fragile mock-ordering + a timing-dependent helper and fixed properly). GitHub CI + Migration Safety Gate + Vercel prod deploy all green on merge; migration confirmed applied to prod DB. **Not yet launched** — flipping the flag + locking the crosswalk is Phase 2, gated. **Phase 2** (verified `Q3_1…Q12_10 → Q01…Q61` positional crosswalk + FTE map + PR-reviewed lock-checklist, then canary flag-flip) and **Rockefeller/LVA import** (parked on their real exports) remain follow-ons. Specs: `PLAN.md`, `docs/specs/v7.6/18o-ops-runbook.md`, ADR-0017; CONTEXT.md "Historical import" term corrected to coach-operated.
...
**Wave M — Custom slides (#19, coach-authored, per-campaign).** A coach adds branded sanitized-HTML interstitial slides to a campaign; participants see them as NON-counted pages woven into the survey.
- Additive nullable `AssessmentCampaign.customSlides JSONB` (migration `20260630000000_add_campaign_custom_slides`; passed `check-migration-safety`).
- `wave-m-flags.ts` (`WAVE_M_CUSTOM_SLIDES_ENABLED`/`_CANARY`/`_KILL`, kill-overrides, campaign-id canary). `custom-slides.ts` (CustomSlide/SafeSlide/PagerPage types + Zod caps ≤10 slides / ≤20 KB / ≤200-char title + pure `mergeCustomSlides`: start / before-section:`<stableKey>` / end-after-Other, `sortOrder`, drop+surface unknown anchor, skip empty).
- **`slide-sanitizer.ts` — a STRICTER profile, NOT a reuse of the admin `sanitizeCustomHtml`** (which self-documents at line ~38 as admin-trust and says "revisit if coach-writable"). Drops `<style>`/`<iframe>`/`data:image/svg+xml`/`srcset`/protocol-relative + all inline styles; keeps `<img>`(https/data) + links(https/mailto/tel) + formatting; forces `referrerpolicy="no-referrer"` on slide imgs; returns strip-warnings.
- Render path: `load-safe-slides.ts` server chokepoint (validate → `sanitizeSlideHtml` → `SafeSlide[]`, flag-gated into `/org-survey/[alias]/me` + `/quiz` payloads; never imported into client bundles). `section-pager.tsx` renders slides as **no-counter interstitials** matching the existing SU-Full phase-tile convention (the "Section N of M" counter + progress count ONLY `kind:"section"` pages; exhaustive `kind` guards on the phase-tile trigger, answered-counts, and the required-answer gate; trailing `end` slide forward button = Submit; Back at index 0 → `onExit`).
- API: create (flag-gated, `expectedVersionId` + anchor-validation against the pinned version, atomic campaign+participants+audit `db.$transaction`, immediate-send emitted post-commit) + PATCH (required `expectedCustomSlides` CAS → 409 on stale, `CLOSED` 409 / `DRAFT`+`ACTIVE` 200, `canManageCampaign` write, sanitize-on-save, audit-in-tx). New read-only `canAccessTemplate`-gated `/api/assessment-templates/[id]/version-sections` endpoint feeds the wizard's before-section picker.
- Authoring: `CustomSlidesPanel.tsx` (sandboxed-iframe live preview + strip-warnings, ≤10 cap) mounted as an optional wizard step + the CampaignDetail editor (DRAFT+ACTIVE); the flag is plumbed server→client as a boolean (never imported into a client component).

**Wave N — Per-respondent longitudinal comparison (#23, coach-facing, scored-only).** One person's scored results across the campaigns they completed — overall trend, per-section/domain deltas, tier movement. Includes Esperto-imported history (Jeff's "Year 1 vs Year 2"). NO migration (reads frozen `AssessmentSubmission.result`).
- `wave-n-flags.ts` (`WAVE_N_RESPONDENT_LONGITUDINAL_ENABLED`/`_CANARY`/`_KILL`, org/template canary). `respondent-longitudinal.ts` loader: dual authz (`canAccessOrganization` AND `canAccessTemplate`, before any load), entry org-bind (id+org+deletedAt) → 404, scored-only scope gate (`reportConfigFor(alias).reportType`, ADR-0016), **match by `normalizedEmail`-within-org** (fallback `respondentId` — links Esperto-external + platform-email rows for the same person), include Esperto-imported (CLOSED/back-dated) submissions, one-point-per-campaign collapse, deterministic tie-break (`submittedAt`/`openAt`/`campaignId`/`submissionId`), capacity bounds (12 cols / 50 resp / 200 subs), **same-`versionId`-only deltas** (cross-version shows value with no delta; degraded points skipped) via new `partitionPointsByVersion` (the preserve-all inverse of `trends.ts`), no raw PII in output.
- Portal route `/portal/assessments/respondents/[respondentId]/longitudinal` → `RespondentLongitudinalView` (inline-SVG overall trend + per-section/domain delta table + tier-movement row only where a tier exists + "different version" badges on genuine cross-version columns + empty/notApplicable/degraded/"need ≥2" states) + fail-safe `RESPONDENT_LONGITUDINAL_VIEW` audit (no emails/names) + PII-stripping `assessment.respondent_longitudinal.*` metrics + `no-store`/`no-referrer`; `forbidden` → `notFound()` with `unstable_rethrow` so 404s aren't swallowed.
- Entry links on the per-respondent report + CampaignDetail respondent rows (`prefetch={false}`, shown only when ≥2 comparable scored submissions, via `longitudinal-eligibility.ts`).

**Process + verification.** brainstorm → `/grill-with-docs` (Q1–Q7) → `/grill-me` (GM1–GM7 — caught the admin-trust sanitizer reuse, the Esperto-link availability, and the `normalizedEmail`-match correction) → `/frontend-design` mockup (signed off) → `/claudex:plan` (3 rounds, 22 findings, all folded into the impl plan) → subagent-driven TDD → **adversarial whole-branch review** (4 worry lenses → adversarial verify): **2 confirmed defects found + fixed** — (1) `partitionPointsByVersion` didn't skip degraded points (a malformed submission fabricated a delta AND poisoned the same-version baseline); (2) the "different version" badge fired on `deltaComparable===false`, which is also true for the baseline column — plus regression tests; refuted findings dropped after verification. `CI=true npx next build --turbopack` green at every integration step; 436 (Wave M) + 152 (Wave N) targeted tests green, zero new failures. ADR-0016 (longitudinal scored-only + same-version deltas); specs `docs/specs/v7.6/18m-wave-m-custom-slides-design.md`, `18n-...`, `18mn-...-implementation-plan.md`; CONTEXT.md terms *Custom slide* + *Per-respondent longitudinal comparison*.
...
**LAUNCHED 2026-06-30:** flipped `WAVE_M_CUSTOM_SLIDES_ENABLED` + `WAVE_N_RESPONDENT_LONGITUDINAL_ENABLED` `=1` on Vercel **Production** (non-sensitive, verified `=1`) + redeployed (deploy `7gwoftxjt`, aliased `platformtest.scalingup.com`, ✓ Ready). Kill = zero the flag + redeploy. **Smoke contract:** M — author a slide on a test INVITED campaign → renders uncounted in `/org-survey`; N — a respondent with ≥2 scored campaigns shows deltas, qualitative → not-applicable, flag-off → 404. **Live prod smoke (Playwright, coach acct): Wave M PASS** (editor sanitizes + sandboxed preview + position picker; `<script>` stripped server-side on the write path too). **Wave N hit a P0, caught by the smoke + fixed same-day:** the flag flip 500'd EVERY coach campaign-detail page with a completed respondent — root cause: the loader (`respondent-longitudinal.ts`) AND eligibility (`longitudinal-eligibility.ts`) filtered `submittedAt: { not: null }` against the **NON-nullable** `AssessmentSubmission.submittedAt DateTime @default(now())` (a Prisma validation error → throw → 500; stubbed unit tests never caught the query-shape-vs-schema mismatch), compounded by the campaign-detail page passing the campaign slug where the TEMPLATE alias was expected (qualitative templates wrongly entered the DB path) and an unguarded `await` in the per-row render loop. **Mitigated in minutes** via `WAVE_N_RESPONDENT_LONGITUDINAL_KILL=1` + redeploy (verified prod healthy). **Hotfix PR #111 (`65ad437`):** removed the invalid filter in both queries, fixed the alias to `campaignForFlag?.template?.alias`, wrapped the page loop + the helper's DB section in try/catch (so a throw can never 500 the render), + regression tests (throw→false; assert no `submittedAt` filter in the `where`; page-level guard). Kill removed + fixed code redeployed (`l2a6ss7aj`); **re-smoke ALL PASS** — campaign-detail 200, full longitudinal view renders on a real ≥2-campaign SU-Full respondent (header + trend + per-section/domain delta table + tier row), qualitative → notApplicable card, entry-link gates correctly (scored-only). **GOTCHA for future queries: `AssessmentSubmission.submittedAt` is NON-nullable — never filter it `{ not: null }`.** The dark-merge + flag-flip + LIVE smoke is what bounded the blast radius to minutes.
...
**Now live for the first time in prod:**
- **Wave B** — `WORKSHOP_CUSTOM_HTML_EDITOR_ENABLED` (admin/staff per-workshop landing-page HTML editor).
- **Wave D** — `WAVE_D_AUTO_SEND_ENABLED` (#2 timing radio defaulting to *Immediately* + #3 auto-send on create / on open), `WAVE_D_RESULTS_EMAIL_ENABLED` (#15, still per-template approval-gated), `WAVE_D_COACH_NOTIFY_ENABLED` (#16), `WAVE_D_CUSTOM_HTML_EMAIL_ENABLED` (#20). Flagless Wave D items (#1 soft-delete, #17 template-on-schedule, #18 select-all) were already live.
- **Wave F** — `WAVE_F_GROUP_REPORT_ENABLED` (#22 LVA CEO/group report; LVA-only via `GROUP_REPORT_ALIASES`).

**Mechanism gotchas:** the flags pre-existed in prod stored **sensitive** (write-only) with empty values, so `vercel env pull` always showed `""` and couldn't confirm a write; and CLI 54.x defaults to `--non-interactive` for agents (ignores piped stdin). Both solved with `vercel env add <flag> production --value 1 --no-sensitive --force` — re-storing non-sensitive so flag state is auditable; verified each reads back exactly `"1"` (no trailing newline — matters for Wave D's strict `isTruthy`). `ASSESSMENT_SENDS_PAUSED` unset (sends armed).

**Live verification (Playwright on prod):** Wave F "View group report" link renders on an LVA campaign + `/(report)/assessments/[id]/report` returns the empty-state (gate open, not 404). Wave D wizard Schedule step renders the timing radio (*Immediately* default), #17 template-name, #16 coach-notify checkbox, and the #15 results-email checkbox correctly **disabled** ("Results email not yet approved for this template"). **End-to-end auto-send smoke:** created a one-recipient QSP-v1 campaign (Test Co, controlled inbox) via "Create & send 1 invitation(s) now" → the Inngest fan-out fired → respondent flipped **New → Invited** with a Sent timestamp → campaign soft-deleted (blast-radius confirm "Responses are retained. This is not reversible." = #1 working).

**Wave J — prepped THEN LAUNCHED (#22 SU-Full group report + #32 Peers benchmark):** verified prod SU-Full state — **v1 already PUBLISHED** (so the publish guard was single-dark behind `WAVE_J_*` only), v2 DRAFT lacked the J-1 `S_BACKGROUND`/FTE section. Ran the guarded **DRAFT-only** SU-Full re-seed (origin/main's seed via temporary overlay — local checkout was on a stale pre-J-1 branch; `forceSupersedeDraft` appends past the divergent draft) → **v3 DRAFT** (`9a23eca…`, 11 sections incl. `S_BACKGROUND`; v1 untouched); deleted the orphaned v2 draft via a guarded one-off (unpublished + 0 campaign refs; `ASSESSMENT_VERSION_DELETED` audit row). **Then LAUNCHED (owner directive "turn it on"):** published SU-Full **v3** (publish guard satisfied) + set `WAVE_J_SUFULL_GROUP_ENABLED=1` (non-sensitive; `_KILL` absent) + redeploy (`kffpx6e5v`). **Verified live** on a real SU-Full campaign (`cmpl8kjsl…`, 1 completion): the scored group report renders — per-domain matrix (People/Strategy/Execution/Cash/Leadership Team), the **Peers benchmark column attaches** (keys match v3 → no `benchmarkKeyMismatch` fail-closed), Team-vs-Peers deviations, the provisional footnote (`v2026-06-28.cohort1.provisional`), and the no-CEO "Team vs Peers" fallback. Scores + Peers are **provisional**, labeled in-report. Kill: `WAVE_J_SUFULL_GROUP_KILL=1` + redeploy, or zero `WAVE_J_SUFULL_GROUP_ENABLED`. [18j ops runbook](docs/specs/v7.6/18j-ops-runbook.md).

**Kill switches:** Wave D → `ASSESSMENT_SENDS_PAUSED=1` + redeploy (stops all assessment sends; auto-send only ever affects campaigns created after the flip — legacy were `invitesSentAt`-backfilled); Wave F → zero `WAVE_F_GROUP_REPORT_ENABLED` + redeploy (read-only, clean). Runbooks: [17d](docs/specs/v7.6/17d-ops-runbook.md) / [17f](docs/specs/v7.6/17f-ops-runbook.md) / [17b](docs/specs/v7.6/17b-ops-runbook.md) / [18j](docs/specs/v7.6/18j-ops-runbook.md).

### 2026-06-30 — SU-Full growth-phase tile (J-1): CEO FTE background + mid-survey interstitial <!-- ENTRY_ISO:2026-06-30 ENTRY_SLUG:wave-j1-phase-tile -->

**Branch `feat/wave-j1-phase-tile` (4 commits off `origin/main` `737e02a`) — additive, NO Prisma migration; DARK (nothing live changes on merge).** Builds the **Scaling Up Full growth-phase tile** that earlier Wave J entries had repeatedly deferred. The June-29/June-30 Wave J + Wave J-rest entries both noted J-1 was "still not buildable" because the SU-Full survey was sliders-only and collected no employee/FTE counts — there was no input from which to compute a growth phase. This sprint adds that input (a CEO-only background section) and the phase classification + in-survey interstitial that consumes it, closing the long-standing block.

- **Seed v2 (DRAFT) — CEO FTE background section (`b9cea58`, `50fe4e6`).** SU-Full is re-seeded to a **new DRAFT v2** that adds a CEO-only `S_BACKGROUND` section at `sortOrder 0`, containing two **NON-SCORED `NUMBER`** questions: `Q_FTE_CONTRACT` ("Number of employees with a permanent or temporary contract (FTE)", **required**, the field that **drives the growth phase**) and `Q_FREELANCE` (optional, **excluded** from the phase computation). The version is appended via `forceSupersedeDraft`, so v1 is **never mutated or published**. Scoring **ignores `NUMBER`** question types, so every existing domain / section / ScaleUp 0–100 rollup is **unchanged** — the background section adds collection, not score.
- **Phase lib (`b9cea58`).** New `src/src/lib/assessments/su-full-phase.ts`: `computeGrowthPhase(contractFte)` maps the single combined contract-FTE figure to bands **1–7 Pioneering / 8–24 Organization / 25–49 Management / 50–149 Delegation / 150+ Standardization**, with `≤0` / `NaN` → `null` (no phase). It also carries the **verbatim P1–P5 in-survey narratives** transcribed from the Esperto source; **P3 and P4 share an identical body** — a documented Esperto source quirk preserved deliberately (not a copy-paste bug), flagged in a lib comment.
- **Single-field driver + C1 caveat.** The phase keys on **one combined contract-FTE figure** (permanent + temporary FTE); the freelance figure is collected but **explicitly excluded**. A single CEO sample report hinted Esperto might classify on permanent-only headcount, but the **verbatim survey question wording is the combined field**, so the build follows the question as written. This `C1` source-fidelity caveat is documented in the `su-full-phase.ts` comment so a future reconcile (if Esperto's exact basis is confirmed) is a one-line change.
- **Survey-side plumbing (`1f56ccc`).** The `/me` survey loader now returns **`isCEO`**, read from `AssessmentCampaignParticipant` and keyed by the **authenticated invitation** (server-derived — not client-spoofable). `OrgSurveyClient` **drops the `S_BACKGROUND` section for non-CEO SU-Full respondents** and threads `templateAlias` + `isCEO` down to `SectionPager`. `SectionPager` shows a **NON-COUNTED mid-survey interstitial** (`phase-tile.tsx`, scoped `.su-assessment-brand` per ADR-0005) when a SU-Full **CEO** leaves the FTE page having entered a phase-resolving answer; "Continue" advances to the next real section. The interstitial is not a survey page and is not counted in "Section N of M". The **public Quick Assessment and every other template are unaffected** (gated on `templateAlias === scaling-up-full` + `isCEO`).
- **DARK / remaining op step.** SU-Full stays **DRAFT**; v2 is **not published**; the re-seed RUN + publish is a **manual, production-mutating operator step** (Jeff-gated, same as the rest of Wave J's SU-Full launch). Merging the branch changes nothing live.

**Process + verification.** Built **subagent-driven TDD** with a final whole-branch review (**MERGE-READY**). `tsc` + ESLint clean on touched files; **362 tests pass in the touched areas**, with pre-existing/environment failures untouched (`exchange.test.ts` 410-vs-425; `migration-verification` and `dashboard-stats` both need `DATABASE_URL`). Commits: `b9cea58` + `50fe4e6` (seed v2 + phase lib), `1f56ccc` (survey-side), `f2de8d3` (comment fix). This entry also clears the standing "phase tile blocked — SU-Full collects no FTE" note carried in the two prior Wave J CHANGELOG entries.

### 2026-06-30 — Wave J-rest + Wave K: SU-Full tier suppression + Appendix B + coach logo <!-- ENTRY_ISO:2026-06-30 ENTRY_SLUG:wave-jk-su-full-rest -->

**Branch `feat/wave-jk-su-full-rest` (5 commits off `origin/main` `9177a1b`) — additive, NO Prisma migration. Three features; only Wave K is live-visible, the two SU-Full features are dark (SU-Full is still DRAFT and the group report is still behind the independent `WAVE_J_*` flag).** Picks up the items deferred from the June-29 Wave J merge.

- **Per-respondent SU-Full tier suppression (`6f4146b` + `99a4875`, ADR-0015).** `BrandedReport` now honors the `report-config` `showTier:false` flag end-to-end: when set, the Scaling Up Full *per-respondent* report renders **no tier band, no tier message, and no sr-only tier label** — matching the no-tier decision already taken for the SU-Full group report (Esperto shows no band, and we can't compute its percentile, so standing = peer-deviation). **DARK:** SU-Full is DRAFT so no live respondent report exists, and every currently-live template keeps `showTier:true` — their rendering is byte-for-byte unchanged. The June-29 Wave J entry had explicitly deferred this per-respondent tier suppression; this closes it.
- **SU-Full group report Appendix B (`93a7f5a` + `d9a86ee`).** A de-identified per-member domain grid appended to the SU-Full *scored* group report: a distinguished **"CEO" row** (label "CEO", not "Person 1" — `d9a86ee` Esperto-fidelity fix) plus **"Person 1..N"** for the rest of the team (no names — pseudonymization, per the Esperto Appendix B convention), columns = the **4 report domains People / Strategy / Execution / Cash** (the "you/self" domain is excluded per the Esperto source), cells = each member's **frozen `domainAvg`** read straight from the stored `ScoreResult` (never recomputed), omit-empty. **DARK:** built only when `scored.domains` is present (i.e. SU-Full), and the SU-Full group report stays gated behind the independent `WAVE_J_SUFULL_GROUP_ENABLED` flag + the SU-Full publish guard (`version.publishedAt`) + the `GROUP_REPORT_ALIASES` allowlist — so merging surfaces nothing live. The June-29 Wave J entry had deferred "Appendix B per-member rows"; this closes it.
- **Wave K — coach logo on reports (`93585b8`).** The campaign coach's logo now renders on the **cover and footer-left** of BOTH the per-respondent AND the group report, across **all templates**. It reuses the existing **`Coach.profileImage`** column via the `AssessmentCampaign.creatorCoach` relation — **NO migration**, no new field. New `CoachLogo.tsx` component; the two report loaders were widened to `select` `creatorCoach.profileImage`; threaded through `RespondentReport` (per-respondent) and the group-report provenance/props; scoped CSS only (ADR-0005, zero global-token leak). The Scaling Up "Generated by Scaling Up Platform" mark stays alongside it. **This is the ONLY live-visible change in the branch, and it is purely additive:** a report whose campaign coach has no `profileImage` — including admin-created PUBLIC campaigns with no coach — renders **byte-identically to before** (graceful SU-only fallback). **Screen + PDF only**; the inline-HTML results email is unchanged (not in scope).

**Deferred this sprint — J-1 phase tile.** The SU-Full growth-phase tile (FTE-band classification + verbatim P1–P5 phase copy) is **still not buildable**: the SU-Full survey seed is sliders only and does not collect employee/FTE counts, so there is no input from which to compute the growth phase. It needs the CEO employee-count questions added to the template plus a re-seed first — a content change whose exact question wording is likely Jeff-gated — before the tile can be built.

**Still blocked (need Jeff), carried forward:** the SU-Full publish / ScaleUp 0–100 weighting formula; the authoritative cohort Peers numbers (current `su-full-benchmarks.ts` values remain provisional); per-question peers; and a benchmark DB table + admin editor (benchmarks are still a static versioned constant).

**Process + verification.** Built **subagent-driven TDD** with a per-task review and a final whole-branch review (**MERGE-READY**). `CI=true npx next build --turbopack` clean; **1014 assessment tests pass**, with **9 pre-existing/environment failures untouched** (`migration-verification` needs `DATABASE_URL`; `dashboard-stats` carries stale `mockCampaign` args — neither touched by this branch). To LAUNCH the SU-Full features still requires publishing SU-Full (Jeff-gated) and flipping `WAVE_J_SUFULL_GROUP_ENABLED`; Wave K's coach logo is live on merge (additive, no flag).

### 2026-06-29 — Wave J: Scaling Up Full scored group report + Peers benchmark (MERGED DARK) <!-- ENTRY_ISO:2026-06-29 ENTRY_SLUG:wave-j-su-full-group-report -->

**Spec 18 §C (Jeff June-22 batch). Wave J = J-3 + J-2, MERGED DARK — additive, NO Prisma migration, default-OFF flag; nothing live changes on merge.** Surfaces the already-built **scored** group report for the Scaling Up Full assessment (CEO-vs-team alignment matrix + per-question bars) and adds a static, versioned, single-cohort **Peers** industry benchmark. SU-Full stays DRAFT and the flag stays off, so merging is invisible; LVA's group report is byte-for-byte unchanged.

**Three enforced dark gates (any one keeps it dark):** (1) an **independent** SU-Full flag set `WAVE_J_SUFULL_GROUP_ENABLED` / `_CANARY` / `_KILL` — fully decoupled from LVA's `WAVE_F_*`, with a **campaign-id-only** canary (a coach/org id can't enable it) and a hard `_KILL` that overrides any canary; (2) a **SU-Full-scoped publish guard** (`alias==="scaling-up-full" && version.publishedAt==null → notApplicable(unpublished)`) enforced in the loader, the portal entry link, AND the direct route — never gates a null-publishedAt LVA; (3) the `GROUP_REPORT_ALIASES` allowlist (alias added atomically with the gates — no intermediate draft-reachable window). Route enablement was moved into the loader (rate-limit still first; pure `report-gate-core` unchanged; disabled ⇒ silent dark 404 with no audit, even for admins; unpublished-but-enabled ⇒ observable notApplicable panel).

**Peers / standing (ADR-0015):** Esperto shows **no tier band** and we lack its peer distribution to compute its "X% score higher" percentile, so standing = **deviation from Peers** (▲/▼). `su-full-benchmarks.ts` holds the provisional single-cohort numbers — domain/section on the **0–10** scale, ScaleUp on **0–100** (Esperto's 0–100 section sum ÷ question-count = the 0–10 matrix value; 47.3/8=5.9), versioned (`SU_FULL_BENCHMARKS_VERSION`) + key-set-bound to the seed + value-locked by snapshot. `applyBenchmarks` attaches `peers`/`devPeers` (CEO−peers) + `devPeersTeam` (team−peers, the no-CEO standing) per domain/section + ScaleUp, **fails closed** (clears ALL peers + flags `benchmarkKeyMismatch`) on any benchmark/seed key-skew — never a partial table. `ScoredGroupReport` renders omit-empty `Peers` + `Dev·Peers` columns (peer-specific null cell with no spurious "(N<2)"), a no-CEO "Team vs Peers" header, a provisional footnote naming the version, and **suppresses SU-Full's invented tier** (render-layer `showTier:false`; the per-respondent report keeps its band, deferred). `benchmarkVersion`/`benchmarkKeyMismatch` flow to the `GROUP_REPORT_VIEW` audit + view metric; `not_applicable` carries a distinguishable `reason`/`template`.

**Process:** designed (one spec) + grilled twice (`/grill-with-docs` design tree, `/grill-me` plan) + two Codex adversarial loops (`/claudex:plan` 3 rounds, then 5) whose findings were integrated as first-class plan tasks; built **subagent-driven TDD** (8 tasks, each implementer + spec-compliance review + code-quality review, gated). Final whole-branch review **MERGE-READY** (three-gate dark traced end-to-end; LVA non-regression provable; data-flow scale-coherent; no migration; no PII). `CI=true npx next build --turbopack` clean; full assessment suites green (+ new tests), 1 pre-existing unrelated `dashboard-stats` fail; capacity smoke ~15ms for a 1-CEO+40-member cohort. Spec [18j design](docs/specs/v7.6/18j-wave-j-su-full-design.md) + [18j source extract](docs/specs/v7.6/18j-su-full-source-extract.md) + [18j plan](docs/specs/v7.6/18j-wave-j-implementation-plan.md) + [18j ops runbook](docs/specs/v7.6/18j-ops-runbook.md) + [ADR-0015](docs/adr/0015-su-full-standing-signal-peer-deviation-not-bands.md). **Deferred:** Appendix B per-member rows; per-question peers; benchmark DB table + admin editor; J-1 phase tile; per-respondent SU-Full tier suppression (with its Peers). **To LAUNCH (all separate, Jeff-gated):** publish SU-Full (a broader step that also opens the wizard/quiz/submit) → canary `WAVE_J_SUFULL_GROUP_CANARY` (campaign ids, ≤25-member cohort) → `WAVE_J_SUFULL_GROUP_ENABLED=1` + redeploy; kill-switch = `WAVE_J_SUFULL_GROUP_KILL=1` or Vercel promote-previous; `benchmarkKeyMismatch=true` is launch-blocking. See [18j ops runbook](docs/specs/v7.6/18j-ops-runbook.md).
### 2026-06-29 — Wave L — LVA group-report fidelity: 0–10 rating scale + report labels/intros (L3+L4) <!-- ENTRY_ISO:2026-06-29 ENTRY_SLUG:wave-l-lva-group-report-fidelity -->

**Wave L (L3 + L4) — LVA group-report source-fidelity shipped (PR #98 `bf0fe2b`, prod — additive, DISPLAY-ONLY, NO migration / NO re-seed / NO flag; the surface is DARK behind default-OFF `WAVE_F_GROUP_REPORT_ENABLED`, so merging changes nothing live until that flag flips).** Closes the fidelity gaps the June-26 LVA source-fidelity audit found between our LVA *group* report and Esperto's rendered "CEO Full Report".

- **L3 — rating value on Esperto's 0–10 scale.** The S3 "Organizational Strengths" rating now displays each factor on a **0–10 axis (clean thirds: Weak=0 / Average=5 / Strong=10)**, the arithmetic **mean over all respondents including the CEO**, **rounded UP (`ceil`) to 1 decimal** — reproducing Esperto's printed values (8.4 / 6.7 / 5.0 / 3.4 / 1.7) exactly. Previously the report printed the raw 1–3 mean (e.g. `2.3`). The formula was reverse-engineered by **pixel-measuring the Esperto group-report bars** at N=3 (an earlier *midpoint* 1.67/5/8.33 attempt was wrong — it gives 7.2 where Esperto shows 8.4 — and was reverted before merge). Gated **LVA-S3-only** (`templateAlias === leadership-vision-alignment` && `stableKey === S3_strengths`); non-LVA rating sections keep `scaledValue = null` and the renderer falls back to the raw mean. **Domain validation:** out-of-domain/imported S3 values (not in {1,2,3}) → `scaledValue null` + `scaleDegraded`, so raw mean/sort and scaled display never silently disagree. Display uses a fixed `toFixed(1)` (so 5.0/10.0 keep the decimal); raw `mean` kept as the sort key. **No change to the 1–3 stored scale, the aggregation math, or the CEO-included denominator.**
- **L4 — report labels + verbatim intros.** A report-layer **factor-label map** for the ~6 factors Esperto's *report* labels differently than the *survey* (`Recruitment of new staff`, `Keeping employees`, `Leadership team`, `The Leadership`, `Internal Communication`, `Financing growth`), keyed by normalized factor slug, applied to BOTH the rating and obstacles sections (survey labels unchanged; S5 "Why is X a hindrance?" headings excluded). Plus **verbatim Esperto section intros** (S1 financials / S2 vision / S3 strengths / S4 obstacles) transcribed char-for-char from the group-report PDF (p3/p4/p7/p8) and rendered under each section heading; S5/S6 have no intro. The seed `description` (a paraphrase) is deliberately NOT used.
- **Provenance (R2-M1):** new `GROUP_RENDER_VERSION = "lva-fidelity-v1"` recorded in the model provenance and the `GROUP_REPORT_VIEW` audit `changes` payload (mirrors the per-respondent `REPORT_FILTER_VERSION` pattern), so a code-only render change is attributable; bump it whenever the scale/label/intro constants change.

New module `src/src/lib/assessments/lva-report-display.ts` (ceil1, scaledRatingValue, domain guard, label/intro maps, `GROUP_RENDER_VERSION`); edits to `group-report-model.ts` (scaledValue/gating/domain/provenance + S3 + S4 label overrides), `QualitativeGroupReport.tsx` (renderer), `report-access-gate.ts` (audit), `GroupReport.tsx`, the report route, `su-report.css`.

**Process:** brainstorm → fidelity audit (read all 5 xlsx tabs + 38 embedded screenshots + the report PDFs; the scoring config is NOT exposed in Esperto's admin, so the rendered reports are authoritative) → scope locked via `/grill-with-docs` + `/grill-me` (fidelity bar = content+numbers parity; L1 radio-matrix / L2 pagination / rehire-bar viz DESCOPED as input/presentation UX with identical data) → hardened via `/claudex:plan` (3 Codex rounds: senior-eng 6m/2l → security 3m/1l → final 0 findings; converged 8→4→0, zero high) → subagent-driven implement + independent code-reviewer **MERGE-READY**. **Verification:** 70/70 targeted tests, ESLint 0 on changed files, `CI=true npx next build --turbopack` clean, full suite at the documented baseline (4179 pass / 28 pre-existing fail — zero new).

**Follow-on (PR #100 `c75f9ba`, merged 2026-06-29, prod):** the S5 "Obstacles and Challenges Explained" headings now use the Esperto report factor labels too, matching the rating + obstacles sections — closing a within-report inconsistency surfaced by the live canary render ("Recruitment of new staff" in the rating section vs "Why is Recruitment of new **employees** a hindrance?" in S5). New `lvaReportQuestionLabel()` substring-replaces the factor name in `S5_why_<slug>` headings (the ~6 slugs that differ; template-agnostic, no-op otherwise); `buildQaSection` threads the alias + applies it LVA-gated. Display-only, group-report-only, still dark behind `WAVE_F`; 64 targeted tests green, ESLint 0.

**Caveats (documented, non-blocking):** (1) the `ceil` rounding is empirically matched only at N=3 (the single validated Esperto sample) — re-validate against a non-N=3 sample before the `WAVE_F` group-report launch; (2) a mixed in-domain/out-of-domain LVA S3 section (rare imported/legacy path) can show the 0–10 legend while a degraded row renders a 1–3 value — accepted on a dark feature, `scaleDegraded` recorded. **To launch:** flip `WAVE_F_GROUP_REPORT_ENABLED` (owner-gated; the LVA group report is still dark). Spec + plan: [docs/specs/v7.6/18l-wave-l-implementation-plan.md](docs/specs/v7.6/18l-wave-l-implementation-plan.md); evidence: [docs/specs/v7.6/18-lva-source-fidelity-audit.md](docs/specs/v7.6/18-lva-source-fidelity-audit.md). See also Wave J (SU-Full scored group report) — separate stream, plan still in review.
...
- **Approvals pulled OUT to a standalone top-level link** (owner decision, June 26 preview review) so its pending badge is always visible — it's the operator's daily queue. Refunds (the other badged queue) stays nested under Financials▾ with a rolled-up badge. "Surveys" relabeled "Workshop Surveys" (disambiguates from the Assessments module). "Assessments →" is a gateway into its existing lane/sidebar (untouched).
- **Disclosure-navigation pattern, hand-rolled** (NOT Radix, NOT `role=menu`): a single `openGroup` state in `AdminNavLinks` so only one panel is open by construction (keyboard-safe — keyboard activation fires `click`, not `mousedown`); `aria-expanded`/`aria-controls`, Esc closes + returns focus to the trigger, outside-click + focus-leaving-the-nav close, `aria-current` on the active leaf, arrow-key cycling as enhancement; active group reads primary-tint (not merely "open"). Mobile mirrors the groups as collapsible sections, **collapsed by default with the active group auto-expanded**.
- **Badges** = two `count()` queries fetched server-side in the dashboard layout (`lib/nav/admin-nav-badges.ts`): Approvals = `approvalQueue` PENDING; Refunds = paid registrations on CANCELED workshops not yet refunded (mirrors `/admin/refunds-needed`). **Fail-soft** (any DB error → zeros, never 500s the admin shell); **zero → no badge**. Not live-polled — the approvals respond flow gets `router.refresh()` so the badge self-heals when the queue clears. The refunds filter is NOT index-backed (no migration in scope); acceptable at this data scale.
- **Responsive:** the full grouped bar shows at `xl` (≥1280); the hamburger (same groups) covers below `xl`. Raised from `lg`→`xl` after the **live browser smoke caught a 1024px overflow** (Financials collided with the theme toggle) — jsdom couldn't see it.
- **Process:** brainstorm → `/grill-with-docs` + `/grill-me` (owner approved a brand-faithful preview; Approvals-out, Refunds-nested, Workshop-Surveys, PENDING-only badge) → `/co-validate` (Codex staff-eng — 6 findings, all accepted: `set -o pipefail` gates, single open-group, real browser smoke, drop the false "indexed" claim, SoT-in-PR, mobile collapsed) → subagent-driven TDD. **23 new tests** (5 model / 3 badges / 9 desktop / 6 mobile), ESLint 0/0 on changed files, `CI=true npx next build --turbopack` green, full suite zero new failures (28 pre-existing). Browser-verified at 1024/1280/390. Also fixes the stale CLAUDE.md "Nav bar has 13 items / Bio" gotcha.
...
The two assessment report-viewing pages — per-respondent **Results report** (`/(report)/assessments/[id]/respondents/[respondentId]/report`) and the campaign group/**Aggregate report** (`/(report)/assessments/[id]/report`) — copy-pasted a cross-cutting request protocol inline (actor resolution → optional flag gate → fail-closed rate-limit guard → fail-closed audit → structured metrics). The **same** fail-closed rate-limit bug shipped to prod **twice** (once per route): the guard's `catch` swallowed the fail-closed `notFound()` because it tested a stale Next digest (`NEXT_NOT_FOUND` vs Next-16's `NEXT_HTTP_ERROR_FALLBACK;404`). **The gate fixes the class structurally** — `notFound()`/`redirect()` are now only ever called OUTSIDE any `try` (the rate-limit check sets a `rateLimited` flag inside the try and calls `notFound()` after the catch), and the loaders return discriminated unions (never throw Next control-flow), so the gate uses **zero `unstable_rethrow`** and the swallow cannot recur.

**Architecture (ADR-0012).** A dependency-injected `viewReport(deps, opts)` core (`lib/assessments/report-gate-core.ts`, pure — only `next/navigation` at runtime; `{ rateLimiter, auditSink, emitMetric }` injected so the protocol is unit-tested against fakes) owns the envelope; it interprets each loader outcome via a single `classify(o) → ReportDisposition` (`ok | forbidden | not-found | passthrough`) over an exhaustive switch (the false-`authz_deny`-on-`not-found` bug class is structurally impossible). `passthrough` outcomes (group `empty`/`notApplicable`) are returned unchanged for the **page** to render its bespoke panel — the gate never swallows domain rendering. Two thin adapters (`viewGroupReport`, `viewRespondentReport` in `report-access-gate.ts`) pre-bind per-surface policy + own the narrow-Db cast; `defaultReportGateDeps()` wires the real deps. Metrics generalized into a positional `emitReportMetric(surface, event, fields?)` (`report-metrics.ts`); `emitGroupReportMetric` becomes a thin wrapper (marker `assessment.group_report.*` preserved byte-for-byte + an additive `surface` field). Seam: the **gate** emits only request-ending events (`rate_limited`/`authz_deny`/`render_failure`/`audit_failure`); the **page** emits success-render events (`view`/`degraded`/`orphan_submission`/`not_applicable`/`empty`).

**PR #73 — gate core + group migration (no-op for group).** Group already matched the gate's target shape (fail-closed audit, structured metrics, canonical rate-limit key), so its behavior is preserved; ships dark behind the existing default-OFF `WAVE_F_GROUP_REPORT_ENABLED`. New `report-gate-core.ts` + `report-access-gate.ts` (group adapter) + `report-metrics.ts`; group page shrinks to adapter call + outcome switch + page-owned metrics; `rate-limit.ts` exports `RateLimitConfig` (one word).

**PR #78 — per-respondent migration + 3 intentional fold-ins** (always-on surface): (1) audit fail-OPEN → fail-CLOSED + captures IP/UA (was `logAudit`, which swallowed write errors and recorded neither); (2) rate-limit key IP-only `report:${ip}` → `report:${actorKey}:${campaignId}:${respondentId}:${ip}` (`actorKey = coachId ?? userId ?? "anon"`; one-time per-IP throttle reset on deploy, no Redis migration); (3) ad-hoc `console.info` → structured `assessment.respondent_report.*` (incl. a **new** `authz_deny` on a true forbidden + a page-owned `view`). `'VIEW_REPORT'` added to the `AuditAction` union (drops the old cast). The respondent page test became the leaf-mocked integration suite (`logAudit` mock → `db.auditLog.create`, + IP/UA + view-metric assertions). Supersedes the auto-closed PR #74 (GitHub closed it when its stacked base branch was deleted on #73's merge; the same commit was re-opened as #78 against `main` after a clean rebase).

**Process.** Surfaced by `/improve-codebase-architecture` (deepening candidate: the report-view protocol had no seam) → `/grill-with-docs` (Q1–Q5, design) → `/grill-me` (GQ1–GQ3 + confirmations, implementation) → `/co-validate` (Codex staff-eng pass, C1–C6 + F1 — Codex's **C1** produced the structural "`notFound()` outside the `try`" fix that eliminated `unstable_rethrow` entirely; C2 caught a group rate-limit-key rebucket; C4 caught the unexported `RateLimitConfig`) → TDD/subagent build. Plan: [plans/report-access-gate-plan.md](report-access-gate-plan.md); decision: [docs/adr/0012-report-access-gate.md](../docs/adr/0012-report-access-gate.md); glossary term **Report access gate** in `CONTEXT.md`. Each PR: ESLint clean · targeted suites green incl. loader regression · `CI=true npx next build --turbopack` exit 0 · full suite zero new failures (28 pre-existing baseline; 4001 → 4008 passing).

---

### 2026-06-22 — All-assessments survey polish (Spec 17 Wave G) <!-- ENTRY_ISO:2026-06-22 ENTRY_SLUG:wave-g-survey-ux -->

**Shipped to prod** (PRs #75 `89132fe`, #76 `d95ea84`, #77 `3f98fe6`; SoT flush this entry) — additive, **NO migration, NO feature flag** (rollback = revert + Vercel promote-previous). Three independent rollback units closing Jeff's Wave G survey-UX punch-list. Built subagent-driven TDD (per task: implementer → spec-review → code-quality-review → whole-branch MERGE-READY) in an isolated git worktree (real `npm ci`, not a node_modules symlink — turbopack rejects out-of-root symlinks) because a parallel stream (report-access-gate, PR #73) was live on `main`. Hardened through a 3-round Codex `/claudex:plan` pass after `/grill-with-docs` + `/grill-me`.

**PR #75 — pager merge + section-heading focus ring.** The shared participant `SectionPager` two-step (intro slide → "Begin section" → questions) is collapsed into ONE page per section: the intro content (domain accent rail + the "What this section covers" callout) renders ABOVE the question list, a single Next advances. Applies UNIFORMLY to BOTH the invited org-survey and the live public Quick Assessment (**G1 decision**: "zero /quiz change" reframed to "no regression"; pinned by tests on both flows). The merged section heading is the single `tabIndex={-1}` focus target and gains a `:focus` ring (`.wf-scope .survey-section-title:focus`, literal `#522583` — `:focus` not `:focus-visible`, since the heading is focused programmatically and `:focus-visible` is unreliable there, esp. Safari). `.su-section-intro` styled as a header region (rail re-anchored via `position:relative`); dead two-step CSS + the unreachable `advance()` guard removed. All required-field validation / focus-first-invalid / submit-latch / `requireAtLeastOneAnswer` contracts preserved. 196 assessment tests green.

**PR #76 — default invitation email + legacy subject hardening + send telemetry.** New client-safe `lib/assessments/invitation-defaults.ts` (`DEFAULT_INVITATION_BODY`/`_SUBJECT`/`_VERSION`; zero imports so it never drags the server email renderer into the client bundle). `sendAssessmentInvitationEmail` normalizes a blank subject/body to the default BEFORE the branded/legacy split (blank = `.trim().length === 0`, NOT `??`) so ALL send paths (initial/reminder/resend/auto-send) are covered; authored values pass through untouched; the Wave D #20 custom-HTML body keeps its content (only its blank subject defaults). **Security:** the legacy renderer's subject now routes through the same hardened `renderSubject` (`SUBJECT_ALLOW` allowlist + control-char/`#t=`/url stripping) — closes a path where `{{invitationUrl}}` or CR/LF could leak the invite credential into the SMTP subject header; legacy body gains org/template token resolution. PII-free send telemetry (`renderer`/`subjectSource`/`bodySource`/`defaultVersion`) on both SMTP sends. The campaign wizard shows the defaults as a PLACEHOLDER (not a value-prefill → no per-campaign override persistence) + adds `{{organizationName}}`/`{{templateName}}` chips. Public `sendAssessmentInvitationEmail` signature UNCHANGED (6 callers untouched). 32 tests green. Ops runbook `docs/specs/v7.6/17g-ops-runbook.md`.

**PR #77 — QSP v2 closing-section description + fail-closed re-seed.** QSP v2 `P5_closing` gets a `description` (the only section platform-wide that lacked one). `runSeed(client, { force = false })` is now FAIL-CLOSED in code (was a hardcoded `forceSupersedeDraft: true`): a divergent unpublished DRAFT aborts by default; superseding requires an explicit, audited `QSP_V2_SEED_FORCE=1` (logs a warning); `versionNumber` surfaced for the ops manifest. **CODE ONLY** — merging runs no seed and creates no version; the DRAFT is created by a separate manual operator step and publishing stays separate (nothing reaches respondents on merge). 234 seed tests green.

Deferred follow-ups: removing the question-less Welcome/Completion pager pages (pending Jeff); #29 LVA content reconcile (Jeff-gated); the QSP v2 re-seed RUN + publish (operator step). Next: Wave H (admin nav group-dropdowns + `platform.scalingup.com` custom domain).: Report access gate shipped (ADR-0012, PRs #73 + #78, prod))
