...
**Visual acceptance.** The customer-overview mockup was reviewed in conversation against Jeff's supplied desktop reference and explicitly approved as “perfect” before implementation. The accepted states cover the collapsed overview, one-at-a-time expansion, automatic close on filter changes, and the existing responsive campaign rows. The shared component keeps its existing wrap-at-small-breakpoint campaign identity layout, while the new customer header is a wrapping, full-width control with no fixed width or horizontal overflow dependency. A direct local-browser capture was attempted after implementation, but localhost navigation was blocked by the browser security policy; no production data or authenticated session was used to bypass it.
